...
class Col : public Mat<eT> {
 public:
  ...
  inline explicit Col(const uword n_elem, const arma_initmode_indicator<do_zeros>&);
  template <bool do_zeros>
  inline explicit Col(const uword in_rows, const uword in_cols,
                      const arma_initmode_indicator<do_zeros>&);
  template <bool do_zeros>
  inline explicit Col(const SizeMat& s, const arma_initmode_indicator<do_zeros>&);
  ...
  template <typename T1>
  inline explicit Col(const SpBase<eT, T1>& X);
  template <typename T1>
  inline Col& operator=(const SpBase<eT, T1>& X);

  inline Col(eT* aux_mem, const uword aux_length, const bool copy_aux_mem = true,
  ...
  arma_frown("use braced initialiser list instead") inline mat_injector<Col> operator<<(
      const eT val);
