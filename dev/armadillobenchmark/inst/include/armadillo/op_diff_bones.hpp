...
class op_diff : public traits_op_default {
 public:
  template <typename eT>
  inline static void apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword k,
                                   const uword dim);
...
class op_diff_vec : public traits_op_passthru {
 public:
  template <typename T1>
