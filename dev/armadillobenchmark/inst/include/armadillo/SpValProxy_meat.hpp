...
arma_inline SpValProxy<T1>::SpValProxy(uword in_row, uword in_col, T1& in_parent,
                                       eT* in_val_ptr)
...
template <typename T1>
inline SpValProxy<T1>& SpValProxy<T1>::operator=(const eT rhs) {
...
template <typename T1>
inline SpValProxy<T1>& SpValProxy<T1>::operator++() {
  if (val_ptr) {
    (*val_ptr) += eT(1);
    parent.invalidate_cache();
    check_zero();
...
template <typename T1>
inline typename T1::elem_type SpValProxy<T1>::operator++(const int) {
  if (val_ptr) {
    (*val_ptr) += eT(1);
    parent.invalidate_cache();
    check_zero();
...
template <typename T1>
arma_inline typename get_pod_type<typename SpValProxy<T1>::eT>::result
...
template <typename T1>
arma_inline void SpValProxy<T1>::check_zero() {
  if (*val_ptr == eT(0)) {
    parent.delete_element(row, col);
    val_ptr = nullptr;
  }
