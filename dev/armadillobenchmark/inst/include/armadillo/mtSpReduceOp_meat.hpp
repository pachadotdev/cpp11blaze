...
inline mtSpReduceOp<out_eT, T1, op_type>::mtSpReduceOp(const T1& in_m) : m(in_m) {
  arma_debug_sigprint();
}

template <typename out_eT, typename T1, typename op_type>
inline mtSpReduceOp<out_eT, T1, op_type>::mtSpReduceOp(const T1& in_m,
...
template <typename out_eT, typename T1, typename op_type>
inline mtSpReduceOp<out_eT, T1, op_type>::~mtSpReduceOp() {
  arma_debug_sigprint();
}
