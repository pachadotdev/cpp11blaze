...
arma_warn_unused inline static typename arma_real_only<eT>::result trunc_exp(const eT x) {
...
template <typename eT>
arma_warn_unused inline static typename arma_integral_only<eT>::result trunc_exp(
    const eT x) {
  return eT(trunc_exp(double(x)));
}

template <typename T>
arma_warn_unused inline static std::complex<T> trunc_exp(const std::complex<T>& x) {
...
template <typename T1>
arma_warn_unused arma_inline const eOpCube<T1, eop_trunc_exp> trunc_exp(
