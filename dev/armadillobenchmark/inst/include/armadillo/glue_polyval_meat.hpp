...
inline void glue_polyval::apply_noalias(Mat<eT>& out, const Mat<eT>& P,
                                        const Mat<eT>& X) {
  arma_debug_sigprint();
...
inline void glue_polyval::apply(Mat<typename T1::elem_type>& out,
                                const Glue<T1, T2, glue_polyval>& expr) {
  arma_debug_sigprint();
  ...
  arma_conform_check(((P.is_vec() == false) && (P.is_empty() == false)),
                     "polyval(): argument P must be a vector");
