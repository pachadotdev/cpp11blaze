...
//! NOTE: use as_scalar() instead; this functionality is kept only for compatibility with
//! old user code
template <typename out_eT>
class conv_to {
 public:
  template <typename in_eT, typename T1>
  arma_frown("use as_scalar() instead") inline static out_eT
      from(const Base<in_eT, T1>& in,
           const typename arma_not_cx<in_eT>::result* junk = nullptr);

  template <typename in_eT, typename T1>
  arma_frown("use as_scalar() instead") inline static out_eT
      from(const Base<in_eT, T1>& in,
           const typename arma_cx_only<in_eT>::result* junk = nullptr);

  template <typename in_eT, typename T1>
  arma_frown("use as_scalar() instead") inline static out_eT
      from(const BaseCube<in_eT, T1>& in,
           const typename arma_not_cx<in_eT>::result* junk = nullptr);
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline out_eT conv_to<out_eT>::from(
    const Base<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename in_eT, typename T1>
arma_warn_unused inline out_eT conv_to<out_eT>::from(
    const Base<in_eT, T1>& in, const typename arma_cx_only<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename in_eT, typename T1>
arma_warn_unused inline out_eT conv_to<out_eT>::from(
    const BaseCube<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename in_eT, typename T1>
arma_warn_unused inline out_eT conv_to<out_eT>::from(
    const BaseCube<in_eT, T1>& in, const typename arma_cx_only<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
class conv_to<Mat<out_eT> > {
 public:
  template <typename in_eT, typename T1>
  inline static Mat<out_eT> from(
      const Base<in_eT, T1>& in,
      const typename arma_not_cx<in_eT>::result* junk = nullptr);
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline Mat<out_eT> conv_to<Mat<out_eT> >::from(
    const Base<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename in_eT, typename T1>
arma_warn_unused inline Mat<out_eT> conv_to<Mat<out_eT> >::from(
    const SpBase<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
  ...
  for (uword count = 0; it != it_end; ++it, ++count) {
    out.at(it.row(), it.col()) = tmp[count];
  }
...
template <typename in_eT, typename T1>
arma_warn_unused inline Mat<out_eT> conv_to<Mat<out_eT> >::from(
    const SpBase<in_eT, T1>& in, const typename arma_cx_only<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename in_eT>
arma_warn_unused inline Mat<out_eT> conv_to<Mat<out_eT> >::from(
    const std::vector<in_eT>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename in_eT>
arma_warn_unused inline Mat<out_eT> conv_to<Mat<out_eT> >::from(
    const std::vector<in_eT>& in, const typename arma_cx_only<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline Row<out_eT> conv_to<Row<out_eT> >::from(
    const Base<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
  ...
  arma_conform_check(((X.is_vec() == false) && (X.is_empty() == false)),
                     "conv_to(): given object cannot be interpreted as a vector");
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline Col<out_eT> conv_to<Col<out_eT> >::from(
    const Base<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
class conv_to<SpMat<out_eT> > {
 public:
  template <typename in_eT, typename T1>
  inline static SpMat<out_eT> from(
      const SpBase<in_eT, T1>& in,
      const typename arma_not_cx<in_eT>::result* junk = nullptr);
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline SpMat<out_eT> conv_to<SpMat<out_eT> >::from(
    const SpBase<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
arma_warn_unused inline SpMat<out_eT> conv_to<SpMat<out_eT> >::from(
  ...
  if (is_same_type<out_eT, in_eT>::yes) {
    ...
    for (uword i = 0; i < X_n_elem; ++i) {
      X_nnz += (X_mem[i] != in_eT(0)) ? uword(1) : uword(0);
    }
...
class conv_to<Cube<out_eT> > {
 public:
  template <typename in_eT, typename T1>
  inline static Cube<out_eT> from(
      const BaseCube<in_eT, T1>& in,
      const typename arma_not_cx<in_eT>::result* junk = nullptr);
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline Cube<out_eT> conv_to<Cube<out_eT> >::from(
    const BaseCube<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
class conv_to<std::vector<out_eT> > {
 public:
  template <typename in_eT, typename T1>
  inline static std::vector<out_eT> from(
      const Base<in_eT, T1>& in,
      const typename arma_not_cx<in_eT>::result* junk = nullptr);
...
template <typename out_eT>
template <typename in_eT, typename T1>
arma_warn_unused inline std::vector<out_eT> conv_to<std::vector<out_eT> >::from(
    const Base<in_eT, T1>& in, const typename arma_not_cx<in_eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
