...
inline void SymEigsSolver<eT, SelectionRule, OpType>::fill_rand(eT* dest, const uword N,
...
inline SymEigsSolver<eT, SelectionRule, OpType>::SymEigsSolver(const OpType& op_,
  ...
  arma_conform_check((nev_ < 1 || nev_ > dim_n - 1),
                     "newarp::SymEigsSolver: nev must satisfy 1 <= nev <= n - 1, n is "
                     "the size of matrix");
  arma_conform_check(
      (ncv_ <= nev_ || ncv_ > dim_n),
