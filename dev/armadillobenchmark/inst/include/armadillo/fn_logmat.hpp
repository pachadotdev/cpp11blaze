...
logmat(const Base<typename T1::elem_type, T1>& X, const uword n_iters = 100u) {
  ...
  return mtOp<std::complex<typename T1::elem_type>, T1, op_logmat>(X.get_ref(), n_iters,
                                                                   uword(0));
}
...
logmat(Mat<std::complex<typename T1::elem_type> >& Y,
       const Base<typename T1::elem_type, T1>& X, const uword n_iters = 100u) {
  arma_debug_sigprint();
...
inline typename enable_if2<(is_supported_blas_type<typename T1::elem_type>::value &&
                            is_cx<typename T1::elem_type>::yes),
                           bool>::result
logmat(Mat<typename T1::elem_type>& Y, const Base<typename T1::elem_type, T1>& X,
