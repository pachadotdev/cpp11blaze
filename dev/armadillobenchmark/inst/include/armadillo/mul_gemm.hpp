...
class gemm_emul_tinysq {
 public:
  ...
  arma_cold inline static void apply(Mat<eT>& C, const TA& A, const TB& B,
    ...
    switch (A.n_rows) {
      case 4:
        gemv_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(
            C.colptr(3), A, B.colptr(3), alpha, beta);
      // fallthrough
      case 3:
        gemv_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(
            C.colptr(2), A, B.colptr(2), alpha, beta);
      // fallthrough
      case 2:
        gemv_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(
            C.colptr(1), A, B.colptr(1), alpha, beta);
      // fallthrough
      case 1:
        gemv_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(
            C.colptr(0), A, B.colptr(0), alpha, beta);
      // fallthrough
      default:;
    }
  }
...
template <const bool do_trans_A = false, const bool do_trans_B = false,
          const bool use_alpha = false, const bool use_beta = false>
...
//! \brief
//! Wrapper for BLAS dgemm function, using template arguments to control the arguments
//! passed to dgemm. Matrix 'C' is assumed to have been set to the correct size (ie.
//! taking into account transposes)
...
class gemm {
 public:
...
#if defined(ARMA_USE_ATLAS)
      {
        ...
        atlas::cblas_gemm<eT>(
            ...
            (do_trans_A) ? (is_cx<eT>::yes ? atlas_CblasConjTrans : atlas_CblasTrans)
                         : atlas_CblasNoTrans,
            (do_trans_B) ? (is_cx<eT>::yes ? atlas_CblasConjTrans : atlas_CblasTrans)
                         : atlas_CblasNoTrans,
            C.n_rows, C.n_cols, (do_trans_A) ? A.n_rows : A.n_cols,
            (use_alpha) ? alpha : eT(1), A.mem, (do_trans_A) ? A.n_rows : C.n_rows, B.mem,
            (do_trans_B) ? C.n_cols : ((do_trans_A) ? A.n_rows : A.n_cols),
            (use_beta) ? beta : eT(0), C.memptr(), C.n_rows);
      }
...
#else
      {
        gemm_emul<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(C, A, B, alpha,
                                                                      beta);
      }
#endif
    }
  ...
  template <typename eT, typename TA, typename TB>
  inline static void apply(Mat<eT>& C, const TA& A, const TB& B, const eT alpha = eT(1),
                           const eT beta = eT(0)) {
    gemm_emul<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(C, A, B, alpha, beta);
  }

  template <typename TA, typename TB>
  arma_inline static void apply(Mat<float>& C, const TA& A, const TB& B,
                                const float alpha = float(1),
                                const float beta = float(0)) {
    gemm<do_trans_A, do_trans_B, use_alpha, use_beta>::apply_blas_type(C, A, B, alpha,
                                                                       beta);
  }

  template <typename TA, typename TB>
  arma_inline static void apply(Mat<double>& C, const TA& A, const TB& B,
  ...
  template <typename TA, typename TB>
  arma_inline static void apply(Mat<std::complex<float> >& C, const TA& A, const TB& B,
  ...
  arma_inline static void apply(
      Mat<std::complex<double> >& C, const TA& A, const TB& B,
