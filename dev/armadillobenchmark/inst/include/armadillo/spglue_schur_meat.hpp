...
inline void spglue_schur::apply(SpMat<typename T1::elem_type>& out,
                                const SpGlue<T1, T2, spglue_schur>& X) {
  arma_debug_sigprint();
...
inline void spglue_schur::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa,
                                        const SpProxy<T2>& pb) {
  arma_debug_sigprint();
  ...
  while ((x_it != x_end) || (y_it != y_end)) {
    ...
    if (x_it == y_it) {
      ...
      if ((x_it_col < y_it_col) ||
          ((x_it_col == y_it_col) && (x_it_row < y_it_row)))  // if y is closer to the end
      {
        ++x_it;
      } else {
        ++y_it;
    ...
    arma_check((count > max_n_nonzero),
               "internal error: spglue_schur::apply_noalias(): count > max_n_nonzero");
  }
...
inline void spglue_schur::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A,
                                        const SpMat<eT>& B) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void spglue_schur_misc::dense_schur_sparse(SpMat<typename T1::elem_type>& out,
  ...
  while (it != it_end) {
    ...
    arma_check(
        (count > max_n_nonzero),
        "internal error: spglue_schur_misc::dense_schur_sparse(): count > max_n_nonzero");
  }
...
template <typename T1, typename T2>
inline void spglue_schur_mixed::apply(
    SpMat<typename eT_promoter<T1, T2>::eT>& out,
    const mtSpGlue<typename eT_promoter<T1, T2>::eT, T1, T2, spglue_schur_mixed>& expr) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void spglue_schur_mixed::dense_schur_sparse(
  ...
  while (it != it_end) {
    if ((out_eT(*it) * out_eT(pa.at(it.row(), it.col()))) != out_eT(0)) {
      ++new_n_nonzero;
    }
