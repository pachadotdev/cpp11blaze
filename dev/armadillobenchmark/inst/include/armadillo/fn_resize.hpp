...
arma_warn_unused inline const Op<T1, op_resize> resize(
    const Base<typename T1::elem_type, T1>& X, const uword in_n_rows,
...
template <typename T1>
arma_warn_unused inline const Op<T1, op_resize> resize(
    const Base<typename T1::elem_type, T1>& X, const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline const OpCube<T1, op_resize> resize(
    const BaseCube<typename T1::elem_type, T1>& X, const uword in_n_rows,
...
template <typename T1>
arma_warn_unused inline const SpOp<T1, spop_resize> resize(
    const SpBase<typename T1::elem_type, T1>& X, const uword in_n_rows,
    const uword in_n_cols) {
  arma_debug_sigprint();
