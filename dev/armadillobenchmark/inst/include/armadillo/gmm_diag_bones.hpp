...
class gmm_diag {
 public:
  ...
  inline void set_params(const Base<eT, T1>& in_means, const Base<eT, T2>& in_dcovs,
  ...
  template <typename T1>
  inline eT log_p(
      const T1& expr, const gmm_empty_arg& junk1 = gmm_empty_arg(),
      typename enable_if<((is_arma_type<T1>::value) &&
                          (resolves_to_colvector<T1>::value == true))>::result* junk2 =
          nullptr) const;
  template <typename T1>
  inline eT log_p(
  ...
  inline Row<eT> log_p(
      const T1& expr, const gmm_empty_arg& junk1 = gmm_empty_arg(),
      typename enable_if<((is_arma_type<T1>::value) &&
                          (resolves_to_colvector<T1>::value == false))>::result* junk2 =
          nullptr) const;
  template <typename T1>
  inline Row<eT> log_p(
      const T1& expr, const uword gaus_id,
  ...
  inline uword assign(
      ...
      typename enable_if<((is_arma_type<T1>::value) &&
                          (resolves_to_colvector<T1>::value == true))>::result* junk =
          nullptr) const;
  template <typename T1>
  inline urowvec assign(
      const T1& expr, const gmm_dist_mode& dist,
      typename enable_if<((is_arma_type<T1>::value) &&
                          (resolves_to_colvector<T1>::value == false))>::result* junk =
          nullptr) const;
  ...
  template <typename T1>
  inline bool learn(const Base<eT, T1>& data, const uword n_gaus,
                    const gmm_dist_mode& dist_mode, const gmm_seed_mode& seed_mode,
                    const uword km_iter, const uword em_iter, const eT var_floor,
                    const bool print_mode);

  template <typename T1>
  inline bool kmeans_wrapper(Mat<eT>& user_means, const Base<eT, T1>& data,
                             const uword n_gaus, const gmm_seed_mode& seed_mode,
 ...
 protected:
  ...
  inline uword internal_scalar_assign(const Mat<eT>& X,
                                      const gmm_dist_mode& dist_mode) const;

  inline void internal_vec_assign(urowvec& out, const Mat<eT>& X,
                                  const gmm_dist_mode& dist_mode) const;

  inline void internal_raw_hist(urowvec& hist, const Mat<eT>& X,
                                const gmm_dist_mode& dist_mode) const;
  ...
  template <uword dist_id>
  inline bool km_iterate(const Mat<eT>& X, const uword max_iter, const bool verbose,
                         const char* signature);

  //

  inline bool em_iterate(const Mat<eT>& X, const uword max_iter, const eT var_floor,
                         const bool verbose);

  inline void em_update_params(const Mat<eT>& X, const umat& boundaries,
                               field<Mat<eT> >& t_acc_means, field<Mat<eT> >& t_acc_dcovs,
                               field<Col<eT> >& t_acc_norm_lhoods,
                               field<Col<eT> >& t_gaus_log_lhoods,
                               Col<eT>& t_progress_log_lhoods);

  inline void em_generate_acc(const Mat<eT>& X, const uword start_index,
                              const uword end_index, Mat<eT>& acc_means,
                              Mat<eT>& acc_dcovs, Col<eT>& acc_norm_lhoods,
                              Col<eT>& gaus_log_lhoods, eT& progress_log_lhood) const;
