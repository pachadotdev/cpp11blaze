...
template <typename T>
arma_warn_unused arma_inline typename arma_scalar_only<T>::result mean(const T& x) {
  return x;
}

template <typename T1>
arma_warn_unused arma_inline const OpCube<T1, op_mean> mean(
...
arma_warn_unused inline typename enable_if2<
    ...
    const mtSpReduceOp<typename T1::elem_type, T1, op_sp_mean> >::result
mean(const T1& x) {
  arma_debug_sigprint();
