...
arma_inline typename enable_if2<is_arma_type<T1>::value, const eOp<T1, eop_neg> >::result
operator-(const T1& X) {
  arma_debug_sigprint();
...
// arma_inline
// typename
// enable_if2< (is_arma_type<T1>::value && is_signed<typename T1::elem_type>::value),
// const eOp<T1, eop_neg> >::result operator- (const T1& X)
//   {
//   arma_debug_sigprint();
//
//   return eOp<T1,eop_neg>(X);
...
// template<typename T1>
// arma_inline
// typename enable_if2< (is_arma_type<T1>::value && (is_signed<typename
// T1::elem_type>::value == false)), const eOp<T1, eop_scalar_times> >::result operator-
// (const T1& X)
//   {
//   arma_debug_sigprint();
//
...
arma_inline typename enable_if2<is_arma_type<T1>::value,
                                const eOp<T1, eop_scalar_minus_post> >::result
...
template <typename T1>
arma_inline typename enable_if2<is_arma_type<T1>::value,
                                const eOp<T1, eop_scalar_minus_pre> >::result
...
arma_inline
    typename enable_if2<(is_arma_type<T1>::value && is_cx<typename T1::elem_type>::no),
                        const mtOp<typename std::complex<typename T1::pod_type>, T1,
                                   op_cx_scalar_minus_pre> >::result
    operator-(const std::complex<typename T1::pod_type>& k, const T1& X) {
  arma_debug_sigprint();

  return mtOp<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_minus_pre>(
      'j', X, k);
}
...
arma_inline
    typename enable_if2<(is_arma_type<T1>::value && is_cx<typename T1::elem_type>::no),
                        const mtOp<typename std::complex<typename T1::pod_type>, T1,
                                   op_cx_scalar_minus_post> >::result
    operator-(const T1& X, const std::complex<typename T1::pod_type>& k) {
  arma_debug_sigprint();

  return mtOp<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_minus_post>(
      'j', X, k);
}
...
inline typename enable_if2<is_arma_sparse_type<T1>::value &&
                               is_signed<typename T1::elem_type>::value,
...
//! sparse - scalar
template <typename T1>
arma_inline typename enable_if2<is_arma_sparse_type<T1>::value,
                                const SpToDOp<T1, op_sp_minus_post> >::result
operator-(const T1& X, const typename T1::elem_type k) {
  arma_debug_sigprint();
