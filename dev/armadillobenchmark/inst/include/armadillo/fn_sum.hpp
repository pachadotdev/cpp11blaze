...
template <typename T>
arma_warn_unused arma_inline typename arma_scalar_only<T>::result sum(const T& x) {
  return x;
}

//! sum of cube
template <typename T1>
arma_warn_unused arma_inline const OpCube<T1, op_sum> sum(
...
arma_warn_unused inline typename enable_if2<
    ...
    const mtSpReduceOp<typename T1::elem_type, T1, op_sp_sum> >::result
sum(const T1& x) {
  arma_debug_sigprint();
