...
inline void op_htrans::apply_mat_noalias(Mat<eT>& out, const Mat<eT>& A,
...
template <typename T>
inline void op_htrans::block_worker(std::complex<T>* Y, const std::complex<T>* X,
                                    const uword X_n_rows, const uword Y_n_rows,
...
template <typename T>
inline void op_htrans::apply_mat_noalias_large(Mat<std::complex<T> >& out,
  ...
  for (uword row = 0; row < n_rows_base; row += block_size) {
    ...
    for (uword col = 0; col < n_cols_base; col += block_size) {
      ...
      op_htrans::block_worker(&Y[col + Y_offset], &X[row + X_offset], n_rows, n_cols,
                              block_size, block_size);
    }

    const uword X_offset = n_cols_base * n_rows;

    op_htrans::block_worker(&Y[n_cols_base + Y_offset], &X[row + X_offset], n_rows,
  ...
  for (uword col = 0; col < n_cols_base; col += block_size) {
    ...
    op_htrans::block_worker(&Y[col + Y_offset], &X[n_rows_base + X_offset], n_rows,
                            n_cols, n_rows_extra, block_size);
  }

  const uword X_offset = n_cols_base * n_rows;

  op_htrans::block_worker(&Y[n_cols_base + Y_offset], &X[n_rows_base + X_offset], n_rows,
                          n_cols, n_rows_extra, n_cols_extra);
}

template <typename eT>
inline void op_htrans::apply_mat_inplace(Mat<eT>& out,
                                         const typename arma_not_cx<eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename eT>
inline void op_htrans::apply_mat(Mat<eT>& out, const Mat<eT>& A,
                                 const typename arma_not_cx<eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
inline void op_htrans::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_htrans::apply_direct(Mat<typename T1::elem_type>& out, const T1& X) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_htrans::apply(
    Mat<typename T1::elem_type>& out, const Op<T1, op_htrans>& in,
...
template <typename T1>
inline void op_htrans2::apply(
    Mat<typename T1::elem_type>& out, const Op<T1, op_htrans2>& in,
    const typename arma_not_cx<typename T1::elem_type>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
