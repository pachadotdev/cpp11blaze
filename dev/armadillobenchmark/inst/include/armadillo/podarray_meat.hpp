...
template <typename eT>
inline podarray<eT>::podarray() : n_elem(0), mem(0) {
  arma_debug_sigprint_this(this);
}

template <typename eT>
inline podarray<eT>::podarray(const podarray& x) : n_elem(x.n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline podarray<eT>::podarray(const uword new_n_elem) : n_elem(new_n_elem) {
  arma_debug_sigprint_this(this);
...
inline podarray<eT>::podarray(const uword new_n_elem,
...
template <typename eT>
inline void podarray<eT>::copy_row(const Mat<eT>& A, const uword row) {
  arma_debug_sigprint();
...
template <typename eT>
inline void podarray<eT>::init_cold(const uword new_n_elem) {
  arma_debug_sigprint();

  mem = (new_n_elem <= podarray_prealloc_n_elem::val) ? mem_local
                                                      : memory::acquire<eT>(new_n_elem);
}
