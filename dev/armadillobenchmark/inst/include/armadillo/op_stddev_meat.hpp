...
inline void op_stddev::apply_noalias(Mat<typename get_pod_type<in_eT>::result>& out,
...
template <typename T1>
inline typename T1::pod_type op_stddev::stddev_vec(
    const Base<typename T1::elem_type, T1>& X, const uword norm_type) {
  arma_debug_sigprint();
...
inline typename get_pod_type<eT>::result op_stddev::stddev_vec(const subview_col<eT>& X,
...
template <typename eT>
inline typename get_pod_type<eT>::result op_stddev::stddev_vec(const subview_row<eT>& X,
                                                               const uword norm_type) {
  arma_debug_sigprint();
