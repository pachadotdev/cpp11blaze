...
inline void glue_solve_gen_default::apply(Mat<typename T1::elem_type>& out,
                                          const Glue<T1, T2, glue_solve_gen_default>& X) {
  arma_debug_sigprint();
...
inline bool glue_solve_gen_default::apply(Mat<eT>& out, const Base<eT, T1>& A_expr,
                                          const Base<eT, T2>& B_expr) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void glue_solve_gen_full::apply(Mat<typename T1::elem_type>& out,
                                       const Glue<T1, T2, glue_solve_gen_full>& X) {
  arma_debug_sigprint();
...
inline bool glue_solve_gen_full::apply(Mat<eT>& actual_out, const Base<eT, T1>& A_expr,
                                       const Base<eT, T2>& B_expr, const uword flags) {
  arma_debug_sigprint();

  typedef typename get_pod_type<eT>::result T;

  if (has_user_flags == true) {
    arma_debug_print("glue_solve_gen_full::apply(): has_user_flags = true");
  }
  if (has_user_flags == false) {
    arma_debug_print("glue_solve_gen_full::apply(): has_user_flags = false");
  }
  ...
  if (has_user_flags) {
    ...
    if (equilibrate) {
      arma_debug_print("equilibrate");
    }
    if (no_approx) {
      arma_debug_print("no_approx");
    }
    if (no_band) {
      arma_debug_print("no_band");
    }
    if (no_sympd) {
      arma_debug_print("no_sympd");
    }
    if (allow_ugly) {
      arma_debug_print("allow_ugly");
    }
    if (likely_sympd) {
      arma_debug_print("likely_sympd");
    }
    if (refine) {
      arma_debug_print("refine");
    }
    if (no_trimat) {
      arma_debug_print("no_trimat");
    }
    if (force_approx) {
    ...
    arma_conform_check(
        (fast && equilibrate),
        "solve(): options 'fast' and 'equilibrate' are mutually exclusive");
    arma_conform_check((fast && refine),
                       "solve(): options 'fast' and 'refine' are mutually exclusive");
    arma_conform_check(
        (no_sympd && likely_sympd),
        "solve(): options 'no_sympd' and 'likely_sympd' are mutually exclusive");
  }
  ...
  if (force_approx) {
    ...
    arma_conform_check(
        no_approx,
        "solve(): options 'no_approx' and 'force_approx' are mutually exclusive");

    if (fast) {
      arma_warn(2, "solve(): option 'fast' ignored for forced approximate solution");
    }
    if (equilibrate) {
      arma_warn(2,
                "solve(): option 'equilibrate' ignored for forced approximate solution");
    }
    if (refine) {
      arma_warn(2, "solve(): option 'refine' ignored for forced approximate solution");
    }
    if (likely_sympd) {
      arma_warn(2,
  ...
  if (A.n_rows == A.n_cols) {
    ...
    if (fast) {
      ...
      if (is_band) {
        ...
        if (is_triu) {
          arma_debug_print(
              "glue_solve_gen_full::apply(): fast + upper triangular matrix");
        }
        if (is_tril) {
          arma_debug_print(
              "glue_solve_gen_full::apply(): fast + lower triangular matrix");
        }
        ...
        if (status == false) {
          ...
          arma_debug_print(
              "glue_solve_gen_full::apply(): auxlib::solve_sympd_fast() failed; "
              "retrying");

          A = A_expr.get_ref();

          status =
              auxlib::solve_square_fast(out, A, B_expr.get_ref());  // A is overwritten
        }
      } else {
      ...
      if (is_band) {
        ...
        if ((status == false) && (rcond == T(0))) {
          ...
          arma_debug_print(
              "glue_solve_gen_full::apply(): auxlib::solve_sympd_refine() failed; "
              "retrying");

          A = A_expr.get_ref();

          status = auxlib::solve_square_refine(out, rcond, A, B_expr.get_ref(),
                                               equilibrate);  // A is overwritten
        }
      } else {
        arma_debug_print("glue_solve_gen_full::apply(): refine + dense");

        status = auxlib::solve_square_refine(out, rcond, A, B_expr,
                                             equilibrate);  // A is overwritten
      }
    } else {
      ...
      if (is_band) {
        ...
        if (is_triu) {
          arma_debug_print(
              "glue_solve_gen_full::apply(): rcond + upper triangular matrix");
        }
        if (is_tril) {
          arma_debug_print(
              "glue_solve_gen_full::apply(): rcond + lower triangular matrix");
        }
        ...
        status = auxlib::solve_sympd_rcond(out, sympd_state, rcond, A,
                                           B_expr.get_ref());  // A is overwritten

        if ((status == false) && (sympd_state == false)) {
          arma_debug_print(
              "glue_solve_gen_full::apply(): auxlib::solve_sympd_rcond() failed; "
              "retrying");

          A = A_expr.get_ref();

          status = auxlib::solve_square_rcond(out, rcond, A,
                                              B_expr.get_ref());  // A is overwritten
        }
      } else {
        status = auxlib::solve_square_rcond(out, rcond, A,
    ...
    if (equilibrate) {
      arma_warn(2, "solve(): option 'equilibrate' ignored for non-square matrix");
    }
    if (refine) {
      arma_warn(2, "solve(): option 'refine' ignored for non-square matrix");
    }
    if (likely_sympd) {
    ...
    if (fast) {
      ...
      status =
          auxlib::solve_rect_rcond(out, rcond, A, B_expr.get_ref());  // A is overwritten
    }
  }

  if ((status == true) && (fast == false) && (allow_ugly == false) &&
      ((rcond < std::numeric_limits<T>::epsilon()) || arma_isnan(rcond))) {
    status = false;
  }
  ...
  if ((status == false) && (no_approx == false)) {
    ...
    if (rcond == T(0)) {
      ...
      arma_warn(2, "solve(): system is singular (rcond: ", rcond,
                "); attempting approx solution");
    }

    // TODO: conditionally recreate A: have a separate state flag which indicates whether
    // A was previously overwritten
...
template <typename T1, typename T2>
inline void glue_solve_tri_default::apply(Mat<typename T1::elem_type>& out,
                                          const Glue<T1, T2, glue_solve_tri_default>& X) {
  arma_debug_sigprint();
...
template <typename eT, typename T1, typename T2>
inline bool glue_solve_tri_default::apply(Mat<eT>& actual_out, const Base<eT, T1>& A_expr,
                                          const Base<eT, T2>& B_expr, const uword flags) {
  arma_debug_sigprint();
  ...
  if (triu) {
    arma_debug_print("triu");
  }
  if (tril) {
    arma_debug_print("tril");
  }
  ...
  arma_conform_check((A.is_square() == false),
                     "solve(): matrix marked as triangular must be square sized");
  ...
  status = auxlib::solve_trimat_rcond(out, rcond, A, B_expr.get_ref(),
                                      layout);  // A is not modified
  ...
  if (status == false) {
    ...
    Mat<eT> triA =
        (triu) ? trimatu(A) : trimatl(A);  // trimatu() and trimatl() return the same type

    status =
        auxlib::solve_approx_svd(out, triA, B_expr.get_ref());  // triA is overwritten
  }
...
template <typename T1, typename T2>
inline void glue_solve_tri_full::apply(Mat<typename T1::elem_type>& out,
                                       const Glue<T1, T2, glue_solve_tri_full>& X) {
  arma_debug_sigprint();
...
template <typename eT, typename T1, typename T2>
inline bool glue_solve_tri_full::apply(Mat<eT>& actual_out, const Base<eT, T1>& A_expr,
                                       const Base<eT, T2>& B_expr, const uword flags) {
  arma_debug_sigprint();
  ...
  if (fast) {
    status =
        auxlib::solve_trimat_fast(out, A, B_expr.get_ref(), layout);  // A is not modified
  } else {
    status = auxlib::solve_trimat_rcond(out, rcond, A, B_expr.get_ref(),
                                        layout);  // A is not modified
  }
