...
normcdf_helper(Mat<typename T1::elem_type>& out,
...
arma_warn_unused inline typename enable_if2<(is_real<eT>::value), eT>::result normcdf(
    const eT x) {
...
arma_warn_unused inline typename enable_if2<(is_real<typename T1::elem_type>::value),
                                            Mat<typename T1::elem_type> >::result
normcdf(const Base<typename T1::elem_type, T1>& X_expr, const typename T1::elem_type mu,
  ...
  normcdf_helper(out, X, mu * ones<Mat<eT> >(arma::size(X)),
