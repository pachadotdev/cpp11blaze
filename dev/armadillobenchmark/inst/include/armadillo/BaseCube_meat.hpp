...
template <typename elem_type, typename derived>
inline elem_type BaseCube<elem_type, derived>::min() const {
  return op_min::min((*this).get_ref());
}

template <typename elem_type, typename derived>
inline elem_type BaseCube<elem_type, derived>::max() const {
  return op_max::max((*this).get_ref());
}

template <typename elem_type, typename derived>
inline uword BaseCube<elem_type, derived>::index_min() const {
...
template <typename elem_type, typename derived>
inline uword BaseCube<elem_type, derived>::index_max() const {
  const ProxyCube<derived> P((*this).get_ref());
    ...
    for (uword s = 0; s < n_s; ++s)
      for (uword c = 0; c < n_c; ++c)
        for (uword r = 0; r < n_r; ++r) {
          if (arma_isfinite(P.at(r, c, s)) == false) {
            return false;
          }
        }
  }
...
template <typename elem_type, typename derived>
inline const CubeToMatOp<derived, op_row_as_mat> BaseCube<elem_type, derived>::row_as_mat(
    const uword in_row) const {
  return CubeToMatOp<derived, op_row_as_mat>((*this).get_ref(), in_row);
}

template <typename elem_type, typename derived>
inline const CubeToMatOp<derived, op_col_as_mat> BaseCube<elem_type, derived>::col_as_mat(
