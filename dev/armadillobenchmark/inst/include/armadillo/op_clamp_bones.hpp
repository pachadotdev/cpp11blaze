...
class op_clamp : public traits_op_passthru {
 public:
  // matrices
  ...
  inline static void apply_direct(Mat<eT>& out, const Mat<eT>& X, const eT min_val,
                                  const eT max_val);
  ...
  template <typename eT>
  inline static void apply_direct(Cube<eT>& out, const Cube<eT>& X, const eT min_val,
                                  const eT max_val);

  template <typename T1>
  inline static void apply_proxy_noalias(Cube<typename T1::elem_type>& out,
                                         const ProxyCube<T1>& P,
                                         const typename T1::elem_type min_val,
                                         const typename T1::elem_type max_val);
};

class op_clamp_cx : public traits_op_passthru {
 public:
  // matrices
