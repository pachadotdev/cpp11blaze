...
arma_warn_unused arma_inline
    typename enable_if2<is_arma_type<T1>::value && resolves_to_vector<T1>::no,
                        const mtOp<uword, T1, op_any> >::result
    any(const T1& X) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused arma_inline
    typename enable_if2<is_arma_type<T1>::value, const mtOp<uword, T1, op_any> >::result
...
//! for compatibility purposes: allows compiling user code designed for earlier versions
//! of Armadillo
template <typename T>
arma_warn_unused arma_inline
    typename enable_if2<is_supported_elem_type<T>::value, bool>::result
    any(const T& val) {
  return (val != T(0));
}
