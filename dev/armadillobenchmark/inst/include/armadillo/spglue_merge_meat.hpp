...
inline void spglue_merge::subview_merge(SpSubview<eT>& sv, const SpMat<eT>& B) {
  arma_debug_sigprint();
  ...
  while (x_it_valid || y_it_valid) {
    ...
    if (x_it_valid && y_it_valid) {
      if ((x_it_row == y_it_row) && (x_it_col == y_it_col)) {
        ...
        {
          const bool x_inside_box =
              ((x_it_row >= sv_row_start) && (x_it_row <= sv_row_end)) &&
              ((x_it_col >= sv_col_start) && (x_it_col <= sv_col_end));
  ...
  arma_check((count != merge_n_nonzero),
...
template <typename eT>
inline void spglue_merge::subview_merge(SpSubview<eT>& sv, const Mat<eT>& B) {
  arma_debug_sigprint();
...
template <typename eT>
inline void spglue_merge::symmat_merge(SpMat<eT>& out, const SpMat<eT>& A,
                                       const SpMat<eT>& B) {
  arma_debug_sigprint();
...
template <typename eT>
inline void spglue_merge::diagview_merge(SpMat<eT>& out, const SpMat<eT>& A,
                                         const SpMat<eT>& B) {
  arma_debug_sigprint();
  ...
  while ((x_it != x_end) || (y_it != y_end)) {
    ...
    if (x_it == y_it) {
      ...
      if ((x_it_col < y_it_col) ||
          ((x_it_col == y_it_col) && (x_it_row < y_it_row)))  // if y is closer to the end
      {
        if (x_it_col != x_it_row) {
          out_val = (*x_it);
        }  // don't take values from the main diagonal of A

        ++x_it;
      } else {
        if (y_it_col == y_it_row) {
          out_val = (*y_it);
          use_y_loc = true;
        }  // take values only from the main diagonal of B
