...
class op_normalise_vec : public traits_op_passthru {
 public:
...
class op_normalise_mat : public traits_op_default {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out,
  ...
  inline static void apply(Mat<eT>& out, const Mat<eT>& A, const uword p,
                           const uword dim);
};
