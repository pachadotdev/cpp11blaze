...
arma_inline const BaseCube<typename T1::elem_type, T1>& operator+(
...
//! BaseCube + scalar
template <typename T1>
arma_inline const eOpCube<T1, eop_scalar_plus> operator+(
...
template <typename T1>
arma_inline const
    mtOpCube<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_plus>
    operator+(const BaseCube<typename T1::pod_type, T1>& X,
              const std::complex<typename T1::pod_type>& k) {
  arma_debug_sigprint();

  return mtOpCube<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_plus>(
      'j', X.get_ref(), k);
}
...
arma_inline const
  ...
  return mtOpCube<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_plus>(
      'j', X.get_ref(), k);  // NOTE: order is swapped
}

//! addition of BaseCube objects with same element type
template <typename T1, typename T2>
arma_inline const eGlueCube<T1, T2, eglue_plus> operator+(
    const BaseCube<typename T1::elem_type, T1>& X,
