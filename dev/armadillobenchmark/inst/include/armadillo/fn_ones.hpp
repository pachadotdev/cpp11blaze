...
arma_warn_unused arma_inline const Gen<obj_type, gen_ones> ones(
...
template <typename obj_type>
arma_warn_unused inline const Gen<obj_type, gen_ones> ones(
  ...
  if (is_Col<obj_type>::value) {
    arma_conform_check((n_cols != 1), "ones(): incompatible size");
  }
  if (is_Row<obj_type>::value) {
    arma_conform_check((n_rows != 1), "ones(): incompatible size");
  }
...
template <typename cube_type>
arma_warn_unused arma_inline const GenCube<typename cube_type::elem_type, gen_ones> ones(
    const SizeCube& s, const typename arma_Cube_only<cube_type>::result* junk = nullptr) {
  arma_debug_sigprint();
  arma_ignore(junk);
