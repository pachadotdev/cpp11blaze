...
template <typename eT, typename T1>
class subview_cube_slices;

template <typename eT, typename T1>
class SpSubview_col_list;
...
#pragma omp atomic read
  ...
  arma_inline void operator=(const int in_state) {
#if defined(ARMA_USE_OPENMP)
...
struct arma_initmode_indicator {};
...
enum struct file_type : unsigned int {
  ...
  arma_binary,  //!< Armadillo binary format (machine dependent), with a header specifying
  ...
  hdf5_binary,  //!< HDF5: open binary format, not specific to Armadillo, which can store
                //!< arbitrary data
  hdf5_binary_trans,  //!< [NOTE: DO NOT USE - deprecated] as per hdf5_binary, but
                      //!< save/load the data with columns transposed to rows
  coord_ascii,  //!< simple co-ordinate format for sparse matrices (indices start at zero)
  ssv_ascii,    //!< similar to csv_ascii; uses semicolon (;) instead of comma (,) as the
                //!< separator
};
