...
class op_logmat : public traits_op_default {
 public:
...
class op_logmat_cx : public traits_op_default {
 public:
  template <typename T1>
  ...
  template <typename T1>
  inline static bool apply_direct_noalias(Mat<typename T1::elem_type>& out,
  ...
  template <typename T>
  inline static bool apply_common(Mat<std::complex<T> >& out, Mat<std::complex<T> >& S,
                                  const uword n_iters);
...
class op_logmat_sympd : public traits_op_default {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out,
