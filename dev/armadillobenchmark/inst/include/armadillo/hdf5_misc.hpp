...
//! (taken from all the possible typedefs of {u8, s8, ..., u64, s64} and the other native
//! types. We can't use the actual u8/s8 typedefs because their relations to the H5T_...
//! types are unclear.
template <>
...
inline herr_t hdf5_search_callback(hid_t loc_id, const char* name, const H5O_info_t* info,
  ...
  if (info->type == H5O_TYPE_DATASET) {
    ...
    for (size_t string_pos = 0; string_pos < search_info->best_match_position;
      ...
      // and then simply take the last substring of name containing that number of
      // slashes, we can do the comparison.
      ...
      // If they are not the same, but we have not found anything and we don't need an
      // exact match, take this.
...
//! If 'exact' is true, failure to find a dataset in the list of names means that -1 is
//! returned. If 'exact' is false and no datasets are found, -1 is returned. The number of
//! dimensions is used to help prune down invalid datasets; 2 dimensions is a matrix, 1
//! dimension is a vector, and 3 dimensions is a cube. If the number of dimensions in a
//! dataset is less than or equal to num_dims, it will be considered -- for instance, a
//! one-dimensional HDF5 vector can be loaded as a single-column matrix.
inline hid_t search_hdf5_file(const std::vector<std::string>& names, hid_t hdf5_file,
                              int num_dims = 2, bool exact = false) {
  hdf5_search_info search_info = {names, num_dims, exact, -1, names.size()};

  // We'll use the H5Ovisit to track potential entries.
  herr_t status = H5Ovisit(hdf5_file, H5_INDEX_NAME, H5_ITER_NATIVE, hdf5_search_callback,
                           void_ptr(&search_info));
...
inline hid_t load_and_convert_hdf5(eT* dest, hid_t dataset, hid_t datatype,
  ...
  if (is_equal) {
    ...
    hid_t status =
        H5Dread(dataset, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, void_ptr(v.memptr()));
