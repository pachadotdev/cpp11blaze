...
class glue_histc : public traits_glue_default {
 public:
  template <typename eT>
  inline static void apply_noalias(Mat<uword>& C, const Mat<eT>& A, const Mat<eT>& B,
                                   const uword dim);
...
class glue_histc_default {
 public:
  ...
  template <typename T1, typename T2>
  inline static void apply(Mat<uword>& C,
