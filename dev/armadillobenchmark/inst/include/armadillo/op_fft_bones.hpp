...
class op_fft_real : public traits_op_passthru {
 public:
...
class op_fft_cx : public traits_op_passthru {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out, const Op<T1, op_fft_cx>& in);

  template <typename eT, bool inverse>
  inline static void apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword a,
                                   const uword b);
};

class op_ifft_cx : public traits_op_passthru {
 public:
  template <typename T1>
