...
//! Matrix 'C' is assumed to have been set to the correct size (ie. taking into account
//! transposes)
 ...
 public:
  ...
  arma_hot inline static void apply(Mat<out_eT>& C, const Mat<in_eT1>& A,
    ...
    if ((do_trans_A == false) && (do_trans_B == false)) {
      ...
      const bool use_mp = (B_n_cols >= 2) && (B.n_elem >= 8192) &&
                          (mp_thread_limit::in_parallel() == false);
#else
      ...
      if (use_mp) {
        ...
        {
          const int n_threads =
              int((std::min)(uword(mp_thread_limit::get()), uword(B_n_cols)));
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
      ...
      gemm_mixed_large<false, false, use_alpha, use_beta>::apply(C, A, B_tmp, alpha,
                                                                 beta);
  ...
  inline static void apply(Mat<out_eT>& C, const Mat<in_eT1>& A, const Mat<in_eT2>& B,
    ...
    if ((is_cx<in_eT1>::yes && do_trans_A) || (is_cx<in_eT2>::yes && do_trans_B)) {
      ...
      gemm_mixed_large<((predo_trans_A) ? false : do_trans_A),
                       ((predo_trans_B) ? false : do_trans_B), use_alpha,
                       use_beta>::apply(C, AA, BB, alpha, beta);
    } else {
      gemm_mixed_large<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(C, A, B, alpha,
                                                                           beta);
    }
  }
};
