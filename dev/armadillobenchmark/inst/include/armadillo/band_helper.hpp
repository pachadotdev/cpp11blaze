...
inline bool is_band(uword& out_KL, uword& out_KU, const Mat<eT>& A, const uword N_min) {
  ...
  if ((A_col0[N - 2] != eT_zero) || (A_col0[N - 1] != eT_zero) ||
      (A_col1[N - 2] != eT_zero) || (A_col1[N - 1] != eT_zero)) {
    return false;
  }
  ...
  if ((A_colNm2[0] != eT_zero) || (A_colNm2[1] != eT_zero) || (A_colNm1[0] != eT_zero) ||
      (A_colNm1[1] != eT_zero)) {
    return false;
  }

  // if we reached this point, go through the entire matrix to work out number of
  ...
  for (uword col = 0; col < N; ++col) {
    ...
    if ((L_count > KL) || (U_count > KU)) {
      ...
      if (n_nonzero > n_nonzero_threshold) {
        return false;
      }
    }
...
inline bool is_band_lower(uword& out_KD, const Mat<eT>& A, const uword N_min) {
  ...
  // if we reached this point, go through the bottom triangle to work out number of
  // subdiagonals

  const uword n_nonzero_threshold =
      (N * N - (N * (N - 1)) / 2) / 4;  // empirically determined
...
inline void compress(Mat<eT>& AB, const Mat<eT>& A, const uword KL, const uword KU,
  ...
  // KL+1 to 2*KL+KU+1  (note: fortran counts from 1) for ?gbsvx, matrix AB size: KL+KU+1
  // x N; band representaiton of A stored in rows    1 to   KL+KU+1  (note: fortran counts
  // from 1)
  //
