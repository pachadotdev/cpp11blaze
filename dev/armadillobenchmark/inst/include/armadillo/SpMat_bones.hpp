...
class SpMat : public SpBase<eT, SpMat<eT> > {
 public:
  ...
  inline SpMat(const Base<uword, T1>& rowind, const Base<uword, T2>& colptr,
               const Base<eT, T3>& values, const uword n_rows, const uword n_cols,
  ...
  template <typename T1, typename T2>
  inline SpMat(const bool add_values, const Base<uword, T1>& locations,
               const Base<eT, T2>& values, const uword n_rows, const uword n_cols,
  ...
  inline SpMat& operator/=(const eT val);
  // operator+=(val) and operator-=(val) are not defined as they don't make sense for
  // sparse matrices
  ...
  template <typename T1>
  inline explicit SpMat(const Op<T1, op_diagmat>& expr);
  template <typename T1>
  inline SpMat& operator=(const Op<T1, op_diagmat>& expr);
  template <typename T1>
  inline SpMat& operator+=(const Op<T1, op_diagmat>& expr);
  template <typename T1>
  inline SpMat& operator-=(const Op<T1, op_diagmat>& expr);
  ...
  template <typename T1>
  inline SpMat(const SpSubview_col_list<eT, T1>& X);
  template <typename T1>
  inline SpMat& operator=(const SpSubview_col_list<eT, T1>& X);
  template <typename T1>
  inline SpMat& operator+=(const SpSubview_col_list<eT, T1>& X);
  template <typename T1>
  inline SpMat& operator-=(const SpSubview_col_list<eT, T1>& X);
  ...
  template <typename T1, typename T2, typename spglue_type>
  inline SpMat(const mtSpGlue<eT, T1, T2, spglue_type>& X);
  template <typename T1, typename T2, typename spglue_type>
  inline SpMat& operator=(const mtSpGlue<eT, T1, T2, spglue_type>& X);
  template <typename T1, typename T2, typename spglue_type>
  inline SpMat& operator+=(const mtSpGlue<eT, T1, T2, spglue_type>& X);
  template <typename T1, typename T2, typename spglue_type>
  inline SpMat& operator-=(const mtSpGlue<eT, T1, T2, spglue_type>& X);
  ...
  inline SpMat(const mtSpReduceOp<eT, T1, op_type>& X);
  template <typename T1, typename op_type>
  inline SpMat& operator=(const mtSpReduceOp<eT, T1, op_type>& X);
  template <typename T1, typename op_type>
  inline SpMat& operator+=(const mtSpReduceOp<eT, T1, op_type>& X);
  template <typename T1, typename op_type>
  inline SpMat& operator-=(const mtSpReduceOp<eT, T1, op_type>& X);
  ...
  inline const SpSubview_row<eT> operator()(const uword row_num,
  ...
  inline const SpSubview_col<eT> operator()(const span& row_span,
  ...
  arma_inline SpSubview<eT> submat(const uword in_row1, const uword in_col1,
                                   const uword in_row2, const uword in_col2);
  arma_inline const SpSubview<eT> submat(const uword in_row1, const uword in_col1,
  ...
  arma_inline SpSubview<eT> operator()(const uword in_row1, const uword in_col1,
                                       const SizeMat& s);
  arma_inline const SpSubview<eT> operator()(const uword in_row1, const uword in_col1,
                                             const SizeMat& s) const;
  ...
  template <typename T1>
  arma_inline SpSubview_col_list<eT, T1> cols(const Base<uword, T1>& ci);
  template <typename T1>
  arma_inline const SpSubview_col_list<eT, T1> cols(const Base<uword, T1>& ci) const;
...
// access the element at the given row and column; if there is nothing at that position, 0
// is returned
#if defined(__cpp_multidimensional_subscript)
  arma_warn_unused arma_inline SpMat_MapMat_val<eT> operator[](const uword in_row,
                                                               const uword in_col);
  arma_warn_unused arma_inline eT operator[](const uword in_row,
                                             const uword in_col) const;
#endif

  arma_warn_unused arma_inline SpMat_MapMat_val<eT> at(const uword in_row,
                                                       const uword in_col);
  ...
  arma_warn_unused inline bool is_symmetric(
      const typename get_pod_type<eT>::result tol) const;

  arma_warn_unused inline bool is_hermitian() const;
  arma_warn_unused inline bool is_hermitian(
      const typename get_pod_type<eT>::result tol) const;
  ...
  inline SpMat& copy_size(const SpMat<eT2>& m);
  template <typename eT2>
  inline SpMat& copy_size(const Mat<eT2>& m);
  ...
  inline void reshape_helper_generic(const uword in_rows,
                                     const uword in_cols);  //! internal use only
  inline void reshape_helper_intovec();                     //! internal use only

  template <typename functor>
  inline SpMat& for_each(functor F);
  template <typename functor>
  inline const SpMat& for_each(functor F) const;

  template <typename functor>
  inline SpMat& transform(functor F);
  ...
  inline void reserve(const uword in_rows, const uword in_cols,
                      const uword new_n_nonzero);

  //! don't use this unless you're writing internal Armadillo code
  inline SpMat(const arma_reserve_indicator&, const uword in_rows, const uword in_cols,
               const uword new_n_nonzero);
  ...
  arma_frown("use load() instead") inline bool quiet_load(
      const std::string name, const file_type type = arma_binary);
  arma_frown("use load() instead") inline bool quiet_load(
      std::istream& is, const file_type type = arma_binary);
  ...
  class const_iterator : public iterator_base {
   public:
    ...
    inline const_iterator(const SpMat& in_M,
                          uword initial_pos = 0);  // assumes initial_pos is valid
    inline const_iterator(
        const SpMat& in_M, uword in_row,
        uword in_col);  // iterator will be at the first nonzero value after the given
                        // position (using forward columnwise traversal)
    inline const_iterator(const SpMat& in_M, uword in_row, uword in_col,
                          uword in_pos);  // if the exact position of the iterator is
                                          // known; in_row is a dummy argument
    ...
    arma_hot inline bool operator==(
        const typename SpSubview<eT>::const_iterator& rhs) const;
    arma_hot inline bool operator!=(
        const typename SpSubview<eT>::const_iterator& rhs) const;
    ...
    arma_hot inline bool operator==(
        const typename SpSubview<eT>::const_row_iterator& rhs) const;
    arma_hot inline bool operator!=(
        const typename SpSubview<eT>::const_row_iterator& rhs) const;
  };
  ...
  class const_row_iterator : public iterator_base {
   public:
    ...
    uword actual_pos;  // hold the true position we are at in the matrix, as column-major
                       // indexing
  ...
  // correctly set. If the new size is smaller, the first new_n_nonzero elements will be
  // copied. n_nonzero is updated.
  ...
  inline void init_xform(const SpBase<eT, T1>& x, const Functor& func);
  template <typename eT2, typename T1, typename Functor>
  inline void init_xform_mt(const SpBase<eT2, T1>& x, const Functor& func);
 ...
 protected:
  ...
  arma_cold inline void init_cold(uword in_rows, uword in_cols,
                                  const uword new_n_nonzero = 0);
  ...
  inline void init_batch_std(const Mat<uword>& locations, const Mat<eT>& values,
                             const bool sort_locations);
  inline void init_batch_add(const Mat<uword>& locations, const Mat<eT>& values,
                             const bool sort_locations);

  inline SpMat(const arma_vec_indicator&, const uword in_vec_state);
  inline SpMat(const arma_vec_indicator&, const uword in_n_rows, const uword in_n_cols,
               const uword in_vec_state);

 private:
  arma_warn_unused arma_hot inline const eT* find_value_csc(const uword in_row,
                                                            const uword in_col) const;

  arma_warn_unused arma_hot inline eT get_value(const uword i) const;
  arma_warn_unused arma_hot inline eT get_value(const uword in_row,
                                                const uword in_col) const;

  arma_warn_unused arma_hot inline eT get_value_csc(const uword i) const;
  arma_warn_unused arma_hot inline eT get_value_csc(const uword in_row,
                                                    const uword in_col) const;

  arma_warn_unused arma_hot inline bool try_set_value_csc(const uword in_row,
                                                          const uword in_col,
                                                          const eT in_val);
  arma_warn_unused arma_hot inline bool try_add_value_csc(const uword in_row,
                                                          const uword in_col,
                                                          const eT in_val);
  arma_warn_unused arma_hot inline bool try_sub_value_csc(const uword in_row,
                                                          const uword in_col,
                                                          const eT in_val);
  arma_warn_unused arma_hot inline bool try_mul_value_csc(const uword in_row,
                                                          const uword in_col,
                                                          const eT in_val);
  arma_warn_unused arma_hot inline bool try_div_value_csc(const uword in_row,
                                                          const uword in_col,
                                                          const eT in_val);

  arma_warn_unused inline eT& insert_element(const uword in_row, const uword in_col,
                                             const eT in_val = eT(0));
  ...
  friend class SpValProxy<SpMat<eT> >;  // allow SpValProxy to call insert_element() and
                                        // delete_element()
  ...
  friend class SpSubview_col_list;
