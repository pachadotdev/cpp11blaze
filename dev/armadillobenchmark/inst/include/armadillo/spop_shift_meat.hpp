...
inline void spop_shift::apply_noalias(SpMat<eT>& out, const SpMat<eT>& X, const uword len,
                                      const uword neg, const uword dim) {
  arma_debug_sigprint();

  arma_conform_check_bounds(((dim == 0) && (len >= X.n_rows)),
                            "shift(): shift amount out of bounds");
  arma_conform_check_bounds(((dim == 1) && (len >= X.n_cols)),
                            "shift(): shift amount out of bounds");
  ...
  if (dim == 0) {
    ...
    for (; it != it_end; ++it) {
      ...
      if (neg == 0) {
        Y_row = (X_row < X_row_threshold) ? (X_row + len) : (X_row - X_row_threshold);
      } else if (neg == 1) {
        Y_row = (X_row >= len) ? (X_row - len) : (X_row + X_row_threshold);
      }
    ...
    for (; it != it_end; ++it) {
      ...
      if (neg == 0) {
        Y_col = (X_col < X_col_threshold) ? (X_col + len) : (X_col - X_col_threshold);
      } else if (neg == 1) {
        Y_col = (X_col >= len) ? (X_col - len) : (X_col + X_col_threshold);
      }
