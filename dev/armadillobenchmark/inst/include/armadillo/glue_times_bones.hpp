...
//! calculates the number of Glue<Tx,Ty, glue_type> instances on the left hand side
//! argument of Glue<Tx,Ty, glue_type> ie. it recursively expands each Tx, until the type
//! of Tx is not "Glue<..,.., glue_type>"  (i.e the "glue_type" changes)
...
class glue_times {
 public:
  ...
  arma_hot inline static void apply_inplace(Mat<typename T1::elem_type>& out,
                                            const T1& X);
  ...
  template <typename eT, const bool do_trans_A, const bool do_trans_B,
            const bool do_scalar_times, typename TA, typename TB>
  arma_hot inline static void apply(Mat<eT>& out, const TA& A, const TB& B, const eT val);

  template <typename eT, const bool do_trans_A, const bool do_trans_B,
            const bool do_trans_C, const bool do_scalar_times, typename TA, typename TB,
            typename TC>
  arma_hot inline static void apply(Mat<eT>& out, const TA& A, const TB& B, const TC& C,
                                    const eT val);

  template <typename eT, const bool do_trans_A, const bool do_trans_B,
            const bool do_trans_C, const bool do_trans_D, const bool do_scalar_times,
            typename TA, typename TB, typename TC, typename TD>
  arma_hot inline static void apply(Mat<eT>& out, const TA& A, const TB& B, const TC& C,
                                    const TD& D, const eT val);
};
