...
// For example, if we want to choose the eigenvalues with largest magnitude, the target
// will be -std::abs(x). The minus sign is due to the fact that std::sort() sorts in
// ascending order.
...
class SortEigenvalue {
 private:
  typedef typename get_pod_type<eT>::result
      TargetType;  // type of the sorting target, will be a floating number type, eg.
                   // double
  typedef std::pair<TargetType, uword>
      PairType;  // type of the sorting pair, including the sorting target and the index
