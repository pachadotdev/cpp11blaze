...
class op_sort_index : public traits_op_col {
 public:
  template <typename T1>
  static inline bool apply_noalias(Mat<uword>& out, const Proxy<T1>& P,
...
class op_stable_sort_index : public traits_op_col {
 public:
  template <typename T1>
  static inline bool apply_noalias(Mat<uword>& out, const Proxy<T1>& P,
                                   const uword sort_type);
  ...
  template <typename T1>
  static inline void apply(Mat<uword>& out,
...
struct arma_sort_index_helper_ascend {
  arma_inline bool operator()(const arma_sort_index_packet<eT>& A,
                              const arma_sort_index_packet<eT>& B) const {
    return (A.val < B.val);
  }
};
...
struct arma_sort_index_helper_ascend<std::complex<T> > {
  ...
  // inline
  // bool
  // operator() (const arma_sort_index_packet<eT>& A, const arma_sort_index_packet<eT>& B)
  // const
  //   {
  //   const T abs_A_val = std::abs(A.val);
  //   const T abs_B_val = std::abs(B.val);
  //
  //   return ( (abs_A_val != abs_B_val) ? (abs_A_val < abs_B_val) : (std::arg(A.val) <
  //   std::arg(B.val)) );
  //   }
};
