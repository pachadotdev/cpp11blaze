...
class op_vecnorm : public traits_op_xvec {
 public:
  ...
  inline static void apply_noalias(Mat<typename get_pod_type<in_eT>::result>& out,
                                   const Mat<in_eT>& X, const uword k, const uword dim);

  template <typename in_eT>
  inline static void apply_rawmem(typename get_pod_type<in_eT>::result& out_val,
                                  const in_eT* mem, const uword N, const uword k);
};

class op_vecnorm_ext : public traits_op_xvec {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::pod_type>& out,
  ...
  template <typename in_eT>
  inline static void apply_rawmem(typename get_pod_type<in_eT>::result& out_val,
                                  const in_eT* mem, const uword N, const uword method_id);
};
