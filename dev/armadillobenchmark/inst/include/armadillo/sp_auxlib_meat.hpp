...
inline sp_auxlib::form_type sp_auxlib::interpret_form_str(const char* form_str) {
  arma_debug_sigprint();
...
inline bool sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X,
                                const uword n_eigvals, const form_type form_val,
  ...
  arma_conform_check((U.M.is_square() == false),
                     "eigs_sym(): given matrix must be square sized");

  if ((arma_config::check_conform) && (sp_auxlib::rudimentary_sym_check(U.M) == false)) {
    if (is_cx<eT>::no) {
      arma_warn(1, "eigs_sym(): given matrix is not symmetric");
    }
    if (is_cx<eT>::yes) {
      arma_warn(1, "eigs_sym(): given matrix is not hermitian");
    }
  }
  ...
  // TODO: investigate optional redirection of "sm" to ARPACK as it's capable of
  // shift-invert;
  // TODO: in shift-invert mode, "sm" maps to "lm" of the shift-inverted matrix (with
  // sigma = 0)
...
#elif defined(ARMA_USE_ARPACK)
  {
    ...
    return sp_auxlib::eigs_sym_arpack<eT, false>(eigval, eigvec, U.M, n_eigvals, form_val,
                                                 sigma, opts);
  }
#else
  {
...
template <typename eT, typename T1>
inline bool sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X,
                                const uword n_eigvals, const eT sigma,
                                const eigs_opts& opts) {
  arma_debug_sigprint();
...
#elif (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    return sp_auxlib::eigs_sym_arpack<eT, true>(eigval, eigvec, U.M, n_eigvals, form_val,
                                                sigma, opts);
  }
#else
  {
...
inline bool sp_auxlib::eigs_sym_newarp(Col<eT>& eigval, Mat<eT>& eigvec,
                                       const SpMat<eT>& X, const uword n_eigvals,
                                       const form_type form_val, const eigs_opts& opts) {
  arma_debug_sigprint();

#if defined(ARMA_USE_NEWARP)
  {
    arma_conform_check((form_val != form_lm) && (form_val != form_sm) &&
                           (form_val != form_la) && (form_val != form_sa),
                       "eigs_sym(): unknown form specified");
    ...
    arma_conform_check(
        (n_eigvals >= op.n_rows),
        "eigs_sym(): n_eigvals must be less than the number of rows in the matrix");
    ...
    // Use max(2*k+1, 20) as default subspace dimension for the sym case; MATLAB uses
    // max(2*k, 20), but we need to be backward-compatible.
    ...
    if (opts.subdim != 0) {
      if (opts.subdim < (n_eigvals + 1)) {
        arma_warn(1,
                  "eigs_sym(): opts.subdim must be greater than k; using k+1 instead of ",
                  opts.subdim);
        ncv = uword(n_eigvals + 1);
      } else if (opts.subdim > n) {
        arma_warn(1,
                  "eigs_sym(): opts.subdim cannot be greater than n_rows; using n_rows "
                  "instead of ",
                  opts.subdim);
        ncv = n;
      } else {
...
inline bool sp_auxlib::eigs_sym_newarp(Col<eT>& eigval, Mat<eT>& eigvec,
                                       const SpMat<eT>& X, const uword n_eigvals,
                                       const eT sigma, const eigs_opts& opts) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_NEWARP)
  {
    ...
    try {
      newarp::SymEigsShiftSolver<eT, newarp::EigsSelect::LARGEST_MAGN,
                                 newarp::SparseGenRealShiftSolve<eT>>
          eigs(op, n_eigvals, ncv, sigma);
      eigs.init();
      nconv = eigs.compute(maxiter, tol);
      eigval = eigs.eigenvalues();
      eigvec = eigs.eigenvectors();
...
inline bool sp_auxlib::eigs_sym_arpack(Col<eT>& eigval, Mat<eT>& eigvec,
                                       const SpMat<eT>& X, const uword n_eigvals,
                                       const form_type form_val, const eT sigma,
                                       const eigs_opts& opts) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // The _saupd() functions allow asking for one more eigenvalue than the _naupd()
    // functions.
    arma_conform_check(
        (n_eigvals >= X.n_rows),
        "eigs_sym(): n_eigvals must be less than the number of rows in the matrix");
    ...
    {
      run_aupd_shiftinvert(n_eigvals, sigma, X, true /* sym, not gen */, n, tol, maxiter,
                           resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork,
                           info);
    } else {
      const SpMat<eT> Xst = X.st();

      run_aupd_plain(n_eigvals, which, X, Xst, true /* sym, not gen */, n, tol, maxiter,
                     resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork,
                     info);
    }
    ...
    // The process has converged, and now we need to recover the actual eigenvectors using
    // seupd()
    ...
    podarray<blas_int> select(ncv,
                              arma_zeros_indicator());  // Logical array of dimension NCV.
    blas_int ldz = n;
    ...
    arpack::seupd(&rvec, &howmny, select.memptr(), eigval.memptr(), eigvec.memptr(), &ldz,
                  (eT*)&sigma, &bmat, &n, which, &nev, &tol, resid.memptr(), &ncv,
                  v.memptr(), &ldv, iparam.memptr(), ipntr.memptr(), workd.memptr(),
                  workl.memptr(), &lworkl, &info);

    // Check for errors.
    if (info != 0) {
      arma_warn(1, "eigs_sym(): ARPACK error ", info, " in seupd()");
      return false;
    }
...
inline bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval,
                                Mat<std::complex<T>>& eigvec, const SpBase<T, T1>& X,
                                const uword n_eigvals, const form_type form_val,
                                const eigs_opts& opts) {
  arma_debug_sigprint();
  ...
  arma_conform_check((U.M.is_square() == false),
                     "eigs_gen(): given matrix must be square sized");
...
#elif defined(ARMA_USE_ARPACK)
  {
    ...
    return sp_auxlib::eigs_gen_arpack<T, false>(eigval, eigvec, U.M, n_eigvals, form_val,
                                                sigma, opts);
  }
#else
  {
...
inline bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval,
                                Mat<std::complex<T>>& eigvec, const SpBase<T, T1>& X,
                                const uword n_eigvals, const std::complex<T> sigma,
                                const eigs_opts& opts) {
  arma_debug_sigprint();
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    return sp_auxlib::eigs_gen_arpack<T, true>(eigval, eigvec, U.M, n_eigvals, form_val,
                                               sigma, opts);
  }
#else
  {
    ...
    arma_stop_logic_error(
        "eigs_gen(): use of ARPACK and SuperLU must be enabled to use 'sigma'");
    return false;
  }
#endif
}
...
inline bool sp_auxlib::eigs_gen_newarp(Col<std::complex<T>>& eigval,
                                       Mat<std::complex<T>>& eigvec, const SpMat<T>& X,
                                       const uword n_eigvals, const form_type form_val,
                                       const eigs_opts& opts) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_NEWARP)
  {
    arma_conform_check((form_val != form_lm) && (form_val != form_sm) &&
                           ...
                           (form_val != form_li) && (form_val != form_si),
                       "eigs_gen(): unknown form specified");
    ...
    if (opts.subdim != 0) {
      if (opts.subdim < (n_eigvals + 3)) {
        arma_warn(
            1, "eigs_gen(): opts.subdim must be greater than k+2; using k+3 instead of ",
            opts.subdim);
        ncv = uword(n_eigvals + 3);
      } else if (opts.subdim > n) {
        arma_warn(1,
                  "eigs_gen(): opts.subdim cannot be greater than n_rows; using n_rows "
                  "instead of ",
                  opts.subdim);
        ncv = n;
      } else {
...
inline bool sp_auxlib::eigs_gen_arpack(Col<std::complex<T>>& eigval,
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // the user-facing functions currently convert X from real to complex if a complex
    // sigma is detected. The check here is just for extra safety, and as a reminder of
    // what's missing.
    T sigmar = real(sigma);
    T sigmai = imag(sigma);
    ...
    // if(form_val == form_sigma)
    {
      if (sigmai != T(0)) {
        arma_stop_logic_error(
            "eigs_gen(): complex 'sigma' not applicable to real matrix");
        return false;
      }

      run_aupd_shiftinvert(n_eigvals, sigmar, X, false /* gen, not sym */, n, tol,
                           maxiter, resid, ncv, v, ldv, iparam, ipntr, workd, workl,
                           lworkl, rwork, info);
    } else {
      const SpMat<T> Xst = X.st();

      run_aupd_plain(n_eigvals, which, X, Xst, false /* gen, not sym */, n, tol, maxiter,
                     resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork,
                     info);
    }
    ...
    podarray<T> z(n * (nev + 1),
                  arma_zeros_indicator());  // real N by NEV array if HOWMNY = 'A'
    podarray<T> workev(3 * ncv, arma_zeros_indicator());

    blas_int ldz = n;

    arpack::neupd(&rvec, &howmny, select.memptr(), dr.memptr(), di.memptr(), z.memptr(),
                  &ldz, (T*)&sigmar, (T*)&sigmai, workev.memptr(), &bmat, &n, which, &nev,
                  &tol, resid.memptr(), &ncv, v.memptr(), &ldv, iparam.memptr(),
                  ipntr.memptr(), workd.memptr(), workl.memptr(), &lworkl, rwork.memptr(),
                  &info);

    // Check for errors.
    if (info != 0) {
      arma_warn(1, "eigs_gen(): ARPACK error ", info, " in neupd()");
      return false;
    }
...
inline bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval,
                                ...
                                const SpBase<std::complex<T>, T1>& X_expr,
                                const uword n_eigvals, const form_type form_val,
                                const eigs_opts& opts) {
  arma_debug_sigprint();
  ...
  return sp_auxlib::eigs_gen<T, false>(eigval, eigvec, U.M, n_eigvals, form_val, sigma,
                                       opts);
}
...
inline bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval,
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    return sp_auxlib::eigs_gen<T, true>(eigval, eigvec, U.M, n_eigvals, form_val, sigma,
                                        opts);
  }
#else
  {
...
inline bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval,
                                Mat<std::complex<T>>& eigvec,
                                const SpMat<std::complex<T>>& X, const uword n_eigvals,
                                const form_type form_val, const std::complex<T> sigma,
                                const eigs_opts& opts) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // if(form_val == form_sigma)
    {
      run_aupd_shiftinvert(n_eigvals, sigma, X, false /* gen, not sym */, n, tol, maxiter,
                           resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork,
                           info);
    } else {
    ...
    podarray<blas_int> select(ncv,
                              arma_zeros_indicator());  // logical array of dimension NCV
    podarray<std::complex<T>> d(
        nev + 1, arma_zeros_indicator());  // complex array of dimension NEV + 1
    podarray<std::complex<T>> z(
        n * nev, arma_zeros_indicator());  // complex N by NEV array if HOWMNY = 'A'
    ...
    arpack::neupd(&rvec, &howmny, select.memptr(), eigval.memptr(),
                  ...
                  (std::complex<T>*)NULL, workev.memptr(), &bmat, &n, which, &nev, &tol,
                  resid.memptr(), &ncv, v.memptr(), &ldv, iparam.memptr(), ipntr.memptr(),
                  workd.memptr(), workl.memptr(), &lworkl, rwork.memptr(), &info);
...
inline bool sp_auxlib::spsolve_simple(Mat<typename T1::elem_type>& X,
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    X = B_expr.get_ref();  // superlu::gssv() uses X as input (the B matrix) and as output
                           // (the solution)

    if (A.is_square() == false) {
      X.soft_reset();
      arma_stop_logic_error(
          "spsolve(): solving under-determined / over-determined systems is currently "
          "not supported");
      return false;
    }

    arma_conform_check((A.n_rows != X.n_rows),
                       "spsolve(): number of rows in the given objects must be the same",
                       [&]() { X.soft_reset(); });
    ...
    if (arma_config::check_nonfinite &&
        (A.internal_has_nonfinite() || X.internal_has_nonfinite())) {
    ...
    if (arma_config::check_conform) {
      ...
      if (overflow) {
        arma_stop_runtime_error(
            "spsolve(): integer overflow: matrix dimensions are too large for integer "
            "type used by SuperLU");
        return false;
      }
    }
    ...
    superlu_array_wrangler<int> perm_c(A.n_cols +
                                       1);  // extra paranoia: increase array length by 1
    ...
    superlu::gssv<eT>(&options, a.get_ptr(), perm_c.get_ptr(), perm_r.get_ptr(),
                      l.get_ptr(), u.get_ptr(), x.get_ptr(), stat.get_ptr(), &info);

    // Process the return code.
    if ((info > 0) && (info <= int(A.n_cols))) {
      // std::ostringstream tmp;
      // tmp << "spsolve(): could not solve system; LU factorisation completed, but
      // detected zero in U(" << (info-1) << ',' << (info-1) << ')'; arma_warn(1,
      // tmp.str());
...
template <typename T1, typename T2>
inline bool sp_auxlib::spsolve_refine(Mat<typename T1::elem_type>& X,
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    X.zeros(A.n_cols, B.n_cols);  // set the elements to zero, as we don't trust the
                                  // SuperLU spaghetti code
    ...
    if (arma_config::check_nonfinite &&
        (A.internal_has_nonfinite() || B.internal_has_nonfinite())) {
      arma_warn(3, "spsolve(): detected non-finite elements");
      return false;
    }
    ...
    const bool status_x = wrap_to_supermatrix(x.get_ref(), X);
    const bool status_a = copy_to_supermatrix(
        a.get_ref(),
        A);  // NOTE: superlu::gssvx() modifies 'a' if equilibration is enabled
    const bool status_b = wrap_to_supermatrix(
        b.get_ref(),
        B);  // NOTE: superlu::gssvx() modifies 'b' if equilibration is enabled
    ...
    superlu::gssvx<eT>(&options, a.get_ptr(), perm_c.get_ptr(), perm_r.get_ptr(),
                       etree.get_ptr(), equed, R.get_ptr(), C.get_ptr(), l.get_ptr(),
                       u.get_ptr(), &work[0], lwork, b.get_ptr(), x.get_ptr(), &rpg,
                       &rcond, ferr.get_ptr(), berr.get_ptr(), &glu, &mu, stat.get_ptr(),
                       &info);
...
template <typename eT>
inline typename get_pod_type<eT>::result sp_auxlib::norm1(superlu::SuperMatrix* A) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename get_pod_type<eT>::result sp_auxlib::lu_rcond(
    superlu::SuperMatrix* L, superlu::SuperMatrix* U,
    typename get_pod_type<eT>::result norm_val) {
  arma_debug_sigprint();
...
inline void sp_auxlib::set_superlu_opts(superlu::superlu_options_t& options,
                                        const superlu_opts& user_opts) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool sp_auxlib::copy_to_supermatrix(superlu::SuperMatrix& out,
                                           const SpMat<eT>& A) {
  arma_debug_sigprint();
  ...
  if ((nc->nzval == nullptr) || (nc->colptr == nullptr) || (nc->rowind == nullptr)) {
    return false;
  }
...
template <typename eT>
inline bool sp_auxlib::copy_to_supermatrix_with_shift(superlu::SuperMatrix& out,
                                                      const SpMat<eT>& A,
                                                      const eT shift) {
  arma_debug_sigprint();
  ...
  arma_conform_check(
      (A.is_square() == false),
      "sp_auxlib::copy_to_supermatrix_with_shift(): given matrix must be square sized");

  if (shift == eT(0)) {
    arma_debug_print(
        "sp_auxlib::copy_to_supermatrix_with_shift(): shift is zero; redirecting to "
        "sp_auxlib::copy_to_supermatrix()");
  ...
  for (uword j = 0; j < n_search_cols; ++j) {
    ...
    const uword* pos_ptr =
        std::lower_bound(start_ptr, end_ptr, wanted_row);  // binary search
  ...
  arma_check((count != out_n_nonzero),
             "internal error: sp_auxlib::copy_to_supermatrix_with_shift(): count != "
             "out_n_nonzero");
...
//     ); arma_conform_check( (A.Mtype != superlu::SLU_GE), "copy_to_spmat(): unknown
//     layout" );
//
...
//
//     out.remove_zeros();  // in case SuperLU has bugs and stores zeros in sparse
//     matrices
//     }
...
inline void sp_auxlib::destroy_supermatrix(superlu::SuperMatrix& out) {
  arma_debug_sigprint();

  // Clean up.
  if (out.Stype == superlu::SLU_NC) {
    superlu::destroy_compcol_mat(&out);
  } else if (out.Stype == superlu::SLU_NCP) {
    superlu::destroy_compcolperm_mat(&out);
  } else if (out.Stype == superlu::SLU_DN) {
    // superlu::destroy_dense_mat(&out);

    // since dn->nzval is set to re-use memory from a Mat object (which manages its own
    // memory), we cannot simply call superlu::destroy_dense_mat(). Only the out.Store
...
inline void sp_auxlib::run_aupd_plain(
    const uword n_eigvals, char* which, const SpMat<T>& X, const SpMat<T>& Xst,
    const bool sym, blas_int& n, eT& tol, blas_int& maxiter, podarray<T>& resid,
    blas_int& ncv, podarray<T>& v, blas_int& ldv, podarray<blas_int>& iparam,
    podarray<blas_int>& ipntr, podarray<T>& workd, podarray<T>& workl, blas_int& lworkl,
    podarray<eT>& rwork, blas_int& info) {
#if defined(ARMA_USE_ARPACK)
  {
    ...
    iparam(2) = maxiter;  // Maximum iterations; all the examples use 300, but they were
                          // written in the ancient times.
    ...
    while (ido != 99) {
      ...
      if (sym) {
        ...
        arpack::saupd(&ido, &bmat, &n, which, &nev, &tol, resid.memptr(), &ncv,
                      v.memptr(), &ldv, iparam.memptr(), ipntr.memptr(), workd.memptr(),
                      workl.memptr(), &lworkl, &info);
      } else {
        arma_debug_print("arpack::naupd()");
        arpack::naupd(&ido, &bmat, &n, which, &nev, &tol, resid.memptr(), &ncv,
                      v.memptr(), &ldv, iparam.memptr(), ipntr.memptr(), workd.memptr(),
                      workl.memptr(), &lworkl, rwork.memptr(), &info);
      }
...
template <typename eT, typename T>
inline void sp_auxlib::run_aupd_shiftinvert(
    const uword n_eigvals, const T sigma, const SpMat<T>& X, const bool sym, blas_int& n,
    eT& tol, blas_int& maxiter, podarray<T>& resid, blas_int& ncv, podarray<T>& v,
    blas_int& ldv, podarray<blas_int>& iparam, podarray<blas_int>& ipntr,
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    char* which =
        which_lm;  // NOTE: which_lm is the assumed operation when using shift-invert
    ...
    iparam(6) = 3;  // Mode 3:  A * x = lambda * M * x, M symmetric semi-definite. OP =
                    // inv[A - sigma*M]*M  (A complex)  or  Real_Part{ inv[A - sigma*M]*M
                    // }  (A real)  and  B = M.
    ...
    const bool status_x =
        sp_auxlib::copy_to_supermatrix_with_shift(x.get_ref(), X, sigma);

    if (status_x == false) {
      arma_stop_runtime_error(
          "run_aupd_shiftinvert(): could not construct SuperLU matrix");
      info = blas_int(-1);
      return;
    }
    ...
    superlu_array_wrangler<int> perm_c(X.n_cols +
                                       1);  // paranoia: increase array length by 1
    ...
    superlu::sp_preorder_mat(&options, x.get_ptr(), perm_c.get_ptr(), etree.get_ptr(),
                             xC.get_ptr());
    superlu::gstrf<T>(&options, xC.get_ptr(), relax, panel_size, etree.get_ptr(), NULL,
                      lwork, perm_c.get_ptr(), perm_r.get_ptr(), l.get_ptr(), u.get_ptr(),
                      &Glu, stat.get_ptr(), &slu_info);
    ...
    while (ido != 99) {
      ...
      switch (ido) {
        ...
        case 1: {
          ...
          const bool status_out_slu =
              sp_auxlib::wrap_to_supermatrix(out_slu.get_ref(), out);
          ...
          superlu::gstrs<T>(trans, l.get_ptr(), u.get_ptr(), perm_c.get_ptr(),
...
template <typename eT>
inline bool sp_auxlib::rudimentary_sym_check(const SpMat<eT>& X) {
  arma_debug_sigprint();
  ...
  while ((it != it_end) && (n_check <= n_check_limit)) {
    ...
    if (it_row != it_col) {
      ...
      if (((delta <= tol) || (delta <= (C * tol))) == false) {
        return false;
      }
...
template <typename T>
inline bool sp_auxlib::rudimentary_sym_check(const SpMat<std::complex<T>>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline eigs_randu_filler<eT>::eigs_randu_filler() {
  arma_debug_sigprint();
...
template <typename eT>
inline void eigs_randu_filler<eT>::fill(podarray<eT>& X, const uword N) {
  arma_debug_sigprint();
  ...
  for (uword i = 0; i < N; ++i) {
    X_mem[i] = eT(local_u_distr(local_engine));
  }
}

template <typename T>
inline eigs_randu_filler<std::complex<T>>::eigs_randu_filler() {
  arma_debug_sigprint();
...
template <typename T>
inline void eigs_randu_filler<std::complex<T>>::fill(podarray<std::complex<T>>& X,
                                                     const uword N) {
  arma_debug_sigprint();
...
inline superlu_supermatrix_wrangler::~superlu_supermatrix_wrangler() {
  ...
  if (all_zero == false) {
    sp_auxlib::destroy_supermatrix(m);
  }
}
...
inline superlu::SuperMatrix& superlu_supermatrix_wrangler::get_ref() {
  used = true;

  return m;
}

inline superlu::SuperMatrix* superlu_supermatrix_wrangler::get_ptr() {
  used = true;
...
inline superlu::SuperLUStat_t* superlu_stat_wrangler::get_ptr() { return &stat; }

//

template <typename eT>
inline superlu_array_wrangler<eT>::~superlu_array_wrangler() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline superlu_array_wrangler<eT>::superlu_array_wrangler() : mem(nullptr) {
  arma_debug_sigprint_this(this);
}

template <typename eT>
inline superlu_array_wrangler<eT>::superlu_array_wrangler(const uword n_elem)
    : mem(nullptr) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline void superlu_array_wrangler<eT>::set_size(const uword n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline void superlu_array_wrangler<eT>::reset() {
  arma_debug_sigprint();
...
template <typename eT>
inline eT* superlu_array_wrangler<eT>::get_ptr() {
  return mem;
}
...
template <typename eT>
inline superlu_worker<eT>::~superlu_worker() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline bool superlu_worker<eT>::factorise(typename get_pod_type<eT>::result& out_rcond,
                                          const SpMat<eT>& A,
                                          const superlu_opts& user_opts) {
  arma_debug_sigprint();
  ...
  superlu::sp_preorder_mat(&options, AA.get_ptr(), perm_c.get_ptr(), etree.get_ptr(),
                           AAc.get_ptr());

  arma_debug_print("superlu::gstrf()");
  superlu::gstrf<eT>(&options, AAc.get_ptr(), relax, panel_size, etree.get_ptr(), NULL,
                     lwork, perm_c.get_ptr(), perm_r.get_ptr(), l_ref.get_ptr(),
                     u_ref.get_ptr(), &Glu, stat.get_ptr(), &info);
...
template <typename eT>
inline bool superlu_worker<eT>::solve(Mat<eT>& X, const Mat<eT>& B) {
  arma_debug_sigprint();
  ...
  arma_debug_print("superlu::gstrs()");
  superlu::gstrs<eT>(trans, l_ref.get_ptr(), u_ref.get_ptr(), perm_c.get_ptr(),
                     perm_r.get_ptr(), XX.get_ptr(), stat.get_ptr(), &info);
