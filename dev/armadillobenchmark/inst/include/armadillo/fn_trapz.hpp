...
arma_warn_unused inline const Glue<T1, T2, glue_trapz> trapz(
...
arma_warn_unused inline const Op<T1, op_trapz> trapz(
    const Base<typename T1::elem_type, T1>& Y, const uword dim = 0) {
  arma_debug_sigprint();
