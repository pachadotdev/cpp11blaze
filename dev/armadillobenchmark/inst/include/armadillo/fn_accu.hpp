...
arma_hot inline typename T1::elem_type accu_proxy_linear(const Proxy<T1>& P) {
  ...
  if (arma_config::openmp && Proxy<T1>::use_mp && mp_gate<eT>::eval(n_elem)) {
    ...
    {
      // NOTE: using parallelisation with manual reduction workaround to take into account
      // complex numbers; NOTE: OpenMP versions lower than 4.0 do not support user-defined
      // reduction

      const int n_threads_max = mp_thread_limit::get();
      const uword n_threads_use =
          (std::min)(uword(podarray_prealloc_n_elem::val), uword(n_threads_max));
...
arma_warn_unused arma_hot inline eT accu(const subview<eT>& X) {
  ...
  if (X_n_cols == 1) {
    return arrayops::accumulate(X.colptr(0), X_n_rows);
  }
...
arma_warn_unused inline typename T1::elem_type accu(
  ...
  return (ProxyCube<expr_type>::use_at) ? accu_cube_proxy_at(P)
                                        : accu_cube_proxy_linear(P);
}

//

template <typename T>
arma_warn_unused inline typename arma_scalar_only<T>::result accu(const T& x) {
  return x;
}
