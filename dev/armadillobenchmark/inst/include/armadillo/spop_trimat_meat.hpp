...
inline void spop_trimat::apply_noalias(SpMat<typename T1::elem_type>& out,
                                       const SpProxy<T1>& P, const bool upper) {
  arma_debug_sigprint();
...
inline void spop_trimatu_ext::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A,
  ...
  if (count < A.n_nonzero) {
    out.mem_resize(count);
  }
}

template <typename T1>
inline void spop_trimatu_ext::apply(SpMat<typename T1::elem_type>& out,
                                    const SpOp<T1, spop_trimatu_ext>& in) {
  arma_debug_sigprint();
...
template <typename eT>
inline void spop_trimatl_ext::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A,
                                            const uword row_offset,
                                            const uword col_offset) {
  arma_debug_sigprint();
...
template <typename T1>
inline void spop_trimatl_ext::apply(SpMat<typename T1::elem_type>& out,
                                    const SpOp<T1, spop_trimatl_ext>& in) {
  arma_debug_sigprint();
