...
arma_warn_unused arma_inline
    typename enable_if2<is_arma_type<T1>::value && resolves_to_vector<T1>::no,
                        const mtOp<uword, T1, op_index_max> >::result
    index_max(const T1& X) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused arma_inline const mtOpCube<uword, T1, op_index_max> index_max(
...
template <typename T1>
arma_warn_unused inline
    typename enable_if2<is_arma_sparse_type<T1>::value, Mat<uword> >::result
    index_max(const T1& X, const uword dim) {
  arma_debug_sigprint();
...
arma_warn_unused inline uword index_max(const SizeMat& s) {
  return (s.n_rows >= s.n_cols) ? uword(0) : uword(1);
}

arma_warn_unused inline uword index_max(const SizeCube& s) {
