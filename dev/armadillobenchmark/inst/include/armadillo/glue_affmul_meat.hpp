...
template <typename T1, typename T2>
inline void glue_affmul::apply_noalias(Mat<typename T1::elem_type>& out, const T1& A,
                                       const T2& B) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void glue_affmul::apply_noalias_square(Mat<typename T1::elem_type>& out,
...
template <typename T1, typename T2>
inline void glue_affmul::apply_noalias_rectangle(Mat<typename T1::elem_type>& out,
                                                 const T1& A, const T2& B) {
  arma_debug_sigprint();
  ...
  switch (A_n_rows) {
    ...
    case 4:  // A is 4x5
    {
      if (B_n_cols == 1) {
        ...
        out_mem[0] =
            A_mem[0] * x + A_mem[4] * y + A_mem[8] * z + A_mem[12] * w + A_mem[16];
        out_mem[1] =
            A_mem[1] * x + A_mem[5] * y + A_mem[9] * z + A_mem[13] * w + A_mem[17];
        out_mem[2] =
            A_mem[2] * x + A_mem[6] * y + A_mem[10] * z + A_mem[14] * w + A_mem[18];
        out_mem[3] =
            A_mem[3] * x + A_mem[7] * y + A_mem[11] * z + A_mem[15] * w + A_mem[19];
      } else
        for (uword col = 0; col < B_n_cols; ++col) {
          const eT* B_mem = B.colptr(col);
          eT* out_mem = out.colptr(col);
...
template <typename T1, typename T2>
inline void glue_affmul::apply_noalias_generic(Mat<typename T1::elem_type>& out,
                                               const T1& A, const T2& B) {
  arma_debug_sigprint();
