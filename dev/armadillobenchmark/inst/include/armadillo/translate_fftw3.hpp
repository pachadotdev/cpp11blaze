...
arma_inline void_ptr plan_dft_1d(int N, eT* input, eT* output, int fftw3_sign,
                                 unsigned int fftw3_flags) {
...
template <typename eT>
arma_inline void execute(void_ptr plan) {
  arma_type_check((is_cx<eT>::value == false));
...
template <typename eT>
arma_inline void destroy_plan(void_ptr plan) {
  arma_type_check((is_cx<eT>::value == false));
