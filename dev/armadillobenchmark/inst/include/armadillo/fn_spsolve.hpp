...
inline bool spsolve_helper(
    Mat<typename T1::elem_type>& out, const SpBase<typename T1::elem_type, T1>& A,
  ...
  } else if (sig == 'l')  // brutal LAPACK solver
  {
    ...
    if (conversion_ok) {
      ...
      if (opts.refine != superlu_opts::REF_NONE) {
        flags |= solve_opts::flag_refine;
      }
