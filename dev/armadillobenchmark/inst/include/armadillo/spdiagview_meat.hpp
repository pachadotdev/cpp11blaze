...
template <typename eT>
arma_inline spdiagview<eT>::spdiagview(const SpMat<eT>& in_m, const uword in_row_offset,
                                       const uword in_col_offset, const uword in_len)
    : m(in_m),
...
inline void spdiagview<eT>::operator=(const spdiagview<eT>& x) {
  ...
  arma_conform_check((d.n_elem != x.n_elem),
                     "spdiagview: diagonals have incompatible lengths");
  ...
  if ((&d_m == &x_m) || ((d.row_offset == 0) && (d.col_offset == 0))) {
    ...
    for (uword i = 0; i < d_n_elem; ++i) {
      d_m.at(i + d_row_offset, i + d_col_offset) =
          x_m.at(i + x_row_offset, i + x_col_offset);
    }
  }
}
...
inline void spdiagview<eT>::operator=(const Base<eT, T1>& o) {
  ...
  if (is_same_type<T1, Gen<Col<eT>, gen_zeros> >::yes) {
    ...
    arma_conform_check((d_n_elem != P.get_n_elem()),
  ...
  arma_conform_check(((d_n_elem != x.n_elem) || ((x.n_rows != 1) && (x.n_cols != 1))),
                     "spdiagview: given object has incompatible size");
...
inline void spdiagview<eT>::operator=(const SpBase<eT, T1>& o) {
  ...
  arma_conform_check(((n_elem != x.n_elem) || ((x.n_rows != 1) && (x.n_cols != 1))),
                     "spdiagview: given object has incompatible size");
...
template <typename eT>
inline SpMat_MapMat_val<eT> spdiagview<eT>::operator[](const uword i) {
...
template <typename eT>
inline SpMat_MapMat_val<eT> spdiagview<eT>::at(const uword i) {
  return (const_cast<SpMat<eT>&>(m)).at(i + row_offset, i + col_offset);
}
...
template <typename eT>
inline SpMat_MapMat_val<eT> spdiagview<eT>::at(const uword row, const uword) {
...
template <typename eT>
inline SpMat_MapMat_val<eT> spdiagview<eT>::operator()(const uword row, const uword col) {
  arma_conform_check_bounds(((row >= n_elem) || (col > 0)),
                            "spdiagview::operator(): out of bounds");
...
template <typename eT>
inline void spdiagview<eT>::replace(const eT old_val, const eT new_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline void spdiagview<eT>::clean(const typename get_pod_type<eT>::result threshold) {
  arma_debug_sigprint();
...
template <typename eT>
inline void spdiagview<eT>::clamp(const eT min_val, const eT max_val) {
  arma_debug_sigprint();
...
inline void spdiagview<eT>::fill(const eT val) {
  ...
  if ((row_offset == 0) && (col_offset == 0) && (m.sync_state != 1)) {
    if (val == eT(0)) {
      SpMat<eT> tmp(arma_reserve_indicator(), m.n_rows, m.n_cols,
                    m.n_nonzero);  // worst case scenario
