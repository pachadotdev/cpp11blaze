...
class SpSubview : public SpBase<eT, SpSubview<eT> > {
 ...
 protected:
  inline SpSubview(const SpMat<eT>& in_m, const uword in_row1, const uword in_col1,
  ...
  */
  ...
  arma_warn_unused inline SpSubview_MapMat_val<eT> operator()(const uword in_row,
                                                              const uword in_col);
  ...
  arma_warn_unused inline SpSubview_MapMat_val<eT> at(const uword in_row,
                                                      const uword in_col);
  ...
  inline SpSubview submat(const uword in_row1, const uword in_col1, const uword in_row2,
                          const uword in_col2);
  inline const SpSubview submat(const uword in_row1, const uword in_col1,
  ...
  // Similar to SpMat iterators but automatically iterates past and ignores values not in
  // the subview.
  class iterator_base {
   public:
  ...
  class const_iterator : public iterator_base {
   public:
    ...
    inline const_iterator(const SpSubview& in_M, uword in_row, uword in_col, uword in_pos,
                          uword skip_pos);
    ...
    arma_inline uword row() const {
      return iterator_base::M->m.row_indices[iterator_base::internal_pos + skip_pos] -
             iterator_base::M->aux_row1;
    }
  ...
  class iterator : public const_iterator {
   public:
    ...
    inline iterator(SpSubview& in_M, const uword in_row, const uword in_col,
                    const uword in_pos, const uword in_skip_pos)
 ...
 private:
  ...
  friend class SpValProxy<SpSubview<eT> >;  // allow SpValProxy to call insert_element()
                                            // and delete_element()
...
class SpSubview_col : public SpSubview<eT> {
 public:
  ...
  arma_warn_unused inline const SpToDOp<SpSubview_col<eT>, op_sp_as_dense> as_dense()
      const;

 protected:
  inline SpSubview_col(const SpMat<eT>& in_m, const uword in_col);
  inline SpSubview_col(const SpMat<eT>& in_m, const uword in_col, const uword in_row1,
...
class SpSubview_row : public SpSubview<eT> {
 public:
  ...
  arma_warn_unused inline const SpToDOp<SpSubview_row<eT>, op_sp_as_dense> as_dense()
      const;

 protected:
  inline SpSubview_row(const SpMat<eT>& in_m, const uword in_row);
  inline SpSubview_row(const SpMat<eT>& in_m, const uword in_row, const uword in_col1,
