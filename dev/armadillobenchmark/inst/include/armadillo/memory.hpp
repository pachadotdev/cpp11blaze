...
class memory {
 public:
  ...
  arma_malloc inline static eT* acquire(const uword n_elem);
...
template <typename eT>
arma_malloc inline eT* memory::acquire(const uword n_elem) {
  if (n_elem == 0) {
    return nullptr;
  }

  arma_conform_check((size_t(n_elem) > (std::numeric_limits<size_t>::max() / sizeof(eT))),
...
#elif defined(ARMA_HAVE_POSIX_MEMALIGN)
  {
    ...
    // TODO: investigate apparent memory leak when using alignment >= 64 (as shown on
    // Fedora 28, glibc 2.27)
    int status = posix_memalign(
        (void**)&memptr, ((alignment >= sizeof(void*)) ? alignment : sizeof(void*)),
        n_bytes);
...
#if (defined(ARMA_HAVE_GCC_ASSUME_ALIGNED) || defined(__cpp_lib_assume_aligned)) && \
    !defined(ARMA_DONT_CHECK_ALIGNMENT)
  {
    return (sizeof(std::size_t) >= sizeof(eT*)) ? ((std::size_t(mem) & 0x0F) == 0)
                                                : false;
  }
#else
  {
