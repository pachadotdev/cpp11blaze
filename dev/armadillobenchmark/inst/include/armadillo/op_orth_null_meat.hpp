...
inline void op_orth::apply(Mat<typename T1::elem_type>& out,
                           const Op<T1, op_orth>& expr) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_null::apply(Mat<typename T1::elem_type>& out,
                           const Op<T1, op_null>& expr) {
  arma_debug_sigprint();
