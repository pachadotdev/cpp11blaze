...
inline void op_sp_plus::apply(Mat<typename T1::elem_type>& out,
                              const SpToDOp<T1, op_sp_plus>& in) {
  arma_debug_sigprint();
...
inline void op_sp_plus::apply_inside_schur(SpMat<eT>& out, const T2& x,
                                           const SpToDOp<T3, op_sp_plus>& y) {
  arma_debug_sigprint();
...
// used for the optimization of sparse / (sparse + scalar)
template <typename eT, typename T2, typename T3>
inline void op_sp_plus::apply_inside_div(SpMat<eT>& out, const T2& x,
                                         const SpToDOp<T3, op_sp_plus>& y) {
  arma_debug_sigprint();
  ...
  arma_conform_assert_same_size(P2.get_n_rows(), P2.get_n_cols(), P3.get_n_rows(),
                                P3.get_n_cols(), "element-wise division");
