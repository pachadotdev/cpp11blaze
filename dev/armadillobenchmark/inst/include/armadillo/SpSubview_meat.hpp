...
template <typename eT>
inline SpSubview<eT>::SpSubview(const SpMat<eT>& in_m, const uword in_row1,
                                const uword in_col1, const uword in_n_rows,
...
template <typename eT>
inline SpSubview<eT>::SpSubview(const SpSubview<eT>& in)
    : m(in.m),
...
template <typename eT>
inline SpSubview<eT>::SpSubview(SpSubview<eT>&& in)
    : m(in.m),
      aux_row1(in.aux_row1),
      aux_col1(in.aux_col1),
      n_rows(in.n_rows),
...
template <typename eT>
inline const SpSubview<eT>& SpSubview<eT>::operator+=(const eT val) {
  arma_debug_sigprint();
...
inline const SpSubview<eT>& SpSubview<eT>::operator=(const Base<eT, T1>& in) {
  arma_debug_sigprint();
  ...
  arma_conform_assert_same_size(n_rows, n_cols, U.M.n_rows, U.M.n_cols,
                                "insertion into sparse submatrix");
...
template <typename eT>
template <typename T1>
inline const SpSubview<eT>& SpSubview<eT>::operator+=(const Base<eT, T1>& x) {
  arma_debug_sigprint();
...
inline const SpSubview<eT>& SpSubview<eT>::operator%=(const Base<eT, T1>& x) {
  ...
  arma_conform_assert_same_size(sv.n_rows, sv.n_cols, B.n_rows, B.n_cols,
...
inline const SpSubview<eT>& SpSubview<eT>::operator/=(const Base<eT, T1>& x) {
  ...
  arma_conform_assert_same_size(A.n_rows, A.n_cols, B.n_rows, B.n_cols,
  ...
  for (uword c = 0; c < B_n_cols; ++c) {
    for (uword r = 0; r < B_n_rows; ++r) {
      ...
      // for efficiency, an element in B is checked before checking the corresponding
      // element in A

      if ((B.at(r, c) == zero) && (A.at(r, c) == zero)) {
        result_ok = false;
        break;
      }
    }
...
template <typename eT>
inline const SpSubview<eT>& SpSubview<eT>::operator=(const SpSubview<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline const SpSubview<eT>& SpSubview<eT>::operator=(const SpBase<eT, T1>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline const SpSubview<eT>& SpSubview<eT>::operator_equ_common(const SpBase<eT, T1>& in) {
  arma_debug_sigprint();
...
inline void SpSubview<eT>::for_each(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline void SpSubview<eT>::for_each(functor F) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline void SpSubview<eT>::transform(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview<eT>::replace(const eT old_val, const eT new_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview<eT>::clean(const typename get_pod_type<eT>::result threshold) {
  arma_debug_sigprint();
  ...
  if (is_cx<eT>::yes) {
    (*this).operator=(tmp);
  } else if (tmp.n_nonzero != n_nonzero) {
    (*this).operator=(tmp);
  }
}

template <typename eT>
inline void SpSubview<eT>::clamp(const eT min_val, const eT max_val) {
  arma_debug_sigprint();

  if (is_cx<eT>::no) {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "SpSubview::clamp(): min_val must be less than max_val");
  } else {
    arma_conform_check(
        (access::tmp_real(min_val) > access::tmp_real(max_val)),
        "SpSubview::clamp(): real(min_val) must be less than real(max_val)");
    arma_conform_check(
        (access::tmp_imag(min_val) > access::tmp_imag(max_val)),
        "SpSubview::clamp(): imag(min_val) must be less than imag(max_val)");
  }
...
template <typename eT>
inline void SpSubview<eT>::fill(const eT val) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview<eT>::zeros() {
  arma_debug_sigprint();
  ...
  for (; m_it != m_it_end; ++m_it) {
    ...
    const bool inside_box = ((m_it_row >= sv_row_start) && (m_it_row <= sv_row_end)) &&
                            ((m_it_col >= sv_col_start) && (m_it_col <= sv_col_end));
...
template <typename eT>
inline void SpSubview<eT>::randu() {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview<eT>::randn() {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview_MapMat_val<eT> SpSubview<eT>::operator[](const uword i) {
...
template <typename eT>
inline eT SpSubview<eT>::operator[](const uword i) const {
  const uword lrow = i % n_rows;
  const uword lcol = i / n_rows;
...
template <typename eT>
inline SpSubview_MapMat_val<eT> SpSubview<eT>::operator()(const uword in_row,
                                                          const uword in_col) {
  arma_conform_check_bounds((in_row >= n_rows) || (in_col >= n_cols),
                            "SpSubview::operator(): index out of bounds");

  return (*this).at(in_row, in_col);
}

template <typename eT>
inline eT SpSubview<eT>::operator()(const uword in_row, const uword in_col) const {
  arma_conform_check_bounds((in_row >= n_rows) || (in_col >= n_cols),
                            "SpSubview::operator(): index out of bounds");
...
template <typename eT>
inline SpSubview_MapMat_val<eT> SpSubview<eT>::at(const uword i) {
  const uword lrow = i % n_rows;
...
template <typename eT>
inline eT SpSubview<eT>::at(const uword i) const {
  const uword lrow = i % n_rows;
  const uword lcol = i / n_cols;
...
template <typename eT>
inline SpSubview_MapMat_val<eT> SpSubview<eT>::at(const uword in_row,
                                                  const uword in_col) {
  return SpSubview_MapMat_val<eT>((*this), m.cache, aux_row1 + in_row, aux_col1 + in_col);
}

template <typename eT>
inline eT SpSubview<eT>::at(const uword in_row, const uword in_col) const {
  return m.at(aux_row1 + in_row, aux_col1 + in_col);
}

template <typename eT>
inline bool SpSubview<eT>::check_overlap(const SpSubview<eT>& x) const {
...
template <typename eT>
inline bool SpSubview<eT>::is_vec() const {
  return ((n_rows == 1) || (n_cols == 1));
}

template <typename eT>
inline SpSubview_row<eT> SpSubview<eT>::row(const uword row_num) {
  arma_debug_sigprint();

  arma_conform_check_bounds(row_num >= n_rows, "SpSubview::row(): out of bounds");

  return SpSubview_row<eT>(const_cast<SpMat<eT>&>(m), row_num + aux_row1, aux_col1,
                           n_cols);
}

template <typename eT>
inline const SpSubview_row<eT> SpSubview<eT>::row(const uword row_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview_col<eT> SpSubview<eT>::col(const uword col_num) {
  arma_debug_sigprint();

  arma_conform_check_bounds(col_num >= n_cols, "SpSubview::col(): out of bounds");

  return SpSubview_col<eT>(const_cast<SpMat<eT>&>(m), col_num + aux_col1, aux_row1,
                           n_rows);
}

template <typename eT>
inline const SpSubview_col<eT> SpSubview<eT>::col(const uword col_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::rows(const uword in_row1,
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::cols(const uword in_col1,
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::submat(const uword in_row1, const uword in_col1,
  ...
  return access::rw(m).submat(in_row1 + aux_row1, in_col1 + aux_col1, in_row2 + aux_row1,
                              in_col2 + aux_col1);
}

template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::submat(const uword in_row1, const uword in_col1,
                                                 const uword in_row2,
                                                 const uword in_col2) const {
  arma_debug_sigprint();
  ...
  return m.submat(in_row1 + aux_row1, in_col1 + aux_col1, in_row2 + aux_row1,
                  in_col2 + aux_col1);
}

template <typename eT>
inline SpSubview<eT> SpSubview<eT>::submat(const span& row_span, const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::submat(const span& row_span,
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::operator()(const uword row_num,
                                               const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::operator()(const uword row_num,
                                                     const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::operator()(const span& row_span,
                                               const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpSubview<eT>::operator()(const span& row_span,
...
template <typename eT>
inline void SpSubview<eT>::swap_rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();

  arma_conform_check((in_row1 >= n_rows) || (in_row2 >= n_rows),
                     "SpSubview::swap_rows(): invalid row index");
...
template <typename eT>
inline void SpSubview<eT>::swap_cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();

  arma_conform_check((in_col1 >= n_cols) || (in_col2 >= n_cols),
                     "SpSubview::swap_cols(): invalid column index");
...
template <typename eT>
inline typename SpSubview<eT>::iterator SpSubview<eT>::begin() {
  m.sync_csc();
...
template <typename eT>
inline typename SpSubview<eT>::const_iterator SpSubview<eT>::cbegin() const {
  m.sync_csc();
...
template <typename eT>
inline typename SpSubview<eT>::row_iterator SpSubview<eT>::begin_row(
    const uword row_num) {
  m.sync_csc();
...
template <typename eT>
inline typename SpSubview<eT>::iterator SpSubview<eT>::end() {
  m.sync_csc();
...
template <typename eT>
inline typename SpSubview<eT>::const_iterator SpSubview<eT>::cend() const {
  m.sync_csc();
...
template <typename eT>
arma_inline bool SpSubview<eT>::is_alias(const SpMat<eT>& X) const {
  return m.is_alias(X);
}

template <typename eT>
inline eT& SpSubview<eT>::insert_element(const uword in_row, const uword in_col,
                                         const eT in_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview<eT>::delete_element(const uword in_row, const uword in_col) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview<eT>::invalidate_cache() const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview_col<eT>::SpSubview_col(const SpMat<eT>& in_m, const uword in_col)
...
template <typename eT>
inline void SpSubview_col<eT>::operator=(const SpSubview<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview_col<eT>::operator=(const SpSubview_col<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void SpSubview_col<eT>::operator=(const SpBase<eT, T1>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void SpSubview_col<eT>::operator=(const Base<eT, T1>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpOp<SpSubview_col<eT>, spop_htrans> SpSubview_col<eT>::t() const {
  return SpOp<SpSubview_col<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpSubview_col<eT>, spop_htrans> SpSubview_col<eT>::ht() const {
  return SpOp<SpSubview_col<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpSubview_col<eT>, spop_strans> SpSubview_col<eT>::st() const {
  return SpOp<SpSubview_col<eT>, spop_strans>(*this);
}

template <typename eT>
inline const SpToDOp<SpSubview_col<eT>, op_sp_as_dense> SpSubview_col<eT>::as_dense()
    const {
...
template <typename eT>
inline SpSubview_row<eT>::SpSubview_row(const SpMat<eT>& in_m, const uword in_row)
...
template <typename eT>
inline void SpSubview_row<eT>::operator=(const SpSubview<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpSubview_row<eT>::operator=(const SpSubview_row<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void SpSubview_row<eT>::operator=(const SpBase<eT, T1>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void SpSubview_row<eT>::operator=(const Base<eT, T1>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpOp<SpSubview_row<eT>, spop_htrans> SpSubview_row<eT>::t() const {
  return SpOp<SpSubview_row<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpSubview_row<eT>, spop_htrans> SpSubview_row<eT>::ht() const {
  return SpOp<SpSubview_row<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpSubview_row<eT>, spop_strans> SpSubview_row<eT>::st() const {
  return SpOp<SpSubview_row<eT>, spop_strans>(*this);
}

template <typename eT>
inline const SpToDOp<SpSubview_row<eT>, op_sp_as_dense> SpSubview_row<eT>::as_dense()
    const {
