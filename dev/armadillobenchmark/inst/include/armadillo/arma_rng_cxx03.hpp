...
inline void arma_rng_cxx03::set_seed(const arma_rng_cxx03::seed_type val) {
  std::srand(val);
}

arma_inline int arma_rng_cxx03::randi_val() {
#if (RAND_MAX == 32767)
  {
...
arma_inline double arma_rng_cxx03::randu_val() {
  return double(double(randi_val()) * (double(1) / double(randi_max_val())));
}

inline double arma_rng_cxx03::randn_val() {
...
template <typename eT>
inline void arma_rng_cxx03::randn_dual_val(eT& out1, eT& out2) {
...
template <typename eT>
inline void arma_rng_cxx03::randi_fill(eT* mem, const uword N, const int a, const int b) {
  if ((a == 0) && (b == RAND_MAX)) {
    for (uword i = 0; i < N; ++i) {
      mem[i] = eT(std::rand());
    }
...
inline int arma_rng_cxx03::randi_max_val() {
#if (RAND_MAX == 32767)
