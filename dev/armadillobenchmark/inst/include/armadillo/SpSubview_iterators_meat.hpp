...
inline SpSubview<eT>::const_iterator::const_iterator(const SpSubview<eT>& in_M,
  ...
  while (cur_pos < (iterator_base::internal_pos + 1)) {
    ...
    while (
        ((lskip_pos + cur_pos) >= iterator_base::M->m.col_ptrs[cur_col + aux_col + 1]) &&
        (cur_col < ln_cols)) {
      ++cur_col;
    }
...
inline SpSubview<eT>::const_iterator::const_iterator(const SpSubview<eT>& in_M,
  ...
  // Because we have to count the points in this subview and not in this subview, this
  // becomes a little difficult and slow.
  const uword aux_col = iterator_base::M->aux_col1;
  const uword aux_row = iterator_base::M->aux_row1;
  const uword ln_rows = iterator_base::M->n_rows;
  const uword ln_cols = iterator_base::M->n_cols;
  ...
  while (cur_col < in_col) {
    ...
    // Have we stepped forward a column (or multiple columns)?
    while (
        ((skip_pos + cur_pos) >= iterator_base::M->m.col_ptrs[cur_col + aux_col + 1]) &&
        (cur_col < ln_cols)) {
      ++cur_col;
    }
  }
...
inline SpSubview<eT>::const_iterator::const_iterator(const SpSubview<eT>& in_M,
                                                     ...
                                                     uword in_pos, uword in_skip_pos)
    : iterator_base(in_M, in_col, in_pos), skip_pos(in_skip_pos) {
  arma_ignore(in_row);
...
template <typename eT>
arma_inline eT SpSubview<eT>::const_iterator::operator*() const {
  return iterator_base::M->m.values[iterator_base::internal_pos + skip_pos];
}

template <typename eT>
inline typename SpSubview<eT>::const_iterator&
SpSubview<eT>::const_iterator::operator++() {
  ...
  while (true) {
    ...
    // Did we move any columns?
    while ((cur_col < ln_cols) && ((lskip_pos + cur_pos) >=
...
inline SpValProxy<SpSubview<eT> > SpSubview<eT>::iterator::operator*() {
  return SpValProxy<SpSubview<eT> >(
      const_iterator::row(), iterator_base::col(), access::rw(*iterator_base::M),
      &(access::rw(iterator_base::M->m
                       .values[iterator_base::internal_pos + const_iterator::skip_pos])));
}
...
inline SpSubview<eT>::const_row_iterator::const_row_iterator(const SpSubview<eT>& in_M,
  ...
  for (uword col = 0; col < iterator_base::M->n_cols; ++col) {
    ...
    if (start_ptr != end_ptr) {
      ...
      if (first_subview_ptr != end_ptr &&
          (*first_subview_ptr) < aux_row + iterator_base::M->n_rows) {
        // There exists at least one element in the subview.
        const uword* pos_ptr =
            std::lower_bound(first_subview_ptr, end_ptr, aux_row + in_row);
...
template <typename eT>
inline typename SpSubview<eT>::const_row_iterator&
SpSubview<eT>::const_row_iterator::operator++() {
  ...
  for (uword col = iterator_base::internal_col + 1; col < M_n_cols; ++col) {
    ...
    if (start_ptr != end_ptr) {
      ...
      if (pos_ptr != end_ptr) {
        ...
        if ((*pos_ptr) == internal_row + aux_row &&
            (*pos_ptr) < aux_row + iterator_base::M->n_rows) {
          // Exact match---so we are done.
          iterator_base::internal_col = col;
          actual_pos = col_offset + (pos_ptr - start_ptr);
          return *this;
        } else if ((*pos_ptr) < next_min_row + aux_row &&
                   (*pos_ptr) < aux_row + iterator_base::M->n_rows) {
        ...
        } else if ((*pos_ptr) == next_min_row + aux_row && col < next_min_col &&
                   (*pos_ptr) < aux_row + iterator_base::M->n_rows) {
  ...
  for (uword col = 0; col <= iterator_base::internal_col; ++col) {
    ...
    if (start_ptr != end_ptr) {
      const uword* pos_ptr =
          std::lower_bound(start_ptr, end_ptr, internal_row + aux_row + 1);

      if (pos_ptr != end_ptr) {
        // We found something in the column, but is the row index correct?
        if ((*pos_ptr) == internal_row + aux_row + 1 &&
            (*pos_ptr) < aux_row + iterator_base::M->n_rows) {
          // Exact match---so we are done.
          iterator_base::internal_col = col;
          internal_row++;
