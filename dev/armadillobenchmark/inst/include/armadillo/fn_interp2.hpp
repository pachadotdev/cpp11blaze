...
inline void interp2_helper_nearest(const Mat<eT>& XG, const Mat<eT>& ZG,
                                   const Mat<eT>& XI, Mat<eT>& ZI, const eT extrap_val,
                                   const uword mode) {
  arma_debug_sigprint();
  ...
  if (mode == 0) {
    ZI.set_size(XI.n_elem, ZG.n_cols);
  }
  if (mode == 1) {
    ZI.set_size(ZG.n_rows, XI.n_elem);
  }
  ...
  for (uword i = 0; i < NI; ++i) {
    ...
    if ((XI_val < XG_min) || (XI_val > XG_max)) {
      if (mode == 0) {
        ZI.row(i).fill(extrap_val);
      }
      if (mode == 1) {
        ZI.col(i).fill(extrap_val);
      }
    } else {
      ...
      if (mode == 0) {
        ZI.row(i) = ZG.row(best_j);
      }
      if (mode == 1) {
        ZI.col(i) = ZG.col(best_j);
      }
    }
  }
}
...
template <typename eT>
inline void interp2_helper_linear(const Mat<eT>& XG, const Mat<eT>& ZG, const Mat<eT>& XI,
                                  Mat<eT>& ZI, const eT extrap_val, const uword mode) {
  arma_debug_sigprint();
  ...
  for (uword i = 0; i < NI; ++i) {
    ...
    if ((XI_val < XG_min) || (XI_val > XG_max)) {
      ...
      if (mode == 0) {
        ZI.row(i) = (eT(1) - weight) * ZG.row(a_best_j) + (weight)*ZG.row(b_best_j);
      }
      if (mode == 1) {
        ZI.col(i) = (eT(1) - weight) * ZG.col(a_best_j) + (weight)*ZG.col(b_best_j);
      }
    }
  }
}
...
inline typename enable_if2<is_real<typename T1::elem_type>::value, void>::result interp2(
    const Base<typename T1::elem_type, T1>& X, const Base<typename T1::elem_type, T2>& Y,
    const Base<typename T1::elem_type, T3>& Z, const Base<typename T1::elem_type, T4>& XI,
    const Base<typename T1::elem_type, T5>& YI, Mat<typename T1::elem_type>& ZI,
  ...
  arma_conform_check(((sig != 'n') && (sig != 'l')),
                     "interp2(): unsupported interpolation type");
  ...
  arma_conform_check((UXG.M.n_elem < 2),
                     "interp2(): X must have at least two unique elements");
  arma_conform_check((UYG.M.n_elem < 2),
                     "interp2(): Y must have at least two unique elements");

  arma_conform_check(
      (UXG.M.n_elem != UZG.M.n_cols),
      "interp2(): number of elements in X must equal the number of columns in Z");
  arma_conform_check(
      (UYG.M.n_elem != UZG.M.n_rows),
      "interp2(): number of elements in Y must equal the number of rows in Z");

  arma_conform_check((UXG.M.is_sorted("strictascend") == false),
                     "interp2(): X must be monotonically increasing");
  arma_conform_check((UYG.M.is_sorted("strictascend") == false),
                     "interp2(): Y must be monotonically increasing");

  arma_conform_check((UXI.M.is_sorted("strictascend") == false),
                     "interp2(): XI must be monotonically increasing");
  arma_conform_check((UYI.M.is_sorted("strictascend") == false),
                     "interp2(): YI must be monotonically increasing");
