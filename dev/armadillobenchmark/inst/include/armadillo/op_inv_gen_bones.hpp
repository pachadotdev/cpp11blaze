...
class op_inv_gen_default : public traits_op_default {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out,
                           const Op<T1, op_inv_gen_default>& in);
...
class op_inv_gen_full : public traits_op_default {
 public:
  ...
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out,
                           const Op<T1, op_inv_gen_full>& in);
...
class op_inv_gen_rcond : public traits_op_default {
 public:
  template <typename T1>
  inline static bool apply_direct(Mat<typename T1::elem_type>& out_inv,
                                  op_inv_gen_state<typename T1::pod_type>& out_state,
                                  const Base<typename T1::elem_type, T1>& expr);
};
...
struct opts_tiny : public opts {
  inline constexpr opts_tiny() : opts(flag_tiny) {}
};
struct opts_allow_approx : public opts {
  inline constexpr opts_allow_approx() : opts(flag_allow_approx) {}
};
...
struct opts_no_ugly : public opts {
  inline constexpr opts_no_ugly() : opts(flag_no_ugly) {}
};
