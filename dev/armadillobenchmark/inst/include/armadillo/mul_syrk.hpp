...
class syrk {
 public:
    ...
    if (A.n_elem <= threshold) {
...
#if defined(ARMA_USE_ATLAS)
      {
        if (use_beta == true) {
          ...
          // NOTE: assuming beta=1; this is okay for now, as currently glue_times only
          // uses beta=1
        ...
        atlas::cblas_syrk<eT>(atlas_CblasColMajor, atlas_CblasUpper,
                              (do_trans_A) ? atlas_CblasTrans : atlas_CblasNoTrans,
                              C.n_cols, (do_trans_A) ? A.n_rows : A.n_cols,
                              (use_alpha) ? alpha : eT(1), A.mem,
                              (do_trans_A) ? A.n_rows : C.n_cols,
                              (use_beta) ? beta : eT(0), C.memptr(), C.n_cols);
  ...
  template <typename eT, typename TA>
  inline static void apply(Mat<eT>& C, const TA& A, const eT alpha = eT(1),
                           const eT beta = eT(0)) {
    if (is_cx<eT>::no) {
      if (A.is_vec()) {
        syrk_vec<do_trans_A, use_alpha, use_beta>::apply(C, A, alpha, beta);
  ...
  arma_inline static void apply(Mat<float>& C, const TA& A, const float alpha = float(1),
                                const float beta = float(0)) {
    syrk<do_trans_A, use_alpha, use_beta>::apply_blas_type(C, A, alpha, beta);
  }

  template <typename TA>
  arma_inline static void apply(Mat<double>& C, const TA& A,
  ...
  template <typename TA>
  arma_inline static void apply(Mat<std::complex<float> >& C, const TA& A,
  ...
  arma_inline static void apply(
      Mat<std::complex<double> >& C, const TA& A,
