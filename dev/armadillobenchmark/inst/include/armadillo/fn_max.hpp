...
template <typename T>
arma_warn_unused arma_inline typename arma_scalar_only<T>::result max(const T& x) {
  return x;
}
...
template <typename T1>
arma_warn_unused arma_inline const OpCube<T1, op_max> max(
...
template <typename T1, typename T2>
arma_warn_unused arma_inline const GlueCube<T1, T2, glue_max> max(
...
arma_warn_unused inline typename enable_if2<
    ...
    const mtSpReduceOp<typename T1::elem_type, T1, op_sp_max> >::result
max(const T1& X) {
  arma_debug_sigprint();
...
arma_warn_unused inline typename enable_if2<
    (is_arma_sparse_type<T1>::value && is_arma_sparse_type<T2>::value &&
     is_same_type<typename T1::elem_type, typename T2::elem_type>::value),
    const SpGlue<T1, T2, spglue_max> >::result
max(const T1& x, const T2& y) {
  arma_debug_sigprint();
...
max(const T1& x, const T2& y) {
  ...
  // TODO: if there is a matrix size mismatch, the debug assert will print the matrix
  // sizes in wrong order
  spglue_max::dense_sparse_max(out, y, x);

  return out;
}

arma_warn_unused inline uword max(const SizeMat& s) {
  return (std::max)(s.n_rows, s.n_cols);
}

arma_warn_unused inline uword max(const SizeCube& s) {
