...
inline bool auxlib::inv(Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (n > 16) {
      ...
      lapack::getri(&n, A.memptr(), &lda, ipiv.memptr(), &work_query[0], &lwork_query,
                    &info);
...
template <typename eT>
inline bool auxlib::inv_rcond(Mat<eT>& A, typename get_pod_type<eT>::result& out_rcond) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_gen(A)
                   : lapack::lange<eT>(&norm_id, &n, &n, A.memptr(), &lda, junk.memptr());
...
template <typename eT>
inline bool auxlib::inv_tr(Mat<eT>& A, const uword layout) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::inv_tr_rcond(Mat<eT>& A, typename get_pod_type<eT>::result& out_rcond,
                                 const uword layout) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::inv_sympd(Mat<eT>& A, bool& out_sympd_state) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // NOTE: for complex matrices, zpotrf() assumes the matrix is hermitian (not simply
    // symmetric)
...
template <typename eT>
inline bool auxlib::inv_sympd(Mat<eT>& out, const Mat<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::inv_sympd_rcond(Mat<eT>& A, bool& out_sympd_state, eT& out_rcond) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_sym(A)
                   : lapack::lansy(&norm_id, &uplo, &n, A.memptr(), &n, work.memptr());
...
template <typename T>
inline bool auxlib::inv_sympd_rcond(Mat<std::complex<T> >& A, bool& out_sympd_state,
                                    T& out_rcond) {
  arma_debug_sigprint();
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<T>::value)
                   ? auxlib::norm1_sym(A)
                   : lapack::lanhe(&norm_id, &uplo, &n, A.memptr(), &n, work.memptr());
...
template <typename eT>
inline bool auxlib::det(eT& out_val, Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // on output A appears to be L+U_alt, where U_alt is U with the main diagonal set to
    // zero
    eT val = A.at(0, 0);
...
template <typename eT>
inline bool auxlib::log_det(eT& out_val, typename get_pod_type<eT>::result& out_sign,
                            Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    eT val = (is_cx<eT>::no)
                 ? std::log((access::tmp_real(A.at(0, 0)) < T(0)) ? A.at(0, 0) * T(-1)
                                                                  : A.at(0, 0))
                 : std::log(A.at(0, 0));
    ...
    for (uword i = 0; i < A.n_rows; ++i) {
      if (blas_int(i) !=
          (ipiv.mem[i] -
           1))  // NOTE: adjustment of -1 is required as Fortran counts from 1
      {
        sign *= -1;
      }
    }
...
template <typename eT>
inline bool auxlib::log_det_sympd(typename get_pod_type<eT>::result& out_val,
                                  Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    for (uword i = 0; i < A.n_rows; ++i) {
      val += std::log(access::tmp_real(A.at(i, i)));
    }
...
inline bool auxlib::lu(Mat<eT>& L, Mat<eT>& U, podarray<blas_int>& ipiv,
                       const Base<eT, T1>& X) {
  arma_debug_sigprint();
...
inline bool auxlib::eig_gen(Mat<std::complex<typename T1::pod_type> >& vals,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (arma_config::check_nonfinite && X.internal_has_nonfinite()) {
      return false;
    }
    ...
    blas_int lwork = 64 * N;  // lwork_min = (vecs_on) ? (std::max)(blas_int(1), 4*N) :
                              // (std::max)(blas_int(1), 3*N)
    blas_int info = 0;
    ...
    lapack::geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals_real.memptr(),
                 vals_imag.memptr(), vl, &ldvl, vr, &ldvr, work.memptr(), &lwork, &info);
...
inline bool auxlib::eig_gen(Mat<std::complex<typename T1::pod_type> >& vals,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals.memptr(), vl, &ldvl, vr,
                    &ldvr, work.memptr(), &lwork, rwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::eig_gen_balance(Mat<std::complex<typename T1::pod_type> >& vals,
                                    Mat<std::complex<typename T1::pod_type> >& vecs,
                                    const bool vecs_on,
                                    const Base<typename T1::pod_type, T1>& expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    T abnrm = T(0);
    blas_int lwork = 64 * N;  // lwork_min = (vecs_on) ? (std::max)(blas_int(1), 2*N) :
                              // (std::max)(blas_int(1), 3*N)
    ...
    podarray<T> work(static_cast<uword>(lwork));
    podarray<blas_int> iwork(
        uword(1));  // iwork not used by lapack::geevx() as sense = 'N'

    podarray<T> vals_real(X.n_rows);
    podarray<T> vals_imag(X.n_rows);

    arma_debug_print("lapack::geevx() -- START");
    lapack::geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N, vals_real.memptr(),
                  vals_imag.memptr(), vl, &ldvl, vr, &ldvr, &ilo, &ihi, scale.memptr(),
                  &abnrm, rconde.memptr(), rcondv.memptr(), work.memptr(), &lwork,
                  iwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::eig_gen_balance(
    Mat<std::complex<typename T1::pod_type> >& vals,
    Mat<std::complex<typename T1::pod_type> >& vecs, const bool vecs_on,
    const Base<std::complex<typename T1::pod_type>, T1>& expr) {
  arma_debug_sigprint();
  ...
  {
    arma_debug_print(
        "auxlib::eig_gen_balance(): redirecting to auxlib::eig_gen() due to crippled "
        "LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N, vals.memptr(), vl,
                     &ldvl, vr, &ldvr, &ilo, &ihi, scale.memptr(), &abnrm,
                     rconde.memptr(), rcondv.memptr(), work.memptr(), &lwork,
                     rwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::eig_gen_twosided(Mat<std::complex<typename T1::pod_type> >& vals,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::geev() -- START");
    lapack::geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals_real.memptr(),
                 vals_imag.memptr(), ltmp.memptr(), &ldvl, rtmp.memptr(), &ldvr,
...
template <typename T1>
inline bool auxlib::eig_gen_twosided(
    Mat<std::complex<typename T1::pod_type> >& vals,
    Mat<std::complex<typename T1::pod_type> >& lvecs,
    Mat<std::complex<typename T1::pod_type> >& rvecs,
    const Base<std::complex<typename T1::pod_type>, T1>& expr) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::cx_geev() -- START");
    lapack::cx_geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals.memptr(), lvecs.memptr(),
                    &ldvl, rvecs.memptr(), &ldvr, work.memptr(), &lwork, rwork.memptr(),
                    &info);
    arma_debug_print("lapack::cx_geev() -- END");
...
template <typename T1>
inline bool auxlib::eig_gen_twosided_balance(
    Mat<std::complex<typename T1::pod_type> >& vals,
    Mat<std::complex<typename T1::pod_type> >& lvecs,
    Mat<std::complex<typename T1::pod_type> >& rvecs,
    const Base<typename T1::pod_type, T1>& expr) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::geevx() -- START");
    lapack::geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N, vals_real.memptr(),
                  vals_imag.memptr(), ltmp.memptr(), &ldvl, rtmp.memptr(), &ldvr, &ilo,
                  &ihi, scale.memptr(), &abnrm, rconde.memptr(), rcondv.memptr(),
...
inline bool auxlib::eig_gen_twosided_balance(
  ...
  {
    arma_debug_print(
        "auxlib::eig_gen_twosided_balance(): redirecting to auxlib::eig_gen() due to "
        "crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::cx_geevx() -- START");
    lapack::cx_geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N, vals.memptr(),
                     lvecs.memptr(), &ldvl, rvecs.memptr(), &ldvr, &ilo, &ihi,
                     scale.memptr(), &abnrm, rconde.memptr(), rcondv.memptr(),
...
inline bool auxlib::eig_pair(Mat<std::complex<typename T1::pod_type> >& vals,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_conform_check(((A.is_square() == false) || (B.is_square() == false)),
                       "eig_pair(): given matrices must be square sized");

    arma_conform_check((A.n_rows != B.n_rows),
                       "eig_pair(): given matrices must have the same size");
    ...
    if (arma_config::check_nonfinite && A.internal_has_nonfinite()) {
      return false;
    }
    if (arma_config::check_nonfinite && B.internal_has_nonfinite()) {
      return false;
    }
    ...
    lapack::ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N, alphar.memptr(),
                 alphai.memptr(), beta.memptr(), vl, &ldvl, vr, &ldvr, work.memptr(),
                 &lwork, &info);
    ...
    if (beta_has_zero) {
      arma_warn(1, "eig_pair(): given matrices appear ill-conditioned");
    }
...
inline bool auxlib::eig_pair(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N, alpha.memptr(),
                    beta.memptr(), vl, &ldvl, vr, &ldvr, work.memptr(), &lwork,
                    rwork.memptr(), &info);
...
template <typename T1, typename T2>
inline bool auxlib::eig_pair_twosided(Mat<std::complex<typename T1::pod_type> >& vals,
                                      Mat<std::complex<typename T1::pod_type> >& lvecs,
                                      Mat<std::complex<typename T1::pod_type> >& rvecs,
                                      const Base<typename T1::pod_type, T1>& A_expr,
                                      const Base<typename T1::pod_type, T2>& B_expr) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::ggev()");
    lapack::ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N, alphar.memptr(),
                 alphai.memptr(), beta.memptr(), ltmp.memptr(), &ldvl, rtmp.memptr(),
                 &ldvr, work.memptr(), &lwork, &info);
...
template <typename T1, typename T2>
inline bool auxlib::eig_pair_twosided(
    Mat<std::complex<typename T1::pod_type> >& vals,
    Mat<std::complex<typename T1::pod_type> >& lvecs,
    Mat<std::complex<typename T1::pod_type> >& rvecs,
    const Base<std::complex<typename T1::pod_type>, T1>& A_expr,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::cx_ggev()");
    lapack::cx_ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N, alpha.memptr(),
                    beta.memptr(), lvecs.memptr(), &ldvl, rvecs.memptr(), &ldvr,
                    work.memptr(), &lwork, rwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::eig_sym(Col<eT>& eigval, Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::syev(&jobz, &uplo, &N, A.memptr(), &N, eigval.memptr(), work.memptr(), &lwork,
                 &info);
...
template <typename T>
inline bool auxlib::eig_sym(Col<T>& eigval, Mat<std::complex<T> >& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::heev(&jobz, &uplo, &N, A.memptr(), &N, eigval.memptr(), work.memptr(), &lwork,
                 rwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::eig_sym(Col<eT>& eigval, Mat<eT>& eigvec, const Mat<eT>& X) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::syev()");
    lapack::syev(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(), work.memptr(),
                 &lwork, &info);
...
template <typename T>
inline bool auxlib::eig_sym(Col<T>& eigval, Mat<std::complex<T> >& eigvec,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::heev()");
    lapack::heev(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(), work.memptr(),
...
template <typename eT>
inline bool auxlib::eig_sym_dc(Col<eT>& eigval, Mat<eT>& eigvec, const Mat<eT>& X) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (N >= 32) {
      ...
      lapack::syevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                    &work_query[0], &lwork_query, &iwork_query[0], &liwork_query, &info);
    ...
    arma_debug_print("lapack::syevd()");
    lapack::syevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(), work.memptr(),
                  &lwork_final, iwork.memptr(), &liwork_final, &info);
...
//! algorithm)
template <typename T>
inline bool auxlib::eig_sym_dc(Col<T>& eigval, Mat<std::complex<T> >& eigvec,
                               const Mat<std::complex<T> >& X) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (N >= 32) {
      ...
      lapack::heevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                    &work_query[0], &lwork_query, &rwork_query[0], &lrwork_query,
                    &iwork_query[0], &liwork_query, &info);
    ...
    arma_debug_print("lapack::heevd()");
    lapack::heevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(), work.memptr(),
                  &lwork_final, rwork.memptr(), &lrwork_final, iwork.memptr(),
                  &liwork_final, &info);
...
template <typename eT>
inline bool auxlib::chol_simple(Mat<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::chol(Mat<eT>& X, const uword layout) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    X = (layout == 0) ? trimatu(X)
                      : trimatl(X);  // trimatu() and trimatl() return the same type
...
template <typename eT>
inline bool auxlib::chol_band(Mat<eT>& X, const uword KD, const uword layout) {
  arma_debug_sigprint();
...
template <typename T>
inline bool auxlib::chol_band(Mat<std::complex<T> >& X, const uword KD,
                              const uword layout) {
  arma_debug_sigprint();
  ...
  {
    arma_debug_print(
        "auxlib::chol_band(): redirecting to auxlib::chol() due to crippled LAPACK");
...
template <typename eT>
inline bool auxlib::chol_band_common(Mat<eT>& X, const uword KD, const uword layout) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::chol_pivot(Mat<eT>& X, Mat<uword>& P, const uword layout) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::pstrf(&uplo, &n, X.memptr(), &n, ipiv.memptr(), &rank, &tol, work.memptr(),
                  &info);
...
template <typename eT, typename T1>
inline bool auxlib::hess(Mat<eT>& H, const Base<eT, T1>& X, Col<eT>& tao) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_conform_check((H.is_square() == false),
                       "hess(): given matrix must be square sized");
    ...
    if (H.n_rows > 2) {
      ...
      lapack::gehrd(&n, &ilo, &ihi, H.memptr(), &lda, tao.memptr(), work.memptr(), &lwork,
                    &info);
...
inline bool auxlib::qr(Mat<eT>& Q, Mat<eT>& R, const Base<eT, T1>& X) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::geqrf(&m, &n, R.memptr(), &m, tau.memptr(), &work_query[0], &lwork_query,
                  &info);
    ...
    arma_debug_print("lapack::geqrf()");
    lapack::geqrf(&m, &n, R.memptr(), &m, tau.memptr(), work.memptr(), &lwork_final,
                  &info);
    ...
    if ((is_float<eT>::value) || (is_double<eT>::value)) {
      ...
      lapack::orgqr(&m, &m, &k, Q.memptr(), &m, tau.memptr(), work.memptr(), &lwork_final,
                    &info);
    } else if ((is_cx_float<eT>::value) || (is_cx_double<eT>::value)) {
      arma_debug_print("lapack::ungqr()");
      lapack::ungqr(&m, &m, &k, Q.memptr(), &m, tau.memptr(), work.memptr(), &lwork_final,
                    &info);
    }
...
inline bool auxlib::qr_econ(Mat<eT>& Q, Mat<eT>& R, const Base<eT, T1>& X) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::geqrf()");
    lapack::geqrf(&m, &n, Q.memptr(), &m, tau.memptr(), &work_query[0], &lwork_query,
                  &info);
    ...
    arma_debug_print("lapack::geqrf()");
    lapack::geqrf(&m, &n, Q.memptr(), &m, tau.memptr(), work.memptr(), &lwork_final,
                  &info);
    ...
    if ((is_float<eT>::value) || (is_double<eT>::value)) {
      arma_debug_print("lapack::orgqr()");
      lapack::orgqr(&m, &n, &k, Q.memptr(), &m, tau.memptr(), work.memptr(), &lwork_final,
                    &info);
    } else if ((is_cx_float<eT>::value) || (is_cx_double<eT>::value)) {
      arma_debug_print("lapack::ungqr()");
      lapack::ungqr(&m, &n, &k, Q.memptr(), &m, tau.memptr(), work.memptr(), &lwork_final,
                    &info);
    }
...
template <typename eT, typename T1>
inline bool auxlib::qr_pivot(Mat<eT>& Q, Mat<eT>& R, Mat<uword>& P,
                             const Base<eT, T1>& X) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min = (std::max)(
        blas_int(3 * n + 1),
        (std::max)(m, n));  // take into account requirements of geqp3() and orgqr()
    blas_int k = (std::min)(m, n);
    blas_int info = 0;
    ...
    lapack::geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(), &work_query[0],
                  &lwork_query, &info);
    ...
    arma_debug_print("lapack::geqp3()");
    lapack::geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(), work.memptr(),
                  &lwork_final, &info);
...
inline bool auxlib::qr_pivot(Mat<std::complex<T> >& Q, Mat<std::complex<T> >& R,
                             Mat<uword>& P, const Base<std::complex<T>, T1>& X) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min = (std::max)(
        blas_int(3 * n + 1),
        (std::max)(m, n));  // take into account requirements of geqp3() and ungqr()
    blas_int k = (std::min)(m, n);
    blas_int info = 0;
    ...
    arma_debug_print("lapack::geqp3()");
    lapack::cx_geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(), &work_query[0],
                     &lwork_query, rwork.memptr(), &info);
    ...
    arma_debug_print("lapack::geqp3()");
    lapack::cx_geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(), work.memptr(),
                     &lwork_final, rwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::svd(Col<eT>& S, Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min =
        (std::max)(blas_int(1), (std::max)((3 * min_mn + (std::max)(m, n)), 5 * min_mn));
    blas_int info = 0;
    ...
    if (A.n_elem >= 1024) {
      ...
      lapack::gesvd<eT>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(),
                        &ldu, V.memptr(), &ldvt, &work_query[0], &lwork_query, &info);
    ...
    arma_debug_print("lapack::gesvd()");
    lapack::gesvd<eT>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(),
                      &ldu, V.memptr(), &ldvt, work.memptr(), &lwork_final, &info);
...
template <typename T>
inline bool auxlib::svd(Col<T>& S, Mat<std::complex<T> >& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_elem >= 256) {
      ...
      lapack::cx_gesvd<T>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(),
                          &ldu, V.memptr(), &ldvt, &work_query[0], &lwork_query,
                          rwork.memptr(), &info);
    ...
    arma_debug_print("lapack::cx_gesvd()");
    lapack::cx_gesvd<T>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(),
                        &ldu, V.memptr(), &ldvt, work.memptr(), &lwork_final,
                        rwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::svd(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, Mat<eT>& A) {
  arma_debug_sigprint();
...
template <typename T>
inline bool auxlib::svd(Mat<std::complex<T> >& U, Col<T>& S, Mat<std::complex<T> >& V,
                        Mat<std::complex<T> >& A) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::svd_econ(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, Mat<eT>& A,
                             const char mode) {
  arma_debug_sigprint();
...
template <typename T>
inline bool auxlib::svd_econ(Mat<std::complex<T> >& U, Col<T>& S,
                             Mat<std::complex<T> >& V, Mat<std::complex<T> >& A,
                             const char mode) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool auxlib::svd_dc(Col<eT>& S, Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_elem >= 1024) {
      ...
      lapack::gesdd<eT>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(), &ldu,
                        V.memptr(), &ldvt, &work_query[0], &lwork_query, iwork.memptr(),
                        &info);
    ...
    arma_debug_print("lapack::gesdd()");
    lapack::gesdd<eT>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(), &ldu,
                      V.memptr(), &ldvt, work.memptr(), &lwork_final, iwork.memptr(),
                      &info);
...
template <typename T>
inline bool auxlib::svd_dc(Col<T>& S, Mat<std::complex<T> >& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    podarray<T> rwork(static_cast<uword>(
        7 * min_mn));  // from LAPACK 3.8 docs: LAPACK <= v3.6 needs 7*mn
    podarray<blas_int> iwork(static_cast<uword>(8 * min_mn));
    ...
    if (A.n_elem >= 256) {
      ...
      lapack::cx_gesdd<T>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(), &ldu,
                          V.memptr(), &ldvt, &work_query[0], &lwork_query, rwork.memptr(),
                          iwork.memptr(), &info);
    ...
    arma_debug_print("lapack::cx_gesdd()");
    lapack::cx_gesdd<T>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(), &ldu,
                        V.memptr(), &ldvt, work.memptr(), &lwork_final, rwork.memptr(),
                        iwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::svd_dc(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork1 =
        3 * min_mn * min_mn +
        (std::max)(max_mn, 4 * min_mn * min_mn + 4 * min_mn);  // as per LAPACK 3.2 docs
    blas_int lwork2 = 4 * min_mn * min_mn + 6 * min_mn +
                      max_mn;  // as per LAPACK 3.8 docs; consistent with LAPACK 3.4 docs
    blas_int lwork_min =
        (std::max)(lwork1, lwork2);  // due to differences between LAPACK 3.2 and 3.8
    blas_int info = 0;
...
template <typename T>
inline bool auxlib::svd_dc(Mat<std::complex<T> >& U, Col<T>& S, Mat<std::complex<T> >& V,
                           Mat<std::complex<T> >& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int ldvt = blas_int(V.n_rows);
    blas_int lwork_min =
        min_mn * min_mn + 2 * min_mn + max_mn;  // as per LAPACK 3.2, 3.4, 3.8 docs
    blas_int lrwork =
        min_mn *
        ((std::max)(5 * min_mn + 7, 2 * max_mn + 2 * min_mn +
                                        1));  // as per LAPACK 3.4 docs; LAPACK 3.8 uses
                                              // 5*min_mn+5 instead of 5*min_mn+7
    blas_int info = 0;
...
template <typename eT>
inline bool auxlib::svd_dc_econ(Mat<eT>& U, Col<eT>& S, Mat<eT>& V, Mat<eT>& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork1 =
        ...
        (std::max)(max_mn, 4 * min_mn * min_mn + 4 * min_mn);  // as per LAPACK 3.2 docs
    blas_int lwork2 =
        4 * min_mn * min_mn + 6 * min_mn +
        max_mn;  // as per LAPACK 3.4 docs; LAPACK 3.8 requires 4*min_mn*min_mn + 7*min_mn
    blas_int lwork_min =
        (std::max)(lwork1, lwork2);  // due to differences between LAPACK 3.2 and 3.4
    blas_int info = 0;
...
template <typename T>
inline bool auxlib::svd_dc_econ(Mat<std::complex<T> >& U, Col<T>& S,
                                Mat<std::complex<T> >& V, Mat<std::complex<T> >& A) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lrwork =
        min_mn * ((std::max)(5 * min_mn + 7,
                             2 * max_mn + 2 * min_mn +
                                 1));  // LAPACK 3.8 uses 5*min_mn+5 instead of 5*min_mn+7
    blas_int info = 0;
...
template <typename T1>
inline bool auxlib::solve_square_fast(Mat<typename T1::elem_type>& out,
  ...
  arma_conform_check((A.n_rows != B_n_rows),
                     "solve(): number of rows in given matrices must be the same",
                     [&]() { out.soft_reset(); });
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    podarray<blas_int> ipiv(
        A.n_rows +
        2);  // +2 for paranoia: some versions of Lapack might be trashing memory

    arma_debug_print("lapack::gesv()");
    lapack::gesv<eT>(&n, &nrhs, A.memptr(), &lda, ipiv.memptr(), out.memptr(), &ldb,
                     &info);
...
template <typename T1>
inline bool auxlib::solve_square_rcond(Mat<typename T1::elem_type>& out,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::getrs<eT>(&trans, &n, &nrhs, A.memptr(), &lda, ipiv.memptr(), out.memptr(),
                      &ldb, &info);
...
//! matrices)
template <typename T1>
inline bool auxlib::solve_square_refine(Mat<typename T1::pod_type>& out,
                                        typename T1::pod_type& out_rcond,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // Mat<eT> B = B_expr.get_ref();  // B is overwritten by lapack::gesvx() if
    // equilibrate is enabled
    ...
    lapack::gesvx(&fact, &trans, &n, &nrhs, A.memptr(), &lda, AF.memptr(), &ldaf,
                  IPIV.memptr(), &equed, R.memptr(), C.memptr(),
                  const_cast<eT*>(B.memptr()), &ldb, out.memptr(), &ldx, &rcond,
                  FERR.memptr(), BERR.memptr(), WORK.memptr(), IWORK.memptr(), &info);

    // NOTE: using const_cast<eT*>(B.memptr()) to allow B to be overwritten for
    // equilibration; NOTE: B is created as a copy of B_expr if equilibration is enabled;
    // otherwise B is a reference to B_expr
...
//! matrices)
template <typename T1>
inline bool auxlib::solve_square_refine(
    Mat<std::complex<typename T1::pod_type> >& out, typename T1::pod_type& out_rcond,
    Mat<std::complex<typename T1::pod_type> >& A,
    const Base<std::complex<typename T1::pod_type>, T1>& B_expr, const bool equilibrate) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // Mat<eT> B = B_expr.get_ref();  // B is overwritten by lapack::cx_gesvx() if
    // equilibrate is enabled
    ...
    lapack::cx_gesvx(&fact, &trans, &n, &nrhs, A.memptr(), &lda, AF.memptr(), &ldaf,
                     IPIV.memptr(), &equed, R.memptr(), C.memptr(),
                     const_cast<eT*>(B.memptr()), &ldb, out.memptr(), &ldx, &rcond,
                     FERR.memptr(), BERR.memptr(), WORK.memptr(), RWORK.memptr(), &info);
...
template <typename T1>
inline bool auxlib::solve_sympd_fast(Mat<typename T1::elem_type>& out,
                                     Mat<typename T1::elem_type>& A,
                                     const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
  ...
  {
    arma_debug_print(
        "auxlib::solve_sympd_fast(): redirecting to auxlib::solve_square_fast() due to "
        "crippled LAPACK");
...
template <typename T1>
inline bool auxlib::solve_sympd_fast_common(
    Mat<typename T1::elem_type>& out, Mat<typename T1::elem_type>& A,
    const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
...
//! solve a system of linear equations via Cholesky decomposition with rcond estimate
//! (real matrices)
template <typename T1>
inline bool auxlib::solve_sympd_rcond(Mat<typename T1::pod_type>& out,
                                      bool& out_sympd_state,
                                      typename T1::pod_type& out_rcond,
                                      Mat<typename T1::pod_type>& A,
                                      const Base<typename T1::pod_type, T1>& B_expr) {
  arma_debug_sigprint();
...
//! (complex matrices)
template <typename T1>
inline bool auxlib::solve_sympd_rcond(
    Mat<std::complex<typename T1::pod_type> >& out, bool& out_sympd_state,
    typename T1::pod_type& out_rcond, Mat<std::complex<typename T1::pod_type> >& A,
  ...
  {
    arma_debug_print(
        "auxlib::solve_sympd_rcond(): redirecting to auxlib::solve_square_rcond() due to "
        "crippled LAPACK");
...
//! solve a system of linear equations via Cholesky decomposition with refinement (real
//! matrices)
template <typename T1>
inline bool auxlib::solve_sympd_refine(Mat<typename T1::pod_type>& out,
                                       typename T1::pod_type& out_rcond,
                                       Mat<typename T1::pod_type>& A,
                                       const Base<typename T1::pod_type, T1>& B_expr,
                                       const bool equilibrate) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // Mat<eT> B = B_expr.get_ref();  // B is overwritten by lapack::posvx() if
    // equilibrate is enabled
    ...
    lapack::posvx(&fact, &uplo, &n, &nrhs, A.memptr(), &lda, AF.memptr(), &ldaf, &equed,
                  S.memptr(), const_cast<eT*>(B.memptr()), &ldb, out.memptr(), &ldx,
                  &rcond, FERR.memptr(), BERR.memptr(), WORK.memptr(), IWORK.memptr(),
                  &info);
...
//! solve a system of linear equations via Cholesky decomposition with refinement (complex
//! matrices)
template <typename T1>
inline bool auxlib::solve_sympd_refine(
    Mat<std::complex<typename T1::pod_type> >& out, typename T1::pod_type& out_rcond,
    Mat<std::complex<typename T1::pod_type> >& A,
    const Base<std::complex<typename T1::pod_type>, T1>& B_expr, const bool equilibrate) {
  arma_debug_sigprint();
  ...
  {
    arma_debug_print(
        "auxlib::solve_sympd_refine(): redirecting to auxlib::solve_square_refine() due "
        "to crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    // Mat<eT> B = B_expr.get_ref();  // B is overwritten by lapack::cx_posvx() if
    // equilibrate is enabled
    ...
    lapack::cx_posvx(&fact, &uplo, &n, &nrhs, A.memptr(), &lda, AF.memptr(), &ldaf,
                     &equed, S.memptr(), const_cast<eT*>(B.memptr()), &ldb, out.memptr(),
                     &ldx, &rcond, FERR.memptr(), BERR.memptr(), WORK.memptr(),
                     RWORK.memptr(), &info);
...
template <typename T1>
inline bool auxlib::solve_rect_fast(Mat<typename T1::elem_type>& out,
                                    Mat<typename T1::elem_type>& A,
                                    const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_elem >= ((is_cx<eT>::yes) ? uword(256) : uword(1024))) {
      ...
      lapack::gels<eT>(&trans, &m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb,
...
//! solve a non-square full-rank system via QR or LQ decomposition with rcond estimate
//! (experimental)
template <typename T1>
inline bool auxlib::solve_rect_rcond(Mat<typename T1::elem_type>& out,
                                     typename T1::pod_type& out_rcond,
                                     Mat<typename T1::elem_type>& A,
                                     const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_rows >= A.n_cols) {
      ...
      // xGELS  docs: for M >= N, A contains details of its QR decomposition as returned
      // by xGEQRF xGEQRF docs: elements on and above the diagonal contain the
      ...
      // xGELS  docs: for M < N, A contains details of its LQ decomposition as returned by
      // xGELQF xGELQF docs: elements on and below the diagonal contain the M-by-min(M,N)
...
template <typename T1>
inline bool auxlib::solve_approx_svd(Mat<typename T1::pod_type>& out,
                                     Mat<typename T1::pod_type>& A,
                                     const Base<typename T1::pod_type, T1>& B_expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // NOTE: assuming LAPACK 3.8+, where the workspace query also obtains liwork in
    // addition to lwork
    ...
    blas_int laenv_result =
        (arma_config::hidden_args)
            ? blas_int(lapack::laenv(&ispec, name, opts, &n1, &n2, &n3, &n4, 6, 1))
            : blas_int(0);
    ...
    blas_int nlvl =
        (std::max)(blas_int(0),
                   blas_int(1) + blas_int(std::log2(double(min_mn) / double(smlsiz_p1))));

    blas_int lwork_min = blas_int(12) * min_mn + blas_int(2) * min_mn * smlsiz +
                         blas_int(8) * min_mn * nlvl + min_mn * nrhs +
                         smlsiz_p1 * smlsiz_p1;
    blas_int liwork_min =
        (std::max)(blas_int(1), (blas_int(3) * min_mn * nlvl + blas_int(11) * min_mn));
    ...
    lapack::gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb, S.memptr(), &rcond,
                  &rank, &work_query[0], &lwork_query, &iwork_query[0], &info);
    ...
    arma_debug_print("lapack::gelsd()");
    lapack::gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb, S.memptr(), &rcond,
                  &rank, work.memptr(), &lwork_final, iwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::solve_approx_svd(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // NOTE: assuming LAPACK 3.8+, where the workspace query also obtains lrwork and
    // liwork in addition to lwork
    ...
    blas_int lrwork_min =
        (m >= n) ? blas_int(10) * n + blas_int(2) * n * smlsiz + blas_int(8) * n * nlvl +
                       blas_int(3) * smlsiz * nrhs +
                       (std::max)((smlsiz_p1) * (smlsiz_p1),
                                  n * (blas_int(1) + nrhs) + blas_int(2) * nrhs)
                 : blas_int(10) * m + blas_int(2) * m * smlsiz + blas_int(8) * m * nlvl +
                       blas_int(3) * smlsiz * nrhs +
                       (std::max)((smlsiz_p1) * (smlsiz_p1),
                                  n * (blas_int(1) + nrhs) + blas_int(2) * nrhs);

    blas_int liwork_min = (std::max)(blas_int(1), (blas_int(3) * blas_int(min_mn) * nlvl +
                                                   blas_int(11) * blas_int(min_mn)));
    ...
    lapack::cx_gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb, S.memptr(),
                     &rcond, &rank, &work_query[0], &lwork_query, &rwork_query[0],
                     &iwork_query[0], &info);
    ...
    arma_debug_print("lapack::cx_gelsd()");
    lapack::cx_gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb, S.memptr(),
                     &rcond, &rank, work.memptr(), &lwork_final, rwork.memptr(),
                     iwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::solve_trimat_fast(Mat<typename T1::elem_type>& out,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::trtrs(&uplo, &trans, &diag, &n, &nrhs, A.memptr(), &n, out.memptr(), &n,
                  &info);
...
template <typename T1>
inline bool auxlib::solve_trimat_rcond(Mat<typename T1::elem_type>& out,
                                       typename T1::pod_type& out_rcond,
                                       const Mat<typename T1::elem_type>& A,
                                       const Base<typename T1::elem_type, T1>& B_expr,
                                       const uword layout) {
...
template <typename T1>
inline bool auxlib::solve_band_fast(Mat<typename T1::pod_type>& out,
                                    Mat<typename T1::pod_type>& A, const uword KL,
                                    const uword KU,
                                    const Base<typename T1::pod_type, T1>& B_expr) {
  arma_debug_sigprint();
...
template <typename T1>
inline bool auxlib::solve_band_fast(
    Mat<std::complex<typename T1::pod_type> >& out,
    Mat<std::complex<typename T1::pod_type> >& A, const uword KL, const uword KU,
    const Base<std::complex<typename T1::pod_type>, T1>& B_expr) {
  arma_debug_sigprint();
  ...
  {
    arma_debug_print(
        "auxlib::solve_band_fast(): redirecting to auxlib::solve_square_fast() due to "
        "crippled LAPACK");
...
template <typename T1>
inline bool auxlib::solve_band_fast_common(
    Mat<typename T1::elem_type>& out, const Mat<typename T1::elem_type>& A,
    const uword KL, const uword KU, const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // for gbsv, matrix AB size: 2*KL+KU+1 x N; band representation of A stored in rows
    // KL+1 to 2*KL+KU+1  (note: fortran counts from 1)
    ...
    lapack::gbsv<eT>(&n, &kl, &ku, &nrhs, AB.memptr(), &ldab, ipiv.memptr(), out.memptr(),
                     &ldb, &info);
...
//! solve a system of linear equations via LU decomposition (real band matrix)
template <typename T1>
inline bool auxlib::solve_band_rcond(Mat<typename T1::pod_type>& out,
                                     typename T1::pod_type& out_rcond,
                                     Mat<typename T1::pod_type>& A, const uword KL,
                                     const uword KU,
                                     const Base<typename T1::pod_type, T1>& B_expr) {
...
inline bool auxlib::solve_band_rcond(
  ...
  {
    arma_debug_print(
        "auxlib::solve_band_rcond(): redirecting to auxlib::solve_square_rcond() due to "
        "crippled LAPACK");
...
//! solve a system of linear equations via LU decomposition (band matrix)
template <typename T1>
inline bool auxlib::solve_band_rcond_common(
    Mat<typename T1::elem_type>& out, typename T1::pod_type& out_rcond,
    const Mat<typename T1::elem_type>& A, const uword KL, const uword KU,
    const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // for gbtrf, matrix AB size: 2*KL+KU+1 x N; band representation of A stored in rows
    // KL+1 to 2*KL+KU+1  (note: fortran counts from 1)
    ...
    // // NOTE: lapack::langb() and lapack::gbtrf() use incompatible storage formats for
    // the band matrix arma_debug_print("lapack::langb()"); norm_val =
    // lapack::langb<eT>(&norm_id, &n, &kl, &ku, AB.memptr(), &ldab, junk.memptr());
    ...
    lapack::gbtrs<eT>(&trans, &n, &kl, &ku, &nrhs, AB.memptr(), &ldab, ipiv.memptr(),
                      out.memptr(), &ldb, &info);
...
//! solve a system of linear equations via LU decomposition with refinement (real band
//! matrices)
template <typename T1>
inline bool auxlib::solve_band_refine(Mat<typename T1::pod_type>& out,
                                      typename T1::pod_type& out_rcond,
                                      Mat<typename T1::pod_type>& A, const uword KL,
                                      const uword KU,
                                      const Base<typename T1::pod_type, T1>& B_expr,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // for gbsvx, matrix AB size: KL+KU+1 x N; band representation of A stored in rows 1
    // to KL+KU+1  (note: fortran counts from 1)
    ...
    lapack::gbsvx(&fact, &trans, &n, &kl, &ku, &nrhs, AB.memptr(), &ldab, AFB.memptr(),
                  &ldafb, IPIV.memptr(), &equed, R.memptr(), C.memptr(), B.memptr(), &ldb,
                  out.memptr(), &ldx, &rcond, FERR.memptr(), BERR.memptr(), WORK.memptr(),
                  IWORK.memptr(), &info);
...
//! solve a system of linear equations via LU decomposition with refinement (complex band
//! matrices)
template <typename T1>
inline bool auxlib::solve_band_refine(
  ...
  {
    arma_debug_print(
        "auxlib::solve_band_refine(): redirecting to auxlib::solve_square_refine() due "
        "to crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    podarray<T> RWORK(N);  // NOTE: according to lapack 3.6.1 docs, the size of RWORK in
                           // zgbsvx is different to RWORK in dgesvx

    arma_debug_print("lapack::cx_gbsvx()");
    lapack::cx_gbsvx(&fact, &trans, &n, &kl, &ku, &nrhs, AB.memptr(), &ldab, AFB.memptr(),
                     &ldafb, IPIV.memptr(), &equed, R.memptr(), C.memptr(), B.memptr(),
                     &ldb, out.memptr(), &ldx, &rcond, FERR.memptr(), BERR.memptr(),
                     WORK.memptr(), RWORK.memptr(), &info);
...
//! (real tridiagonal band matrix)
template <typename T1>
inline bool auxlib::solve_tridiag_fast(Mat<typename T1::pod_type>& out,
                                       Mat<typename T1::pod_type>& A,
                                       const Base<typename T1::pod_type, T1>& B_expr) {
  arma_debug_sigprint();
...
//! solve a system of linear equations via Gaussian elimination with partial pivoting
//! (complex tridiagonal band matrix)
template <typename T1>
inline bool auxlib::solve_tridiag_fast(
    Mat<std::complex<typename T1::pod_type> >& out,
    Mat<std::complex<typename T1::pod_type> >& A,
    const Base<std::complex<typename T1::pod_type>, T1>& B_expr) {
  arma_debug_sigprint();
  ...
  {
    arma_debug_print(
        "auxlib::solve_tridiag_fast(): redirecting to auxlib::solve_square_fast() due to "
        "crippled LAPACK");
...
template <typename T1>
inline bool auxlib::solve_tridiag_fast_common(
    Mat<typename T1::elem_type>& out, const Mat<typename T1::elem_type>& A,
    const Base<typename T1::elem_type, T1>& B_expr) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::gtsv<eT>(&n, &nrhs, tridiag.colptr(0), tridiag.colptr(1), tridiag.colptr(2),
                     out.memptr(), &ldb, &info);
...
template <typename eT, typename T1>
inline bool auxlib::schur(Mat<eT>& U, Mat<eT>& S, const Base<eT, T1>& X,
                          const bool calc_U) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_conform_check((S.is_square() == false),
                       "schur(): given matrix must be square sized");
    ...
    lapack::gees(&jobvs, &sort, select, &n, S.memptr(), &n, &sdim, wr.memptr(),
                 wi.memptr(), U.memptr(), &ldvs, work.memptr(), &lwork, bwork.memptr(),
                 &info);
...
template <typename T, typename T1>
inline bool auxlib::schur(Mat<std::complex<T> >& U, Mat<std::complex<T> >& S,
                          const Base<std::complex<T>, T1>& X, const bool calc_U) {
  arma_debug_sigprint();
...
inline bool auxlib::schur(Mat<std::complex<T> >& U, Mat<std::complex<T> >& S,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gees(&jobvs, &sort, select, &n, S.memptr(), &n, &sdim, w.memptr(),
                    U.memptr(), &ldvs, work.memptr(), &lwork, rwork.memptr(),
                    bwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::syl(Mat<eT>& X, const Mat<eT>& A, const Mat<eT>& B,
                        const Mat<eT>& C) {
  arma_debug_sigprint();
  ...
  {
    arma_conform_check((A.is_square() == false) || (B.is_square() == false),
                       "syl(): given matrices must be square sized");

    arma_conform_check((C.n_rows != A.n_rows) || (C.n_cols != B.n_cols),
    ...
    lapack::trsyl<eT>(&trana, &tranb, &isgn, &m, &n, T1.memptr(), &m, T2.memptr(), &n,
                      Y.memptr(), &m, &scale, &info);
...
inline bool auxlib::qz(Mat<T>& A, Mat<T>& B, Mat<T>& vsl, Mat<T>& vsr,
                       const Base<T, T1>& X_expr, const Base<T, T2>& Y_expr,
                       const char mode) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_conform_check(((A.is_square() == false) || (B.is_square() == false)),
                       "qz(): given matrices must be square sized", [&]() {
                         A.soft_reset();
                         B.soft_reset();
                       });

    arma_conform_check((A.n_rows != B.n_rows),
                       "qz(): given matrices must have the same size");
    ...
    lapack::gges(&jobvsl, &jobvsr, &eigsort, selctg, &N, A.memptr(), &N, B.memptr(), &N,
                 &sdim, alphar.memptr(), alphai.memptr(), beta.memptr(), vsl.memptr(), &N,
                 vsr.memptr(), &N, work.memptr(), &lwork, bwork.memptr(), &info);
...
template <typename T, typename T1, typename T2>
inline bool auxlib::qz(Mat<std::complex<T> >& A, Mat<std::complex<T> >& B,
                       Mat<std::complex<T> >& vsl, Mat<std::complex<T> >& vsr,
                       const Base<std::complex<T>, T1>& X_expr,
                       const Base<std::complex<T>, T2>& Y_expr, const char mode) {
  arma_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gges(&jobvsl, &jobvsr, &eigsort, selctg, &N, A.memptr(), &N, B.memptr(),
                    &N, &sdim, alpha.memptr(), beta.memptr(), vsl.memptr(), &N,
                    vsr.memptr(), &N, work.memptr(), &lwork, rwork.memptr(),
                    bwork.memptr(), &info);
...
template <typename eT>
inline eT auxlib::rcond(Mat<eT>& A) {
#if defined(ARMA_USE_LAPACK)
  {
    arma_conform_assert_blas_size(A);
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_gen(A)
                   : lapack::lange(&norm_id, &m, &n, A.memptr(), &lda, work.memptr());
    ...
    lapack::gecon(&norm_id, &n, A.memptr(), &lda, &norm_val, &rcond, work.memptr(),
                  iwork.memptr(), &info);
...
template <typename T>
inline T auxlib::rcond(Mat<std::complex<T> >& A) {
#if defined(ARMA_USE_LAPACK)
  {
    typedef typename std::complex<T> eT;
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_gen(A)
                   : lapack::lange(&norm_id, &m, &n, A.memptr(), &lda, junk.memptr());
    ...
    lapack::cx_gecon(&norm_id, &n, A.memptr(), &lda, &norm_val, &rcond, work.memptr(),
                     rwork.memptr(), &info);
...
template <typename eT>
inline eT auxlib::rcond_sympd(Mat<eT>& A, bool& calc_ok) {
#if defined(ARMA_USE_LAPACK)
  {
    arma_conform_assert_blas_size(A);
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_sym(A)
                   : lapack::lansy(&norm_id, &uplo, &n, A.memptr(), &lda, work.memptr());
    ...
    lapack::pocon(&uplo, &n, A.memptr(), &lda, &norm_val, &rcond, work.memptr(),
                  iwork.memptr(), &info);
...
template <typename T>
inline T auxlib::rcond_sympd(Mat<std::complex<T> >& A, bool& calc_ok) {
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_debug_print(
        "auxlib::rcond_sympd(): redirecting to auxlib::rcond() due to crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_sym(A)
                   : lapack::lanhe(&norm_id, &uplo, &n, A.memptr(), &lda, rwork.memptr());
    ...
    lapack::cx_pocon(&uplo, &n, A.memptr(), &lda, &norm_val, &rcond, work.memptr(),
                     rwork.memptr(), &info);
...
template <typename eT>
inline eT auxlib::rcond_trimat(const Mat<eT>& A, const uword layout) {
#if defined(ARMA_USE_LAPACK)
  {
    arma_conform_assert_blas_size(A);
    ...
    lapack::trcon(&norm_id, &uplo, &diag, &n, A.memptr(), &n, &rcond, work.memptr(),
                  iwork.memptr(), &info);
...
template <typename T>
inline T auxlib::rcond_trimat(const Mat<std::complex<T> >& A, const uword layout) {
#if defined(ARMA_USE_LAPACK)
  {
    typedef typename std::complex<T> eT;
    ...
    lapack::cx_trcon(&norm_id, &uplo, &diag, &n, A.memptr(), &n, &rcond, work.memptr(),
                     rwork.memptr(), &info);
...
template <typename eT>
inline eT auxlib::lu_rcond(const Mat<eT>& A, const eT norm_val) {
#if defined(ARMA_USE_LAPACK)
  {
    char norm_id = '1';
    blas_int n = blas_int(A.n_rows);  // assuming square matrix
...
template <typename T>
inline T auxlib::lu_rcond(const Mat<std::complex<T> >& A, const T norm_val) {
#if defined(ARMA_USE_LAPACK)
  {
    typedef typename std::complex<T> eT;
...
template <typename eT>
inline eT auxlib::lu_rcond_sympd(const Mat<eT>& A, const eT norm_val) {
#if defined(ARMA_USE_LAPACK)
  {
    char uplo = 'L';
    blas_int n = blas_int(A.n_rows);  // assuming square matrix
    ...
    arma_debug_print("lapack::pocon()");
    lapack::pocon(&uplo, &n, A.memptr(), &n, &norm_val, &rcond, work.memptr(),
                  iwork.memptr(), &info);
...
template <typename T>
inline T auxlib::lu_rcond_sympd(const Mat<std::complex<T> >& A, const T norm_val) {
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_ignore(A);
    arma_ignore(norm_val);
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    arma_debug_print("lapack::cx_pocon()");
    lapack::cx_pocon(&uplo, &n, A.memptr(), &n, &norm_val, &rcond, work.memptr(),
                     rwork.memptr(), &info);
...
template <typename eT>
inline eT auxlib::lu_rcond_band(const Mat<eT>& AB, const uword KL, const uword KU,
                                const podarray<blas_int>& ipiv, const eT norm_val) {
#if defined(ARMA_USE_LAPACK)
  {
    const uword N = AB.n_cols;  // order of the original square matrix A
    ...
    lapack::gbcon<eT>(&norm_id, &n, &kl, &ku, AB.memptr(), &ldab, ipiv.memptr(),
                      &norm_val, &rcond, work.memptr(), iwork.memptr(), &info);
...
template <typename T>
inline T auxlib::lu_rcond_band(const Mat<std::complex<T> >& AB, const uword KL,
                               const uword KU, const podarray<blas_int>& ipiv,
                               const T norm_val) {
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_ignore(AB);
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gbcon<T>(&norm_id, &n, &kl, &ku, AB.memptr(), &ldab, ipiv.memptr(),
                        &norm_val, &rcond, work.memptr(), rwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::crippled_lapack(const Base<typename T1::elem_type, T1>&) {
#if defined(ARMA_CRIPPLED_LAPACK)
  {
...
template <typename eT>
inline bool auxlib::rudimentary_sym_check(const Mat<eT>& X) {
  arma_debug_sigprint();
...
template <typename T>
inline bool auxlib::rudimentary_sym_check(const Mat<std::complex<T> >& X) {
  arma_debug_sigprint();
  ...
  if (std::abs(X_mem[0].imag()) > tol) {
    return false;
  }  // check top-left
  if (std::abs(X_mem[X.n_elem - 1].imag()) > tol) {
    return false;
  }  // check bottom-right
...
template <typename eT>
inline typename get_pod_type<eT>::result auxlib::norm1_gen(const Mat<eT>& A) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename get_pod_type<eT>::result auxlib::norm1_sym(const Mat<eT>& A) {
  arma_debug_sigprint();
  ...
  for (uword col = 0; col < N; ++col) {
    ...
    for (uword c = 0; c < col; ++c) {
      acc_val += std::abs(A.at(col, c));
    }

    for (uword r = col; r < N; ++r) {
      acc_val += std::abs(colmem[r]);
    }
...
template <typename eT>
inline typename get_pod_type<eT>::result auxlib::norm1_band(const Mat<eT>& A,
                                                            const uword KL,
                                                            const uword KU) {
  arma_debug_sigprint();
...
inline blas_int select_ouc(const T* x_ptr, const T* y_ptr, const T* z_ptr) {
  ...
  if (z == T(0)) {
    return (x == T(0)) ? blas_int(0) : blas_int(1);  // consider an infinite eig value to
...
template <typename T>
inline blas_int cx_select_lhp(const std::complex<T>* x_ptr,
                              const std::complex<T>* y_ptr) {
  arma_debug_sigprint();
...
template <typename T>
inline blas_int cx_select_rhp(const std::complex<T>* x_ptr,
                              const std::complex<T>* y_ptr) {
  arma_debug_sigprint();
...
template <typename T>
inline blas_int cx_select_iuc(const std::complex<T>* x_ptr,
                              const std::complex<T>* y_ptr) {
  arma_debug_sigprint();
...
template <typename T>
inline blas_int cx_select_ouc(const std::complex<T>* x_ptr,
                              const std::complex<T>* y_ptr) {
  arma_debug_sigprint();
  ...
  if ((y.real() == T(0)) && (y.imag() == T(0))) {
    return ((x.real() == T(0)) && (x.imag() == T(0)))
               ? blas_int(0)
               : blas_int(1);  // consider an infinite eig value to be outside of the unit
                               // circle
  }
...
// - we're using LAPACK ?gges() defined to expect pointer-to-function to be passed as
// pointer-to-object
// - explicit casting between pointer-to-function and pointer-to-object is a non-standard
// extension in C
...
template <typename T>
inline void_ptr ptr_cast(blas_int (*function)(const std::complex<T>*,
                                              const std::complex<T>*)) {
  union converter {
