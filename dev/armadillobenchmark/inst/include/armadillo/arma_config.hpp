...
struct arma_config {
  ...
  static constexpr uword mat_prealloc =
  ...
  static constexpr uword mp_threshold =
      (sword(ARMA_OPENMP_THRESHOLD) > 0) ? uword(ARMA_OPENMP_THRESHOLD) : 320;
#else
  static constexpr uword mp_threshold = 320;
#endif

#if defined(ARMA_OPENMP_THREADS)
  static constexpr uword mp_threads =
      (sword(ARMA_OPENMP_THREADS) > 0) ? uword(ARMA_OPENMP_THREADS) : 8;
#else
  ...
  static constexpr uword warn_level =
      (sword(ARMA_WARN_LEVEL) > 0) ? uword(ARMA_WARN_LEVEL) : 0;
};
