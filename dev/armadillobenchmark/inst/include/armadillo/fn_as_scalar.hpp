...
struct as_scalar_errmsg {
  arma_cold arma_noinline static std::string incompat_size_string(const uword n_rows,
                                                                  const uword n_cols) {
  ...
  arma_cold arma_noinline static std::string incompat_size_string(const uword n_rows,
    ...
    tmp << "as_scalar(): expected 1x1x1 cube; got " << n_rows << 'x' << n_cols << 'x'
        << n_slices;
...
struct as_scalar_redirect<2> {
  ...
  inline static void check_size(const uword A_n_rows, const uword A_n_cols,
...
struct as_scalar_redirect<3> {
  ...
  inline static typename T1::elem_type apply(
...
inline typename T1::elem_type as_scalar_redirect<N>::apply(const T1& X) {
  ...
  if ((arma_config::check_conform) && (P.get_n_elem() != 1)) {
    arma_conform_check_bounds(
        true, as_scalar_errmsg::incompat_size_string(P.get_n_rows(), P.get_n_cols()));
  }
...
template <typename T1, typename T2>
inline typename T1::elem_type as_scalar_redirect<2>::apply(
  ...
  constexpr bool proxy_is_mat = (is_Mat<typename Proxy<T1>::stored_type>::value &&
                                 is_Mat<typename Proxy<T2>::stored_type>::value);

  constexpr bool use_at = (Proxy<T1>::use_at) || (Proxy<T2>::use_at);

  constexpr bool fast_unwrap =
      (partial_unwrap<T1>::is_fast && partial_unwrap<T2>::is_fast);
  ...
  if (proxy_is_mat || use_at || fast_unwrap) {
    ...
    const uword A_n_rows = (tmp1.do_trans == false) ? (TA::is_row ? 1 : A.n_rows)
                                                    : (TA::is_col ? 1 : A.n_cols);
    const uword A_n_cols = (tmp1.do_trans == false) ? (TA::is_col ? 1 : A.n_cols)
                                                    : (TA::is_row ? 1 : A.n_rows);

    const uword B_n_rows = (tmp2.do_trans == false) ? (TB::is_row ? 1 : B.n_rows)
                                                    : (TB::is_col ? 1 : B.n_cols);
    const uword B_n_cols = (tmp2.do_trans == false) ? (TB::is_col ? 1 : B.n_cols)
                                                    : (TB::is_row ? 1 : B.n_rows);

    if ((arma_config::check_conform) &&
        ((A_n_rows != 1) || (B_n_cols != 1) || (A_n_cols != B_n_rows))) {
    ...
    return (tmp1.do_times || tmp2.do_times) ? (val * tmp1.get_val() * tmp2.get_val())
                                            : val;
  } else {
...
inline void as_scalar_redirect<2>::check_size(const uword A_n_rows, const uword A_n_cols,
                                              const uword B_n_rows,
                                              const uword B_n_cols) {
  arma_conform_assert_mul_size(A_n_rows, A_n_cols, B_n_rows, B_n_cols,
                               "matrix multiplication");

  arma_conform_check_bounds(((A_n_rows != 1) || (B_n_cols != 1)),
                            as_scalar_errmsg::incompat_size_string(A_n_rows, B_n_cols));
}

template <typename T1, typename T2, typename T3>
inline typename T1::elem_type as_scalar_redirect<3>::apply(
    const Glue<Glue<T1, T2, glue_times>, T3, glue_times>& X) {
  arma_debug_sigprint();
  ...
  if (tmp2_do_diagmat == false) {
    ...
    if ((arma_config::check_conform) && (tmp.n_elem != 1)) {
      arma_conform_check_bounds(
          true, as_scalar_errmsg::incompat_size_string(tmp.n_rows, tmp.n_cols));
    }
    ...
    const uword B_n_rows =
        (B_is_vec) ? B.n_elem : ((tmp2.do_trans == false) ? B.n_rows : B.n_cols);
    const uword B_n_cols =
        (B_is_vec) ? B.n_elem : ((tmp2.do_trans == false) ? B.n_cols : B.n_rows);
    ...
    arma_conform_check_bounds((A_n_rows != 1) || (C_n_cols != 1) ||
                                  (A_n_cols != B_n_rows) || (B_n_cols != C_n_rows),
...
template <typename T1, typename T2, typename T3>
inline typename T1::elem_type as_scalar_diag(
    const Glue<Glue<T1, T2, glue_times_diag>, T3, glue_times>& X) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
arma_warn_unused inline typename T1::elem_type as_scalar(
    const Glue<T1, T2, glue_times>& X,
    const typename arma_not_cx<typename T1::elem_type>::result* junk = nullptr) {
  arma_debug_sigprint();
  arma_ignore(junk);

  if (is_glue_times_diag<T1>::value) {
...
template <typename T>
arma_warn_unused arma_inline typename arma_scalar_only<T>::result as_scalar(const T& x) {
  return x;
}
...
arma_warn_unused inline typename T1::elem_type as_scalar(
  ...
  if ((arma_config::check_conform) && (A.n_elem != 1)) {
    arma_conform_check_bounds(true,
                              as_scalar_errmsg::incompat_size_string(A.n_rows, A.n_cols));
  }
