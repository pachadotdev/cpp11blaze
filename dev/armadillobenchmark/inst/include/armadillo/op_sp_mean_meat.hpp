...
inline void op_sp_mean::apply(
    Mat<typename T1::elem_type>& out,
    const mtSpReduceOp<typename T1::elem_type, T1, op_sp_mean>& in) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_sp_mean::apply_fast(Mat<typename T1::elem_type>& out, const SpProxy<T1>& p,
                                   const uword dim) {
  arma_debug_sigprint();
  ...
  if (dim == 0)  // find the mean in each column
  {
    ...
    if (SpProxy<T1>::use_iterator) {
      ...
      for (uword col = 0; col < p_n_cols; ++col) {
        out_mem[col] =
            arrayops::accumulate(&p.get_values()[p.get_col_ptrs()[col]],
...
template <typename T1>
inline void op_sp_mean::apply_slow(Mat<typename T1::elem_type>& out, const SpProxy<T1>& p,
                                   const uword dim) {
  arma_debug_sigprint();
    ...
    for (uword col = 0; col < p_n_cols; ++col) {
      ...
      if (SpProxy<T1>::use_iterator) {
        ...
        out.at(0, col) = op_sp_mean::direct_mean(
            ...
            p.get_col_ptrs()[col + 1] - p.get_col_ptrs()[col], p_n_rows);
      }
    }
  } else if (dim == 1)  // find the mean in each row
  {
...
inline eT op_sp_mean::direct_mean(const eT* const X, const uword length, const uword N) {
  arma_debug_sigprint();

  typedef typename get_pod_type<eT>::result T;

  const eT result =
      ((length > 0) && (N > 0)) ? eT(arrayops::accumulate(X, length) / T(N)) : eT(0);

  return arma_isfinite(result) ? result : op_sp_mean::direct_mean_robust(X, length, N);
}

template <typename eT>
inline eT op_sp_mean::direct_mean_robust(const eT* const X, const uword length,
                                         const uword N) {
  arma_debug_sigprint();
...
template <typename T1>
inline typename T1::elem_type op_sp_mean::mean_all(
  ...
  if (SpProxy<T1>::use_iterator) {
    ...
    return op_sp_mean::iterator_mean(it, end, p.get_n_elem() - p.get_n_nonzero(),
...
inline typename T1::elem_type op_sp_mean::mean_all(const SpOp<T1, spop_type>& expr) {
  arma_debug_sigprint();
...
inline eT op_sp_mean::iterator_mean(T1& it, const T1& end, const uword n_zero,
                                    const eT junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
  ...
  return arma_isfinite(result)
             ? result
             : op_sp_mean::iterator_mean_robust(backup_it, end, n_zero, eT(0));
}

template <typename T1, typename eT>
inline eT op_sp_mean::iterator_mean_robust(T1& it, const T1& end, const uword n_zero,
                                           const eT junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
