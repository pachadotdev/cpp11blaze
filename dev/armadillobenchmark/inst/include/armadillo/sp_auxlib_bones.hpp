...
class sp_auxlib {
 public:
  ...
  inline static bool eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X,
  ...
  inline static bool eigs_sym_newarp(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X,
                                     const uword n_eigvals, const form_type form_val,
                                     const eigs_opts& opts);
  ...
  inline static bool eigs_sym_arpack(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X,
                                     const uword n_eigvals, const form_type form_val,
  ...
  inline static bool eigs_gen_arpack(Col<std::complex<T> >& eigval,
                                     Mat<std::complex<T> >& eigvec, const SpMat<T>& X,
                                     const uword n_eigvals, const form_type form_val,
                                     const std::complex<T> sigma, const eigs_opts& opts);
  ...
  inline static bool eigs_gen(Col<std::complex<T> >& eigval,
                              ...
                              const SpBase<std::complex<T>, T1>& X, const uword n_eigvals,
  ...
  inline static bool spsolve_simple(Mat<typename T1::elem_type>& out,
                                    const SpBase<typename T1::elem_type, T1>& A,
                                    const Base<typename T1::elem_type, T2>& B,
                                    const superlu_opts& user_opts);

  template <typename T1, typename T2>
  inline static bool spsolve_refine(Mat<typename T1::elem_type>& out,
  ...
  template <typename eT>
  inline static typename get_pod_type<eT>::result lu_rcond(
      superlu::SuperMatrix* L, superlu::SuperMatrix* U,
      typename get_pod_type<eT>::result norm_val);

  inline static void set_superlu_opts(superlu::superlu_options_t& options,
                                      const superlu_opts& user_opts);
  ...
  template <typename eT>
  inline static bool copy_to_supermatrix_with_shift(superlu::SuperMatrix& out,
                                                    const SpMat<eT>& A, const eT shift);
 ...
 private:
  ...
  inline static void run_aupd_plain(const uword n_eigvals, char* which, const SpMat<T>& X,
                                    const SpMat<T>& Xst, const bool sym, blas_int& n,
                                    eT& tol, blas_int& maxiter, podarray<T>& resid,
                                    blas_int& ncv, podarray<T>& v, blas_int& ldv,
                                    podarray<blas_int>& iparam, podarray<blas_int>& ipntr,
                                    podarray<T>& workd, podarray<T>& workl,
                                    blas_int& lworkl, podarray<eT>& rwork,
                                    blas_int& info);

  template <typename eT, typename T>
  inline static void run_aupd_shiftinvert(const uword n_eigvals, const T sigma,
                                          const SpMat<T>& X, const bool sym, blas_int& n,
                                          eT& tol, blas_int& maxiter, podarray<T>& resid,
                                          blas_int& ncv, podarray<T>& v, blas_int& ldv,
                                          podarray<blas_int>& iparam,
                                          podarray<blas_int>& ipntr, podarray<T>& workd,
                                          podarray<T>& workl, blas_int& lworkl,
...
class superlu_worker {
 ...
 public:
  ...
  inline bool factorise(typename get_pod_type<eT>::result& out_rcond, const SpMat<eT>& A,
                        const superlu_opts& user_opts);
