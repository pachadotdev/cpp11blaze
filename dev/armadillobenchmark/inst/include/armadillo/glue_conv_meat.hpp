...
inline void glue_conv::apply(Mat<eT>& out, const Mat<eT>& A, const Mat<eT>& B,
  ...
  Col<eT> xx((x_n_elem + 2 * h_n_elem_m1),
             arma_zeros_indicator());  // zero padded version of x
  ...
  if ((arma_config::openmp) && (x_n_elem >= 128) && (h_n_elem >= 64) &&
...
//   const uword   h_n_elem_m1 = h_n_elem - 1;
//   const uword   x_n_elem    = x.n_elem;
//   const uword out_n_elem    = ((h_n_elem + x_n_elem) > 0) ? (h_n_elem + x_n_elem - 1) :
//   uword(0);
//
...
//
//
//   Col<eT> xx( (x_n_elem + 2*h_n_elem_m1), arma_zeros_indicator() );  // zero padded
//   version of x
//
//   const eT*  x_mem =  x.memptr();
//         eT* xx_mem = xx.memptr();
