...
arma_warn_unused inline
  ...
  const bool is_vec = (T1::is_xvec) || (T1::is_row) || (T1::is_col) ||
                      (P.get_n_rows() == 1) || (P.get_n_cols() == 1);
  ...
  if (is_vec) {
    ...
    if (k == uword(1)) {
      return op_norm::mat_norm_1(U.M);
    }
    if (k == uword(2)) {
      return op_norm::mat_norm_2(U.M);
    }
...
arma_warn_unused inline typename enable_if2<is_arma_type<T1>::value, double>::result norm(
  ...
  if (resolves_to_colvector<T1>::value) {
    return norm(conv_to<Col<double> >::from(X), k);
  }
  if (resolves_to_rowvector<T1>::value) {
    return norm(conv_to<Row<double> >::from(X), k);
  }
...
arma_warn_unused inline typename enable_if2<is_arma_type<T1>::value, double>::result norm(
  ...
  if (resolves_to_colvector<T1>::value) {
    return norm(conv_to<Col<double> >::from(X), method);
  }
  if (resolves_to_rowvector<T1>::value) {
    return norm(conv_to<Row<double> >::from(X), method);
  }
...
arma_warn_unused inline
  ...
  const bool is_vec =
      (T1::is_xvec) || (T1::is_row) || (T1::is_col) || (X.n_rows == 1) || (X.n_cols == 1);

  if (is_vec) {
    return spop_norm::vec_norm_k(X.values, X.n_nonzero, k);
  } else {
    if (k == uword(1)) {
      return spop_norm::mat_norm_1(X);
    }
    if (k == uword(2)) {
      return spop_norm::mat_norm_2(X);
    }
...
template <typename T1>
arma_warn_unused inline typename T1::pod_type norm2est(
