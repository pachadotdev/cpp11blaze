...
template <typename oT>
arma_inline subview_field<oT>::subview_field(const field<oT>& in_f, const uword in_row1,
...
arma_inline oT& subview_field<oT>::operator[](const uword i) {
  ...
  const uword index = (in_slice + aux_slice1) * (f.n_rows * f.n_cols) +
                      (in_col + aux_col1) * f.n_rows + aux_row1 + in_row;
...
template <typename oT>
arma_inline oT& subview_field<oT>::at(const uword in_row, const uword in_col) {
  return at(in_row, in_col, 0);
}

template <typename oT>
arma_inline const oT& subview_field<oT>::at(const uword in_row,
                                            const uword in_col) const {
  return at(in_row, in_col, 0);
}
...
template <typename oT>
arma_inline const oT& subview_field<oT>::at(const uword in_row, const uword in_col,
                                            const uword in_slice) const {
  const uword index = (in_slice + aux_slice1) * (f.n_rows * f.n_cols) +
                      (in_col + aux_col1) * f.n_rows + aux_row1 + in_row;
...
template <typename oT>
arma_inline bool subview_field<oT>::is_empty() const {
  return (n_elem == 0);
}

template <typename oT>
inline bool subview_field<oT>::check_overlap(const subview_field<oT>& x) const {
