...
arma_inline const eOpCube<T1, eop_scalar_div_post> operator/(
...
//! scalar / BaseCube
template <typename T1>
arma_inline const eOpCube<T1, eop_scalar_div_pre> operator/(
...
template <typename T1>
arma_inline const
    mtOpCube<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_div_pre>
    operator/(const std::complex<typename T1::pod_type>& k,
              const BaseCube<typename T1::pod_type, T1>& X) {
  arma_debug_sigprint();

  return mtOpCube<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_div_pre>(
      'j', X.get_ref(), k);
}
...
inline const mtGlueCube<
    typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2,
    glue_mixed_div>
