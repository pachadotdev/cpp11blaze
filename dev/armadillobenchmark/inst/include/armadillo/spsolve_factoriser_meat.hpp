...
inline void spsolve_factoriser::cleanup() {
  ...
  {
    if (elem_type_indicator == 1) {
      delete_worker<superlu_worker<float> >();
    } else if (elem_type_indicator == 2) {
      delete_worker<superlu_worker<double> >();
    } else if (elem_type_indicator == 3) {
      delete_worker<superlu_worker<cx_float> >();
    } else if (elem_type_indicator == 4) {
      delete_worker<superlu_worker<cx_double> >();
    }
  }
#endif
...
inline bool spsolve_factoriser::factorise(
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    if (is_float<eT>::value) {
      elem_type_indicator = 1;
    } else if (is_double<eT>::value) {
      elem_type_indicator = 2;
    } else if (is_cx_float<eT>::value) {
      elem_type_indicator = 3;
    } else if (is_cx_double<eT>::value) {
      elem_type_indicator = 4;
    }
...
#else
  {
    ...
    arma_stop_logic_error(
        "spsolve_factoriser::factorise(): use of SuperLU must be enabled");
    return false;
  }
#endif
}
