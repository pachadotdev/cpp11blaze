...
template <typename T1>
inline void op_clamp::apply_proxy_noalias(Mat<typename T1::elem_type>& out,
                                          const Proxy<T1>& P,
...
template <typename T1>
inline void op_clamp::apply_proxy_noalias(Cube<typename T1::elem_type>& out,
...
template <typename T1>
inline void op_clamp_cx::apply_proxy_noalias(Mat<typename T1::elem_type>& out,
                                             const Proxy<T1>& P,
                                             const typename T1::elem_type min_val,
                                             const typename T1::elem_type max_val) {
  arma_debug_sigprint();
  ...
  if (Proxy<T1>::use_at == false) {
    ...
    for (uword col = 0; col < n_cols; ++col)
      for (uword row = 0; row < n_rows; ++row) {
        ...
        (*out_mem) = std::complex<T>(val_real, val_imag);
        out_mem++;
      }
  }
}
...
template <typename T1>
inline void op_clamp_cx::apply_proxy_noalias(Cube<typename T1::elem_type>& out,
                                             const ProxyCube<T1>& P,
                                             const typename T1::elem_type min_val,
                                             const typename T1::elem_type max_val) {
  arma_debug_sigprint();
