...
// #pragma message ("WARNING: disabled use of 64 bit integers, as std::size_t is smaller
// than 64 bits")
#undef ARMA_64BIT_WORD
#endif
#endif
...
// NOTE: Apple accelerate framework has broken implementations of functions that return a
// float value, NOTE: such as sdot(), slange(), clange(), slansy(), clanhe(), slangb(),
// snrm2(), sasum()
...
// TODO: __INTEL_CLANG_COMPILER indicates the clang based intel compiler, distinct from
// the classic intel compiler
...
#pragma message( \
    "WARNING: this compiler is pretending to be Clang but it may not be fully compatible;")
#pragma message( \
    "WARNING: to allow this compiler to use Clang features such as data alignment attributes,")
...
#pragma warning(disable : 4146)  // unary minus operator applied to unsigned type, result
                                 // still unsigned
#pragma warning(disable : 4180)  // qualifier has no meaning
#pragma warning( \
    disable : 4244)  // possible loss of data when converting types (see also 4305)
...
#pragma warning(disable : 4519)  // default template args are only allowed on a class
                                 // template (C++11)
...
#define ARMA_USE_OPENMP
     ...
     (!defined(_OPENMP) || (defined(_OPENMP) && (_OPENMP < 201107))))
...
#pragma message( \
    "WARNING: use of OpenMP disabled; compiler support for OpenMP 3.1+ not detected")
...
#pragma message( \
    "WARNING: compiler is in fast math mode; some functions may be unreliable.")
#pragma message("WARNING: to suppress this warning and related warnings,")
#pragma message( \
    "WARNING: #define ARMA_DONT_PRINT_FAST_MATH_WARNING before #include <armadillo>")
#endif
...
#define ARMA_PRINT_EXCEPTIONS_INTERNAL
    ...
    (!defined(ARMA_ALIEN_MEM_ALLOC_FUNCTION) && defined(ARMA_ALIEN_MEM_FREE_FUNCTION))
#error \
    "*** both ARMA_ALIEN_MEM_ALLOC_FUNCTION and ARMA_ALIEN_MEM_FREE_FUNCTION must be defined ***"
#endif
