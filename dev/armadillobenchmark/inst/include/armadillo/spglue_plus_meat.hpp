...
inline void spglue_plus::apply(SpMat<typename T1::elem_type>& out,
...
inline void spglue_plus::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa,
                                       const SpProxy<T2>& pb) {
  arma_debug_sigprint();
  ...
  while ((x_it != x_end) || (y_it != y_end)) {
    ...
    arma_check((count > max_n_nonzero),
               "internal error: spglue_plus::apply_noalias(): count > max_n_nonzero");
  }
...
inline void spglue_plus::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A,
                                       const SpMat<eT>& B) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void spglue_plus_mixed::apply(
    SpMat<typename eT_promoter<T1, T2>::eT>& out,
    const mtSpGlue<typename eT_promoter<T1, T2>::eT, T1, T2, spglue_plus_mixed>& expr) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void spglue_plus_mixed::dense_plus_sparse(
