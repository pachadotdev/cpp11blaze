...
class op_sort : public traits_op_default {
 public:
  ...
  template <typename eT>
  inline static void apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword sort_type,
                                   const uword dim);
...
class op_sort_vec : public traits_op_passthru {
 public:
  template <typename T1>
