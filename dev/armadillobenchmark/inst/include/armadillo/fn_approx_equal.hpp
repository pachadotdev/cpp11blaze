...
arma_inline bool internal_approx_equal_abs_diff(
    const eT& x, const eT& y, const typename get_pod_type<eT>::result tol) {
  ...
  if (x != y) {
                            ...
                            // std::complex<double>
    {
      ...
      if (eop_aux::arma_abs((cond_rel<is_cx<eT>::no>::gt(x, y)) ? (x - y) : (y - x)) >
          tol) {
        return false;
      }
    }
...
template <typename eT>
arma_inline bool internal_approx_equal_rel_diff(
    const eT& a, const eT& b, const typename get_pod_type<eT>::result tol) {
  typedef typename get_pod_type<eT>::result T;
  ...
  if (a != b) {
                            ...
                            // std::complex<double>
    {
      ...
      const T abs_d =
          eop_aux::arma_abs((cond_rel<is_cx<eT>::no>::gt(a, b)) ? (a - b) : (b - a));
