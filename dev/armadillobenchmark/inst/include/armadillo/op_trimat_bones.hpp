...
// NOTE: as several instances elsewhere rely on trimatu() and trimatl() producing the same
// type
class op_trimat : public traits_op_default {
 public:
...
class op_trimatu_ext : public traits_op_default {
 public:
  template <typename T1>
  ...
  template <typename eT>
  inline static void fill_zeros(Mat<eT>& A, const uword row_offset,
                                const uword col_offset);
};

class op_trimatl_ext : public traits_op_default {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out,
