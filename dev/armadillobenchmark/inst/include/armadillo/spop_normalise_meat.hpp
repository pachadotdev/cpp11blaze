...
inline void spop_normalise::apply(SpMat<typename T1::elem_type>& out,
                                  const SpOp<T1, spop_normalise>& expr) {
  arma_debug_sigprint();
...
inline void spop_normalise::apply_direct(SpMat<eT>& out, const SpMat<eT>& X,
                                         const uword p) {
  arma_debug_sigprint();
