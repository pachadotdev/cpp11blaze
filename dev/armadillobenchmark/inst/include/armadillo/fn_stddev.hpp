...
arma_warn_unused inline
    typename enable_if2<is_arma_type<T1>::value && resolves_to_vector<T1>::no,
                        const mtOp<typename T1::pod_type, T1, op_stddev> >::result
    stddev(const T1& X, const uword norm_type = 0) {
  arma_debug_sigprint();
...
template <typename T>
arma_warn_unused inline typename arma_scalar_only<T>::result stddev(const T&) {
  return T(0);
}
...
arma_warn_unused inline typename enable_if2<
    ...
    const mtSpReduceOp<typename T1::pod_type, T1, op_sp_stddev> >::result
stddev(const T1& X, const uword norm_type = 0) {
  arma_debug_sigprint();
