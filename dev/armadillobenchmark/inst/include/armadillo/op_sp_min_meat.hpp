...
inline void op_sp_min::apply(
    Mat<typename T1::elem_type>& out,
    const mtSpReduceOp<typename T1::elem_type, T1, op_sp_min>& in) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_sp_min::apply_proxy(
    Mat<typename T1::elem_type>& out, const SpProxy<T1>& p, const uword dim,
...
template <typename T1>
inline typename T1::elem_type op_sp_min::vector_min(
    const T1& x, const typename arma_not_cx<typename T1::elem_type>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename T1>
inline typename arma_not_cx<typename T1::elem_type>::result op_sp_min::min(
...
template <typename T1>
inline typename arma_not_cx<typename T1::elem_type>::result op_sp_min::min_with_index(
    const SpProxy<T1>& P, uword& index_of_min_val) {
  arma_debug_sigprint();
...
template <typename T1>
inline typename T1::elem_type op_sp_min::vector_min(
    const T1& x, const typename arma_cx_only<typename T1::elem_type>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename T1>
inline typename arma_cx_only<typename T1::elem_type>::result op_sp_min::min(
    const SpBase<typename T1::elem_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename T1>
inline typename arma_cx_only<typename T1::elem_type>::result op_sp_min::min_with_index(
    const SpProxy<T1>& P, uword& index_of_min_val) {
  arma_debug_sigprint();
