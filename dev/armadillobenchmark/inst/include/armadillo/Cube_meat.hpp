...
template <typename eT>
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols,
...
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols,
                      const uword in_n_slices, const arma_initmode_indicator<do_zeros>&)
    : n_rows(in_n_rows),
      n_cols(in_n_cols),
      n_elem_slice(in_n_rows * in_n_cols),
      n_slices(in_n_slices),
...
template <typename eT>
template <bool do_zeros>
inline Cube<eT>::Cube(const SizeCube& s, const arma_initmode_indicator<do_zeros>&)
    : n_rows(s.n_rows),
      n_cols(s.n_cols),
      n_elem_slice(s.n_rows * s.n_cols),
      n_slices(s.n_slices),
...
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols,
                      const uword in_n_slices, const fill::fill_class<fill_type>&)
    : n_rows(in_n_rows),
      n_cols(in_n_cols),
      n_elem_slice(in_n_rows * in_n_cols),
      n_slices(in_n_slices),
  ...
  arma_static_check((is_same_type<fill_type, fill::fill_eye>::yes),
                    "Cube::Cube(): unsupported fill type");
}
...
template <typename eT>
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols,
                      const uword in_n_slices, const fill::scalar_holder<eT> f)
    : n_rows(in_n_rows),
      n_cols(in_n_cols),
      n_elem_slice(in_n_rows * in_n_cols),
      n_slices(in_n_slices),
...
template <typename eT>
inline Cube<eT>::Cube(const SizeCube& s, const fill::scalar_holder<eT> f)
    : n_rows(s.n_rows),
      n_cols(s.n_cols),
      n_elem_slice(s.n_rows * s.n_cols),
      n_slices(s.n_slices),
...
template <typename eT>
inline Cube<eT>& Cube<eT>::operator=(Cube<eT>&& in_cube) {
  arma_debug_sigprint(arma_str::format("this: %x; in_cube: %x") % this % &in_cube);
...
template <typename eT>
inline void Cube<eT>::init_cold() {
  arma_debug_sigprint(arma_str::format("n_rows: %u; n_cols: %u; n_slices: %u") % n_rows %
                      n_cols % n_slices);
  ...
  arma_conform_check((((n_rows > 0x0FFF) || (n_cols > 0x0FFF) || (n_slices > 0xFF))
  ...
  if (n_elem <= Cube_prealloc::mem_n_elem) {
    if (n_elem > 0) {
      arma_debug_print("Cube::init(): using local memory");
    }
...
template <typename eT>
inline void Cube<eT>::init_warm(const uword in_n_rows, const uword in_n_cols,
                                const uword in_n_slices) {
  arma_debug_sigprint(arma_str::format("in_n_rows: %u; in_n_cols: %u; in_n_slices: %u") %
...
inline void Cube<eT>::init(const BaseCube<typename Cube<eT>::pod_type, T1>& X,
    ...
    for (uword uslice = 0; uslice < local_n_slices; ++uslice)
      for (uword ucol = 0; ucol < local_n_cols; ++ucol)
        for (uword urow = 0; urow < local_n_rows; ++urow) {
          *out_mem =
              std::complex<T>(PX.at(urow, ucol, uslice), PY.at(urow, ucol, uslice));
          out_mem++;
        }
  }
}
...
template <typename eT>
inline void Cube<eT>::delete_mat() {
  arma_debug_sigprint();

  if ((n_slices > 0) && (mat_ptrs != nullptr)) {
    for (uword s = 0; s < n_slices; ++s) {
      raw_mat_ptr_type mat_ptr = raw_mat_ptr_type(
          mat_ptrs[s]);  // explicit cast to indicate load from std::atomic<Mat<eT>*>

      if (mat_ptr != nullptr) {
        arma_debug_print(arma_str::format("Cube::delete_mat(): destroying matrix %u") %
                         s);
        delete mat_ptr;
        mat_ptrs[s] = nullptr;
      }
...
template <typename eT>
inline void Cube<eT>::create_mat() {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>* Cube<eT>::create_mat_ptr(const uword in_slice) const {
  arma_debug_sigprint();

  arma_debug_print(arma_str::format("Cube::create_mat_ptr(): creating matrix %u") %
                   in_slice);
...
template <typename eT>
inline Mat<eT>* Cube<eT>::get_mat_ptr(const uword in_slice) const {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::operator=(const eT val) {
  arma_debug_sigprint();
...
//! construct a cube from a given cube
template <typename eT>
inline Cube<eT>& Cube<eT>::operator=(const Cube<eT>& x) {
  arma_debug_sigprint(arma_str::format("this: %x; in_cube: %x") % this % &x);
...
//! memory and copying).
template <typename eT>
inline Cube<eT>::Cube(eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols,
                      const uword aux_n_slices, const bool copy_aux_mem,
                      const bool strict, const bool prealloc_mat)
    : n_rows(aux_n_rows),
      n_cols(aux_n_cols),
...
//! the array is copied.
template <typename eT>
inline Cube<eT>::Cube(const eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols,
...
template <typename eT>
inline Cube<eT>& Cube<eT>::operator+=(const Cube<eT>& m) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
inline Cube<eT>::Cube(const BaseCube<typename Cube<eT>::pod_type, T1>& A,
...
//! construct a cube from a subview_cube instance (eg. construct a cube from a delayed
//! subcube operation)
template <typename eT>
...
//! subcube operation)
template <typename eT>
inline Cube<eT>& Cube<eT>::operator=(const subview_cube<eT>& X) {
  arma_debug_sigprint();
...
inline Cube<eT>::Cube(const subview_cube_slices<eT, T1>& X)
    : n_rows(0),
      n_cols(0),
      n_elem_slice(0),
      n_slices(0),
...
template <typename eT>
template <typename T1>
inline Cube<eT>& Cube<eT>::operator=(const subview_cube_slices<eT, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::row(const uword in_row) {
  arma_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified row)
template <typename eT>
arma_inline const subview_cube<eT> Cube<eT>::row(const uword in_row) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::col(const uword in_col) {
  arma_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified column)
template <typename eT>
arma_inline const subview_cube<eT> Cube<eT>::col(const uword in_col) const {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Cube<eT>::slice(const uword in_slice) {
  arma_debug_sigprint();
...
//! provide the reference to the matrix representing a single slice
template <typename eT>
inline const Mat<eT>& Cube<eT>::slice(const uword in_slice) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified rows)
template <typename eT>
arma_inline const subview_cube<eT> Cube<eT>::rows(const uword in_row1,
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();
...
//! creation of subview_cube (subcube comprised of specified columns)
template <typename eT>
arma_inline const subview_cube<eT> Cube<eT>::cols(const uword in_col1,
...
arma_inline subview_cube<eT> Cube<eT>::slices(const uword in_slice1,
  ...
  arma_conform_check_bounds((in_slice1 > in_slice2) || (in_slice2 >= n_slices),
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1,
                                               const uword in_slice1, const uword in_row2,
                                               const uword in_col2,
                                               const uword in_slice2) {
  arma_debug_sigprint();

  arma_conform_check_bounds((in_row1 > in_row2) || (in_col1 > in_col2) ||
                                (in_slice1 > in_slice2) || (in_row2 >= n_rows) ||
                                (in_col2 >= n_cols) || (in_slice2 >= n_slices),
  ...
  return subview_cube<eT>(*this, in_row1, in_col1, in_slice1, subcube_n_rows,
                          subcube_n_cols, subcube_n_slices);
}

//! creation of subview_cube (generic subcube)
template <typename eT>
arma_inline const subview_cube<eT> Cube<eT>::subcube(
    const uword in_row1, const uword in_col1, const uword in_slice1, const uword in_row2,
...
//! creation of subview_cube (generic subcube)
template <typename eT>
inline subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1,
                                          const uword in_slice1, const SizeCube& s) {
  arma_debug_sigprint();
  ...
  arma_conform_check_bounds(
      (in_row1 >= l_n_rows) || (in_col1 >= l_n_cols) || (in_slice1 >= l_n_slices) ||
          ...
          ((in_slice1 + s_n_slices) > l_n_slices),
      "Cube::subcube(): indices or size out of bounds");

  return subview_cube<eT>(*this, in_row1, in_col1, in_slice1, s_n_rows, s_n_cols,
                          s_n_slices);
}

//! creation of subview_cube (generic subcube)
template <typename eT>
inline const subview_cube<eT> Cube<eT>::subcube(const uword in_row1, const uword in_col1,
                                                const uword in_slice1,
                                                const SizeCube& s) const {
  arma_debug_sigprint();
...
//! creation of subview_cube (generic subcube)
template <typename eT>
inline subview_cube<eT> Cube<eT>::subcube(const span& row_span, const span& col_span,
  ...
  arma_conform_check_bounds(
      (row_all ? false : ((in_row1 > in_row2) || (in_row2 >= local_n_rows))) ||
          ...
          (slice_all ? false
                     : ((in_slice1 > in_slice2) || (in_slice2 >= local_n_slices))),
      "Cube::subcube(): indices out of bounds or incorrectly used");
...
//! creation of subview_cube (generic subcube)
template <typename eT>
inline const subview_cube<eT> Cube<eT>::subcube(const span& row_span,
                                                const span& col_span,
                                                const span& slice_span) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::operator()(const span& row_span, const span& col_span,
                                             const span& slice_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT> Cube<eT>::operator()(const span& row_span,
                                                   const span& col_span,
                                                   const span& slice_span) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::operator()(const uword in_row1, const uword in_col1,
                                             const uword in_slice1, const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT> Cube<eT>::operator()(const uword in_row1,
                                                   const uword in_col1,
                                                   const uword in_slice1,
                                                   const SizeCube& s) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1) {
  arma_debug_sigprint();

  arma_conform_check_bounds(((in_row1 >= n_rows) || (in_col1 >= n_cols)),
...
template <typename eT>
arma_inline const subview_cube<eT> Cube<eT>::tube(const uword in_row1,
                                                  const uword in_col1) const {
  arma_debug_sigprint();
...
arma_inline subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1,
  ...
  return subview_cube<eT>(*this, in_row1, in_col1, 0, subcube_n_rows, subcube_n_cols,
                          n_slices);
}
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::tube(const span& row_span, const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT> Cube<eT>::tube(const span& row_span,
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::head_slices(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT> Cube<eT>::head_slices(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::tail_slices(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT> Cube<eT>::tail_slices(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline subview_elem1<eT, T1> Cube<eT>::elem(const Base<uword, T1>& a) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline const subview_elem1<eT, T1> Cube<eT>::elem(const Base<uword, T1>& a) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline subview_elem1<eT, T1> Cube<eT>::operator()(const Base<uword, T1>& a) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline const subview_elem1<eT, T1> Cube<eT>::operator()(
...
template <typename eT>
arma_inline subview_cube_each1<eT> Cube<eT>::each_slice() {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const subview_cube_each1<eT> Cube<eT>::each_slice() const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline subview_cube_each2<eT, T1> Cube<eT>::each_slice(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline const subview_cube_each2<eT, T1> Cube<eT>::each_slice(
...
template <typename eT>
inline Cube<eT>& Cube<eT>::each_slice(const std::function<void(Mat<eT>&)>& F) {
  arma_debug_sigprint();
...
template <typename eT>
inline const Cube<eT>& Cube<eT>::each_slice(
    const std::function<void(const Mat<eT>&)>& F) const {
  arma_debug_sigprint();
...
template <typename eT>
inline const Cube<eT>& Cube<eT>::each_slice(const std::function<void(const Mat<eT>&)>& F,
                                            const bool use_mp) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline subview_cube_slices<eT, T1> Cube<eT>::slices(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline const subview_cube_slices<eT, T1> Cube<eT>::slices(
    const Base<uword, T1>& indices) const {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::shed_row(const uword row_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::shed_col(const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::shed_slice(const uword slice_num) {
  arma_debug_sigprint();

  arma_conform_check_bounds(slice_num >= n_slices,
                            "Cube::shed_slice(): index out of bounds");
...
template <typename eT>
inline void Cube<eT>::shed_rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::shed_cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::shed_slices(const uword in_slice1, const uword in_slice2) {
  arma_debug_sigprint();
  ...
  if (n_keep_back > 0) {
    X.slices(n_keep_front, (n_keep_front + n_keep_back - 1)) =
        slices((in_slice2 + 1), (n_slices - 1));
  }
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::shed_slices(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
  ...
  arma_conform_check(((tmp1.is_vec() == false) && (tmp1.is_empty() == false)),
                     "Cube::shed_slices(): list of indices must be a vector");
  ...
  const Col<uword>& slices_to_shed =
  ...
  if (arma_config::check_conform) {
    for (uword i = 0; i < N; ++i) {
      arma_conform_check_bounds((slices_to_shed_mem[i] >= n_slices),
                                "Cube::shed_slices(): indices out of bounds");
    }
  }
...
template <typename eT>
inline void Cube<eT>::insert_rows(const uword row_num, const uword N,
...
inline void Cube<eT>::insert_rows(const uword row_num, const uword N) {
  ...
  arma_conform_check_bounds((row_num > t_n_rows),
                            "Cube::insert_rows(): index out of bounds");
...
template <typename eT>
inline void Cube<eT>::insert_cols(const uword col_num, const uword N,
                                  const bool set_to_zero) {
  arma_debug_sigprint();
...
inline void Cube<eT>::insert_cols(const uword col_num, const uword N) {
  ...
  arma_conform_check_bounds((col_num > t_n_cols),
                            "Cube::insert_cols(): index out of bounds");
...
template <typename eT>
inline void Cube<eT>::insert_slices(const uword slice_num, const uword N,
                                    const bool set_to_zero) {
  arma_debug_sigprint();
...
inline void Cube<eT>::insert_slices(const uword slice_num, const uword N) {
  ...
  arma_conform_check_bounds((slice_num > t_n_slices),
                            "Cube::insert_slices(): index out of bounds");
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_rows(const uword row_num, const BaseCube<eT, T1>& X) {
  arma_debug_sigprint();
  ...
  arma_conform_check(((C.n_cols != n_cols) || (C.n_slices != n_slices)),
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_cols(const uword col_num, const BaseCube<eT, T1>& X) {
  arma_debug_sigprint();
  ...
  arma_conform_check(((C.n_rows != n_rows) || (C.n_slices != n_slices)),
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_slices(const uword slice_num, const BaseCube<eT, T1>& X) {
  arma_debug_sigprint();
  ...
  arma_conform_check(((C.n_rows != n_rows) || (C.n_cols != n_cols)),
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_slices(const uword slice_num, const Base<eT, T1>& X) {
  arma_debug_sigprint();

  const quasi_unwrap<T1> U(X.get_ref());

  const Cube<eT> C(const_cast<eT*>(U.M.memptr()), U.M.n_rows, U.M.n_cols, uword(1), false,
                   true);

  (*this).insert_slices(slice_num, C);
}

//! create a cube from GenCube, ie. run the previously delayed element generation
//! operations
template <typename eT>
...
template <typename eT>
template <typename gen_type>
inline Cube<eT>& Cube<eT>::operator=(const GenCube<eT, gen_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT>& Cube<eT>::operator=(const OpCube<T1, op_type>& X) {
  arma_debug_sigprint();
...
//! in-place cube element-wise multiplication, with the right-hand-side operand having
//! delayed operations
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT>& Cube<eT>::operator%=(const OpCube<T1, op_type>& X) {
...
//! in-place cube element-wise division, with the right-hand-side operand having delayed
//! operations
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT>& Cube<eT>::operator/=(const OpCube<T1, op_type>& X) {
...
template <typename eT>
template <typename T1, typename eop_type>
inline Cube<eT>& Cube<eT>::operator=(const eOpCube<T1, eop_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT>& Cube<eT>::operator=(const mtOpCube<eT, T1, op_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT>& Cube<eT>::operator=(const GlueCube<T1, T2, glue_type>& X) {
  arma_debug_sigprint();
...
//! in-place cube element-wise multiplication, with the right-hand-side operands having
//! delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT>& Cube<eT>::operator%=(const GlueCube<T1, T2, glue_type>& X) {
...
//! in-place cube element-wise division, with the right-hand-side operands having delayed
//! operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT>& Cube<eT>::operator/=(const GlueCube<T1, T2, glue_type>& X) {
...
template <typename eT>
template <typename T1, typename T2, typename eglue_type>
inline Cube<eT>& Cube<eT>::operator=(const eGlueCube<T1, T2, eglue_type>& X) {
  arma_debug_sigprint();

  arma_type_check((is_same_type<eT, typename T1::elem_type>::no));
  arma_type_check((is_same_type<eT, typename T2::elem_type>::no));

  const bool bad_alias = ((X.P1.has_subview && X.P1.is_alias(*this)) ||
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT>& Cube<eT>::operator=(const mtGlueCube<eT, T1, T2, glue_type>& X) {
  arma_debug_sigprint();
...
//! linear element accessor (treats the cube as a vector); no bounds check; assumes memory
//! is aligned
template <typename eT>
arma_inline const eT& Cube<eT>::at_alt(const uword i) const {
...
//! linear element accessor (treats the cube as a vector); bounds checking not done when
//! ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline eT& Cube<eT>::operator()(const uword i) {
...
//! ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline const eT& Cube<eT>::operator()(const uword i) const {
  arma_conform_check_bounds((i >= n_elem), "Cube::operator(): index out of bounds");
...
//! linear element accessor (treats the cube as a vector); no bounds check.
template <typename eT>
arma_inline eT& Cube<eT>::at(const uword i) {
  return access::rw(mem[i]);
}

//! linear element accessor (treats the cube as a vector); no bounds check
template <typename eT>
arma_inline const eT& Cube<eT>::at(const uword i) const {
  return mem[i];
}

//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline eT& Cube<eT>::operator()(const uword in_row, const uword in_col,
...
//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline const eT& Cube<eT>::operator()(const uword in_row, const uword in_col,
...
//! element accessor; no bounds check
template <typename eT>
arma_inline eT& Cube<eT>::at(const uword in_row, const uword in_col,
                             const uword in_slice) {
  return access::rw(mem[in_slice * n_elem_slice + in_col * n_rows + in_row]);
}
...
//! element accessor; no bounds check
template <typename eT>
arma_inline const eT& Cube<eT>::at(const uword in_row, const uword in_col,
                                   const uword in_slice) const {
  return mem[in_slice * n_elem_slice + in_col * n_rows + in_row];
}
...
//! prefix ++
template <typename eT>
arma_inline const Cube<eT>& Cube<eT>::operator++() {
...
template <typename eT>
arma_inline void Cube<eT>::operator++(int) {
...
template <typename eT>
arma_inline bool Cube<eT>::is_empty() const {
  return (n_elem == 0);
}

template <typename eT>
inline bool Cube<eT>::internal_is_finite() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::internal_has_inf() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::internal_has_nan() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::internal_has_nonfinite() const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool Cube<eT>::in_range(const uword i) const {
  return (i < n_elem);
}

//! returns true if the given start and end indices are currently in range
template <typename eT>
arma_inline bool Cube<eT>::in_range(const span& x) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool Cube<eT>::in_range(const uword in_row, const uword in_col,
                                    const uword in_slice) const {
  return ((in_row < n_rows) && (in_col < n_cols) && (in_slice < n_slices));
}

template <typename eT>
inline bool Cube<eT>::in_range(const span& row_span, const span& col_span,
                               const span& slice_span) const {
  arma_debug_sigprint();
  ...
  const bool slices_ok =
      slice_span.whole ? true : ((in_slice1 <= in_slice2) && (in_slice2 < n_slices));

  return (rows_ok && cols_ok && slices_ok);
}

template <typename eT>
inline bool Cube<eT>::in_range(const uword in_row, const uword in_col,
                               const uword in_slice, const SizeCube& s) const {
  const uword l_n_rows = n_rows;
  const uword l_n_cols = n_cols;
  const uword l_n_slices = n_slices;

  if ((in_row >= l_n_rows) || (in_col >= l_n_cols) || (in_slice >= l_n_slices) ||
      ((in_row + s.n_rows) > l_n_rows) || ((in_col + s.n_cols) > l_n_cols) ||
...
template <typename eT>
arma_inline eT* Cube<eT>::memptr() {
  return const_cast<eT*>(mem);
}

//! returns a pointer to array of eTs used by the cube
template <typename eT>
arma_inline const eT* Cube<eT>::memptr() const {
  return mem;
}

//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline eT* Cube<eT>::slice_memptr(const uword uslice) {
  return const_cast<eT*>(&mem[uslice * n_elem_slice]);
}

//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline const eT* Cube<eT>::slice_memptr(const uword uslice) const {
  return &mem[uslice * n_elem_slice];
}

//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline eT* Cube<eT>::slice_colptr(const uword uslice, const uword col) {
  return const_cast<eT*>(&mem[uslice * n_elem_slice + col * n_rows]);
}

//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline const eT* Cube<eT>::slice_colptr(const uword uslice, const uword col) const {
  return &mem[uslice * n_elem_slice + col * n_rows];
}

//! change the cube to have user specified dimensions (data is not preserved)
template <typename eT>
inline Cube<eT>& Cube<eT>::set_size(const uword new_n_rows, const uword new_n_cols,
...
template <typename eT>
inline Cube<eT>& Cube<eT>::reshape(const uword new_n_rows, const uword new_n_cols,
                                   const uword new_n_slices) {
  arma_debug_sigprint();
...
//! change the cube to have user specified dimensions (data is preserved)
template <typename eT>
inline Cube<eT>& Cube<eT>::resize(const uword new_n_rows, const uword new_n_cols,
                                  const uword new_n_slices) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::set_size(const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::reshape(const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::resize(const SizeCube& s) {
  arma_debug_sigprint();
...
//! change the cube (without preserving data) to have the same dimensions as the given
//! cube
template <typename eT>
template <typename eT2>
inline Cube<eT>& Cube<eT>::copy_size(const Cube<eT2>& m) {
  arma_debug_sigprint();
...
inline Cube<eT>& Cube<eT>::for_each(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline const Cube<eT>& Cube<eT>::for_each(functor F) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline Cube<eT>& Cube<eT>::transform(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline Cube<eT>& Cube<eT>::imbue(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::replace(const eT old_val, const eT new_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::clean(const typename get_pod_type<eT>::result threshold) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::clamp(const eT min_val, const eT max_val) {
  arma_debug_sigprint();

  if (is_cx<eT>::no) {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "Cube::clamp(): min_val must be less than max_val");
  } else {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "Cube::clamp(): real(min_val) must be less than real(max_val)");
    arma_conform_check((access::tmp_imag(min_val) > access::tmp_imag(max_val)),
                       "Cube::clamp(): imag(min_val) must be less than imag(max_val)");
  }
...
template <typename eT>
inline Cube<eT>& Cube<eT>::fill(const eT val) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::zeros(const uword new_n_rows, const uword new_n_cols,
                                 const uword new_n_slices) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::zeros(const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::ones(const uword new_n_rows, const uword new_n_cols,
                                const uword new_n_slices) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::ones(const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::randu(const uword new_n_rows, const uword new_n_cols,
                                 const uword new_n_slices) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::randu(const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::randn(const uword new_n_rows, const uword new_n_cols,
                                 const uword new_n_slices) {
  arma_debug_sigprint();
...
template <typename eT>
inline Cube<eT>& Cube<eT>::randn(const SizeCube& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::soft_reset() {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::set_imag(const BaseCube<typename Cube<eT>::pod_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Cube<eT>::min(uword& index_of_min_val) const {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Cube<eT>::max(uword& index_of_max_val) const {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Cube<eT>::min(uword& row_of_min_val, uword& col_of_min_val,
...
template <typename eT>
inline eT Cube<eT>::max(uword& row_of_max_val, uword& col_of_max_val,
...
template <typename eT>
inline bool Cube<eT>::save(const std::string name, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (save_okay == false) {
    arma_warn(3, "Cube::save(): write failed; file: ", name);
  }
...
template <typename eT>
inline bool Cube<eT>::save(const hdf5_name& spec, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (append && replace) {
    arma_stop_runtime_error(
        "Cube::save(): only one of 'append' or 'replace' options can be used");
    return false;
  }
...
template <typename eT>
inline bool Cube<eT>::save(std::ostream& os, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (save_okay == false) {
    arma_warn(3, "Cube::save(): stream write failed");
  }
...
template <typename eT>
inline bool Cube<eT>::load(const std::string name, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::load(const hdf5_name& spec, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::load(std::istream& is, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::quiet_save(const std::string name, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::quiet_save(std::ostream& os, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::quiet_load(const std::string name, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Cube<eT>::quiet_load(std::istream& is, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::iterator Cube<eT>::begin() {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::const_iterator Cube<eT>::begin() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::const_iterator Cube<eT>::cbegin() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::iterator Cube<eT>::end() {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::const_iterator Cube<eT>::end() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::const_iterator Cube<eT>::cend() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::slice_iterator Cube<eT>::begin_slice(const uword slice_num) {
  arma_debug_sigprint();

  arma_conform_check_bounds((slice_num >= n_slices),
                            "begin_slice(): index out of bounds");

  return slice_memptr(slice_num);
}

template <typename eT>
inline typename Cube<eT>::const_slice_iterator Cube<eT>::begin_slice(
    const uword slice_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::slice_iterator Cube<eT>::end_slice(const uword slice_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::const_slice_iterator Cube<eT>::end_slice(
    const uword slice_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::clear() {
  reset();
}

//! returns true if the cube has no elements
template <typename eT>
inline bool Cube<eT>::empty() const {
  return (n_elem == 0);
}

//! returns the number of elements in this cube
template <typename eT>
inline uword Cube<eT>::size() const {
  return n_elem;
}

template <typename eT>
inline eT& Cube<eT>::front() {
...
template <typename eT>
inline const eT& Cube<eT>::front() const {
  arma_conform_check((n_elem == 0), "Cube::front(): cube is empty");
...
template <typename eT>
inline const eT& Cube<eT>::back() const {
  arma_conform_check((n_elem == 0), "Cube::back(): cube is empty");
...
template <typename eT>
inline void Cube<eT>::swap(Cube<eT>& B) {
  Cube<eT>& A = (*this);

  arma_debug_sigprint(arma_str::format("A: %x; B: %x") % &A % &B);

  if ((A.mem_state == 0) && (B.mem_state == 0) &&
      (A.n_elem > Cube_prealloc::mem_n_elem) && (B.n_elem > Cube_prealloc::mem_n_elem)) {
    A.delete_mat();
    B.delete_mat();
    ...
    A.create_mat();
    B.create_mat();
  } else if ((A.mem_state == 0) && (B.mem_state == 0) &&
             (A.n_elem <= Cube_prealloc::mem_n_elem) &&
             (B.n_elem <= Cube_prealloc::mem_n_elem)) {
    A.delete_mat();
    B.delete_mat();
...
template <typename eT>
inline void Cube<eT>::steal_mem(Cube<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::steal_mem(Cube<eT>& x, const bool is_move) {
  arma_debug_sigprint();
  ...
  if ((mem_state <= 1) && ((x.n_alloc > Cube_prealloc::mem_n_elem) ||
                           (x.mem_state == 1) || (is_move && (x.mem_state == 2)))) {
...
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::mem_setup() {
  ...
  if (fixed_n_elem > 0) {
    ...
    access::rw(Cube<eT>::mem) =
        (fixed_n_elem > Cube_prealloc::mem_n_elem) ? mem_local_extra : mem_local;
    Cube<eT>::mat_ptrs = (fixed_n_slices > Cube_prealloc::mat_ptrs_size)
                             ? mat_ptrs_local_extra
                             : mat_ptrs_local;
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(
    const fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>& X) {
  arma_debug_sigprint_this(this);
...
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(
  ...
  arma_static_check((is_same_type<fill_type, fill::fill_eye>::yes),
                    "Cube::fixed::fixed(): unsupported fill type");
}
...
template <typename T1, typename T2>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(
    const BaseCube<pod_type, T1>& A, const BaseCube<pod_type, T2>& B) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
inline Cube<eT>& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator=(
    const fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator[](
    const uword i) {
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols,
                                      fixed_n_slices>::operator[](const uword i) const {
  return (use_extra) ? mem_local_extra[i] : mem_local[i];
}

template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(
    const uword i) {
  return (use_extra) ? mem_local_extra[i] : mem_local[i];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline const eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(
    const uword i) const {
  return (use_extra) ? mem_local_extra[i] : mem_local[i];
}
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline eT& Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator[](
    const uword in_row, const uword in_col, const uword in_slice) {
...
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator[](
    const uword in_row, const uword in_col, const uword in_slice) const {
  const uword i = in_slice * fixed_n_elem_slice + in_col * fixed_n_rows + in_row;
...
//! prefix ++
template <typename eT>
arma_inline void Cube_aux::prefix_pp(Cube<eT>& x) {
  eT* memptr = x.memptr();
...
template <typename T>
arma_inline void Cube_aux::prefix_pp(Cube<std::complex<T> >& x) {
  x += T(1);
}

//! postfix ++
template <typename eT>
arma_inline void Cube_aux::postfix_pp(Cube<eT>& x) {
  eT* memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
template <typename T>
arma_inline void Cube_aux::postfix_pp(Cube<std::complex<T> >& x) {
  x += T(1);
}

//! prefix --
template <typename eT>
arma_inline void Cube_aux::prefix_mm(Cube<eT>& x) {
  eT* memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! prefix -- for complex numbers (work around for limitations of the std::complex class)
template <typename T>
arma_inline void Cube_aux::prefix_mm(Cube<std::complex<T> >& x) {
  x -= T(1);
}

//! postfix --
template <typename eT>
arma_inline void Cube_aux::postfix_mm(Cube<eT>& x) {
  eT* memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! postfix ++ for complex numbers (work around for limitations of the std::complex class)
template <typename T>
arma_inline void Cube_aux::postfix_mm(Cube<std::complex<T> >& x) {
  x -= T(1);
}

template <typename eT, typename T1>
inline void Cube_aux::set_real(Cube<eT>& out, const BaseCube<eT, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT, typename T1>
inline void Cube_aux::set_imag(Cube<eT>&, const BaseCube<eT, T1>&) {
  arma_debug_sigprint();
}
...
inline void Cube_aux::set_real(Cube<std::complex<T> >& out, const BaseCube<T, T1>& X) {
  ...
  arma_conform_assert_same_size(out.n_rows, out.n_cols, out.n_slices, local_n_rows,
                                local_n_cols, local_n_slices, "Cube::set_real()");
...
inline void Cube_aux::set_imag(Cube<std::complex<T> >& out, const BaseCube<T, T1>& X) {
  ...
  arma_conform_assert_same_size(out.n_rows, out.n_cols, out.n_slices, local_n_rows,
                                local_n_cols, local_n_slices, "Cube::set_imag()");
