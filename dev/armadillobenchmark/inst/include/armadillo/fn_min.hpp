...
template <typename T>
arma_warn_unused arma_inline typename arma_scalar_only<T>::result min(const T& x) {
  return x;
}
...
arma_warn_unused inline uword min(const SizeMat& s) {
  return (std::min)(s.n_rows, s.n_cols);
}

arma_warn_unused inline uword min(const SizeCube& s) {
