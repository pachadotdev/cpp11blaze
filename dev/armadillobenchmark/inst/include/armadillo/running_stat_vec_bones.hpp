...
struct rsv_get_elem_type {
  typedef typename rsv_get_elem_type_worker<obj_type, is_Mat<obj_type>::value>::result
      elem_type;
};
...
struct rsv_get_return_type1 {
  typedef typename rsv_get_return_type1_worker<obj_type, is_Mat<obj_type>::value>::result
      return_type1;
};
...
class running_stat_vec {
 public:
  ...
  inline running_stat_vec(
      const bool in_calc_cov =
          false);  // TODO: investigate char* overload, eg. "calc_cov", "no_calc_cov"
  ...
  inline void operator()(const Base<T, T1>& X);
  template <typename T1>
  inline void operator()(const Base<std::complex<T>, T1>& X);
...
class running_stat_vec_aux {
 public:
  template <typename obj_type>
  inline static void update_stats(
