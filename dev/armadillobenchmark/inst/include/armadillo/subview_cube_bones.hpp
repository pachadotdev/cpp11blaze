...
//! (ie. where the subcube starts and ends as well as a reference/pointer to the original
//! cube),
...
class subview_cube : public BaseCube<eT, subview_cube<eT> > {
 ...
 protected:
  arma_inline subview_cube(const Cube<eT>& in_m, const uword in_row1, const uword in_col1,
                           const uword in_slice1, const uword in_n_rows,
 ...
 public:
  ...
  inline void inplace_op(const BaseCube<eT, T1>& x, const char* identifier);
  template <typename op_type>
  inline void inplace_op(const subview_cube<eT>& x, const char* identifier);
  ...
  arma_inline eT& operator()(const uword in_row, const uword in_col,
  ...
  class iterator {
   public:
    ...
    inline iterator(subview_cube<eT>& in_sv, const uword in_row, const uword in_col,
                    const uword in_slice);
  ...
  class const_iterator {
   public:
    ...
    inline const_iterator(const subview_cube<eT>& in_sv, const uword in_row,
