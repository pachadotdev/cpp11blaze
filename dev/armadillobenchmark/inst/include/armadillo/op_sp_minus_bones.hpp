...
class op_sp_minus_pre : public traits_op_passthru {
 public:
  ...
  inline static void apply_inside_schur(SpMat<eT>& out, const T2& x,
                                        const SpToDOp<T3, op_sp_minus_pre>& y);

  // used for the optimization of sparse / (scalar - sparse)
  template <typename eT, typename T2, typename T3>
  inline static void apply_inside_div(SpMat<eT>& out, const T2& x,
                                      const SpToDOp<T3, op_sp_minus_pre>& y);
};

// Subtract a scalar from a sparse object; the output will be a dense object.
class op_sp_minus_post : public traits_op_passthru {
 public:
  template <typename T1>
  inline static void apply(Mat<typename T1::elem_type>& out,
