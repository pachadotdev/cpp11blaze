...
inline void op_roots::apply(
    Mat<std::complex<typename T1::pod_type> >& out,
    const mtOp<std::complex<typename T1::pod_type>, T1, op_roots>& expr) {
  arma_debug_sigprint();
...
template <typename T1>
inline bool op_roots::apply_direct(Mat<std::complex<typename T1::pod_type> >& out,
...
inline bool op_roots::apply_noalias(
    Mat<std::complex<typename get_pod_type<eT>::result> >& out, const Mat<eT>& X) {
  arma_debug_sigprint();
  ...
  const uword n_tail_zeros = (indices.n_elem > 0)
                                 ? uword((Y.n_elem - 1) - indices[indices.n_elem - 1])
                                 : uword(0);
