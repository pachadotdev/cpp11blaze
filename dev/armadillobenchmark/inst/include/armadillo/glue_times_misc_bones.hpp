...
class dense_sparse_helper {
 public:
  ...
  arma_inline static typename arma_not_cx<eT>::result dot(const eT* A_mem,
                                                          const SpMat<eT>& B,
                                                          const uword col);

  template <typename eT>
  arma_inline static typename arma_cx_only<eT>::result dot(const eT* A_mem,
                                                           const SpMat<eT>& B,
                                                           const uword col);
};
...
class glue_times_dense_sparse {
 public:
  ...
  template <typename T1, typename T2>
  inline static void apply_noalias(Mat<typename T1::elem_type>& out, const T1& x,
                                   const T2& y);
...
class glue_times_sparse_dense {
 public:
  ...
  template <typename T1, typename T2>
  inline static void apply_noalias_trans(Mat<typename T1::elem_type>& out, const T1& x,
                                         const T2& y);
