...
arma_warn_unused inline
    ...
    normalise(const T1& X, const uword p = uword(2),
...
template <typename T1>
arma_warn_unused inline const SpOp<T1, spop_normalise> normalise(
...
arma_warn_unused arma_inline
    typename enable_if2<is_supported_blas_type<T>::value, Col<T> >::result
