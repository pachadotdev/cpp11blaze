...
inline typename enable_if2<(is_real<typename T1::elem_type>::value), void>::result
log_normpdf_helper(Mat<typename T1::elem_type>& out,
                   const Base<typename T1::elem_type, T1>& X_expr,
                   const Base<typename T1::elem_type, T2>& M_expr,
                   const Base<typename T1::elem_type, T3>& S_expr) {
  arma_debug_sigprint();
  ...
  arma_conform_check(
      ((PX.get_n_rows() != PM.get_n_rows()) || (PX.get_n_cols() != PM.get_n_cols()) ||
       (PM.get_n_rows() != PS.get_n_rows()) || (PM.get_n_cols() != PS.get_n_cols())),
      "log_normpdf(): size mismatch");
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
      for (uword i = 0; i < N; ++i) {
        ...
        out_mem[i] =
            (eT(-0.5) * (tmp * tmp)) - (std::log(sigma) + Datum<eT>::log_sqrt2pi);
      }
    }
#endif
  } else {
...
arma_warn_unused inline typename enable_if2<(is_real<eT>::value), eT>::result log_normpdf(
    const eT x) {
...
template <typename eT>
arma_warn_unused inline typename enable_if2<(is_real<eT>::value), eT>::result log_normpdf(
    const eT x, const eT mu, const eT sigma) {
...
arma_warn_unused inline typename enable_if2<(is_real<eT>::value), Mat<eT> >::result
...
arma_warn_unused inline typename enable_if2<(is_real<typename T1::elem_type>::value),
...
log_normpdf(const Base<typename T1::elem_type, T1>& X_expr) {
  ...
  log_normpdf_helper(out, X, zeros<Mat<eT> >(arma::size(X)),
                     ones<Mat<eT> >(arma::size(X)));
...
arma_warn_unused inline typename enable_if2<(is_real<typename T1::elem_type>::value),
                                            Mat<typename T1::elem_type> >::result
log_normpdf(const Base<typename T1::elem_type, T1>& X_expr,
            const typename T1::elem_type mu, const typename T1::elem_type sigma) {
  arma_debug_sigprint();
  ...
  log_normpdf_helper(out, X, mu * ones<Mat<eT> >(arma::size(X)),
                     sigma * ones<Mat<eT> >(arma::size(X)));
