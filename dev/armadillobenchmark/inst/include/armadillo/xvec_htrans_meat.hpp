...
inline xvec_htrans<eT>::xvec_htrans(const eT* const in_mem, const uword in_n_rows,
...
inline eT xvec_htrans<eT>::at(const uword in_row, const uword in_col) const {
  ...
  return access::alt_conj(mem[in_row + in_col]);  // either in_row or in_col must be zero,
                                                  // as we're storing a vector
}
