...
inline std::ostream& get_cout_stream() { return (ARMA_COUT_STREAM); }

inline std::ostream& get_cerr_stream() { return (ARMA_CERR_STREAM); }

arma_deprecated inline std::ostream& get_stream_err1() { return get_cerr_stream(); }

arma_deprecated inline std::ostream& get_stream_err2() { return get_cerr_stream(); }

arma_frown(
    "this function does nothing; instead use ARMA_COUT_STREAM or ARMA_WARN_LEVEL; see "
    "documentation") inline void set_cout_stream(const std::ostream&) {}

arma_frown(
    "this function does nothing; instead use ARMA_CERR_STREAM or ARMA_WARN_LEVEL; see "
    "documentation") inline void set_cerr_stream(const std::ostream&) {}

arma_frown(
    "this function does nothing; instead use ARMA_CERR_STREAM or ARMA_WARN_LEVEL; see "
    "documentation") inline void set_stream_err1(const std::ostream&) {}

arma_frown(
    "this function does nothing; instead use ARMA_CERR_STREAM or ARMA_WARN_LEVEL; see "
    "documentation") inline void set_stream_err2(const std::ostream&) {}

template <typename T>
arma_frown(
    "this function does nothing; instead use ARMA_COUT_STREAM or ARMA_WARN_LEVEL; see "
    "documentation") inline std::ostream& arma_cout_stream(std::ostream*) {
...
arma_frown(
    "this function does nothing; instead use ARMA_CERR_STREAM or ARMA_WARN_LEVEL; see "
    "documentation") inline std::ostream& arma_cerr_stream(std::ostream*) {
...
arma_cold arma_noinline static void arma_stop_logic_error(const char* x, const char* y) {
...
template <typename T1>
arma_cold arma_noinline static void arma_print(const T1& x) {
...
template <typename T1>
inline void arma_bktprint(const T1& x) {
  get_cerr_stream() << " [" << x << ']' << std::endl;
}

template <typename T1, typename T2>
inline void arma_bktprint(const T1& x, const T2& y) {
  get_cerr_stream() << " [" << x << y << ']' << std::endl;
}
...
template <typename T1>
arma_cold arma_noinline static void arma_plain_warn(const T1& arg1) {
  get_cerr_stream() << "\nwarning: " << arg1 << std::endl;
}

template <typename T1, typename T2>
arma_cold arma_noinline static void arma_plain_warn(const T1& arg1, const T2& arg2) {
...
template <typename T1>
inline void arma_warn(const uword level, const T1& arg1) {
  constexpr uword config_level =
      (sword(ARMA_WARN_LEVEL) > 0) ? uword(ARMA_WARN_LEVEL) : uword(0);

  if ((config_level > 0) && (level <= config_level)) {
    arma_plain_warn(arg1);
  }
}

template <typename T1, typename T2>
inline void arma_warn(const uword level, const T1& arg1, const T2& arg2) {
  constexpr uword config_level =
      (sword(ARMA_WARN_LEVEL) > 0) ? uword(ARMA_WARN_LEVEL) : uword(0);

  if ((config_level > 0) && (level <= config_level)) {
    arma_plain_warn(arg1, arg2);
  }
}

template <typename T1, typename T2, typename T3>
inline void arma_warn(const uword level, const T1& arg1, const T2& arg2, const T3& arg3) {
  constexpr uword config_level =
      (sword(ARMA_WARN_LEVEL) > 0) ? uword(ARMA_WARN_LEVEL) : uword(0);

  if ((config_level > 0) && (level <= config_level)) {
    arma_plain_warn(arg1, arg2, arg3);
  }
}
...
inline void arma_warn(const uword level, const T1& arg1, const T2& arg2, const T3& arg3,
  ...
  if ((config_level > 0) && (level <= config_level)) {
    arma_plain_warn(arg1, arg2, arg3, arg4);
  }
}
...
template <typename T1>
arma_hot inline void arma_check(const bool state, const T1& x) {
  if (state) {
...
arma_hot inline void arma_check(const bool state, const char* x, const Functor& fn) {
  if (state) {
    fn();
    arma_stop_logic_error(x);
  }
}

arma_hot inline void arma_check(const bool state, const char* x, const char* y) {
  if (state) {
    arma_stop_logic_error(x, y);
  }
}
...
template <typename T1>
arma_hot inline void arma_check_bounds(const bool state, const T1& x) {
  if (state) {
    arma_stop_bounds_error(arma_str::str_wrapper(x));
  }
}

template <typename T1>
arma_hot inline void arma_check_bad_alloc(const bool state, const T1& x) {
  if (state) {
...
arma_hot arma_inline void arma_set_error(bool& err_state, char*& err_msg,
                                         const bool expression, const char* message) {
  if (expression) {
    err_state = true;
...
arma_cold arma_noinline static std::string arma_incompat_size_string(const uword A_n_rows,
  ...
  tmp << x << ": incompatible matrix dimensions: " << A_n_rows << 'x' << A_n_cols
      << " and " << B_n_rows << 'x' << B_n_cols;

  return tmp.str();
}

arma_cold arma_noinline static std::string arma_incompat_size_string(
    const uword A_n_rows, const uword A_n_cols, const uword A_n_slices,
    const uword B_n_rows, const uword B_n_cols, const uword B_n_slices, const char* x) {
  std::ostringstream tmp;

  tmp << x << ": incompatible cube dimensions: " << A_n_rows << 'x' << A_n_cols << 'x'
      << A_n_slices << " and " << B_n_rows << 'x' << B_n_cols << 'x' << B_n_slices;
...
arma_cold arma_noinline static std::string arma_incompat_size_string(
    const subview_cube<eT>& Q, const Mat<eT>& A, const char* x) {
  std::ostringstream tmp;

  tmp << x << ": interpreting matrix as cube with dimensions: " << A.n_rows << 'x'
      << A.n_cols << 'x' << 1 << " or " << A.n_rows << 'x' << 1 << 'x' << A.n_cols
      << " or " << 1 << 'x' << A.n_rows << 'x' << A.n_cols
      << " is incompatible with cube dimensions: " << Q.n_rows << 'x' << Q.n_cols << 'x'
      << Q.n_slices;
...
arma_hot arma_inline void arma_assert_same_size(const uword A_n_rows,
                                                const uword A_n_cols,
                                                const uword B_n_rows,
                                                const uword B_n_cols, const char* x) {
...
arma_hot inline void arma_assert_same_size(const Mat<eT1>& A, const Mat<eT2>& B,
                                           const char* x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Proxy<eT1>& A, const Proxy<eT2>& B,
                                           const char* x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview<eT1>& A, const subview<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Mat<eT1>& A, const subview<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview<eT1>& A, const Mat<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Mat<eT1>& A, const Proxy<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Proxy<eT1>& A, const Mat<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.get_n_rows();
  const uword A_n_cols = A.get_n_cols();
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Proxy<eT1>& A, const subview<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.get_n_rows();
  const uword A_n_cols = A.get_n_cols();
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview<eT1>& A, const Proxy<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const SpMat<eT1>& A, const SpMat<eT2>& B,
                                           const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
arma_hot inline void arma_assert_same_size(const uword A_n_rows, const uword A_n_cols,
                                           const uword A_n_slices, const uword B_n_rows,
                                           const uword B_n_cols, const uword B_n_slices,
                                           const char* x) {
  if ((A_n_rows != B_n_rows) || (A_n_cols != B_n_cols) || (A_n_slices != B_n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A_n_rows, A_n_cols, A_n_slices,
                                                    B_n_rows, B_n_cols, B_n_slices, x));
  }
}

//! stop if given cubes do not have the same size
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Cube<eT1>& A, const Cube<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, A.n_slices,
                                                    B.n_rows, B.n_cols, B.n_slices, x));
  }
}

template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Cube<eT1>& A, const subview_cube<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, A.n_slices,
                                                    B.n_rows, B.n_cols, B.n_slices, x));
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview_cube<eT1>& A, const Cube<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, A.n_slices,
                                                    B.n_rows, B.n_cols, B.n_slices, x));
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview_cube<eT1>& A,
                                           const subview_cube<eT2>& B, const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, A.n_slices,
                                                    B.n_rows, B.n_cols, B.n_slices, x));
  }
...
arma_hot inline void arma_assert_same_size(const subview_cube<eT>& A,
                                           const ProxyCube<T1>& B, const char* x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const ProxyCube<eT1>& A,
                                           const ProxyCube<eT2>& B, const char* x) {
  const uword A_n_rows = A.get_n_rows();
  const uword A_n_cols = A.get_n_cols();
...
//
// functions for checking whether a cube or subcube can be interpreted as a matrix (ie.
// single slice)

template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Cube<eT1>& A, const Mat<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != 1)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, A.n_slices,
                                                    B.n_rows, B.n_cols, 1, x));
  }
}

template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Mat<eT1>& A, const Cube<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (1 != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, 1, B.n_rows,
                                                    B.n_cols, B.n_slices, x));
  }
}

template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview_cube<eT1>& A, const Mat<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != 1)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, A.n_slices,
                                                    B.n_rows, B.n_cols, 1, x));
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Mat<eT1>& A, const subview_cube<eT2>& B,
                                           const char* x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (1 != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(A.n_rows, A.n_cols, 1, B.n_rows,
                                                    B.n_cols, B.n_slices, x));
...
template <typename eT, typename T1>
inline void arma_assert_cube_as_mat(const Mat<eT>& M, const T1& Q, const char* x,
  ...
  if (M_vec_state == 0) {
    if (((Q_n_rows == 1) || (Q_n_cols == 1) || (Q_n_slices == 1)) == false) {
      ...
      tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x' << Q_n_cols
          << 'x' << Q_n_slices << " as a matrix; one of the dimensions must be 1";
    ...
    if (Q_n_slices == 1) {
      if ((M_vec_state == 1) && (Q_n_cols != 1)) {
        ...
        tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
            << Q_n_cols << 'x' << Q_n_slices << " as a column vector";
      ...
      if ((M_vec_state == 2) && (Q_n_rows != 1)) {
        ...
        tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
            << Q_n_cols << 'x' << Q_n_slices << " as a row vector";
      ...
      if ((Q_n_cols != 1) && (Q_n_rows != 1)) {
        ...
        tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
            << Q_n_cols << 'x' << Q_n_slices << " as a vector";
  ...
  if (check_compat_size) {
    ...
    if (M_vec_state == 0) {
      if ((((Q_n_rows == M_n_rows) && (Q_n_cols == M_n_cols)) ||
           ((Q_n_rows == M_n_rows) && (Q_n_slices == M_n_cols)) ||
           ((Q_n_cols == M_n_rows) && (Q_n_slices == M_n_cols))) == false) {
        std::ostringstream tmp;

        tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
            << Q_n_cols << 'x' << Q_n_slices << " as a matrix with dimensions "
            << M_n_rows << 'x' << M_n_cols;
      ...
      if (Q_n_slices == 1) {
        if ((M_vec_state == 1) && (Q_n_rows != M_n_rows)) {
          ...
          tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
              << Q_n_cols << 'x' << Q_n_slices << " as a column vector with dimensions "
              << M_n_rows << 'x' << M_n_cols;
        ...
        if ((M_vec_state == 2) && (Q_n_cols != M_n_cols)) {
          ...
          tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
              << Q_n_cols << 'x' << Q_n_slices << " as a row vector with dimensions "
              << M_n_rows << 'x' << M_n_cols;
        ...
        if (((M_n_cols == Q_n_slices) || (M_n_rows == Q_n_slices)) == false) {
          ...
          tmp << x << ": can't interpret cube with dimensions " << Q_n_rows << 'x'
              << Q_n_cols << 'x' << Q_n_slices << " as a vector with dimensions "
              << M_n_rows << 'x' << M_n_cols;
...
//
// functions for checking whether two matrices have dimensions that are compatible with
// the matrix multiply operation

arma_hot inline void arma_assert_mul_size(const uword A_n_rows, const uword A_n_cols,
                                          const uword B_n_rows, const uword B_n_cols,
                                          const char* x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const Mat<eT1>& A, const Mat<eT2>& B,
                                          const char* x) {
  const uword A_n_cols = A.n_cols;
  const uword B_n_rows = B.n_rows;
  ...
  if (A_n_cols != B_n_rows) {
    arma_stop_logic_error(
        arma_incompat_size_string(A.n_rows, A_n_cols, B_n_rows, B.n_cols, x));
  }
}
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const Mat<eT1>& A, const Mat<eT2>& B,
                                          const bool do_trans_A, const bool do_trans_B,
                                          const char* x) {
  ...
  if (final_A_n_cols != final_B_n_rows) {
    ...
    arma_stop_logic_error(arma_incompat_size_string(final_A_n_rows, final_A_n_cols,
                                                    final_B_n_rows, final_B_n_cols, x));
  }
}

template <const bool do_trans_A, const bool do_trans_B>
arma_hot inline void arma_assert_trans_mul_size(const uword A_n_rows,
                                                const uword A_n_cols,
                                                const uword B_n_rows,
                                                const uword B_n_cols, const char* x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const Mat<eT1>& A, const subview<eT2>& B,
                                          const char* x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const subview<eT1>& A, const Mat<eT2>& B,
                                          const char* x) {
  if (A.n_cols != B.n_rows) {
    arma_stop_logic_error(
        arma_incompat_size_string(A.n_rows, A.n_cols, B.n_rows, B.n_cols, x));
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const subview<eT1>& A, const subview<eT2>& B,
                                          const char* x) {
  if (A.n_cols != B.n_rows) {
    arma_stop_logic_error(
        arma_incompat_size_string(A.n_rows, A.n_cols, B.n_rows, B.n_cols, x));
...
template <typename T1>
arma_hot inline void arma_assert_blas_size(const T1& A) {
  if (sizeof(uword) >= sizeof(blas_int)) {
    ...
    if (overflow) {
      arma_stop_runtime_error(
          "integer overflow: matrix dimensions are too large for integer type used by "
          "BLAS and LAPACK");
    }
  }
}
...
template <typename T1, typename T2>
arma_hot inline void arma_assert_blas_size(const T1& A, const T2& B) {
  if (sizeof(uword) >= sizeof(blas_int)) {
    bool overflow;
...
template <typename T1>
arma_hot inline void arma_assert_atlas_size(const T1& A) {
...
// TODO: remove support for ATLAS in next major version
template <typename T1, typename T2>
arma_hot inline void arma_assert_atlas_size(const T1& A, const T2& B) {
  if (sizeof(uword) >= sizeof(int)) {
    bool overflow;
...
#define arma_debug_sigprint  \
  arma_sigprint(ARMA_FNSIG); \
  arma_bktprint
#define arma_debug_sigprint_this \
  arma_sigprint(ARMA_FNSIG);     \
  arma_thisprint
...
arma_frown("use arma_conform_check() instead") inline void arma_debug_check(
    bool state, const char* msg) {
  arma_conform_check(state, msg);
}
...
class arma_first_debug_message {
 public:
  inline arma_first_debug_message() {
