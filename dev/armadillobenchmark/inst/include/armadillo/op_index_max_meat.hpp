...
inline void op_index_max::apply(Mat<uword>& out,
                                const mtOp<uword, T1, op_index_max>& in) {
  arma_debug_sigprint();
...
inline void op_index_max::apply_noalias(Mat<uword>& out, const Mat<eT>& X,
                                        const uword dim) {
  arma_debug_sigprint();
  ...
  if (dim == 0) {
    ...
    for (uword col = 1; col < X_n_cols; ++col) {
      ...
      for (uword row = 0; row < X_n_rows; ++row) {
        ...
        T col_val = (is_cx<eT>::yes) ? T(eop_aux::arma_abs(col_mem[row]))
                                     : T(access::tmp_real(col_mem[row]));
...
template <typename T1>
inline void op_index_max::apply(Cube<uword>& out,
                                const mtOpCube<uword, T1, op_index_max>& in) {
  arma_debug_sigprint();
...
template <typename eT>
inline void op_index_max::apply_noalias(Cube<uword>& out, const Cube<eT>& X,
                                        const uword dim,
