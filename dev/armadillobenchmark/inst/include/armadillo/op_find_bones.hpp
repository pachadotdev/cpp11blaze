...
class op_find : public traits_op_col {
 public:
  template <typename T1>
  inline static uword helper(Mat<uword>& indices,
                             const Base<typename T1::elem_type, T1>& X);

  template <typename T1, typename op_type>
  inline static uword helper(
  ...
  inline static uword helper(
      Mat<uword>& indices, const mtGlue<uword, T1, T2, glue_type>& X,
