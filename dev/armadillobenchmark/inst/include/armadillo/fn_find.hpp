...
template <typename T1>
arma_warn_unused inline const mtOp<uword, T1, op_find> find(
    const Base<typename T1::elem_type, T1>& X, const uword k,
    const char* direction = "first") {
  arma_debug_sigprint();

  const char sig = (direction != nullptr) ? direction[0] : char(0);

  arma_conform_check(((sig != 'f') && (sig != 'F') && (sig != 'l') && (sig != 'L')),
...
template <typename T1>
arma_warn_unused inline uvec find(const BaseCube<typename T1::elem_type, T1>& X,
                                  const uword k, const char* direction = "first") {
  arma_debug_sigprint();
...
arma_warn_unused inline uvec find(const mtOpCube<uword, T1, op_rel_type>& X,
                                  const uword k = 0, const char* direction = "first") {
  arma_debug_sigprint();
...
arma_warn_unused inline uvec find(const mtGlueCube<uword, T1, T2, glue_rel_type>& X,
                                  const uword k = 0, const char* direction = "first") {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline Col<uword> find(const SpBase<typename T1::elem_type, T1>& X,
                                        const uword k = 0) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline Col<uword> find(const SpBase<typename T1::elem_type, T1>& X,
                                        const uword k, const char* direction) {
  arma_debug_sigprint();
...
arma_warn_unused inline typename enable_if2<is_arma_type<T1>::value,
                                            const mtOp<uword, T1, op_find_nan> >::result
find_nan(const T1& X) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline uvec find_nonfinite(
...
template <typename T1>
arma_warn_unused inline uvec find_nan(const BaseCube<typename T1::elem_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline Col<uword> find_finite(
...
template <typename T1>
arma_warn_unused inline Col<uword> find_nonfinite(
    const SpBase<typename T1::elem_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline Col<uword> find_nan(const SpBase<typename T1::elem_type, T1>& X) {
  arma_debug_sigprint();
