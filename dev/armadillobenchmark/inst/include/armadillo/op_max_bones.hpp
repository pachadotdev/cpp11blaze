...
class op_max : public traits_op_xvec {
 public:
  //
  // dense matrices
  ...
  inline static void apply_noalias(
      Mat<eT>& out, const Mat<eT>& X, const uword dim,
  ...
  template <typename eT>
  inline static void apply_noalias(
      Cube<eT>& out, const Cube<eT>& X, const uword dim,
      const typename arma_not_cx<eT>::result* junk = nullptr);
