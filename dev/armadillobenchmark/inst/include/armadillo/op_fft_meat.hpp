...
class fft_engine_wrapper {
 public:
  ...
  inline ~fft_engine_wrapper() {
    arma_debug_sigprint();

    if (worker_kissfft != nullptr) {
      delete worker_kissfft;
    }
    if (worker_fftw3 != nullptr) {
      delete worker_fftw3;
    }
  }

  inline fft_engine_wrapper(const uword N_samples, const uword N_exec) {
    arma_debug_sigprint();

    const bool use_fftw3 = N_samples >= (threshold / N_exec);

    worker_kissfft = (use_fftw3 == false)
                         ? new fft_engine_kissfft<cx_type, inverse>(N_samples)
                         : nullptr;
    worker_fftw3 =
        (use_fftw3 == true) ? new fft_engine_fftw3<cx_type, inverse>(N_samples) : nullptr;
  }
  ...
  inline void run(cx_type* Y, const cx_type* X) {
    ...
    if (worker_kissfft != nullptr) {
      (*worker_kissfft).run(Y, X);
    } else if (worker_fftw3 != nullptr) {
      (*worker_fftw3).run(Y, X);
    }
  }
};
...
inline void op_fft_real::apply(
  ...
  // no need to worry about aliasing, as we're going from a real object to complex
  // complex, which by definition cannot alias
...
inline void op_fft_cx::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword a,
                                     const uword b) {
  arma_debug_sigprint();
