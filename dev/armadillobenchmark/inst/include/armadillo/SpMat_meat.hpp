...
template <typename eT>
inline SpMat<eT>::SpMat(const arma_reserve_indicator&, const uword in_rows,
                        const uword in_cols, const uword new_n_nonzero)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
inline SpMat<eT>::SpMat(const arma_layout_indicator&, const SpMat<eT2>& x)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const char* text) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const std::string& text) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(SpMat<eT>&& in_mat) {
  arma_debug_sigprint(arma_str::format("this: %x; in_mat: %x") % this % &in_mat);
...
template <typename eT>
inline SpMat<eT>::SpMat(const MapMat<eT>& x)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const MapMat<eT>& x) {
  arma_debug_sigprint();
...
inline SpMat<eT>::SpMat(const Base<uword, T1>& locations_expr,
  ...
  arma_conform_check((vals.is_vec() == false),
                     "SpMat::SpMat(): given 'values' object must be a vector");
  arma_conform_check((locs.n_rows != 2),
                     "SpMat::SpMat(): locations matrix must have two rows");
  arma_conform_check(
      (locs.n_cols != vals.n_elem),
      "SpMat::SpMat(): number of locations is different than number of values");
  ...
  for (uword i = 0; i < N_old; ++i) {
    N_new += (vals[i] != eT(0)) ? uword(1) : uword(0);
  }
...
template <typename T1, typename T2>
inline SpMat<eT>::SpMat(const Base<uword, T1>& locations_expr,
                        const Base<eT, T2>& vals_expr, const uword in_n_rows,
                        const uword in_n_cols, const bool sort_locations,
...
template <typename eT>
template <typename T1, typename T2>
inline SpMat<eT>::SpMat(const bool add_values, const Base<uword, T1>& locations_expr,
                        const Base<eT, T2>& vals_expr, const uword in_n_rows,
                        const uword in_n_cols, const bool sort_locations,
                        const bool check_for_zeros)
    : n_rows(0),
  ...
  if (check_for_zeros) {
    ...
    if (N_new != N_old) {
      ...
      add_values ? init_batch_add(filtered_locs, filtered_vals, sort_locations)
                 : init_batch_std(filtered_locs, filtered_vals, sort_locations);
    } else {
      add_values ? init_batch_add(locs, vals, sort_locations)
                 : init_batch_std(locs, vals, sort_locations);
    }
  } else {
    add_values ? init_batch_add(locs, vals, sort_locations)
...
inline SpMat<eT>::SpMat(const Base<uword, T1>& rowind_expr,
                        const Base<uword, T2>& colptr_expr,
                        const Base<eT, T3>& values_expr, const uword in_n_rows,
                        const uword in_n_cols, const bool check_for_zeros)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
  ...
  arma_conform_check((rowind.is_vec() == false),
                     "SpMat::SpMat(): given 'rowind' object must be a vector");
  arma_conform_check((colptr.is_vec() == false),
                     "SpMat::SpMat(): given 'colptr' object must be a vector");
  arma_conform_check((vals.is_vec() == false),
                     "SpMat::SpMat(): given 'values' object must be a vector");
  ...
  arma_conform_check(
      (rowind.n_elem != vals.n_elem),
      "SpMat::SpMat(): number of row indices is not equal to number of values");
  arma_conform_check(
      (colptr.n_elem != (n_cols + 1)),
      "SpMat::SpMat(): number of column pointers is not equal to n_cols+1");
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const eT val) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const SpMat<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator*=(const SpMat<eT>& y) {
  arma_debug_sigprint();
...
inline SpMat<eT>& SpMat<eT>::operator/=(const SpMat<eT>& x) {
  ...
  arma_conform_assert_same_size(n_rows, n_cols, x.n_rows, x.n_cols,
...
template <typename eT>
template <typename T1, typename T2>
inline SpMat<eT>::SpMat(const SpBase<typename SpMat<eT>::pod_type, T1>& A,
...
template <typename eT>
template <typename T1>
inline SpMat<eT>& SpMat<eT>::operator=(const Base<eT, T1>& expr) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline SpMat<eT>& SpMat<eT>::operator+=(const Base<eT, T1>& x) {
  arma_debug_sigprint();
...
inline SpMat<eT>& SpMat<eT>::operator%=(const Base<eT, T1>& x) {
  ...
  arma_conform_assert_same_size(n_rows, n_cols, B.n_rows, B.n_cols,
...
template <typename eT>
template <typename T1>
inline SpMat<eT>::SpMat(const Op<T1, op_diagmat>& expr)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
template <typename T1>
inline SpMat<eT>& SpMat<eT>::operator=(const Op<T1, op_diagmat>& expr) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const SpSubview<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator*=(const SpSubview<eT>& y) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline SpMat<eT>::SpMat(const SpSubview_col_list<eT, T1>& X)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
template <typename T1>
inline SpMat<eT>& SpMat<eT>::operator=(const SpSubview_col_list<eT, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>::SpMat(const spdiagview<eT>& X)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::operator=(const spdiagview<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename spop_type>
inline SpMat<eT>& SpMat<eT>::operator=(const SpOp<T1, spop_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename spglue_type>
inline SpMat<eT>& SpMat<eT>::operator=(const SpGlue<T1, T2, spglue_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename spop_type>
inline SpMat<eT>& SpMat<eT>::operator=(const mtSpOp<eT, T1, spop_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename spglue_type>
inline SpMat<eT>::SpMat(const mtSpGlue<eT, T1, T2, spglue_type>& X)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
template <typename T1, typename T2, typename spglue_type>
inline SpMat<eT>& SpMat<eT>::operator=(const mtSpGlue<eT, T1, T2, spglue_type>& X) {
  arma_debug_sigprint();
...
inline SpMat<eT>::SpMat(const mtSpReduceOp<eT, T1, op_type>& X)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
template <typename T1, typename op_type>
inline SpMat<eT>& SpMat<eT>::operator=(const mtSpReduceOp<eT, T1, op_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline SpSubview_row<eT> SpMat<eT>::row(const uword row_num) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const SpSubview_row<eT> SpMat<eT>::row(const uword row_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview_row<eT> SpMat<eT>::operator()(const uword row_num,
                                               const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview_row<eT> SpMat<eT>::operator()(const uword row_num,
...
template <typename eT>
arma_inline SpSubview_col<eT> SpMat<eT>::col(const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const SpSubview_col<eT> SpMat<eT>::col(const uword col_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview_col<eT> SpMat<eT>::operator()(const span& row_span,
                                               const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview_col<eT> SpMat<eT>::operator()(const span& row_span,
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const SpSubview<eT> SpMat<eT>::rows(const uword in_row1,
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const SpSubview<eT> SpMat<eT>::cols(const uword in_col1,
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::submat(const uword in_row1, const uword in_col1,
...
template <typename eT>
arma_inline const SpSubview<eT> SpMat<eT>::submat(const uword in_row1,
                                                  const uword in_col1,
                                                  const uword in_row2,
                                                  const uword in_col2) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::submat(const span& row_span, const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpMat<eT>::submat(const span& row_span,
                                             const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::operator()(const span& row_span, const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpMat<eT>::operator()(const span& row_span,
                                                 const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::operator()(const uword in_row1, const uword in_col1,
                                                const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const SpSubview<eT> SpMat<eT>::operator()(const uword in_row1,
                                                      const uword in_col1,
                                                      const SizeMat& s) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::head_rows(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpMat<eT>::head_rows(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::tail_rows(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpMat<eT>::tail_rows(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::head_cols(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpMat<eT>::head_cols(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::tail_cols(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const SpSubview<eT> SpMat<eT>::tail_cols(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline SpSubview_col_list<eT, T1> SpMat<eT>::cols(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline const SpSubview_col_list<eT, T1> SpMat<eT>::cols(
...
template <typename eT>
inline spdiagview<eT> SpMat<eT>::diag(const sword in_id) {
  arma_debug_sigprint();
...
//! creation of spdiagview (diagonal)
template <typename eT>
inline const spdiagview<eT> SpMat<eT>::diag(const sword in_id) const {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::swap_rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();

  arma_conform_check_bounds(((in_row1 >= n_rows) || (in_row2 >= n_rows)),
  ...
  // The easier way to do this, instead of collecting all the elements in one row and then
  // swapping with the other, will be to iterate over each column of the matrix (since we
  // store in column-major format) and then swap the two elements in the two rows at that
  // time. We will try to avoid using the at() call since it is expensive, instead
  // preferring to use an iterator to track our position.
  ...
  for (uword lcol = 0; lcol < n_cols; lcol++) {
    ...
    // There are four cases: we found both elements; we found one element (loc1); we found
    // one element (loc2); we found zero elements. If we found zero elements no work needs
    // to be done and we can continue to the next column.
    ...
    {
      // We need to find the correct place to move our value to.  It will be forward (not
      // backwards) because in_row2 > in_row1. Each iteration of the loop swaps the
      // current value (loc1) with (loc1 + 1); in this manner we move our value down to
      // where it should be.
      ...
      // We need to find the correct place to move our value to.  It will be backwards
      // (not forwards) because in_row1 < in_row2. Each iteration of the loop swaps the
      // current value (loc2) with (loc2 - 1); in this manner we move our value up to
      // where it should be.
...
template <typename eT>
inline void SpMat<eT>::swap_cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();

  arma_conform_check_bounds(((in_col1 >= n_cols) || (in_col2 >= n_cols)),
...
template <typename eT>
inline void SpMat<eT>::shed_row(const uword row_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::shed_col(const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::shed_rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::shed_cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();
  ...
  if (diff > 0) {
    ...
    if (col_end != n_nonzero) {
      ...
      arrayops::copy(new_row_indices + col_beg, row_indices + col_end,
                     n_nonzero - col_end);
    }
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::operator[](const uword i) {
...
template <typename eT>
arma_inline eT SpMat<eT>::operator[](const uword i) const {
  return get_value(i);
}

template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::at(const uword i) {
  const uword in_col = i / n_rows;
  const uword in_row = i % n_rows;
...
template <typename eT>
arma_inline eT SpMat<eT>::at(const uword i) const {
  return get_value(i);
}
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::operator[](const uword in_row,
                                                       const uword in_col) {
  return SpMat_MapMat_val<eT>((*this), cache, in_row, in_col);
}

template <typename eT>
arma_inline eT SpMat<eT>::operator[](const uword in_row, const uword in_col) const {
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::at(const uword in_row, const uword in_col) {
  return SpMat_MapMat_val<eT>((*this), cache, in_row, in_col);
}

template <typename eT>
arma_inline eT SpMat<eT>::at(const uword in_row, const uword in_col) const {
  return get_value(in_row, in_col);
}
 ...
 */
template <typename eT>
arma_inline bool SpMat<eT>::is_empty() const {
  return (n_elem == 0);
}

//! returns true if the object can be interpreted as a column or row vector
template <typename eT>
arma_inline bool SpMat<eT>::is_vec() const {
  return ((n_rows == 1) || (n_cols == 1));
}

//! returns true if the object can be interpreted as a row vector
template <typename eT>
arma_inline bool SpMat<eT>::is_rowvec() const {
  return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT>
arma_inline bool SpMat<eT>::is_colvec() const {
  return (n_cols == 1);
}

//! returns true if the object has the same number of non-zero rows and columnns
template <typename eT>
arma_inline bool SpMat<eT>::is_square() const {
  return (n_rows == n_cols);
}

template <typename eT>
inline bool SpMat<eT>::is_symmetric() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::is_symmetric(
...
template <typename eT>
inline bool SpMat<eT>::is_hermitian() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::is_hermitian(
    const typename get_pod_type<elem_type>::result tol) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::internal_is_finite() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::internal_has_inf() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::internal_has_nan() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::internal_has_nonfinite() const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const uword i) const {
  return (i < n_elem);
}

//! returns true if the given start and end indices are currently in range
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const span& x) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const uword in_row, const uword in_col) const {
  return ((in_row < n_rows) && (in_col < n_cols));
}

template <typename eT>
arma_inline bool SpMat<eT>::in_range(const span& row_span, const uword in_col) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const uword in_row, const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const span& row_span, const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const uword in_row, const uword in_col,
                                     const SizeMat& s) const {
  const uword l_n_rows = n_rows;
  const uword l_n_cols = n_cols;
...
template <typename eT>
template <typename eT2>
inline SpMat<eT>& SpMat<eT>::copy_size(const SpMat<eT2>& m) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename eT2>
inline SpMat<eT>& SpMat<eT>::copy_size(const Mat<eT2>& m) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::set_size(const uword new_n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::set_size(const uword in_rows, const uword in_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::set_size(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::resize(const uword in_rows, const uword in_cols) {
  arma_debug_sigprint();
  ...
  if ((n_elem == 0) || (n_nonzero == 0)) {
    return set_size(in_rows, in_cols);
  }
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::resize(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::reshape(const uword in_rows, const uword in_cols) {
  arma_debug_sigprint();

  arma_check(((in_rows * in_cols) != n_elem),
             "SpMat::reshape(): changing the number of elements in a sparse matrix is "
  ...
  if (vec_state == 1) {
    arma_conform_check(
        (in_cols != 1),
        "SpMat::reshape(): object is a column vector; requested size is not compatible");
  }
  if (vec_state == 2) {
    arma_conform_check(
        (in_rows != 1),
        "SpMat::reshape(): object is a row vector; requested size is not compatible");
  }

  if (n_nonzero == 0) {
    return (*this).zeros(in_rows, in_cols);
  }
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::reshape(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::reshape_helper_generic(const uword in_rows, const uword in_cols) {
  arma_debug_sigprint();
  ...
  // Iterate over all the points to do this.  We won't be deleting any points, but we will
  // be modifying columns and rows. We'll have to store a new set of column vectors.
...
template <typename eT>
inline void SpMat<eT>::reshape_helper_intovec() {
  arma_debug_sigprint();
...
inline SpMat<eT>& SpMat<eT>::for_each(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline const SpMat<eT>& SpMat<eT>::for_each(functor F) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline SpMat<eT>& SpMat<eT>::transform(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::replace(const eT old_val, const eT new_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::clean(const typename get_pod_type<eT>::result threshold) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::clamp(const eT min_val, const eT max_val) {
  arma_debug_sigprint();

  if (is_cx<eT>::no) {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "SpMat::clamp(): min_val must be less than max_val");
  } else {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "SpMat::clamp(): real(min_val) must be less than real(max_val)");
    arma_conform_check((access::tmp_imag(min_val) > access::tmp_imag(max_val)),
                       "SpMat::clamp(): imag(min_val) must be less than imag(max_val)");
  }
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::zeros() {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::zeros(const uword new_n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::zeros(const uword in_rows, const uword in_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::zeros(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::eye(const uword in_rows, const uword in_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::eye(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::speye() {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::speye(const uword in_n_rows, const uword in_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::speye(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::sprandu(const uword in_rows, const uword in_cols,
                                     const double density) {
  arma_debug_sigprint();

  arma_conform_check(((density < double(0)) || (density > double(1))),
                     "sprandu(): density must be in the [0,1] interval");
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::sprandu(const SizeMat& s, const double density) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::sprandn(const uword in_rows, const uword in_cols,
                                     const double density) {
  arma_debug_sigprint();

  arma_conform_check(((density < double(0)) || (density > double(1))),
                     "sprandn(): density must be in the [0,1] interval");
...
template <typename eT>
inline SpMat<eT>& SpMat<eT>::sprandn(const SizeMat& s, const double density) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::reset_cache() {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::reserve(const uword in_rows, const uword in_cols,
                               const uword new_n_nonzero) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void SpMat<eT>::set_real(const SpBase<typename SpMat<eT>::pod_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void SpMat<eT>::set_imag(const SpBase<typename SpMat<eT>::pod_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::save(const std::string name, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (save_okay == false) {
    arma_warn(3, "SpMat::save(): write failed; file: ", name);
  }
...
template <typename eT>
inline bool SpMat<eT>::save(const csv_name& spec, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (with_header) {
    ...
    for (uword i = 0; i < spec.header_ro.n_elem; ++i) {
      ...
      if (token.find(separator) != std::string::npos) {
        arma_warn(
            1,
            "SpMat::save(): token within the header contains the separator character: '",
            token, "'");
        return false;
      }
    }
  ...
  if (save_okay == false) {
    arma_warn(3, "SpMat::save(): write failed; file: ", spec.filename);
  }
...
template <typename eT>
inline bool SpMat<eT>::save(std::ostream& os, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (save_okay == false) {
    arma_warn(3, "SpMat::save(): stream write failed");
  }
...
template <typename eT>
inline bool SpMat<eT>::load(const std::string name, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::load(const csv_name& spec, const file_type type) {
  arma_debug_sigprint();
  ...
  if (strict) {
    arma_warn(1, "SpMat::load(): option 'strict' not implemented for sparse matrices");
  }
...
template <typename eT>
inline bool SpMat<eT>::load(std::istream& is, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::quiet_save(const std::string name, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::quiet_save(std::ostream& os, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::quiet_load(const std::string name, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::quiet_load(std::istream& is, const file_type type) {
  arma_debug_sigprint();
...
/**
 * Initialize the matrix to the specified size.  Data is not preserved, so the matrix is
 * assumed to be entirely sparse (empty).
 */
template <typename eT>
inline void SpMat<eT>::init(uword in_rows, uword in_cols, const uword new_n_nonzero) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::init_cold(uword in_rows, uword in_cols,
                                 const uword new_n_nonzero) {
  arma_debug_sigprint();
  ...
  if (vec_state > 0) {
    if ((in_rows == 0) && (in_cols == 0)) {
      ...
      if (vec_state == 1) {
        arma_conform_check(
            (in_cols != 1),
            "SpMat::init(): object is a column vector; requested size is not compatible");
      }
      if (vec_state == 2) {
        arma_conform_check(
            (in_rows != 1),
            "SpMat::init(): object is a row vector; requested size is not compatible");
      }
    }
  }
  ...
  arma_conform_check((((in_rows > ARMA_MAX_UHWORD) || (in_cols > ARMA_MAX_UHWORD))
...
template <typename eT>
inline void SpMat<eT>::init(const std::string& text) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::init(const SpMat<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::init(const MapMat<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::init_simple(const SpMat<eT>& x) {
  arma_debug_sigprint();
  ...
  if ((x.n_nonzero == 0) && (n_nonzero == 0) && (n_rows == x.n_rows) &&
      (n_cols == x.n_cols) && (values != nullptr)) {
    invalidate_cache();
  } else {
    init(x.n_rows, x.n_cols, x.n_nonzero);
  }
...
template <typename eT>
inline void SpMat<eT>::init_batch_std(const Mat<uword>& locs, const Mat<eT>& vals,
  ...
  if (sort_locations) {
    ...
    if (actually_sorted == false) {
      // see op_sort_index_bones.hpp for the definition of arma_sort_index_packet and
      ...
      for (uword i = 0; i < locs_n_cols; ++i) {
        ...
        arma_conform_check(((row_i >= n_rows) || (col_i >= n_cols)),
        ...
        if (i > 0) {
          ...
          arma_conform_check(((row_i == row_im1) && (col_i == col_im1)),
                             "SpMat::SpMat(): detected identical locations");
        }
    ...
    for (uword i = 0; i < locs_n_cols; ++i) {
      ...
      if (i > 0) {
        ...
        arma_conform_check(
            ...
            "SpMat::SpMat(): out of order points; either pass sort_locations = true, or "
            "sort points in column-major ordering");

        arma_conform_check(((col_i == col_im1) && (row_i == row_im1)),
                           "SpMat::SpMat(): detected identical locations");
      }
...
template <typename eT>
inline void SpMat<eT>::init_batch_add(const Mat<uword>& locs, const Mat<eT>& vals,
                                      const bool sort_locations) {
  arma_debug_sigprint();
  ...
  if (sort_locations) {
    ...
    if (actually_sorted == false) {
      ...
      // first element
      {
        ...
        arma_conform_check(((locs_i[0] >= n_rows) || (locs_i[1] >= n_cols)),
                           "SpMat::SpMat(): invalid row or column index");
...
template <typename eT>
inline SpMat<eT>::SpMat(const arma_vec_indicator&, const uword in_vec_state)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
//! constructor used by SpRow and SpCol classes
template <typename eT>
inline SpMat<eT>::SpMat(const arma_vec_indicator&, const uword in_n_rows,
                        const uword in_n_cols, const uword in_vec_state)
    : n_rows(0),
      n_cols(0),
      n_elem(0),
      n_nonzero(0),
...
template <typename eT>
inline void SpMat<eT>::mem_resize(const uword new_n_nonzero) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::sync() const {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::remove_zeros() {
  arma_debug_sigprint();

  sync_csc();

  invalidate_cache();  // placed here, as remove_zeros() is used during matrix
                       // modification
...
template <typename eT>
inline void SpMat<eT>::steal_mem(SpMat<eT>& x) {
  arma_debug_sigprint();
  ...
  if ((*this).vec_state == x.vec_state) {
    ...
    if (((*this).vec_state == 1) && (x.n_cols == 1)) {
      layout_ok = true;
    }
    if (((*this).vec_state == 2) && (x.n_rows == 1)) {
      layout_ok = true;
    }
  }
...
template <typename eT>
inline void SpMat<eT>::steal_mem_simple(SpMat<eT>& x) {
  arma_debug_sigprint();
...
inline void SpMat<eT>::init_xform(const SpBase<eT, T1>& A, const Functor& func) {
  arma_debug_sigprint();
...
inline void SpMat<eT>::init_xform_mt(const SpBase<eT2, T1>& A, const Functor& func) {
  arma_debug_sigprint();

  const SpProxy<T1> P(A.get_ref());

  if (P.is_alias(*this) || (is_SpMat<typename SpProxy<T1>::stored_type>::value)) {
    // NOTE: unwrap_spmat will convert a submatrix to a matrix, which in effect takes care
    // of aliasing with submatrices; NOTE: however, when more delayed ops are implemented,
    // more elaborate handling of aliasing will be necessary
    ...
    for (uword i = 0; i < nnz; ++i) {
      ...
      t_values_i = func(x_values[i]);  // NOTE: func() must produce a value of type eT
                                       // (ie. act as a convertor between eT2 and eT)
    ...
    while (it != it_end) {
      const eT val = func(*it);  // NOTE: func() must produce a value of type eT (ie. act
                                 // as a convertor between eT2 and eT)
...
template <typename eT>
arma_inline bool SpMat<eT>::is_alias(const SpMat<eT>& X) const {
  return (&X == this);
}

template <typename eT>
inline typename SpMat<eT>::iterator SpMat<eT>::begin() {
  arma_debug_sigprint();
...
template <typename eT>
inline typename SpMat<eT>::const_iterator SpMat<eT>::begin() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename SpMat<eT>::const_iterator SpMat<eT>::cbegin() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename SpMat<eT>::iterator SpMat<eT>::end() {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_iterator SpMat<eT>::end() const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_iterator SpMat<eT>::cend() const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::col_iterator SpMat<eT>::begin_col(const uword col_num) {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_col_iterator SpMat<eT>::begin_col(
    const uword col_num) const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::col_iterator SpMat<eT>::begin_col_no_sync(
    const uword col_num) {
  return col_iterator(*this, 0, col_num);
}

template <typename eT>
inline typename SpMat<eT>::const_col_iterator SpMat<eT>::begin_col_no_sync(
    const uword col_num) const {
  return const_col_iterator(*this, 0, col_num);
}

template <typename eT>
inline typename SpMat<eT>::col_iterator SpMat<eT>::end_col(const uword col_num) {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_col_iterator SpMat<eT>::end_col(
    const uword col_num) const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::col_iterator SpMat<eT>::end_col_no_sync(const uword col_num) {
  return col_iterator(*this, 0, col_num + 1);
}

template <typename eT>
inline typename SpMat<eT>::const_col_iterator SpMat<eT>::end_col_no_sync(
    const uword col_num) const {
  return const_col_iterator(*this, 0, col_num + 1);
}

template <typename eT>
inline typename SpMat<eT>::row_iterator SpMat<eT>::begin_row(const uword row_num) {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_row_iterator SpMat<eT>::begin_row(
...
template <typename eT>
inline typename SpMat<eT>::row_iterator SpMat<eT>::end_row() {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_row_iterator SpMat<eT>::end_row() const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::row_iterator SpMat<eT>::end_row(const uword row_num) {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_row_iterator SpMat<eT>::end_row(
    const uword row_num) const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::row_col_iterator SpMat<eT>::begin_row_col() {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_row_col_iterator SpMat<eT>::begin_row_col() const {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::row_col_iterator SpMat<eT>::end_row_col() {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::const_row_col_iterator SpMat<eT>::end_row_col() const {
  sync_csc();
...
template <typename eT>
inline void SpMat<eT>::clear() {
  (*this).reset();
}

template <typename eT>
inline bool SpMat<eT>::empty() const {
  return (n_elem == 0);
}

template <typename eT>
inline uword SpMat<eT>::size() const {
  return n_elem;
}

template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::front() {
...
template <typename eT>
arma_inline eT SpMat<eT>::front() const {
  arma_conform_check((n_elem == 0), "SpMat::front(): matrix is empty");
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::back() {
...
template <typename eT>
arma_inline eT SpMat<eT>::back() const {
  arma_conform_check((n_elem == 0), "SpMat::back(): matrix is empty");
...
template <typename eT>
inline eT SpMat<eT>::get_value(const uword i) const {
...
template <typename eT>
inline eT SpMat<eT>::get_value(const uword in_row, const uword in_col) const {
  const MapMat<eT>& const_cache = cache;  // declare as const for clarity of intent

  // get the element from the cache if it has more recent data than CSC

  return (sync_state == 1) ? const_cache.at(in_row, in_col)
                           : get_value_csc(in_row, in_col);
}

template <typename eT>
inline eT SpMat<eT>::get_value_csc(const uword i) const {
...
template <typename eT>
inline const eT* SpMat<eT>::find_value_csc(const uword in_row, const uword in_col) const {
...
template <typename eT>
inline eT SpMat<eT>::get_value_csc(const uword in_row, const uword in_col) const {
...
template <typename eT>
inline bool SpMat<eT>::try_set_value_csc(const uword in_row, const uword in_col,
                                         const eT in_val) {
  const eT* val_ptr = find_value_csc(in_row, in_col);
...
template <typename eT>
inline bool SpMat<eT>::try_add_value_csc(const uword in_row, const uword in_col,
                                         const eT in_val) {
  const eT* val_ptr = find_value_csc(in_row, in_col);
...
template <typename eT>
inline bool SpMat<eT>::try_sub_value_csc(const uword in_row, const uword in_col,
                                         const eT in_val) {
  const eT* val_ptr = find_value_csc(in_row, in_col);
...
template <typename eT>
inline bool SpMat<eT>::try_mul_value_csc(const uword in_row, const uword in_col,
                                         const eT in_val) {
  const eT* val_ptr = find_value_csc(in_row, in_col);

  // element not found, ie. it's zero; succeed if given value is finite; zero multiplied
  // by anything is zero, except for nan and inf
  if (val_ptr == nullptr) {
    return arma_isfinite(in_val);
  }
...
template <typename eT>
inline bool SpMat<eT>::try_div_value_csc(const uword in_row, const uword in_col,
                                         const eT in_val) {
  const eT* val_ptr = find_value_csc(in_row, in_col);

  // element not found, ie. it's zero; succeed if given value is not zero and not nan;
  // zero divided by anything is zero, except for zero and nan
  if (val_ptr == nullptr) {
    return ((in_val != eT(0)) && (arma_isnan(in_val) == false));
  }
 ...
 */
template <typename eT>
inline eT& SpMat<eT>::insert_element(const uword in_row, const uword in_col,
                                     const eT val) {
  arma_debug_sigprint();
 ...
 */
template <typename eT>
inline void SpMat<eT>::delete_element(const uword in_row, const uword in_col) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline void SpMat<eT>::invalidate_cache() const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline void SpMat<eT>::invalidate_csc() const {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::sync_cache() const {
  arma_debug_sigprint();
  ...
  // reading and writing to sync_state uses std::memory_order_seq_cst which has an implied
  // fence; data races are prevented via the mutex
...
template <typename eT>
inline void SpMat<eT>::sync_cache_simple() const {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::sync_csc() const {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::sync_csc_simple() const {
  arma_debug_sigprint();
...
inline void SpMat_aux::set_real(SpMat<eT>& out, const SpBase<eT, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT, typename T1>
inline void SpMat_aux::set_imag(SpMat<eT>&, const SpBase<eT, T1>&) {
  arma_debug_sigprint();
}
...
inline void SpMat_aux::set_real(SpMat<std::complex<T> >& out, const SpBase<T, T1>& X) {
  ...
  SpMat<eT> tmp(Y,
                arma::imag(out));  // arma:: prefix required due to bugs in GCC 4.4 - 4.6
...
inline void SpMat_aux::set_imag(SpMat<std::complex<T> >& out, const SpBase<T, T1>& X) {
  ...
  SpMat<eT> tmp(arma::real(out),
                Y);  // arma:: prefix required due to bugs in GCC 4.4 - 4.6
