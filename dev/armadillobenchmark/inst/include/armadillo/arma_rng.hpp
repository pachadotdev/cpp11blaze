...
// NOTE: i don't have the patience to keep looking into these broken platforms
     ...
     defined(__MSYS__) || defined(__MSYS2__))
#undef ARMA_USE_THREAD_LOCAL
#endif
...
inline std::mt19937_64& arma_rng::get_producer() {
#if defined(ARMA_USE_THREAD_LOCAL)
...
inline void arma_rng::warmup_producer(std::mt19937_64& producer) {
...
inline void arma_rng::lock_producer() {
#if defined(ARMA_GUARD_PRODUCER)
...
inline void arma_rng::unlock_producer() {
#if defined(ARMA_GUARD_PRODUCER)
...
#if defined(ARMA_GUARD_PRODUCER)
inline std::mutex& arma_rng::get_producer_mutex() {
...
arma_cold inline void arma_rng::set_seed_random() {
  ...
  if (have_seed == false) {
    ...
    auto since_epoch_usec =
        std::chrono::duration_cast<std::chrono::microseconds>(tp_now.time_since_epoch())
            .count();
...
struct arma_rng::randi {
  inline operator eT() {
...
#elif defined(ARMA_USE_CXX11_RNG)
    {
      constexpr double scale =
          double(std::numeric_limits<int>::max()) / double(std::mt19937_64::max());
  ...
  inline static void fill(eT* mem, const uword N, const int a, const int b) {
...
#elif defined(ARMA_USE_CXX11_RNG)
    {
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_i_distr(producer));
      }
    ...
    {
      if (N == uword(1)) {
        arma_rng_cxx03::randi_fill(mem, uword(1), a, b);
        return;
      }
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_i_distr(local_engine));
      }
    }
#endif
  }
...
struct arma_rng::randu {
  ...
  inline static void fill(eT* mem, const uword N) {
...
#elif defined(ARMA_USE_CXX11_RNG)
    {
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_u_distr(producer));
      }
    ...
    {
      if (N == uword(1)) {
        mem[0] = eT(arma_rng_cxx03::randu_val());
        return;
      }
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_u_distr(local_engine));
      }
    }
#endif
  }
  ...
  inline static void fill(eT* mem, const uword N, const double a, const double b) {
    ...
    {
      if (N == uword(1)) {
        mem[0] = eT(arma_rng_cxx03::randu_val() * (b - a) + a);
        return;
      }
...
struct arma_rng::randu<std::complex<T> > {
  ...
  inline static void fill(std::complex<T>* mem, const uword N, const double a,
                          const double b) {
#if defined(ARMA_RNG_ALT)
    {
      const double r = b - a;
  ...
  inline static void fill(eT* mem, const uword N) {
#if defined(ARMA_RNG_ALT)
    {
      ...
      for (i = 0, j = 1; j < N; i += 2, j += 2) {
        arma_rng_alt::randn_dual_val(mem[i], mem[j]);
      }
...
#elif defined(ARMA_USE_CXX11_RNG)
    {
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_n_distr(producer));
      }
    ...
    {
      if (N == uword(1)) {
        mem[0] = eT(arma_rng_cxx03::randn_val());
        return;
      }
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_n_distr(local_engine));
      }
    }
#endif
  }

  inline static void fill(eT* mem, const uword N, const double mu, const double sd) {
#if defined(ARMA_RNG_ALT)
    {
      // NOTE: old method to avoid regressions in user code that assumes specific sequence
...
struct arma_rng::randg {
  inline static void fill(eT* mem, const uword N, const double a, const double b) {
#if defined(ARMA_USE_CXX11_RNG)
    {
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_g_distr(producer));
      }
...
#else
    {
      ...
      for (uword i = 0; i < N; ++i) {
        mem[i] = eT(local_g_distr(local_engine));
      }
    }
#endif
  }
