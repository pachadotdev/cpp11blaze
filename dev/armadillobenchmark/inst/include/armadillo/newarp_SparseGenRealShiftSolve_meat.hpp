...
inline SparseGenRealShiftSolve<eT>::SparseGenRealShiftSolve(const SpMat<eT>& mat_obj,
                                                            const eT shift)
...
#endif
{
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    // Copy A-shift*I to x
    const bool status_x =
        sp_auxlib::copy_to_supermatrix_with_shift(x.get_ref(), mat_obj, shift);

    if (status_x == false) {
      arma_stop_runtime_error(
          "newarp::SparseGenRealShiftSolve::SparseGenRealShiftSolve(): could not "
          "construct SuperLU matrix");
      return;
    }
    ...
    superlu::sp_preorder_mat(&options, x.get_ptr(), perm_c.get_ptr(), etree.get_ptr(),
                             xC.get_ptr());
    superlu::gstrf<eT>(&options, xC.get_ptr(), relax, panel_size, etree.get_ptr(), NULL,
                       lwork, perm_c.get_ptr(), perm_r.get_ptr(), l.get_ptr(),
...
inline void SparseGenRealShiftSolve<eT>::perform_op(eT* x_in, eT* y_out) const {
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    if (status_out_slu == false) {
      arma_stop_runtime_error(
          "newarp::SparseGenRealShiftSolve::perform_op(): could not construct SuperLU "
          "matrix");
      return;
    }
    ...
    if (info != 0) {
      arma_stop_runtime_error(
          "newarp::SparseGenRealShiftSolve::perform_op(): could not solve linear "
          "equation");
      return;
    }
