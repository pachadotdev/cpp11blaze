...
// NOTE: mtSpReduceOp is derived from SpBase so the default user-accessible output is in
// sparse storage format. NOTE. This is to mimic the behaviour of Octave and to keep
// compatibility with existing user code. NOTE: NOTE: However, for simplicity and
// efficiency, each mtSpReduceOp op_type::apply() function NOTE: must output to a dense
// matrix, ie. apply(Mat, ...). NOTE: The SpMat class handles mtSpReduceOp by converting
// the dense output to a sparse representation. NOTE: The Mat class has an explicit
// constructor to efficiently handle mtSpReduceOp.
...
class mtSpReduceOp : public SpBase<out_eT, mtSpReduceOp<out_eT, T1, op_type> > {
 public:
  ...
  inline mtSpReduceOp(const T1& in_m, const uword in_aux_uword_a,
