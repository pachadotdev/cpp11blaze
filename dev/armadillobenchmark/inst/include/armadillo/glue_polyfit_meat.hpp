...
inline bool glue_polyfit::apply_noalias(Mat<eT>& out, const Col<eT>& X, const Col<eT>& Y,
                                        const uword N) {
  arma_debug_sigprint();
...
inline bool glue_polyfit::apply_direct(Mat<typename T1::elem_type>& out,
                                       const Base<typename T1::elem_type, T1>& X_expr,
                                       const Base<typename T1::elem_type, T2>& Y_expr,
                                       const uword N) {
  arma_debug_sigprint();
  ...
  arma_conform_check((((X.is_vec() == false) && (X.is_empty() == false)) ||
                      ((Y.is_vec() == false) && (Y.is_empty() == false))),
                     "polyfit(): given object must be a vector");

  arma_conform_check((X.n_elem != Y.n_elem),
                     "polyfit(): given vectors must have the same number of elements");
  ...
  arma_conform_check((N >= X.n_elem),
                     "polyfit(): N must be less than the number of elements in X");
...
template <typename T1, typename T2>
inline void glue_polyfit::apply(Mat<typename T1::elem_type>& out,
                                const Glue<T1, T2, glue_polyfit>& expr) {
  arma_debug_sigprint();
