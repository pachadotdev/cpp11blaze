...
inline void glue_histc::apply_noalias(Mat<uword>& C, const Mat<eT>& A, const Mat<eT>& B,
                                      const uword dim) {
  arma_debug_sigprint();

  arma_conform_check(((B.is_vec() == false) && (B.is_empty() == false)),
  ...
  arma_conform_check(
      ((Col<eT>(const_cast<eT*>(B.memptr()), B_n_elem, false, false))
...
template <typename T1, typename T2>
inline void glue_histc_default::apply(
    Mat<uword>& C, const mtGlue<uword, T1, T2, glue_histc_default>& expr) {
  arma_debug_sigprint();
