...
inline void glue_mvnrnd_vec::apply(Mat<typename T1::elem_type>& out,
                                   const Glue<T1, T2, glue_mvnrnd_vec>& expr) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline void glue_mvnrnd::apply(Mat<typename T1::elem_type>& out,
                               const Glue<T1, T2, glue_mvnrnd>& expr) {
  arma_debug_sigprint();
...
template <typename T1, typename T2>
inline bool glue_mvnrnd::apply_direct(Mat<typename T1::elem_type>& out,
                                      const Base<typename T1::elem_type, T1>& M,
  ...
  arma_conform_check((UM.M.is_colvec() == false) && (UM.M.is_empty() == false),
                     "mvnrnd(): given mean must be a column vector");
  arma_conform_check((UC.M.is_square() == false),
                     "mvnrnd(): given covariance matrix must be square sized");
  arma_conform_check(
      (UM.M.n_rows != UC.M.n_rows),
      "mvnrnd(): number of rows in given mean vector and covariance matrix must match");
...
inline bool glue_mvnrnd::apply_noalias(Mat<eT>& out, const Mat<eT>& M, const Mat<eT>& C,
                                       const uword N) {
  arma_debug_sigprint();

  Mat<eT> D;

  const bool chol_status =
      op_chol::apply_direct(D, C, 1);  // '1' means "lower triangular"

  if (chol_status == false) {
    // C is not symmetric positive definite, so find approximate square root of C

    Col<eT> eigval;  // NOTE: eT is constrained to be real (ie. float or double) in
                     // fn_mvnrnd.hpp
    Mat<eT> eigvec;
    ...
    for (uword i = 0; i < eigval_n_elem; ++i) {
      ...
      if ((val < tol) || (arma_isfinite(val) == false)) {
        return false;
      }
    }

    for (uword i = 0; i < eigval_n_elem; ++i) {
      if (eigval_mem[i] < eT(0)) {
        eigval_mem[i] = eT(0);
      }
    }
