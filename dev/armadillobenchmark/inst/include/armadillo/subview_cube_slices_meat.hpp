...
inline void subview_cube_slices<eT, T1>::inplace_rand(const uword rand_mode) {
  ...
  for (uword si_count = 0; si_count < si_n_elem; ++si_count) {
    ...
    if (rand_mode == 0) {
      arma_rng::randu<eT>::fill(m_slice_ptr, m_n_elem_slice);
    }
    if (rand_mode == 1) {
      arma_rng::randn<eT>::fill(m_slice_ptr, m_n_elem_slice);
    }
  }
}
...
inline void subview_cube_slices<eT, T1>::inplace_op(const eT val) {
  ...
  for (uword si_count = 0; si_count < si_n_elem; ++si_count) {
    ...
    if (is_same_type<op_type, op_internal_equ>::yes) {
      arrayops::inplace_set(m_slice_ptr, val, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_plus>::yes) {
      arrayops::inplace_plus(m_slice_ptr, val, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_minus>::yes) {
      arrayops::inplace_minus(m_slice_ptr, val, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_schur>::yes) {
      arrayops::inplace_mul(m_slice_ptr, val, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_div>::yes) {
      arrayops::inplace_div(m_slice_ptr, val, m_n_elem_slice);
    }
  }
}
...
inline void subview_cube_slices<eT, T1>::inplace_op(const BaseCube<eT, expr>& x) {
  ...
  for (uword si_count = 0; si_count < si_n_elem; ++si_count) {
    ...
    if (is_same_type<op_type, op_internal_equ>::yes) {
      arrayops::copy(m_slice_ptr, X_slice_ptr, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_plus>::yes) {
      arrayops::inplace_plus(m_slice_ptr, X_slice_ptr, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_minus>::yes) {
      arrayops::inplace_minus(m_slice_ptr, X_slice_ptr, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_schur>::yes) {
      arrayops::inplace_mul(m_slice_ptr, X_slice_ptr, m_n_elem_slice);
    }
    if (is_same_type<op_type, op_internal_div>::yes) {
      arrayops::inplace_div(m_slice_ptr, X_slice_ptr, m_n_elem_slice);
    }
  }
}
