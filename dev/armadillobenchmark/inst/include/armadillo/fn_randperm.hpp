...
inline void internal_randperm_helper(obj_type& x, const uword N, const uword N_keep) {
  arma_debug_sigprint();
...
template <typename obj_type>
arma_warn_unused inline typename enable_if2<is_Mat<obj_type>::value, obj_type>::result
randperm(const uword N) {
  ...
  if (N > 0) {
    internal_randperm_helper(x, N, N);
  }

  return x;
}

arma_warn_unused inline uvec randperm(const uword N) {
  arma_debug_sigprint();
...
randperm(const uword N, const uword M) {
  ...
  if ((N > 0) && (M > 0)) {
    internal_randperm_helper(x, N, M);
  }

  return x;
}

arma_warn_unused inline uvec randperm(const uword N, const uword M) {
  arma_debug_sigprint();
