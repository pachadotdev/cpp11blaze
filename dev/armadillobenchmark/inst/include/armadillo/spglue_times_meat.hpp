...
template <typename T1, typename T2>
inline void spglue_times::apply(
    SpMat<typename T1::elem_type>& out,
    const SpGlue<SpOp<T1, spop_scalar_times>, T2, spglue_times>& X) {
  arma_debug_sigprint();
...
inline void spglue_times::apply_noalias(SpMat<eT>& c, const SpMat<eT>& x,
                                        const SpMat<eT>& y) {
  arma_debug_sigprint();
  ...
  arma_conform_assert_mul_size(x_n_rows, x_n_cols, y_n_rows, y_n_cols,
...
template <typename T1, typename T2>
inline void spglue_times_mixed::apply(
    SpMat<typename eT_promoter<T1, T2>::eT>& out,
    const mtSpGlue<typename eT_promoter<T1, T2>::eT, T1, T2, spglue_times_mixed>& expr) {
  arma_debug_sigprint();
