...
template <typename eT>
inline Mat<eT>::Mat()
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0), mem() {
  arma_debug_sigprint_this(this);
}
...
template <typename eT>
template <bool do_zeros>
inline Mat<eT>::Mat(const SizeMat& s, const arma_initmode_indicator<do_zeros>&)
    : n_rows(s.n_rows),
      n_cols(s.n_cols),
      n_elem(s.n_rows * s.n_cols),
      n_alloc(),
...
template <typename eT>
inline Mat<eT>::Mat(const SizeMat& s, const fill::scalar_holder<eT> f)
    : n_rows(s.n_rows),
      n_cols(s.n_cols),
      n_elem(s.n_rows * s.n_cols),
      n_alloc(),
...
//! constructor used by Row and Col classes
template <typename eT>
inline Mat<eT>::Mat(const arma_vec_indicator&, const uword in_n_rows,
                    const uword in_n_cols, const uhword in_vec_state)
    : n_rows(in_n_rows),
      n_cols(in_n_cols),
      n_elem(in_n_rows * in_n_cols),
      n_alloc(),
...
template <typename eT>
inline Mat<eT>::Mat(const arma_fixed_indicator&, const uword in_n_rows,
                    const uword in_n_cols, const uhword in_vec_state, const eT* in_mem)
    : n_rows(in_n_rows),
      n_cols(in_n_cols),
      n_elem(in_n_rows * in_n_cols),
      n_alloc(0),
...
inline void Mat<eT>::init_cold() {
  ...
  arma_conform_check((((n_rows > ARMA_MAX_UHWORD) || (n_cols > ARMA_MAX_UHWORD))
  ...
  if (n_elem <= arma_config::mat_prealloc) {
    if (n_elem > 0) {
      arma_debug_print("Mat::init(): using local memory");
    }
...
template <typename eT>
inline void Mat<eT>::init_warm(uword in_n_rows, uword in_n_cols) {
  arma_debug_sigprint(arma_str::format("in_n_rows: %u; in_n_cols: %u") % in_n_rows %
                      in_n_cols);
  ...
  if (t_vec_state > 0) {
    if ((in_n_rows == 0) && (in_n_cols == 0)) {
      ...
      if (t_vec_state == 1) {
        arma_conform_set_error(err_state, err_msg, (in_n_cols != 1), error_message_2);
      }  // TODO: (in_n_cols > 1) ?
      if (t_vec_state == 2) {
        arma_conform_set_error(err_state, err_msg, (in_n_rows != 1), error_message_3);
      }  // TODO: (in_n_rows > 1) ?
    }
  }
...
//! create the matrix from a textual description
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const char* text) {
  arma_debug_sigprint();
...
//! create the matrix from a textual description
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const std::string& text) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Mat<eT>::init(const std::string& text_orig) {
  arma_debug_sigprint();

  const bool replace_commas =
      (is_cx<eT>::yes) ? false : (text_orig.find(',') != std::string::npos);
  ...
  while (line_start < text.length()) {
    ...
    if (t_n_rows == 0) {
      ...
      if (has_semicolon || has_token) {
        arma_check((line_n_cols != t_n_cols),
  ...
  if ((has_semicolon == false) && (has_token == false) && (t_n_rows >= 1)) {
    --t_n_rows;
  }
...
//! create the matrix from std::vector
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const std::vector<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const std::initializer_list<eT>& list) {
  arma_debug_sigprint();
...
inline Mat<eT>::Mat(Mat<eT>&& X)
  ...
  if ((X.n_alloc > arma_config::mat_prealloc) || (X.mem_state == 1) ||
      (X.mem_state == 2)) {
  ...
  } else  // condition: (X.n_alloc <= arma_config::mat_prealloc) || (X.mem_state == 0) ||
          // (X.mem_state == 3)
  {
    init_cold();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(Mat<eT>&& X) {
  arma_debug_sigprint(arma_str::format("this: %x; X: %x") % this % &X);
...
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const eT val) {
  arma_debug_sigprint();
...
//! construct a matrix from a given matrix
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const Mat<eT>& in_mat) {
  arma_debug_sigprint(arma_str::format("this: %x; in_mat: %x") % this % &in_mat);
...
template <typename eT>
inline void Mat<eT>::init(const std::initializer_list<eT>& list) {
  arma_debug_sigprint();
...
inline void Mat<eT>::init(const std::initializer_list<std::initializer_list<eT> >& list) {
  ...
  if (t.mem_state == 3) {
    arma_conform_check(
        ((x_n_rows != t.n_rows) || (x_n_cols != t.n_cols)),
...
inline void Mat<eT>::init(const Base<typename Mat<eT>::pod_type, T1>& X,
  ...
  arma_type_check((is_same_type<std::complex<T>, eT>::no));  //!< compile-time abort if
...
template <typename eT>
inline void Mat<eT>::swap(Mat<eT>& B) {
  Mat<eT>& A = (*this);
  ...
  if (A.vec_state == B.vec_state) {
    ...
    const bool A_absorbs_B = (A_vec_state == 0) ||
                             ((A_vec_state == 1) && (B.n_cols == 1)) ||
                             ((A_vec_state == 2) && (B.n_rows == 1));
    const bool B_absorbs_A = (B_vec_state == 0) ||
                             ((B_vec_state == 1) && (A.n_cols == 1)) ||
                             ((B_vec_state == 2) && (A.n_rows == 1));
  ...
  } else if ((A_mem_state <= 2) && (B_mem_state <= 2) && (A.n_elem == B.n_elem) &&
             layout_ok) {
    std::swap(access::rw(A.n_rows), access::rw(B.n_rows));
    std::swap(access::rw(A.n_cols), access::rw(B.n_cols));
...
template <typename eT>
inline void Mat<eT>::steal_mem(Mat<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Mat<eT>::steal_mem(Mat<eT>& x, const bool is_move) {
  arma_debug_sigprint();
  ...
  const bool layout_ok = (t_vec_state == x_vec_state) ||
                         ((t_vec_state == 1) && (x_n_cols == 1)) ||
                         ((t_vec_state == 2) && (x_n_rows == 1));

  if (layout_ok && (t_mem_state <= 1) &&
      ((x_n_alloc > arma_config::mat_prealloc) || (x_mem_state == 1) ||
       (is_move && (x_mem_state == 2)))) {
...
template <typename eT>
inline void Mat<eT>::steal_mem_col(Mat<eT>& x, const uword max_n_rows) {
  arma_debug_sigprint();
  ...
  if ((this != &x) && (t_vec_state <= 1) && (t_mem_state <= 1) && (x_mem_state <= 1)) {
    if ((x_mem_state == 0) && ((x_n_alloc <= arma_config::mat_prealloc) ||
...
//! memory and copying). the default is to copy the array.

template <typename eT>
inline Mat<eT>::Mat(eT* aux_mem, const uword aux_n_rows, const uword aux_n_cols,
                    const bool copy_aux_mem, const bool strict)
    : n_rows(aux_n_rows),
      n_cols(aux_n_cols),
...
template <typename eT>
inline Mat<eT>::Mat(const char junk, const eT* aux_mem, const uword aux_n_rows,
...
template <typename eT>
inline Mat<eT>& Mat<eT>::operator+=(const Mat<eT>& m) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline Mat<eT>& Mat<eT>::operator=(const BaseCube<eT, T1>& X) {
  arma_debug_sigprint();
...
inline Mat<eT>& Mat<eT>::operator+=(const BaseCube<eT, T1>& X) {
  ...
  if (in_n_slices == 1) {
    ...
    if (out_vec_state == 0) {
      ...
      } else if ((in_n_rows == 1) && (in_n_cols == out_n_rows) &&
                 (in_n_slices == out_n_cols)) {
        for (uword slice = 0; slice < in_n_slices; ++slice) {
          eT* out_colptr = out.colptr(slice);
...
template <typename eT>
template <typename T1, typename T2>
inline Mat<eT>::Mat(const Base<typename Mat<eT>::pod_type, T1>& A,
...
//! construct a matrix from subview (eg. construct a matrix from a delayed submatrix
//! operation)
template <typename eT>
...
//! operation)
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const subview<eT>& X) {
  arma_debug_sigprint();
...
//! construct a matrix from a subview_cube instance
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const subview_cube<eT>& X) {
  arma_debug_sigprint();
...
//! in-place element-wise matrix mutiplication (using a single-slice subcube on the
//! right-hand-side)
template <typename eT>
inline Mat<eT>& Mat<eT>::operator%=(const subview_cube<eT>& X) {
  arma_debug_sigprint();
...
//! in-place element-wise matrix division (using a single-slice subcube on the
//! right-hand-side)
template <typename eT>
inline Mat<eT>& Mat<eT>::operator/=(const subview_cube<eT>& X) {
  arma_debug_sigprint();
...
//! construct a matrix from diagview (eg. construct a matrix from a delayed diag
//! operation)
template <typename eT>
...
//! operation)
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const diagview<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline Mat<eT>& Mat<eT>::operator=(const subview_elem1<eT, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
inline Mat<eT>& Mat<eT>::operator=(const subview_elem2<eT, T1, T2>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline Mat<eT>& Mat<eT>::operator=(const SpBase<eT, T1>& m) {
  arma_debug_sigprint();
...
inline Mat<eT>& Mat<eT>::operator+=(const SpBase<eT, T1>& m) {
  ...
  arma_conform_assert_same_size(n_rows, n_cols, p.get_n_rows(), p.get_n_cols(),
                                "addition");
...
inline Mat<eT>& Mat<eT>::operator%=(const SpBase<eT, T1>& m) {
  ...
  while (it != it_end) {
    ...
    const uword next_loc =
        (it == it_end) ? (p.get_n_cols() * n_rows) : (it.col() * n_rows) + it.row();
...
template <typename eT>
inline Mat<eT>::Mat(const SpSubview<eT>& X)
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0), mem() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const SpSubview<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::operator=(const spdiagview<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline mat_injector<Mat<eT> > Mat<eT>::operator<<(const eT val) {
...
template <typename eT>
inline subview_row<eT> Mat<eT>::operator()(const uword row_num, const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_row<eT> Mat<eT>::operator()(const uword row_num,
...
template <typename eT>
inline subview_col<eT> Mat<eT>::operator()(const span& row_span, const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_col<eT> Mat<eT>::operator()(const span& row_span,
...
template <typename eT>
inline Col<eT> Mat<eT>::unsafe_col(const uword col_num) {
  arma_debug_sigprint();
...
//! it will cause memory corruption and/or a crash
template <typename eT>
inline const Col<eT> Mat<eT>::unsafe_col(const uword col_num) const {
  arma_debug_sigprint();
...
arma_inline subview<eT> Mat<eT>::rows(const uword in_row1, const uword in_row2) {
  ...
  arma_conform_check_bounds((in_row1 > in_row2) || (in_row2 >= n_rows),
...
template <typename eT>
arma_inline subview_cols<eT> Mat<eT>::cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();

  arma_conform_check_bounds((in_col1 > in_col2) || (in_col2 >= n_cols),
...
//! creation of subview (submatrix comprised of specified column vectors)
template <typename eT>
arma_inline const subview_cols<eT> Mat<eT>::cols(const uword in_col1,
...
//! creation of subview (submatrix comprised of specified row vectors)
template <typename eT>
inline subview<eT> Mat<eT>::rows(const span& row_span) {
  arma_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified row vectors)
template <typename eT>
inline const subview<eT> Mat<eT>::rows(const span& row_span) const {
  arma_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified column vectors)
template <typename eT>
arma_inline subview_cols<eT> Mat<eT>::cols(const span& col_span) {
  arma_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified column vectors)
template <typename eT>
arma_inline const subview_cols<eT> Mat<eT>::cols(const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1,
                                        const uword in_row2, const uword in_col2) {
  arma_debug_sigprint();

  arma_conform_check_bounds((in_row1 > in_row2) || (in_col1 > in_col2) ||
...
template <typename eT>
arma_inline const subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1,
                                              const uword in_row2,
                                              const uword in_col2) const {
  arma_debug_sigprint();
...
arma_inline subview<eT> Mat<eT>::submat(const uword in_row1, const uword in_col1,
  ...
  arma_conform_check_bounds(
      ((in_row1 >= l_n_rows) || (in_col1 >= l_n_cols) ||
       ((in_row1 + s_n_rows) > l_n_rows) || ((in_col1 + s_n_cols) > l_n_cols)),
...
//! creation of subview (submatrix)
template <typename eT>
inline subview<eT> Mat<eT>::submat(const span& row_span, const span& col_span) {
  arma_debug_sigprint();
...
//! creation of subview (generic submatrix)
template <typename eT>
inline const subview<eT> Mat<eT>::submat(const span& row_span,
                                         const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview<eT> Mat<eT>::operator()(const span& row_span, const span& col_span) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview<eT> Mat<eT>::operator()(const span& row_span,
                                             const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview<eT> Mat<eT>::operator()(const uword in_row1, const uword in_col1,
                                       const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview<eT> Mat<eT>::operator()(const uword in_row1, const uword in_col1,
                                             const SizeMat& s) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview<eT> Mat<eT>::head_rows(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview<eT> Mat<eT>::head_rows(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview<eT> Mat<eT>::tail_rows(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview<eT> Mat<eT>::tail_rows(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview_cols<eT> Mat<eT>::head_cols(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cols<eT> Mat<eT>::head_cols(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
inline subview_cols<eT> Mat<eT>::tail_cols(const uword N) {
  arma_debug_sigprint();
...
template <typename eT>
inline const subview_cols<eT> Mat<eT>::tail_cols(const uword N) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline subview_elem1<eT, T1> Mat<eT>::operator()(const Base<uword, T1>& a) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline const subview_elem1<eT, T1> Mat<eT>::operator()(
    const Base<uword, T1>& a) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
arma_inline subview_elem2<eT, T1, T2> Mat<eT>::elem(const Base<uword, T1>& ri,
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2> Mat<eT>::elem(
    const Base<uword, T1>& ri, const Base<uword, T2>& ci) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
arma_inline subview_elem2<eT, T1, T2> Mat<eT>::submat(const Base<uword, T1>& ri,
                                                      const Base<uword, T2>& ci) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2> Mat<eT>::submat(
    const Base<uword, T1>& ri, const Base<uword, T2>& ci) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
arma_inline subview_elem2<eT, T1, T2> Mat<eT>::operator()(const Base<uword, T1>& ri,
                                                          const Base<uword, T2>& ci) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2> Mat<eT>::operator()(
    const Base<uword, T1>& ri, const Base<uword, T2>& ci) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline subview_elem2<eT, T1, T1> Mat<eT>::rows(const Base<uword, T1>& ri) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline const subview_elem2<eT, T1, T1> Mat<eT>::rows(
    const Base<uword, T1>& ri) const {
  arma_debug_sigprint();
...
arma_inline subview_elem2<eT, T2, T2> Mat<eT>::cols(const Base<uword, T2>& ci) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T2>
arma_inline const subview_elem2<eT, T2, T2> Mat<eT>::cols(
...
template <typename eT>
arma_inline subview_each1<Mat<eT>, 0> Mat<eT>::each_col() {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline subview_each1<Mat<eT>, 1> Mat<eT>::each_row() {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const subview_each1<Mat<eT>, 0> Mat<eT>::each_col() const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline const subview_each1<Mat<eT>, 1> Mat<eT>::each_row() const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline subview_each2<Mat<eT>, 0, T1> Mat<eT>::each_col(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline subview_each2<Mat<eT>, 1, T1> Mat<eT>::each_row(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline const subview_each2<Mat<eT>, 0, T1> Mat<eT>::each_col(
...
template <typename eT>
template <typename T1>
inline const subview_each2<Mat<eT>, 1, T1> Mat<eT>::each_row(
    const Base<uword, T1>& indices) const {
  arma_debug_sigprint();
...
//! apply a lambda function to each column, where each column is interpreted as a column
//! vector
template <typename eT>
inline Mat<eT>& Mat<eT>::each_col(const std::function<void(Col<eT>&)>& F) {
  arma_debug_sigprint();
...
template <typename eT>
inline const Mat<eT>& Mat<eT>::each_col(
    const std::function<void(const Col<eT>&)>& F) const {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::each_row(const std::function<void(Row<eT>&)>& F) {
  arma_debug_sigprint();
...
template <typename eT>
inline const Mat<eT>& Mat<eT>::each_row(
    const std::function<void(const Row<eT>&)>& F) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline diagview<eT> Mat<eT>::diag(const sword in_id) {
  arma_debug_sigprint();
...
//! creation of diagview (diagonal)
template <typename eT>
arma_inline const diagview<eT> Mat<eT>::diag(const sword in_id) const {
  arma_debug_sigprint();
...
template <typename eT>
inline void Mat<eT>::swap_rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();

  const uword local_n_rows = n_rows;
  const uword local_n_cols = n_cols;

  arma_conform_check_bounds((in_row1 >= local_n_rows) || (in_row2 >= local_n_rows),
...
template <typename eT>
inline void Mat<eT>::swap_cols(const uword in_colA, const uword in_colB) {
  arma_debug_sigprint();

  const uword local_n_rows = n_rows;
  const uword local_n_cols = n_cols;

  arma_conform_check_bounds((in_colA >= local_n_cols) || (in_colB >= local_n_cols),
...
template <typename eT>
inline void Mat<eT>::shed_row(const uword row_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Mat<eT>::shed_col(const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Mat<eT>::shed_rows(const uword in_row1, const uword in_row2) {
  arma_debug_sigprint();
  ...
  if (n_keep_back > 0) {
    X.rows(n_keep_front, (n_keep_front + n_keep_back - 1)) =
        rows((in_row2 + 1), (n_rows - 1));
  }
...
template <typename eT>
inline void Mat<eT>::shed_cols(const uword in_col1, const uword in_col2) {
  arma_debug_sigprint();
  ...
  if (n_keep_back > 0) {
    X.cols(n_keep_front, (n_keep_front + n_keep_back - 1)) =
        cols((in_col2 + 1), (n_cols - 1));
  }
...
template <typename eT>
template <typename T1>
inline void Mat<eT>::shed_rows(const Base<uword, T1>& indices) {
  arma_debug_sigprint();

  const unwrap_check_mixed<T1> U(indices.get_ref(), *this);
  const Mat<uword>& tmp1 = U.M;

  arma_conform_check(((tmp1.is_vec() == false) && (tmp1.is_empty() == false)),
                     "Mat::shed_rows(): list of indices must be a vector");
  ...
  const Col<uword>& rows_to_shed =
      (tmp2.is_sorted("strictascend") == false)
  ...
  if (arma_config::check_conform) {
    for (uword i = 0; i < N; ++i) {
      arma_conform_check_bounds((rows_to_shed_mem[i] >= n_rows),
                                "Mat::shed_rows(): indices out of bounds");
    }
  }
...
template <typename eT>
template <typename T1>
inline void Mat<eT>::shed_cols(const Base<uword, T1>& indices) {
  arma_debug_sigprint();
  ...
  arma_conform_check(((tmp1.is_vec() == false) && (tmp1.is_empty() == false)),
                     "Mat::shed_cols(): list of indices must be a vector");
  ...
  if (arma_config::check_conform) {
    for (uword i = 0; i < N; ++i) {
      arma_conform_check_bounds((cols_to_shed_mem[i] >= n_cols),
                                "Mat::shed_cols(): indices out of bounds");
    }
  }
...
template <typename eT>
inline void Mat<eT>::insert_rows(const uword row_num, const uword N,
...
inline void Mat<eT>::insert_rows(const uword row_num, const uword N) {
  ...
  arma_conform_check_bounds((row_num > t_n_rows),
...
template <typename eT>
inline void Mat<eT>::insert_cols(const uword col_num, const uword N,
                                 const bool set_to_zero) {
  arma_debug_sigprint();
...
inline void Mat<eT>::insert_cols(const uword col_num, const uword N) {
  ...
  arma_conform_check_bounds((col_num > t_n_cols),
...
template <typename eT>
template <typename T1>
inline void Mat<eT>::insert_rows(const uword row_num, const Base<eT, T1>& X) {
  arma_debug_sigprint();
  ...
  arma_conform_set_error(err_state, err_msg, (row_num > t_n_rows), error_message_1);

  arma_conform_set_error(err_state, err_msg,
  ...
  if (C_n_rows > 0) {
    ...
    if (t_n_cols > 0) {
      ...
      if ((t_n_cols > 0) && (B_n_rows > 0)) {
        out.rows(row_num + C_n_rows, t_n_rows + C_n_rows - 1) =
...
template <typename eT>
template <typename T1>
inline void Mat<eT>::insert_cols(const uword col_num, const Base<eT, T1>& X) {
  arma_debug_sigprint();
  ...
  arma_conform_set_error(err_state, err_msg, (col_num > t_n_cols), error_message_1);
  ...
  if (C_n_cols > 0) {
    ...
    if (t_n_rows > 0) {
      ...
      if (B_n_cols > 0) {
        out.cols(col_num + C_n_cols, t_n_cols + C_n_cols - 1) =
...
template <typename eT>
template <typename T1, typename gen_type>
inline Mat<eT>& Mat<eT>::operator=(const Gen<T1, gen_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator=(const Op<T1, op_type>& X) {
  arma_debug_sigprint();
...
//! in-place matrix subtraction, with the right-hand-side operand having delayed
//! operations
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator-=(const Op<T1, op_type>& X) {
...
//! in-place matrix multiplication, with the right-hand-side operand having delayed
//! operations
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator*=(const Op<T1, op_type>& X) {
...
//! in-place matrix element-wise multiplication, with the right-hand-side operand having
//! delayed operations
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator%=(const Op<T1, op_type>& X) {
...
//! in-place matrix element-wise division, with the right-hand-side operand having delayed
//! operations
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator/=(const Op<T1, op_type>& X) {
...
template <typename eT>
template <typename T1, typename eop_type>
inline Mat<eT>& Mat<eT>::operator=(const eOp<T1, eop_type>& X) {
  arma_debug_sigprint();

  arma_type_check((is_same_type<eT, typename T1::elem_type>::no));

  const bool bad_alias =
      (eOp<T1, eop_type>::proxy_type::has_subview && X.P.is_alias(*this));
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator=(const mtOp<eT, T1, op_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>::Mat(const CubeToMatOp<T1, op_type>& X)
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0), mem() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator=(const CubeToMatOp<T1, op_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>::Mat(const SpToDOp<T1, op_type>& X)
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0), mem() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator=(const SpToDOp<T1, op_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>::Mat(const mtSpReduceOp<eT, T1, op_type>& X)
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0), mem() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>& Mat<eT>::operator=(const mtSpReduceOp<eT, T1, op_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator=(const Glue<T1, T2, glue_type>& X) {
  arma_debug_sigprint();
...
//! in-place matrix subtraction, with the right-hand-side operands having delayed
//! operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator-=(const Glue<T1, T2, glue_type>& X) {
...
//! in-place matrix multiplications, with the right-hand-side operands having delayed
//! operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator*=(const Glue<T1, T2, glue_type>& X) {
...
//! in-place matrix element-wise multiplication, with the right-hand-side operands having
//! delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator%=(const Glue<T1, T2, glue_type>& X) {
...
//! in-place matrix element-wise division, with the right-hand-side operands having
//! delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator/=(const Glue<T1, T2, glue_type>& X) {
...
template <typename eT>
template <typename T1, typename T2>
inline Mat<eT>& Mat<eT>::operator+=(const Glue<T1, T2, glue_times>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename eglue_type>
inline Mat<eT>& Mat<eT>::operator=(const eGlue<T1, T2, eglue_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator=(const mtGlue<eT, T1, T2, glue_type>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>::Mat(const SpToDGlue<T1, T2, glue_type>& X)
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0), mem() {
  arma_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>& Mat<eT>::operator=(const SpToDGlue<T1, T2, glue_type>& X) {
  arma_debug_sigprint();
...
//! linear element accessor (treats the matrix as a vector); no bounds check; assumes
//! memory is aligned
template <typename eT>
arma_inline const eT& Mat<eT>::at_alt(const uword ii) const {
...
//! linear element accessor (treats the matrix as a vector); bounds checking not done when
//! ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline eT& Mat<eT>::operator()(const uword ii) {
...
//! ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline const eT& Mat<eT>::operator()(const uword ii) const {
  arma_conform_check_bounds((ii >= n_elem), "Mat::operator(): index out of bounds");
...
//! linear element accessor (treats the matrix as a vector); no bounds check.
template <typename eT>
arma_inline eT& Mat<eT>::at(const uword ii) {
  return access::rw(mem[ii]);
}

//! linear element accessor (treats the matrix as a vector); no bounds check
template <typename eT>
arma_inline const eT& Mat<eT>::at(const uword ii) const {
  return mem[ii];
}

//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline eT& Mat<eT>::operator()(const uword in_row, const uword in_col) {
  arma_conform_check_bounds(((in_row >= n_rows) || (in_col >= n_cols)),
...
//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline const eT& Mat<eT>::operator()(const uword in_row, const uword in_col) const {
  arma_conform_check_bounds(((in_row >= n_rows) || (in_col >= n_cols)),
                            "Mat::operator(): index out of bounds");
...
template <typename eT>
arma_inline eT& Mat<eT>::at(const uword in_row, const uword in_col) {
  return access::rw(mem[in_row + in_col * n_rows]);
}

//! element accessor; no bounds check
template <typename eT>
arma_inline const eT& Mat<eT>::at(const uword in_row, const uword in_col) const {
  return mem[in_row + in_col * n_rows];
}
...
template <typename eT>
arma_inline bool Mat<eT>::is_empty() const {
  return (n_elem == 0);
}
...
template <typename eT>
arma_inline bool Mat<eT>::is_rowvec() const {
  return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT>
arma_inline bool Mat<eT>::is_colvec() const {
  return (n_cols == 1);
}
...
template <typename eT>
inline bool Mat<eT>::internal_is_finite() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::internal_has_inf() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::internal_has_nan() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::internal_has_nonfinite() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::is_sorted(const char* direction) const {
  arma_debug_sigprint();

  return (*this).is_sorted(direction,
                           (((vec_state == 2) || (n_rows == 1)) ? uword(1) : uword(0)));
}

template <typename eT>
inline bool Mat<eT>::is_sorted(const char* direction, const uword dim) const {
  arma_debug_sigprint();
  ...
  arma_conform_check(((sig1 != 'a') && (sig1 != 'd') && (sig1 != 's')),
                     "Mat::is_sorted(): unknown sort direction");
  ...
  if (sig1 == 's') {
    arma_conform_check(((sig2 != 'a') && (sig2 != 'd')),
                       "Mat::is_sorted(): unknown sort direction");
  }
...
inline bool Mat<eT>::is_sorted_helper(const comparator& comp, const uword dim) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool Mat<eT>::in_range(const uword ii) const {
  return (ii < n_elem);
}

//! returns true if the given start and end indices are currently in range
template <typename eT>
arma_inline bool Mat<eT>::in_range(const span& x) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool Mat<eT>::in_range(const uword in_row, const uword in_col) const {
  return ((in_row < n_rows) && (in_col < n_cols));
}

template <typename eT>
arma_inline bool Mat<eT>::in_range(const span& row_span, const uword in_col) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool Mat<eT>::in_range(const uword in_row, const span& col_span) const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline bool Mat<eT>::in_range(const span& row_span, const span& col_span) const {
  arma_debug_sigprint();
  ...
  const bool rows_ok =
      row_span.whole ? true : ((in_row1 <= in_row2) && (in_row2 < n_rows));
  const bool cols_ok =
      col_span.whole ? true : ((in_col1 <= in_col2) && (in_col2 < n_cols));

  return (rows_ok && cols_ok);
}

template <typename eT>
arma_inline bool Mat<eT>::in_range(const uword in_row, const uword in_col,
                                   const SizeMat& s) const {
  const uword l_n_rows = n_rows;
  const uword l_n_cols = n_cols;

  if ((in_row >= l_n_rows) || (in_col >= l_n_cols) || ((in_row + s.n_rows) > l_n_rows) ||
      ((in_col + s.n_cols) > l_n_cols)) {
    return false;
  } else {
    return true;
  }
...
template <typename eT>
arma_inline eT* Mat<eT>::colptr(const uword in_col) {
  return &access::rw(mem[in_col * n_rows]);
}

//! returns a pointer to array of eTs for a specified column; no bounds check
template <typename eT>
arma_inline const eT* Mat<eT>::colptr(const uword in_col) const {
...
template <typename eT>
inline Mat<eT>& Mat<eT>::set_size(const uword new_n_elem) {
  arma_debug_sigprint();
...
//! change the matrix to have user specified dimensions (data is not preserved)
template <typename eT>
inline Mat<eT>& Mat<eT>::set_size(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::set_size(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::resize(const uword new_n_elem) {
  arma_debug_sigprint();
...
//! change the matrix to have user specified dimensions (data is preserved)
template <typename eT>
inline Mat<eT>& Mat<eT>::resize(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::resize(const SizeMat& s) {
  arma_debug_sigprint();
...
//! change the matrix to have user specified dimensions (data is preserved)
template <typename eT>
inline Mat<eT>& Mat<eT>::reshape(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::reshape(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline void Mat<eT>::reshape(const uword new_n_rows, const uword new_n_cols,
                             const uword dim) {
  arma_debug_sigprint();
...
//! change the matrix (without preserving data) to have the same dimensions as the given
//! expression
template <typename eT>
template <typename eT2, typename expr>
inline Mat<eT>& Mat<eT>::copy_size(const Base<eT2, expr>& X) {
  arma_debug_sigprint();
...
inline Mat<eT>& Mat<eT>::for_each(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline const Mat<eT>& Mat<eT>::for_each(functor F) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline Mat<eT>& Mat<eT>::transform(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline Mat<eT>& Mat<eT>::imbue(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::replace(const eT old_val, const eT new_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::clean(const typename get_pod_type<eT>::result threshold) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::clamp(const eT min_val, const eT max_val) {
  arma_debug_sigprint();

  if (is_cx<eT>::no) {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "Mat::clamp(): min_val must be less than max_val");
  } else {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "Mat::clamp(): real(min_val) must be less than real(max_val)");
    arma_conform_check((access::tmp_imag(min_val) > access::tmp_imag(max_val)),
                       "Mat::clamp(): imag(min_val) must be less than imag(max_val)");
  }
...
template <typename eT>
inline Mat<eT>& Mat<eT>::fill(const eT val) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename fill_type>
inline Mat<eT>& Mat<eT>::fill(const fill::fill_class<fill_type>&) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::zeros(const uword new_n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::zeros(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::zeros(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::ones(const uword new_n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::ones(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::ones(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::randu(const uword new_n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::randu(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::randu(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::randn(const uword new_n_elem) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::randn(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::randn(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::eye(const uword new_n_rows, const uword new_n_cols) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>& Mat<eT>::eye(const SizeMat& s) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void Mat<eT>::set_real(const Base<typename Mat<eT>::pod_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void Mat<eT>::set_imag(const Base<typename Mat<eT>::pod_type, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Mat<eT>::min(uword& index_of_min_val) const {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Mat<eT>::max(uword& index_of_max_val) const {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Mat<eT>::min(uword& row_of_min_val, uword& col_of_min_val) const {
  arma_debug_sigprint();
...
template <typename eT>
inline eT Mat<eT>::max(uword& row_of_max_val, uword& col_of_max_val) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::save(const std::string name, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (save_okay == false) {
    arma_warn(3, "Mat::save(): write failed; file: ", name);
  }
...
template <typename eT>
inline bool Mat<eT>::save(const hdf5_name& spec, const file_type type) const {
  arma_debug_sigprint();

  // handling of hdf5_binary_trans kept for compatibility with earlier versions of
  // Armadillo
  ...
  const bool do_trans =
      bool(spec.opts.flags & hdf5_opts::flag_trans) || (type == hdf5_binary_trans);
  ...
  if (append && replace) {
    arma_stop_runtime_error(
        "Mat::save(): only one of 'append' or 'replace' options can be used");
    return false;
  }
...
template <typename eT>
inline bool Mat<eT>::save(const csv_name& spec, const file_type type) const {
  arma_debug_sigprint();
  ...
  const bool with_header =
      bool(spec.opts.flags & csv_opts::flag_with_header) && (no_header == false);
  const bool use_semicolon =
      bool(spec.opts.flags & csv_opts::flag_semicolon) || (type == ssv_ascii);

  arma_debug_print("Mat::save(csv_name): enabled flags:");

  if (do_trans) {
    arma_debug_print("trans");
  }
  if (no_header) {
    arma_debug_print("no_header");
  }
  if (with_header) {
    arma_debug_print("with_header");
  }
  if (use_semicolon) {
    arma_debug_print("semicolon");
  }
  ...
  if (with_header) {
    ...
    for (uword i = 0; i < spec.header_ro.n_elem; ++i) {
      ...
      if (token.find(separator) != std::string::npos) {
        arma_warn(
            1, "Mat::save(): token within the header contains the separator character: '",
            token, "'");
        return false;
      }
    }
...
template <typename eT>
inline bool Mat<eT>::save(std::ostream& os, const file_type type) const {
  arma_debug_sigprint();
  ...
  if (save_okay == false) {
    arma_warn(3, "Mat::save(): stream write failed");
  }
...
template <typename eT>
inline bool Mat<eT>::load(const std::string name, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::load(const hdf5_name& spec, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::load(const csv_name& spec, const file_type type) {
  arma_debug_sigprint();
  ...
  if (do_trans) {
    ...
    load_okay = diskio::load_csv_ascii(tmp_mat, spec.filename, err_msg, spec.header_rw,
                                       with_header, separator, strict);
    ...
    load_okay = diskio::load_csv_ascii(*this, spec.filename, err_msg, spec.header_rw,
                                       with_header, separator, strict);
  }
...
template <typename eT>
inline bool Mat<eT>::load(std::istream& is, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_save(const std::string name, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_save(const hdf5_name& spec, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_save(const csv_name& spec, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_save(std::ostream& os, const file_type type) const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_load(const std::string name, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_load(const hdf5_name& spec, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_load(const csv_name& spec, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_load(std::istream& is, const file_type type) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>::row_iterator::row_iterator()
    : M(nullptr), current_row(0), current_col(0) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>::row_iterator::row_iterator(const row_iterator& X)
    : M(X.M), current_row(X.current_row), current_col(X.current_col) {
  arma_debug_sigprint();
}
...
inline Mat<eT>::row_iterator::row_iterator(Mat<eT>& in_M, const uword in_row,
                                           const uword in_col)
    : M(&in_M), current_row(in_row), current_col(in_col) {
  arma_debug_sigprint();
}

template <typename eT>
inline eT& Mat<eT>::row_iterator::operator*() {
  return M->at(current_row, current_col);
}

template <typename eT>
inline typename Mat<eT>::row_iterator& Mat<eT>::row_iterator::operator++() {
  current_col++;
...
template <typename eT>
inline typename Mat<eT>::row_iterator Mat<eT>::row_iterator::operator++(int) {
...
template <typename eT>
inline bool Mat<eT>::row_iterator::operator!=(
    const typename Mat<eT>::row_iterator& X) const {
...
template <typename eT>
inline bool Mat<eT>::row_iterator::operator!=(
    const typename Mat<eT>::const_row_iterator& X) const {
  return ((current_row != X.current_row) || (current_col != X.current_col));
}
...
template <typename eT>
inline Mat<eT>::const_row_iterator::const_row_iterator()
    : M(nullptr), current_row(0), current_col(0) {
  arma_debug_sigprint();

  // NOTE: this instance of const_row_iterator is invalid (it does not point to a valid
  // element)
}

template <typename eT>
inline Mat<eT>::const_row_iterator::const_row_iterator(
    const typename Mat<eT>::row_iterator& X)
    : M(X.M), current_row(X.current_row), current_col(X.current_col) {
  arma_debug_sigprint();
}
...
template <typename eT>
inline Mat<eT>::const_row_iterator::const_row_iterator(
    const typename Mat<eT>::const_row_iterator& X)
    : M(X.M), current_row(X.current_row), current_col(X.current_col) {
  arma_debug_sigprint();
}

template <typename eT>
inline Mat<eT>::const_row_iterator::const_row_iterator(const Mat<eT>& in_M,
                                                       const uword in_row,
                                                       const uword in_col)
    : M(&in_M), current_row(in_row), current_col(in_col) {
  arma_debug_sigprint();
...
template <typename eT>
inline const eT& Mat<eT>::const_row_iterator::operator*() const {
  return M->at(current_row, current_col);
}

template <typename eT>
inline typename Mat<eT>::const_row_iterator& Mat<eT>::const_row_iterator::operator++() {
  current_col++;
...
template <typename eT>
inline typename Mat<eT>::const_row_iterator Mat<eT>::const_row_iterator::operator++(int) {
...
template <typename eT>
inline bool Mat<eT>::const_row_iterator::operator!=(
    const typename Mat<eT>::row_iterator& X) const {
  return ((current_row != X.current_row) || (current_col != X.current_col));
}
...
inline Mat<eT>::row_col_iterator::row_col_iterator()
    : M(nullptr), current_ptr(nullptr), current_col(0), current_row(0) {
  arma_debug_sigprint();
...
template <typename eT>
inline Mat<eT>::row_col_iterator::row_col_iterator(Mat<eT>& in_M, const uword in_row,
                                                   const uword in_col)
    : M(&in_M),
...
template <typename eT>
inline eT& Mat<eT>::row_col_iterator::operator*() {
  return *current_ptr;
}

template <typename eT>
inline typename Mat<eT>::row_col_iterator& Mat<eT>::row_col_iterator::operator++() {
  if (current_col < M->n_cols) {
    current_ptr++;
    current_row++;
...
template <typename eT>
inline typename Mat<eT>::row_col_iterator Mat<eT>::row_col_iterator::operator++(int) {
...
template <typename eT>
inline uword Mat<eT>::row_col_iterator::row() const {
  return current_row;
}

template <typename eT>
inline uword Mat<eT>::row_col_iterator::col() const {
  return current_col;
}

template <typename eT>
inline bool Mat<eT>::row_col_iterator::operator==(const row_col_iterator& rhs) const {
...
template <typename eT>
inline bool Mat<eT>::row_col_iterator::operator==(
    const const_row_col_iterator& rhs) const {
  return (current_ptr == rhs.current_ptr);
}
...
template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(
    const const_row_col_iterator& in_it)
    : M(in_it.M),
      current_ptr(in_it.current_ptr),
      current_col(in_it.col()),
      current_row(in_it.row()) {
...
template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const Mat<eT>& in_M,
                                                               const uword in_row,
                                                               const uword in_col)
    : M(&in_M),
      current_ptr(&in_M.at(in_row, in_col)),
...
template <typename eT>
inline const eT& Mat<eT>::const_row_col_iterator::operator*() const {
  return *current_ptr;
}
...
template <typename eT>
inline uword Mat<eT>::const_row_col_iterator::row() const {
  return current_row;
}

template <typename eT>
inline uword Mat<eT>::const_row_col_iterator::col() const {
  return current_col;
}

template <typename eT>
inline bool Mat<eT>::const_row_col_iterator::operator==(
    const const_row_col_iterator& rhs) const {
  return (current_ptr == rhs.current_ptr);
}
...
template <typename eT>
inline bool Mat<eT>::const_row_col_iterator::operator==(
    const row_col_iterator& rhs) const {
  return (current_ptr == rhs.current_ptr);
}
...
template <typename eT>
inline typename Mat<eT>::iterator Mat<eT>::begin() {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_iterator Mat<eT>::begin() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_iterator Mat<eT>::cbegin() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::iterator Mat<eT>::end() {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_iterator Mat<eT>::end() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_iterator Mat<eT>::cend() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::col_iterator Mat<eT>::begin_col(const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_col_iterator Mat<eT>::begin_col(
    const uword col_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::col_iterator Mat<eT>::end_col(const uword col_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_col_iterator Mat<eT>::end_col(const uword col_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::row_iterator Mat<eT>::begin_row(const uword row_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_row_iterator Mat<eT>::begin_row(
...
template <typename eT>
inline typename Mat<eT>::row_iterator Mat<eT>::end_row(const uword row_num) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::const_row_iterator Mat<eT>::end_row(const uword row_num) const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename Mat<eT>::row_col_iterator Mat<eT>::begin_row_col() {
  return row_col_iterator(*this);
}

template <typename eT>
inline typename Mat<eT>::const_row_col_iterator Mat<eT>::begin_row_col() const {
  return const_row_col_iterator(*this);
}

template <typename eT>
inline typename Mat<eT>::row_col_iterator Mat<eT>::end_row_col() {
  return row_col_iterator(*this, 0, n_cols);
}

template <typename eT>
inline typename Mat<eT>::const_row_col_iterator Mat<eT>::end_row_col() const {
  return const_row_col_iterator(*this, 0, n_cols);
}

//! resets this matrix to an empty matrix
template <typename eT>
inline void Mat<eT>::clear() {
  reset();
}

//! returns true if the matrix has no elements
template <typename eT>
inline bool Mat<eT>::empty() const {
  return (n_elem == 0);
}

//! returns the number of elements in this matrix
template <typename eT>
inline uword Mat<eT>::size() const {
  return n_elem;
}

template <typename eT>
inline eT& Mat<eT>::front() {
...
template <typename eT>
inline const eT& Mat<eT>::front() const {
  arma_conform_check((n_elem == 0), "Mat::front(): matrix is empty");
...
template <typename eT>
inline const eT& Mat<eT>::back() const {
  arma_conform_check((n_elem == 0), "Mat::back(): matrix is empty");
...
arma_inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed()
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0,
              ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
  arma_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(
    const fixed<fixed_n_rows, fixed_n_cols>& X)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0,
              ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const fill::scalar_holder<eT> f)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0,
              ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
  arma_debug_sigprint_this(this);
...
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(
    const fill::fill_class<fill_type>&)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0,
              ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
  arma_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols>
template <typename T1, typename T2>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const Base<pod_type, T1>& A,
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(
    const fixed<fixed_n_rows, fixed_n_cols>& X) {
  arma_debug_sigprint();
...
inline Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(
  ...
  arma_conform_assert_same_size(fixed_n_rows, fixed_n_cols, X.get_n_rows(),
...
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const
    Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type,
       op_htrans>
    Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::t() const {
  return Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type,
            op_htrans>(*this);
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at_alt(
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator[](const uword ii) {
  return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}

template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator[](
    const uword ii) const {
  return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword ii) {
  return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}

template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(
    const uword ii) const {
  return (use_extra) ? mem_local_extra[ii] : mem_local[ii];
}
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator[](
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword in_row,
                                                               const uword in_col) {
  const uword iq = in_row + in_col * fixed_n_rows;
...
arma_inline eT& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(
    const uword in_row, const uword in_col) {
  arma_conform_check_bounds(((in_row >= fixed_n_rows) || (in_col >= fixed_n_cols)),
                            "Mat::operator(): index out of bounds");
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::colptr(const uword in_col) {
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::colptr(
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::memptr() {
  return (use_extra) ? mem_local_extra : mem_local;
}

template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const eT* Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::memptr() const {
  return (use_extra) ? mem_local_extra : mem_local;
}

template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline bool Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::is_vec() const {
  return ((fixed_n_rows == 1) || (fixed_n_cols == 1));
}

template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fill(const eT val) {
  arma_debug_sigprint();
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::zeros() {
  arma_debug_sigprint();
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline const Mat<eT>& Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::ones() {
  arma_debug_sigprint();
...
//! prefix ++
template <typename eT>
inline void Mat_aux::prefix_pp(Mat<eT>& x) {
  eT* memptr = x.memptr();
...
template <typename T>
inline void Mat_aux::prefix_pp(Mat<std::complex<T> >& x) {
  x += T(1);
}

//! postfix ++
template <typename eT>
inline void Mat_aux::postfix_pp(Mat<eT>& x) {
  eT* memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
template <typename T>
inline void Mat_aux::postfix_pp(Mat<std::complex<T> >& x) {
  x += T(1);
}

//! prefix --
template <typename eT>
inline void Mat_aux::prefix_mm(Mat<eT>& x) {
  eT* memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! prefix -- for complex numbers (work around for limitations of the std::complex class)
template <typename T>
inline void Mat_aux::prefix_mm(Mat<std::complex<T> >& x) {
  x -= T(1);
}

//! postfix --
template <typename eT>
inline void Mat_aux::postfix_mm(Mat<eT>& x) {
  eT* memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! postfix ++ for complex numbers (work around for limitations of the std::complex class)
template <typename T>
inline void Mat_aux::postfix_mm(Mat<std::complex<T> >& x) {
  x -= T(1);
}

template <typename eT, typename T1>
inline void Mat_aux::set_real(Mat<eT>& out, const Base<eT, T1>& X) {
  arma_debug_sigprint();
...
template <typename eT, typename T1>
inline void Mat_aux::set_imag(Mat<eT>&, const Base<eT, T1>&) {
  arma_debug_sigprint();
}
...
inline void Mat_aux::set_real(Mat<std::complex<T> >& out, const Base<T, T1>& X) {
  ...
  arma_conform_assert_same_size(out.n_rows, out.n_cols, local_n_rows, local_n_cols,
                                "Mat::set_real()");
