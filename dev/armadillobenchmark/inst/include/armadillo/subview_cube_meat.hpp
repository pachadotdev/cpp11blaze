...
template <typename eT>
arma_inline subview_cube<eT>::subview_cube(const Cube<eT>& in_m, const uword in_row1,
                                           const uword in_col1, const uword in_slice1,
...
template <typename eT>
inline subview_cube<eT>::subview_cube(const subview_cube<eT>& in)
    : m(in.m),
...
template <typename eT>
inline subview_cube<eT>::subview_cube(subview_cube<eT>&& in)
    : m(in.m),
      aux_row1(in.aux_row1),
      aux_col1(in.aux_col1),
      aux_slice1(in.aux_slice1),
...
inline void subview_cube<eT>::inplace_op(const eT val) {
  arma_debug_sigprint();
  ...
  for (uword s = 0; s < t_n_slices; ++s)
    for (uword c = 0; c < t_n_cols; ++c) {
      if (is_same_type<op_type, op_internal_plus>::yes) {
        arrayops::inplace_plus(slice_colptr(s, c), val, t_n_rows);
      }
      if (is_same_type<op_type, op_internal_minus>::yes) {
        arrayops::inplace_minus(slice_colptr(s, c), val, t_n_rows);
      }
      if (is_same_type<op_type, op_internal_schur>::yes) {
        arrayops::inplace_mul(slice_colptr(s, c), val, t_n_rows);
      }
      if (is_same_type<op_type, op_internal_div>::yes) {
        arrayops::inplace_div(slice_colptr(s, c), val, t_n_rows);
      }
    }
}
...
inline void subview_cube<eT>::inplace_op(const BaseCube<eT, T1>& in,
  ...
  const bool use_mp =
      arma_config::openmp && ProxyCube<T1>::use_mp && mp_gate<eT>::eval(t.n_elem);
  ...
  if ((is_Cube<typename ProxyCube<T1>::stored_type>::value) || (use_mp) ||
      (has_overlap)) {
    const unwrap_cube_check<typename ProxyCube<T1>::stored_type> tmp(P.Q, has_overlap);
    const Cube<eT>& B = tmp.M;

    if ((is_same_type<op_type, op_internal_equ>::yes) && (t.aux_row1 == 0) &&
        (t_n_rows == t.m.n_rows)) {
      for (uword s = 0; s < t_n_slices; ++s) {
      ...
      for (uword s = 0; s < t_n_slices; ++s)
        for (uword c = 0; c < t_n_cols; ++c) {
          if (is_same_type<op_type, op_internal_equ>::yes) {
            arrayops::copy(t.slice_colptr(s, c), B.slice_colptr(s, c), t_n_rows);
          }
          if (is_same_type<op_type, op_internal_plus>::yes) {
            arrayops::inplace_plus(t.slice_colptr(s, c), B.slice_colptr(s, c), t_n_rows);
          }
          if (is_same_type<op_type, op_internal_minus>::yes) {
            arrayops::inplace_minus(t.slice_colptr(s, c), B.slice_colptr(s, c), t_n_rows);
          }
          if (is_same_type<op_type, op_internal_schur>::yes) {
            arrayops::inplace_mul(t.slice_colptr(s, c), B.slice_colptr(s, c), t_n_rows);
          }
          if (is_same_type<op_type, op_internal_div>::yes) {
            arrayops::inplace_div(t.slice_colptr(s, c), B.slice_colptr(s, c), t_n_rows);
          }
        }
    }
  } else  // use the Proxy
...
template <typename eT>
template <typename op_type>
inline void subview_cube<eT>::inplace_op(const subview_cube<eT>& x,
                                         const char* identifier) {
  arma_debug_sigprint();
  ...
  for (uword s = 0; s < t_n_slices; ++s)
    for (uword c = 0; c < t_n_cols; ++c) {
      if (is_same_type<op_type, op_internal_equ>::yes) {
        arrayops::copy(t.slice_colptr(s, c), x.slice_colptr(s, c), t_n_rows);
      }
      if (is_same_type<op_type, op_internal_plus>::yes) {
        arrayops::inplace_plus(t.slice_colptr(s, c), x.slice_colptr(s, c), t_n_rows);
      }
      if (is_same_type<op_type, op_internal_minus>::yes) {
        arrayops::inplace_minus(t.slice_colptr(s, c), x.slice_colptr(s, c), t_n_rows);
      }
      if (is_same_type<op_type, op_internal_schur>::yes) {
        arrayops::inplace_mul(t.slice_colptr(s, c), x.slice_colptr(s, c), t_n_rows);
      }
      if (is_same_type<op_type, op_internal_div>::yes) {
        arrayops::inplace_div(t.slice_colptr(s, c), x.slice_colptr(s, c), t_n_rows);
      }
    }
}

template <typename eT>
inline void subview_cube<eT>::operator=(const eT val) {
  arma_debug_sigprint();
...
inline void subview_cube<eT>::operator=(const BaseCube<eT, T1>& in) {
  arma_debug_sigprint();
...
//! x.subcube(...) = y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::operator=(const subview_cube<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void subview_cube<eT>::operator=(const Base<eT, T1>& in) {
  arma_debug_sigprint();
  ...
  if (((x_n_rows == 1) || (x_n_cols == 1)) && (t_n_rows == 1) && (t_n_cols == 1) &&
      (x.n_elem == t_n_slices)) {
  ...
  if (((x_n_rows == 1) || (x_n_cols == 1)) && (t_n_rows == 1) && (t_n_cols == 1) &&
    ...
    if (arma_config::check_conform) {
      arma_stop_logic_error(
          arma_incompat_size_string(t, x, "element-wise multiplication"));
    }
  }
}
...
inline void subview_cube<eT>::operator=(const GenCube<eT, gen_type>& in) {
  arma_debug_sigprint();

  arma_conform_assert_same_size(n_rows, n_cols, n_slices, in.n_rows, in.n_cols,
                                in.n_slices, "copy into subcube");
...
inline void subview_cube<eT>::for_each(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline void subview_cube<eT>::for_each(functor F) const {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline void subview_cube<eT>::transform(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename functor>
inline void subview_cube<eT>::imbue(functor F) {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::each_slice(const std::function<void(Mat<eT>&)>& F) {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::each_slice(
...
template <typename eT>
inline void subview_cube<eT>::replace(const eT old_val, const eT new_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::clean(const typename get_pod_type<eT>::result threshold) {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::clamp(const eT min_val, const eT max_val) {
  arma_debug_sigprint();

  if (is_cx<eT>::no) {
    arma_conform_check((access::tmp_real(min_val) > access::tmp_real(max_val)),
                       "subview_cube::clamp(): min_val must be less than max_val");
  } else {
    arma_conform_check(
        (access::tmp_real(min_val) > access::tmp_real(max_val)),
        "subview_cube::clamp(): real(min_val) must be less than real(max_val)");
    arma_conform_check(
        (access::tmp_imag(min_val) > access::tmp_imag(max_val)),
        "subview_cube::clamp(): imag(min_val) must be less than imag(max_val)");
  }
...
template <typename eT>
inline void subview_cube<eT>::fill(const eT val) {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::zeros() {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::ones() {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::randu() {
  arma_debug_sigprint();
...
template <typename eT>
inline void subview_cube<eT>::randn() {
  arma_debug_sigprint();
...
template <typename eT>
inline bool subview_cube<eT>::is_finite() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool subview_cube<eT>::is_zero(const typename get_pod_type<eT>::result tol) const {
  arma_debug_sigprint();
  ...
  for (uword slice = 0; slice < local_n_slices; ++slice) {
    for (uword col = 0; col < local_n_cols; ++col) {
      if (arrayops::is_zero(slice_colptr(slice, col), local_n_rows, tol) == false) {
        return false;
      }
    }
  }
...
template <typename eT>
inline bool subview_cube<eT>::has_inf() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool subview_cube<eT>::has_nan() const {
  arma_debug_sigprint();
...
template <typename eT>
inline bool subview_cube<eT>::has_nonfinite() const {
  arma_debug_sigprint();
...
template <typename eT>
inline eT subview_cube<eT>::at_alt(const uword i) const {
  return operator[](i);
}

template <typename eT>
inline eT& subview_cube<eT>::operator[](const uword i) {
  ...
  const uword index = (in_slice + aux_slice1) * m.n_elem_slice +
                      (in_col + aux_col1) * m.n_rows + aux_row1 + in_row;

  return access::rw((const_cast<Cube<eT>&>(m)).mem[index]);
}

template <typename eT>
inline eT subview_cube<eT>::operator[](const uword i) const {
  const uword in_slice = i / n_elem_slice;
  const uword offset = in_slice * n_elem_slice;
  const uword j = i - offset;
...
inline eT& subview_cube<eT>::operator()(const uword i) {
  arma_conform_check_bounds((i >= n_elem),
                            "subview_cube::operator(): index out of bounds");
...
template <typename eT>
arma_inline eT& subview_cube<eT>::operator()(const uword in_row, const uword in_col,
                                             const uword in_slice) {
  arma_conform_check_bounds(
      ((in_row >= n_rows) || (in_col >= n_cols) || (in_slice >= n_slices)),
      "subview_cube::operator(): location out of bounds");
...
template <typename eT>
arma_inline eT& subview_cube<eT>::at(const uword in_row, const uword in_col,
                                     const uword in_slice) {
  const uword index = (in_slice + aux_slice1) * m.n_elem_slice +
                      (in_col + aux_col1) * m.n_rows + aux_row1 + in_row;
...
arma_inline eT* subview_cube<eT>::slice_colptr(const uword in_slice, const uword in_col) {
  return &access::rw((const_cast<Cube<eT>&>(m))
                         .mem[(in_slice + aux_slice1) * m.n_elem_slice +
...
arma_inline const eT* subview_cube<eT>::slice_colptr(const uword in_slice,
  ...
  return &m.mem[(in_slice + aux_slice1) * m.n_elem_slice +
                (in_col + aux_col1) * m.n_rows + aux_row1];
}

template <typename eT>
template <typename eT2>
inline bool subview_cube<eT>::check_overlap(const subview_cube<eT2>& x) const {
  ...
  const bool outside_rows =
      ((x_row_start >= t_row_end_p1) || (t_row_start >= x_row_end_p1));
  const bool outside_cols =
      ((x_col_start >= t_col_end_p1) || (t_col_start >= x_col_end_p1));
  const bool outside_slices =
      ((x_slice_start >= t_slice_end_p1) || (t_slice_start >= x_slice_end_p1));

  return ((outside_rows == false) && (outside_cols == false) &&
          (outside_slices == false));
}

template <typename eT>
inline bool subview_cube<eT>::check_overlap(const Mat<eT>& x) const {
  const subview_cube<eT>& t = *this;
...
//! cube X = Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::extract(Cube<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();

  // NOTE: we're assuming that the cube has already been set to the correct size and there
  // is no aliasing; size setting and alias checking is done by either the Cube contructor
  // or operator=()
  ...
  arma_debug_print(
      arma_str::format("out.n_rows: %u; out.n_cols: %u; out.n_slices: %u; in.m.n_rows: "
                       "%u; in.m.n_cols: %u; in.m.n_slices: %u") %
      out.n_rows % out.n_cols % out.n_slices % in.m.n_rows % in.m.n_cols % in.m.n_slices);
...
//! cube X += Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::plus_inplace(Cube<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  for (uword slice = 0; slice < n_slices; ++slice) {
    for (uword col = 0; col < n_cols; ++col) {
      arrayops::inplace_plus(out.slice_colptr(slice, col), in.slice_colptr(slice, col),
                             n_rows);
    }
  }
}
...
//! cube X -= Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::minus_inplace(Cube<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  for (uword slice = 0; slice < n_slices; ++slice) {
    for (uword col = 0; col < n_cols; ++col) {
      arrayops::inplace_minus(out.slice_colptr(slice, col), in.slice_colptr(slice, col),
                              n_rows);
    }
  }
}
...
//! cube X %= Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::schur_inplace(Cube<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  for (uword slice = 0; slice < n_slices; ++slice) {
    for (uword col = 0; col < n_cols; ++col) {
      arrayops::inplace_mul(out.slice_colptr(slice, col), in.slice_colptr(slice, col),
                            n_rows);
    }
  }
}
...
//! cube X /= Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::div_inplace(Cube<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  for (uword slice = 0; slice < n_slices; ++slice) {
    for (uword col = 0; col < n_cols; ++col) {
      arrayops::inplace_div(out.slice_colptr(slice, col), in.slice_colptr(slice, col),
                            n_rows);
    }
  }
}
...
//! mat X = Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::extract(Mat<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
...
//! mat X += Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::plus_inplace(Mat<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  if (in_n_slices == 1) {
    if ((arma_config::check_conform) &&
        ((out_n_rows != in_n_rows) || (out_n_cols != in_n_cols))) {
      std::ostringstream tmp;

      tmp << "in-place addition: " << out_n_rows << 'x' << out_n_cols
          << " output matrix is incompatible with " << in_n_rows << 'x' << in_n_cols
          << 'x' << in_n_slices << " cube interpreted as " << in_n_rows << 'x'
          << in_n_cols << " matrix";
...
//! mat X -= Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::minus_inplace(Mat<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  if (in_n_slices == 1) {
    if ((arma_config::check_conform) &&
      ...
      tmp << "in-place subtraction: " << out_n_rows << 'x' << out_n_cols
          << " output matrix is incompatible with " << in_n_rows << 'x' << in_n_cols
          << 'x' << in_n_slices << " cube interpreted as " << in_n_rows << 'x'
          << in_n_cols << " matrix";
...
//! mat X %= Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::schur_inplace(Mat<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  if (in_n_slices == 1) {
    if ((arma_config::check_conform) &&
      ...
      tmp << "in-place element-wise multiplication: " << out_n_rows << 'x' << out_n_cols
          << " output matrix is incompatible with " << in_n_rows << 'x' << in_n_cols
          << 'x' << in_n_slices << " cube interpreted as " << in_n_rows << 'x'
          << in_n_cols << " matrix";
...
//! mat X /= Y.subcube(...)
template <typename eT>
inline void subview_cube<eT>::div_inplace(Mat<eT>& out, const subview_cube<eT>& in) {
  arma_debug_sigprint();
  ...
  if (in_n_slices == 1) {
    if ((arma_config::check_conform) &&
      ...
      tmp << "in-place element-wise division: " << out_n_rows << 'x' << out_n_cols
          << " output matrix is incompatible with " << in_n_rows << 'x' << in_n_cols
          << 'x' << in_n_slices << " cube interpreted as " << in_n_rows << 'x'
          << in_n_cols << " matrix";
...
template <typename eT>
inline typename subview_cube<eT>::iterator subview_cube<eT>::begin() {
  return iterator(*this, aux_row1, aux_col1, aux_slice1);
}

template <typename eT>
inline typename subview_cube<eT>::const_iterator subview_cube<eT>::begin() const {
  return const_iterator(*this, aux_row1, aux_col1, aux_slice1);
}

template <typename eT>
inline typename subview_cube<eT>::const_iterator subview_cube<eT>::cbegin() const {
  return const_iterator(*this, aux_row1, aux_col1, aux_slice1);
}

template <typename eT>
inline typename subview_cube<eT>::iterator subview_cube<eT>::end() {
  return iterator(*this, aux_row1, aux_col1, aux_slice1 + n_slices);
}

template <typename eT>
inline typename subview_cube<eT>::const_iterator subview_cube<eT>::end() const {
  return const_iterator(*this, aux_row1, aux_col1, aux_slice1 + n_slices);
}

template <typename eT>
inline typename subview_cube<eT>::const_iterator subview_cube<eT>::cend() const {
  return const_iterator(*this, aux_row1, aux_col1, aux_slice1 + n_slices);
}
...
template <typename eT>
inline subview_cube<eT>::iterator::iterator()
    : M(nullptr),
      current_ptr(nullptr),
      current_row(0),
      current_col(0),
...
template <typename eT>
inline subview_cube<eT>::iterator::iterator(const iterator& X)
    : M(X.M),
...
template <typename eT>
inline subview_cube<eT>::iterator::iterator(subview_cube<eT>& in_sv, const uword in_row,
                                            const uword in_col, const uword in_slice)
...
template <typename eT>
inline eT& subview_cube<eT>::iterator::operator*() {
  return (*current_ptr);
}

template <typename eT>
inline typename subview_cube<eT>::iterator& subview_cube<eT>::iterator::operator++() {
  current_row++;
...
template <typename eT>
inline typename subview_cube<eT>::iterator subview_cube<eT>::iterator::operator++(int) {
...
template <typename eT>
inline bool subview_cube<eT>::iterator::operator==(const iterator& rhs) const {
...
template <typename eT>
inline bool subview_cube<eT>::iterator::operator==(const const_iterator& rhs) const {
  return (current_ptr == rhs.current_ptr);
}
...
template <typename eT>
inline subview_cube<eT>::const_iterator::const_iterator()
    : M(nullptr),
      current_ptr(nullptr),
      current_row(0),
      current_col(0),
...
template <typename eT>
inline subview_cube<eT>::const_iterator::const_iterator(const iterator& X)
    : M(X.M),
      current_ptr(X.current_ptr),
      current_row(X.current_row),
      current_col(X.current_col),
...
template <typename eT>
inline subview_cube<eT>::const_iterator::const_iterator(const const_iterator& X)
    : M(X.M),
      current_ptr(X.current_ptr),
      current_row(X.current_row),
      current_col(X.current_col),
...
template <typename eT>
inline subview_cube<eT>::const_iterator::const_iterator(const subview_cube<eT>& in_sv,
                                                        const uword in_row,
                                                        const uword in_col,
                                                        const uword in_slice)
    : M(&(in_sv.m)),
...
template <typename eT>
inline const eT& subview_cube<eT>::const_iterator::operator*() {
  return (*current_ptr);
}

template <typename eT>
inline typename subview_cube<eT>::const_iterator&
...
template <typename eT>
inline bool subview_cube<eT>::const_iterator::operator==(const iterator& rhs) const {
  return (current_ptr == rhs.current_ptr);
}
...
template <typename eT>
inline bool subview_cube<eT>::const_iterator::operator==(
