...
inline void glue_quantile::worker(eTb* out_mem, Col<eTa>& Y, const Mat<eTb>& P) {
  arma_debug_sigprint();
  ...
  for (uword i = 0; i < P_n_elem; ++i) {
    ...
    if (P_i < P_min) {
      out_val =
          (P_i < eTb(0)) ? eTb(-std::numeric_limits<eTb>::infinity()) : eTb(Y.min());
    } else if (P_i > P_max) {
...
template <typename eTa, typename eTb>
inline void glue_quantile::apply_noalias(Mat<eTb>& out, const Mat<eTa>& X,
                                         const Mat<eTb>& P, const uword dim) {
  arma_debug_sigprint();

  arma_conform_check(((P.is_vec() == false) && (P.is_empty() == false)),
                     "quantile(): parameter 'P' must be a vector");
  ...
  if (dim == 0) {
    ...
    if (X_n_rows == 1) {
      ...
      for (uword row = 0; row < X_n_rows; ++row) {
        ...
        for (uword col = 0; col < X_n_cols; ++col) {
          Y_mem[col] = X.at(row, col);
        }

        glue_quantile::worker(tmp_mem, Y, P);

        for (uword i = 0; i < P_n_elem; ++i) {
          out.at(row, i) = tmp_mem[i];
        }
      }
    }
  }
...
inline void glue_quantile::apply(
    Mat<typename T2::elem_type>& out,
    const mtGlue<typename T2::elem_type, T1, T2, glue_quantile>& expr) {
  arma_debug_sigprint();
  ...
  arma_conform_check((UA.M.internal_has_nan() || UB.M.internal_has_nan()),
                     "quantile(): detected NaN");
...
template <typename T1, typename T2>
inline void glue_quantile_default::apply(
    Mat<typename T2::elem_type>& out,
    const mtGlue<typename T2::elem_type, T1, T2, glue_quantile_default>& expr) {
  arma_debug_sigprint();
  ...
  const uword dim =
      (T1::is_xvec) ? uword(UA.M.is_rowvec() ? 1 : 0) : uword((T1::is_row) ? 1 : 0);
