...
arma_inline typename arma_not_cx<eT>::result dense_sparse_helper::dot(const eT* A_mem,
                                                                      const SpMat<eT>& B,
                                                                      const uword col) {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline typename arma_cx_only<eT>::result dense_sparse_helper::dot(const eT* A_mem,
                                                                       const SpMat<eT>& B,
                                                                       const uword col) {
  arma_debug_sigprint();
...
inline void glue_times_dense_sparse::apply(
    Mat<typename T1::elem_type>& out,
    const SpToDGlue<T1, T2, glue_times_dense_sparse>& expr) {
  arma_debug_sigprint();

  typedef typename T1::elem_type eT;

  if (is_op_diagmat<T1>::value) {
    out = SpMat<eT>(expr.A) * expr.B;
    return;
  }  // SpMat has specialised handling for op_diagmat
...
template <typename T1, typename T2>
inline void glue_times_dense_sparse::apply_noalias(Mat<typename T1::elem_type>& out,
  ...
  if ((resolves_to_rowvector<T1>::value) || (A.n_rows == 1)) {
    ...
    if ((arma_config::openmp) && (mp_thread_limit::in_parallel() == false) &&
        (B.n_cols >= 2) && mp_gate<eT>::eval(B.n_nonzero)) {
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
  ...
  } else if ((arma_config::openmp) && (mp_thread_limit::in_parallel() == false) &&
             (A.n_rows <= (A.n_cols / uword(100)))) {
#if defined(ARMA_USE_OPENMP)
    {
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
      for (uword i = 0; i < B_n_cols; ++i) {
        ...
        const uvec indices(const_cast<uword*>(&(B.row_indices[col_offset_1])),
                           col_offset_delta, false, false);
        const Col<eT> B_col(const_cast<eT*>(&(B.values[col_offset_1])), col_offset_delta,
                            false, false);
...
template <typename T1, typename T2>
inline void glue_times_dense_sparse::apply_mixed(
  ...
  if ((is_same_type<eT1, out_eT>::no) && (is_same_type<eT2, out_eT>::yes)) {
    ...
    for (uword i = 0; i < B.n_nonzero; ++i) {
      access::rw(BB.values[i]) = out_eT(B.values[i]);
    }
...
template <typename T1, typename T2>
inline void glue_times_sparse_dense::apply(
    Mat<typename T1::elem_type>& out,
    const SpToDGlue<T1, T2, glue_times_sparse_dense>& expr) {
  arma_debug_sigprint();

  typedef typename T1::elem_type eT;

  if (is_op_diagmat<T2>::value) {
    out = expr.A * SpMat<eT>(expr.B);
    return;
  }  // SpMat has specialised handling for op_diagmat

  const quasi_unwrap<T2> UB(expr.B);

  if ((sp_strip_trans<T1>::do_htrans && is_cx<eT>::no) ||
      (sp_strip_trans<T1>::do_strans)) {
...
template <typename T1, typename T2>
inline void glue_times_sparse_dense::apply_noalias(Mat<typename T1::elem_type>& out,
                                                   const T1& x, const T2& y) {
  arma_debug_sigprint();
  ...
  if ((resolves_to_colvector<T2>::value) || (B_n_cols == 1)) {
  ...
  } else if (B_n_cols >= (B_n_rows / uword(100))) {
    ...
    if (A_n_rows == B_n_cols) {
      ...
      op_strans::apply_mat(
          out, out);  // since 'out' is square-sized, this will do an inplace transpose
    } else {
      Mat<eT> tmp;
...
template <typename T1, typename T2>
inline void glue_times_sparse_dense::apply_noalias_trans(Mat<typename T1::elem_type>& out,
                                                         const T1& x, const T2& y) {
  arma_debug_sigprint();
  ...
  const unwrap_spmat<T1> UA(x);
  const SpMat<eT>& A =
      UA.M;  // NOTE: this is the given matrix without the transpose operation applied
  ...
  arma_conform_assert_mul_size(A_n_cols, A_n_rows, B_n_rows, B_n_cols,
                               "matrix multiplication");
  ...
  if ((resolves_to_colvector<T2>::value) || (B_n_cols == 1)) {
    ...
    if ((arma_config::openmp) && (mp_thread_limit::in_parallel() == false) &&
        (A_n_cols >= 2) && mp_gate<eT>::eval(A.n_nonzero)) {
...
template <typename T1, typename T2>
inline void glue_times_sparse_dense::apply_mixed(
    Mat<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result>&
        out,
    const T1& X, const T2& Y) {
  arma_debug_sigprint();
  ...
  if ((is_same_type<eT1, out_eT>::no) && (is_same_type<eT2, out_eT>::yes)) {
    ...
    for (uword i = 0; i < A.n_nonzero; ++i) {
      access::rw(AA.values[i]) = out_eT(A.values[i]);
    }
