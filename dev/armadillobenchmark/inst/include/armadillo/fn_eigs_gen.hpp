...
arma_warn_unused inline
    typename enable_if2<is_real<typename T1::pod_type>::value,
...
arma_warn_unused inline
  ...
  // The reason is that we are still not able to apply truly complex shifts to real
  // matrices
  if ((is_real<typename T1::elem_type>::yes) && (std::imag(sigma) != T(0))) {
    status =
        sp_auxlib::eigs_gen(eigval, eigvec, conv_to<SpMat<std::complex<T> > >::from(X),
                            n_eigvals, sigma, opts);
  } else {
...
//! eigenvalues of general sparse matrix X
template <typename T1>
inline typename enable_if2<is_real<typename T1::pod_type>::value, bool>::result eigs_gen(
...
//! this form is deprecated; use eigs_gen(eigval, eigvec, X, n_eigvals, form, opts)
//! instead
template <typename T1>
arma_deprecated inline
    typename enable_if2<is_real<typename T1::pod_type>::value, bool>::result
