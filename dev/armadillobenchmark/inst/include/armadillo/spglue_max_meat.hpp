...
inline void spglue_max::apply(SpMat<typename T1::elem_type>& out,
                              const SpGlue<T1, T2, spglue_max>& X) {
  arma_debug_sigprint();
...
inline void spglue_max::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa,
                                      const SpProxy<T2>& pb) {
  arma_debug_sigprint();

  arma_conform_assert_same_size(pa.get_n_rows(), pa.get_n_cols(), pb.get_n_rows(),
                                pb.get_n_cols(), "element-wise max()");
  ...
  while ((x_it != x_end) || (y_it != y_end)) {
    ...
    arma_check((count > max_n_nonzero),
               "internal error: spglue_max::apply_noalias(): count > max_n_nonzero");
  }
...
inline void spglue_max::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A,
                                      const SpMat<eT>& B) {
  arma_debug_sigprint();
...
template <typename eT, typename T1, typename T2>
inline void spglue_max::dense_sparse_max(Mat<eT>& out, const Base<eT, T1>& X,
                                         const SpBase<eT, T2>& Y) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename enable_if2<is_cx<eT>::no, eT>::result spglue_max::elem_max(const eT& a,
                                                                           const eT& b) {
  return (std::max)(a, b);
}
...
template <typename eT>
inline typename enable_if2<is_cx<eT>::yes, eT>::result spglue_max::elem_max(const eT& a,
                                                                            const eT& b) {
