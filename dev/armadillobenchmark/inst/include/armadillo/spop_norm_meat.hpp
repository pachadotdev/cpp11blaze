...
inline typename get_pod_type<eT>::result spop_norm::mat_norm_1(const SpMat<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename get_pod_type<eT>::result spop_norm::mat_norm_2(
    const SpMat<eT>& X, const typename arma_real_only<eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
template <typename eT>
inline typename get_pod_type<eT>::result spop_norm::mat_norm_2(
    const SpMat<eT>& X, const typename arma_cx_only<eT>::result* junk) {
  arma_debug_sigprint();
  arma_ignore(junk);
...
// we're calling eigs_gen(), which currently requires ARPACK
#if !defined(ARMA_USE_ARPACK)
  {
    arma_stop_logic_error(
        "norm(): use of ARPACK must be enabled for norm of complex matrices");
    return T(0);
  }
#endif
...
template <typename eT>
inline typename get_pod_type<eT>::result spop_norm::mat_norm_inf(const SpMat<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline typename get_pod_type<eT>::result spop_norm::vec_norm_k(const eT* mem,
                                                               const uword N,
                                                               const uword k) {
  arma_debug_sigprint();
