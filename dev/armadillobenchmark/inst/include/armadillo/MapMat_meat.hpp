...
template <typename eT>
inline MapMat<eT>::MapMat() : n_rows(0), n_cols(0), n_elem(0), map_ptr(nullptr) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline MapMat<eT>::MapMat(const uword in_n_rows, const uword in_n_cols)
    : n_rows(in_n_rows),
      n_cols(in_n_cols),
...
template <typename eT>
inline MapMat<eT>::MapMat(const SizeMat& s)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem(s.n_rows * s.n_cols), map_ptr(nullptr) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline MapMat<eT>::MapMat(const MapMat<eT>& x)
    : n_rows(0), n_cols(0), n_elem(0), map_ptr(nullptr) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline MapMat<eT>::MapMat(const SpMat<eT>& x)
    : n_rows(0), n_cols(0), n_elem(0), map_ptr(nullptr) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
inline MapMat<eT>::MapMat(MapMat<eT>&& x)
    : n_rows(x.n_rows), n_cols(x.n_cols), n_elem(x.n_elem), map_ptr(x.map_ptr) {
  arma_debug_sigprint_this(this);
...
template <typename eT>
arma_inline MapMat_val<eT> MapMat<eT>::operator[](const uword index) {
  return MapMat_val<eT>(*this, index);
}

template <typename eT>
inline eT MapMat<eT>::operator[](const uword index) const {
  map_type& map_ref = (*map_ptr);
...
template <typename eT>
arma_inline MapMat_val<eT> MapMat<eT>::at(const uword in_row, const uword in_col) {
...
template <typename eT>
inline eT MapMat<eT>::at(const uword in_row, const uword in_col) const {
  const uword index = (n_rows * in_col) + in_row;
...
template <typename eT>
arma_inline MapMat_val<eT> MapMat<eT>::operator()(const uword in_row,
                                                  const uword in_col) {
  arma_conform_check_bounds(((in_row >= n_rows) || (in_col >= n_cols)),
                            "MapMat::operator(): index out of bounds");
...
template <typename eT>
inline eT MapMat<eT>::operator()(const uword in_row, const uword in_col) const {
  arma_conform_check_bounds(((in_row >= n_rows) || (in_col >= n_cols)),
                            "MapMat::operator(): index out of bounds");
...
template <typename eT>
inline bool MapMat<eT>::is_empty() const {
  return (n_elem == 0);
}

template <typename eT>
inline bool MapMat<eT>::is_vec() const {
  return ((n_rows == 1) || (n_cols == 1));
}

template <typename eT>
inline bool MapMat<eT>::is_rowvec() const {
  return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT>
inline bool MapMat<eT>::is_colvec() const {
  return (n_cols == 1);
}

template <typename eT>
inline bool MapMat<eT>::is_square() const {
  return (n_rows == n_cols);
}

// this function is for debugging purposes only
template <typename eT>
inline void MapMat<eT>::sprandu(const uword in_n_rows, const uword in_n_cols,
  ...
  const Col<uword> indx =
      linspace<Col<uword> >(0, ((n_elem > 0) ? uword(n_elem - 1) : uword(0)), N);
...
inline void MapMat<eT>::print(const std::string& extra_text) const {
  ...
  const double density =
      (n_elem > 0) ? ((double(n_nonzero) / double(n_elem)) * double(100)) : double(0);

  get_cout_stream() << "[matrix size: " << n_rows << 'x' << n_cols
                    << "; n_nonzero: " << n_nonzero << "; density: " << density
                    << "%]\n\n";
...
inline void MapMat<eT>::init_cold() {
  ...
  const char* error_message =
      "MapMat(): requested size is too large; suggest to enable ARMA_64BIT_WORD";
#endif
...
template <typename eT>
arma_inline MapMat_val<eT>::MapMat_val(MapMat<eT>& in_parent, const uword in_index)
    : parent(in_parent), index(in_index) {
  arma_debug_sigprint();
}
...
template <typename eT>
arma_inline typename get_pod_type<eT>::result MapMat_val<eT>::real() const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline typename get_pod_type<eT>::result MapMat_val<eT>::imag() const {
  arma_debug_sigprint();
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT>::SpMat_MapMat_val(SpMat<eT>& in_s_parent,
                                                   MapMat<eT>& in_m_parent,
                                                   const uword in_row, const uword in_col)
    : s_parent(in_s_parent), m_parent(in_m_parent), row(in_row), col(in_col) {
  arma_debug_sigprint();
}

template <typename eT>
inline SpMat_MapMat_val<eT>::operator eT() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename get_pod_type<eT>::result SpMat_MapMat_val<eT>::real() const {
  arma_debug_sigprint();
...
template <typename eT>
inline typename get_pod_type<eT>::result SpMat_MapMat_val<eT>::imag() const {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator=(
    const SpMat_MapMat_val<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator=(const eT in_val) {
  arma_debug_sigprint();
...
template <typename eT>
inline eT SpMat_MapMat_val<eT>::operator++(int) {
  arma_debug_sigprint();
...
template <typename eT>
inline void SpMat_MapMat_val<eT>::set(const eT in_val) {
  arma_debug_sigprint();

  const bool done =
      (s_parent.sync_state == 0) ? s_parent.try_set_value_csc(row, col, in_val) : false;
...
template <typename eT>
inline void SpMat_MapMat_val<eT>::add(const eT in_val) {
  arma_debug_sigprint();

  const bool done =
      (s_parent.sync_state == 0) ? s_parent.try_add_value_csc(row, col, in_val) : false;
...
template <typename eT>
inline void SpMat_MapMat_val<eT>::sub(const eT in_val) {
  arma_debug_sigprint();

  const bool done =
      (s_parent.sync_state == 0) ? s_parent.try_sub_value_csc(row, col, in_val) : false;
...
template <typename eT>
inline void SpMat_MapMat_val<eT>::mul(const eT in_val) {
  arma_debug_sigprint();

  const bool done =
      (s_parent.sync_state == 0) ? s_parent.try_mul_value_csc(row, col, in_val) : false;
  ...
  if (done == false) {
    ...
    if (it != it_end) {
      ...
      // element not found, ie. it's zero; zero multiplied by anything is zero, except for
      // nan and inf
...
template <typename eT>
inline void SpMat_MapMat_val<eT>::div(const eT in_val) {
  arma_debug_sigprint();

  const bool done =
      (s_parent.sync_state == 0) ? s_parent.try_div_value_csc(row, col, in_val) : false;
  ...
  if (done == false) {
    ...
    if (it != it_end) {
      ...
      // element not found, ie. it's zero; zero divided by anything is zero, except for
      // zero and nan
...
template <typename eT>
arma_inline SpSubview_MapMat_val<eT>::SpSubview_MapMat_val(SpSubview<eT>& in_sv_parent,
                                                           MapMat<eT>& in_m_parent,
                                                           const uword in_row,
                                                           const uword in_col)
...
template <typename eT>
inline SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator=(
    const SpSubview_MapMat_val<eT>& x) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator=(const eT in_val) {
  arma_debug_sigprint();
  ...
  if (sv_parent.m.n_nonzero > old_n_nonzero) {
    access::rw(sv_parent.n_nonzero)++;
  }
  if (sv_parent.m.n_nonzero < old_n_nonzero) {
    access::rw(sv_parent.n_nonzero)--;
  }
...
template <typename eT>
inline eT SpSubview_MapMat_val<eT>::operator++(int) {
  arma_debug_sigprint();
