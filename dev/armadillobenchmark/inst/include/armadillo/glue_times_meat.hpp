...
inline void glue_times_redirect2_helper<true>::apply(Mat<typename T1::elem_type>& out,
  ...
  if (arma_config::optimise_invexpr &&
    ...
    if (status == false) {
      ...
      arma_stop_runtime_error(
          "matrix multiplication: problem with matrix inverse; suggest to use solve() "
          "instead");
    }
...
inline void glue_times_redirect3_helper<true>::apply(
  ...
  if (arma_config::optimise_invexpr &&
    ...
    glue_times::apply<eT, partial_unwrap<T2>::do_trans, partial_unwrap<T3>::do_trans,
                      (partial_unwrap<T2>::do_times || partial_unwrap<T3>::do_times)>(
        BC, B, C, alpha);
...
inline void glue_times::apply_inplace_plus(Mat<typename T1::elem_type>& out,
  ...
  if ((is_outer_product<T1>::value) || (has_op_inv_any<T1>::value) ||
      (has_op_inv_any<T2>::value)) {
...
template <typename eT, const bool do_trans_A, const bool do_trans_B, typename TA,
          typename TB>
arma_inline uword glue_times::mul_storage_cost(const TA& A, const TB& B) {
  const uword final_A_n_rows =
      (do_trans_A == false) ? (TA::is_row ? 1 : A.n_rows) : (TA::is_col ? 1 : A.n_cols);
  const uword final_B_n_cols =
      (do_trans_B == false) ? (TB::is_col ? 1 : B.n_cols) : (TB::is_row ? 1 : B.n_rows);
...
template <typename eT, const bool do_trans_A, const bool do_trans_B,
          const bool do_trans_C, const bool use_alpha, typename TA, typename TB,
          typename TC>
inline void glue_times::apply(Mat<eT>& out, const TA& A, const TB& B, const TC& C,
  ...
  const uword storage_cost_AB =
      glue_times::mul_storage_cost<eT, do_trans_A, do_trans_B>(A, B);
  const uword storage_cost_BC =
      glue_times::mul_storage_cost<eT, do_trans_B, do_trans_C>(B, C);
...
template <typename eT, const bool do_trans_A, const bool do_trans_B,
          const bool do_trans_C, const bool do_trans_D, const bool use_alpha, typename TA,
...
inline void glue_times::apply(Mat<eT>& out, const TA& A, const TB& B, const TC& C,
  ...
  const uword storage_cost_AC =
      glue_times::mul_storage_cost<eT, do_trans_A, do_trans_C>(A, C);
  const uword storage_cost_BD =
      glue_times::mul_storage_cost<eT, do_trans_B, do_trans_D>(B, D);

  if (storage_cost_AC <= storage_cost_BD) {
    // out = (A*B*C)*D

    glue_times::apply<eT, do_trans_A, do_trans_B, do_trans_C, use_alpha>(tmp, A, B, C,
                                                                         alpha);
    ...
    glue_times::apply<eT, do_trans_B, do_trans_C, do_trans_D, use_alpha>(tmp, B, C, D,
                                                                         alpha);
