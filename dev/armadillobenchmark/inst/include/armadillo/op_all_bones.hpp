...
class op_all : public traits_op_xvec {
 public:
  template <typename T1>
  static inline bool all_vec_helper(const Base<typename T1::elem_type, T1>& X);

  template <typename eT>
  static inline bool all_vec_helper(const subview<eT>& X);

  template <typename T1>
  static inline bool all_vec_helper(const Op<T1, op_vectorise_col>& X);

  template <typename T1, typename op_type>
  static inline bool all_vec_helper(
