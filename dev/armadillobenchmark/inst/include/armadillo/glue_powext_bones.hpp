...
class glue_powext : public traits_glue_or {
 public:
  ...
  inline static Mat<typename parent::elem_type> apply(
  ...
  inline static Cube<eT> apply(const subview_cube_each1<eT>& X, const Base<eT, T2>& Y);
};

class glue_powext_cx : public traits_glue_or {
 public:
  template <typename T1, typename T2>
  ...
  template <typename T>
  inline static void apply(Cube<std::complex<T> >& out, const Cube<std::complex<T> >& A,
                           const Cube<T>& B);

  template <typename T, typename T2>
  inline static Cube<std::complex<T> > apply(
