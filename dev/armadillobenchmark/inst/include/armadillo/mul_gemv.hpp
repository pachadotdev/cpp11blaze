...
template <const bool do_trans_A = false, const bool use_alpha = false,
...
class gemv_emul_tinysq {
 public:
  ...
  arma_inline static void assign(eT* y, const eT acc, const eT alpha, const eT beta) {
    if (use_beta == false) {
  ...
  arma_cold inline static void apply(eT* y, const TA& A, const eT* x,
    ...
    switch (A.n_rows) {
      ...
      case 3: {
        ...
        const eT acc0 =
            Am[pos<0, 0>::n3] * x0 + Am[pos<0, 1>::n3] * x1 + Am[pos<0, 2>::n3] * x2;
        const eT acc1 =
            Am[pos<1, 0>::n3] * x0 + Am[pos<1, 1>::n3] * x1 + Am[pos<1, 2>::n3] * x2;
        const eT acc2 =
            Am[pos<2, 0>::n3] * x0 + Am[pos<2, 1>::n3] * x1 + Am[pos<2, 2>::n3] * x2;
      ...
      case 4: {
        ...
        const eT acc0 = Am[pos<0, 0>::n4] * x0 + Am[pos<0, 1>::n4] * x1 +
                        Am[pos<0, 2>::n4] * x2 + Am[pos<0, 3>::n4] * x3;
        const eT acc1 = Am[pos<1, 0>::n4] * x0 + Am[pos<1, 1>::n4] * x1 +
                        Am[pos<1, 2>::n4] * x2 + Am[pos<1, 3>::n4] * x3;
        const eT acc2 = Am[pos<2, 0>::n4] * x0 + Am[pos<2, 1>::n4] * x1 +
                        Am[pos<2, 2>::n4] * x2 + Am[pos<2, 3>::n4] * x3;
        const eT acc3 = Am[pos<3, 0>::n4] * x0 + Am[pos<3, 1>::n4] * x1 +
                        Am[pos<3, 2>::n4] * x2 + Am[pos<3, 3>::n4] * x3;
...
class gemv_emul_helper {
 public:
  template <typename eT, typename TA>
  arma_hot inline static typename arma_not_cx<eT>::result dot_row_col(const TA& A,
                                                                      const eT* x,
                                                                      const uword row,
                                                                      const uword N) {
    eT acc1 = eT(0);
  ...
  template <typename eT, typename TA>
  arma_hot inline static typename arma_cx_only<eT>::result dot_row_col(const TA& A,
                                                                       const eT* x,
                                                                       const uword row,
                                                                       const uword N) {
...
//! 'y' is assumed to have been set to the correct size (ie. taking into account the
//! transpose)
...
//! \brief
//! Wrapper for BLAS gemv function, using template arguments to control the arguments
//! passed to gemv. 'y' is assumed to have been set to the correct size (ie. taking into
...
class gemv {
 public:
    ...
    if ((A.n_rows <= 4) && (A.n_rows == A.n_cols) && (is_cx<eT>::no)) {
...
#if defined(ARMA_USE_ATLAS)
      {
        ...
        if (is_cx<eT>::no) {
          ...
          atlas::cblas_gemm<eT>(
              atlas_CblasColMajor,
              (do_trans_A) ? (is_cx<eT>::yes ? atlas_CblasConjTrans : atlas_CblasTrans)
                           : atlas_CblasNoTrans,
              atlas_CblasNoTrans, (do_trans_A) ? A.n_cols : A.n_rows, 1,
              (do_trans_A) ? A.n_rows : A.n_cols, (use_alpha) ? alpha : eT(1), A.mem,
          ...
          atlas::cblas_gemv<eT>(
              ...
              (do_trans_A) ? (is_cx<eT>::yes ? atlas_CblasConjTrans : atlas_CblasTrans)
                           : atlas_CblasNoTrans,
              A.n_rows, A.n_cols, (use_alpha) ? alpha : eT(1), A.mem, A.n_rows, x, 1,
              (use_beta) ? beta : eT(0), y, 1);
        }
      }
  ...
  template <typename eT, typename TA>
  arma_inline static void apply(eT* y, const TA& A, const eT* x, const eT alpha = eT(1),
                                const eT beta = eT(0)) {
    gemv_emul<do_trans_A, use_alpha, use_beta>::apply(y, A, x, alpha, beta);
  }

  template <typename TA>
  arma_inline static void apply(float* y, const TA& A, const float* x,
  ...
  template <typename TA>
  arma_inline static void apply(double* y, const TA& A, const double* x,
  ...
  template <typename TA>
  arma_inline static void apply(std::complex<float>* y, const TA& A,
  ...
  arma_inline static void apply(
      std::complex<double>* y, const TA& A, const std::complex<double>* x,
