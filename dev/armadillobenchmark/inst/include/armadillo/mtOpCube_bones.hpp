...
class mtOpCube : public BaseCube<out_eT, mtOpCube<out_eT, T1, op_type> > {
 public:
  ...
  inline mtOpCube(const T1& in_m, const uword in_aux_uword_a, const uword in_aux_uword_b,
                  const uword in_aux_uword_c);
  inline mtOpCube(const T1& in_m, const in_eT in_aux, const uword in_aux_uword_a,
                  const uword in_aux_uword_b, const uword in_aux_uword_c);

  inline mtOpCube(const char junk, const T1& in_m, const out_eT in_aux);

  inline mtOpCube(const mtOpCube_dual_aux_indicator&, const T1& in_m,
                  const in_eT in_aux_a, const out_eT in_aux_b);
  ...
  arma_aligned out_eT aux_out_eT;  //!< auxiliary data, using the element type as
                                   //!< specified by the out_eT template parameter
