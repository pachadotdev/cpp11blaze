...
class op_pinv_default : public traits_op_default {
 public:
...
class op_pinv : public traits_op_default {
 public:
  template <typename T1>
  ...
  inline static bool apply_diag(Mat<eT>& out, const Mat<eT>& A,
                                typename get_pod_type<eT>::result tol);

  template <typename eT>
  inline static bool apply_sym(Mat<eT>& out, const Mat<eT>& A,
                               typename get_pod_type<eT>::result tol,
                               const uword method_id);

  template <typename eT>
  inline static bool apply_gen(Mat<eT>& out, Mat<eT>& A,
                               typename get_pod_type<eT>::result tol,
                               const uword method_id);
};
