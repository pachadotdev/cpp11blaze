...
class Row : public Mat<eT> {
 public:
  ...
  inline explicit Row(const uword N, const arma_initmode_indicator<do_zeros>&);
  template <bool do_zeros>
  inline explicit Row(const uword in_rows, const uword in_cols,
                      const arma_initmode_indicator<do_zeros>&);
  template <bool do_zeros>
  inline explicit Row(const SizeMat& s, const arma_initmode_indicator<do_zeros>&);
  ...
  template <typename T1>
  inline explicit Row(const SpBase<eT, T1>& X);
  template <typename T1>
  inline Row& operator=(const SpBase<eT, T1>& X);

  inline Row(eT* aux_mem, const uword aux_length, const bool copy_aux_mem = true,
  ...
  arma_frown("use braced initialiser list instead") inline mat_injector<Row> operator<<(
      const eT val);
  ...
  arma_inline const subview_row<eT> subvec(const uword in_col1,
  ...
  arma_deprecated inline void insert_cols(const uword col_num, const uword N,
