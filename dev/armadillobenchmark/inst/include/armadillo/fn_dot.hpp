...
template <typename T1, typename T2>
arma_hot inline typename T1::elem_type dot_helper(const SpProxy<T1>& pa,
                                                  const SpProxy<T2>& pb) {
  ...
  while ((a_it != a_end) && (b_it != b_end)) {
    if (a_it == b_it) {
      ...
      ++a_it;
      ++b_it;
    } else if ((a_it.col() < b_it.col()) ||
               ((a_it.col() == b_it.col()) && (a_it.row() < b_it.row()))) {
      // a_it is "behind"
      ++a_it;
    } else {
      // b_it is "behind"
...
template <typename T1, typename T2>
arma_warn_unused arma_hot inline typename enable_if2<
...
dot(const T1& x, const T2& y) {
  ...
  if (((SpProxy<T1>::use_iterator == false) && (SpProxy<T2>::use_iterator == false)) &&
