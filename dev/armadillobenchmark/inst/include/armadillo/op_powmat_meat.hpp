...
inline bool op_powmat::apply_direct(Mat<typename T1::elem_type>& out,
  ...
  if (y_neg) {
    if (y == uword(1)) {
      ...
      const bool inv_status =
          op_inv_gen_default::apply_direct(X_inv, X.get_ref(), "powmat()");
...
inline void op_powmat::apply_direct_positive(Mat<eT>& out, const Mat<eT>& X,
  ...
  if (X.is_diagmat()) {
    ...
    for (uword i = 0; i < N; ++i) {
      tmp[i] = eop_aux::pow(X.at(i, i), int(y));
    }
...
inline bool op_powmat_cx::apply_direct(Mat<std::complex<typename T1::pod_type> >& out,
  ...
  if (A.is_diagmat()) {
    ...
    for (uword i = 0; i < N; ++i) {
      tmp[i] = eop_aux::pow(std::complex<in_T>(A.at(i, i)), y);
    }
