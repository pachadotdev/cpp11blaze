...
inline SpCol<eT>::SpCol() : SpMat<eT>(arma_vec_indicator(), 1) {
  arma_debug_sigprint();
}
...
template <typename eT>
inline SpCol<eT>::SpCol(const SizeMat& s) : SpMat<eT>(arma_vec_indicator(), 0, 0, 1) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpCol<eT>::SpCol(const char* text) : SpMat<eT>(arma_vec_indicator(), 1) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpCol<eT>::SpCol(const std::string& text) : SpMat<eT>(arma_vec_indicator(), 1) {
  arma_debug_sigprint();
...
inline SpCol<eT>::SpCol(const Base<eT, T1>& X) : SpMat<eT>(arma_vec_indicator(), 1) {
  arma_debug_sigprint();
...
template <typename eT>
inline SpCol<eT>::SpCol(const Col<eT>& X) : SpMat<eT>(arma_vec_indicator(), 1) {
  arma_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline SpCol<eT>::SpCol(const SpBase<eT, T1>& X) : SpMat<eT>(arma_vec_indicator(), 1) {
  arma_debug_sigprint();
...
inline SpCol<eT>::SpCol(const SpBase<typename SpCol<eT>::pod_type, T1>& A,
...
template <typename eT>
inline const SpOp<SpCol<eT>, spop_htrans> SpCol<eT>::t() const {
  return SpOp<SpCol<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpCol<eT>, spop_htrans> SpCol<eT>::ht() const {
  return SpOp<SpCol<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpCol<eT>, spop_strans> SpCol<eT>::st() const {
  return SpOp<SpCol<eT>, spop_strans>(*this);
}

template <typename eT>
inline const SpToDOp<SpCol<eT>, op_sp_as_dense> SpCol<eT>::as_dense() const {
...
inline void SpCol<eT>::shed_row(const uword row_num) {
  ...
  arma_conform_check_bounds(row_num >= SpMat<eT>::n_rows,
                            "SpCol::shed_row(): out of bounds");
  ...
  if (start != end) {
    ...
    if (end != SpMat<eT>::n_nonzero) {
      ...
      arrayops::copy(new_row_indices + start, SpMat<eT>::row_indices + end,
                     (SpMat<eT>::n_nonzero - end));
      arrayops::inplace_minus(new_row_indices + start, diff,
                              (SpMat<eT>::n_nonzero - end));
    }
...
//   arma_debug_sigprint();
//
//   arma_conform_check(set_to_zero == false, "SpCol::insert_rows(): cannot set nonzero
//   values");
//
//   arma_conform_check_bounds((row_num > SpMat<eT>::n_rows), "SpCol::insert_rows(): out
//   of bounds");
//
...
inline typename SpCol<eT>::row_iterator SpCol<eT>::begin_row(const uword row_num) {
  ...
  arma_conform_check_bounds((row_num >= SpMat<eT>::n_rows),
                            "SpCol::begin_row(): index out of bounds");
...
template <typename eT>
inline typename SpCol<eT>::const_row_iterator SpCol<eT>::begin_row(
...
inline typename SpCol<eT>::row_iterator SpCol<eT>::end_row(const uword row_num) {
  ...
  arma_conform_check_bounds((row_num >= SpMat<eT>::n_rows),
                            "SpCol::end_row(): index out of bounds");
...
template <typename eT>
inline typename SpCol<eT>::const_row_iterator SpCol<eT>::end_row(
    const uword row_num) const {
  arma_debug_sigprint();
