...
inline void arma_ostream_state::restore(std::ostream& o) const {
  o.flags(orig_flags);
...
inline std::streamsize arma_ostream::modify_stream(std::ostream& o, const eT* data,
                                                   const uword n_elem) {
  ...
  for (uword i = 0; i < n_elem; ++i) {
    ...
    if (arma_isfinite(val) == false) {
      continue;
    }

    if ((cond_rel < (sizeof(eT) > 4) &&
         (is_same_type<uword, eT>::yes || is_same_type<sword, eT>::yes) >
             ::geq(val, eT(+10000000000))) ||
        (cond_rel < (sizeof(eT) > 4) &&
         is_same_type<sword, eT>::yes > ::leq(val, eT(-10000000000)))) {
      use_layout_D = true;
      break;
    }
    ...
    if ((val >= eT(+100)) ||
        ...
        //( (is_non_integral<eT>::value) && (is_signed<eT>::value) && (val < eT(0)) &&
        //(val >= eT(-1e-4)) )
...
template <typename T>
inline std::streamsize arma_ostream::modify_stream(std::ostream& o,
...
inline std::streamsize arma_ostream::modify_stream(
  ...
  for (typename SpMat<eT>::const_iterator it = begin; it.pos() < n_elem; ++it) {
    ...
    if (val >= eT(+100) || ((is_signed<eT>::value) && (val <= eT(-100))) ||
        ((is_non_integral<eT>::value) && (val > eT(0)) && (val <= eT(+1e-4))) ||
        ((is_non_integral<eT>::value) && (is_signed<eT>::value) && (val < eT(0)) &&
         (val >= eT(-1e-4)))) {
      use_layout_C = true;
    ...
    if ((val >= eT(+10)) || ((is_signed<eT>::value) && (val <= eT(-10)))) {
      use_layout_B = true;
    }
  }
...
template <typename eT>
inline void arma_ostream::print_elem(std::ostream& o, const eT& x, const bool modify) {
  if (x == eT(0)) {
...
template <typename eT>
inline void arma_ostream::raw_print_elem(std::ostream& o, const eT& x) {
...
template <typename T>
inline void arma_ostream::print_elem(std::ostream& o, const std::complex<T>& x,
                                     const bool modify) {
...
template <typename T>
inline void arma_ostream::raw_print_elem(std::ostream& o, const std::complex<T>& x) {
...
inline void arma_ostream::print(std::ostream& o, const Mat<eT>& m, const bool modify) {
  ...
  const std::streamsize cell_width =
      modify ? arma_ostream::modify_stream(o, m.memptr(), m.n_elem) : o.width();
...
inline void arma_ostream::print(std::ostream& o, const Cube<eT>& x, const bool modify) {
  ...
  if (x.is_empty() == false) {
    for (uword slice = 0; slice < x.n_slices; ++slice) {
      const Mat<eT> tmp(const_cast<eT*>(x.slice_memptr(slice)), x.n_rows, x.n_cols,
                        false);

      o << "[cube slice: " << slice << ']' << '\n';
      arma_ostream::print(o, tmp, modify);

      if ((slice + 1) < x.n_slices) {
        o << '\n';
      }
    }
  } else {
...
//! Assumes type oT can be printed, ie. oT has std::ostream& operator<< (std::ostream&,
//! const oT&)
...
inline void arma_ostream::print(std::ostream& o, const SpMat<eT>& m, const bool modify) {
  ...
  const double density =
      (m.n_elem > 0) ? (double(m_n_nonzero) / double(m.n_elem) * double(100)) : double(0);

  o << "[matrix size: " << m.n_rows << 'x' << m.n_cols << "; n_nonzero: " << m_n_nonzero;

  if (density == double(0)) {
    o.precision(0);
  } else if (density >= (double(10.0) - std::numeric_limits<double>::epsilon())) {
    o.precision(1);
  } else if (density > (double(0.01) - std::numeric_limits<double>::epsilon())) {
    o.precision(2);
  } else if (density > (double(0.001) - std::numeric_limits<double>::epsilon())) {
    o.precision(3);
  } else if (density > (double(0.0001) - std::numeric_limits<double>::epsilon())) {
    o.precision(4);
  } else {
    o.unsetf(ios::fixed);
    o.setf(ios::scientific);
  ...
  if (m_n_nonzero > 0) {
    const std::streamsize cell_width =
        modify ? arma_ostream::modify_stream<eT>(o, m.begin(), m_n_nonzero) : o.width();
    ...
    while (it != it_end) {
      ...
      // TODO: change the maximum number of spaces before and after each location to be
      // dependent on n_rows and n_cols
...
inline void arma_ostream::brief_print(std::ostream& o, const Mat<eT>& m,
  ...
  if ((print_row_ellipsis == true) && (print_col_ellipsis == true)) {
    ...
    const std::streamsize cell_width =
        arma_ostream::modify_stream(o, X.memptr(), X.n_elem);
...
inline void arma_ostream::brief_print(std::ostream& o, const SpMat<eT>& m) {
  ...
  const std::streamsize cell_width =
      arma_ostream::modify_stream(o, storage_val.memptr(), 10);
