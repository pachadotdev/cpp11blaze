...
inline void op_sp_stddev::apply(
...
template <typename T1>
inline void op_sp_stddev::apply_slow(Mat<typename T1::pod_type>& out,
  ...
  if (dim == 0)  // find variance in each column
  {
    ...
    for (uword col = 0; col < p_n_cols; ++col) {
      if (SpProxy<T1>::use_iterator) {
        ...
        out.at(0, col) =
            std::sqrt(op_sp_var::iterator_var(it, end, n_zero, norm_type, in_eT(0)));
      } else {
        // We can use direct memory access to calculate the variance.
        out.at(0, col) = std::sqrt(op_sp_var::direct_var(
...
template <typename T1>
inline typename T1::pod_type op_sp_stddev::stddev_vec(const T1& X,
  ...
  return std::sqrt(
      op_sp_var::direct_var(tmp.M.values, tmp.M.n_nonzero, tmp.M.n_elem, norm_type));
}
