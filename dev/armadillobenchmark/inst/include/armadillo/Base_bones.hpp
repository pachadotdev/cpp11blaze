...
struct Base_extra_yes {
  arma_warn_unused inline const Op<derived, op_inv_gen_default> i()
      const;  //!< matrix inverse

  arma_warn_unused inline bool is_sympd() const;
  arma_warn_unused inline bool is_sympd(
...
template <typename elem_type, typename derived>
struct Base_extra<elem_type, derived, true> {
  typedef Base_extra_yes<elem_type, derived> result;
};

template <typename elem_type, typename derived>
struct Base_extra<elem_type, derived, false> {
...
struct Base_eval_expr {
  arma_warn_unused inline Mat<elem_type> eval()
      const;  //!< force the immediate evaluation of a delayed expression
};
...
struct Base_trans_cx {
  ...
  arma_warn_unused arma_inline const Op<derived, op_strans> st()
      const;  // simple transpose: no complex conjugates
};
...
struct Base_trans_default {
  arma_warn_unused arma_inline const Op<derived, op_htrans> t() const;
  arma_warn_unused arma_inline const Op<derived, op_htrans> ht() const;
  arma_warn_unused arma_inline const Op<derived, op_htrans> st()
      const;  // return op_htrans instead of op_strans, as it's handled better by matrix
              // multiplication code
};
...
//! Class for static polymorphism, modelled after the "Curiously Recurring Template
//! Pattern" (CRTP). Used for type-safe downcasting in functions that restrict their
//! input(s) to be classes that are derived from Base (eg. Mat, Op, Glue, diagview,
//! subview). A Base object can be converted to a Mat object by the unwrap class.

template <typename elem_type, typename derived>
struct Base : public Base_extra<elem_type, derived,
                                is_supported_blas_type<elem_type>::value>::result,
  ...
  arma_cold inline void raw_print(std::ostream& user_stream,
                                  const std::string extra_text = "") const;

  arma_cold inline void brief_print(const std::string extra_text = "") const;
  arma_cold inline void brief_print(std::ostream& user_stream,
                                    const std::string extra_text = "") const;
  ...
  arma_warn_unused inline bool is_zero(
      const typename get_pod_type<elem_type>::result tol = 0) const;
