...
// (4) ensure rudimentary diagonal dominance: (real(A_ii) + real(A_jj)) >
// 2*abs(real(A_ij)) the above conditions are necessary, but not sufficient; doing it
// properly would be too computationally expensive for our purposes more info:
...
inline typename enable_if2<is_cx<eT>::no, bool>::result guess_sympd_worker(
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      ...
      if ((A_delta > tol) && (A_delta > (A_abs_max * tol))) {
        return false;
      }

      const eT A_ii = (*A_ii_ptr);

      if ((A_ij_abs + A_ij_abs) >= (A_ii + A_jj)) {
        return false;
      }
...
inline typename enable_if2<is_cx<eT>::yes, bool>::result guess_sympd_worker(
  ...
  for (uword j = 0; j < N; ++j) {
    ...
    if ((A_jj_real <= T(0)) || (std::abs(A_jj_imag) > tol)) {
      return false;
    }
  ...
  if (arma_isfinite(square_max_diag) == false) {
    return false;
  }
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      ...
      if (square_A_ij_abs >= square_max_diag) {
        return false;
      }
      ...
      if ((A_real_delta > tol) && (A_real_delta > (A_real_abs_max * tol))) {
        return false;
      }

      const T A_imag_delta =
          (std::abs)(A_ij_imag + A_ji_imag);  // take into account complex conjugate
      const T A_imag_abs_max = (std::max)(A_ij_imag_abs, A_ji_imag_abs);

      if ((A_imag_delta > tol) && (A_imag_delta > (A_imag_abs_max * tol))) {
        return false;
      }

      const T A_ii_real = std::real(*A_ii_ptr);

      if ((A_ij_real_abs + A_ij_real_abs) >= (A_ii_real + A_jj_real)) {
        return false;
      }
...
inline typename enable_if2<is_cx<eT>::no, void>::result analyse_matrix_worker(
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      ...
      if (is_approx_sympd) {
        // if( (A_ij_abs >= max_diag) || (A_ji_abs >= max_diag) )  { is_approx_sympd =
        // false; }
        if (A_ij_abs >= max_diag) {
          is_approx_sympd = false;
        }
...
inline typename enable_if2<is_cx<eT>::yes, void>::result analyse_matrix_worker(
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      ...
      if (is_approx_sympd) {
        // avoid using std::abs(), as that is time consuming due to division and
        // std::sqrt()
        const T square_A_ij_abs = (A_ij_real * A_ij_real) + (A_ij_imag * A_ij_imag);
