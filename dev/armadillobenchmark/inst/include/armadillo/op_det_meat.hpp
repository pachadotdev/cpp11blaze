...
inline bool op_det::apply_direct(typename T1::elem_type& out_val,
  ...
  arma_conform_check((A.is_square() == false),
                     "det(): given matrix must be square sized");
  ...
  if ((is_cx<eT>::no) && (N <= 3)) {
    ...
    if (N == 2) {
      det_val = op_det::apply_tiny_2x2(A);
    }
    if (N == 3) {
      det_val = op_det::apply_tiny_3x3(A);
    }

    const T abs_det_val = std::abs(det_val);

    if ((abs_det_val > det_min) && (abs_det_val < det_max)) {
      out_val = det_val;
      return true;
    }
  ...
  if (A.is_diagmat()) {
    out_val = op_det::apply_diagmat(A);
    return true;
  }
...
template <typename T1>
inline typename T1::elem_type op_det::apply_diagmat(
    const Base<typename T1::elem_type, T1>& expr) {
  arma_debug_sigprint();
...
template <typename T1>
inline typename T1::elem_type op_det::apply_trimat(
    const Base<typename T1::elem_type, T1>& expr) {
  arma_debug_sigprint();
...
inline eT op_det::apply_tiny_2x2(const Mat<eT>& X) {
  arma_debug_sigprint();
...
template <typename eT>
inline eT op_det::apply_tiny_3x3(const Mat<eT>& X) {
  arma_debug_sigprint();
  ...
  const eT val1 = Xm[pos<0, 0>::n3] * (Xm[pos<2, 2>::n3] * Xm[pos<1, 1>::n3] -
                                       Xm[pos<2, 1>::n3] * Xm[pos<1, 2>::n3]);
