...
class op_find_unique : public traits_op_col {
 public:
  ...
  static inline bool apply_helper(Mat<uword>& out, const Proxy<T1>& P,
...
struct arma_find_unique_comparator {
  arma_inline bool operator()(const arma_find_unique_packet<eT>& A,
                              const arma_find_unique_packet<eT>& B) const {
    return (A.val < B.val);
  }
};
...
struct arma_find_unique_comparator<std::complex<T> > {
  arma_inline bool operator()(const arma_find_unique_packet<std::complex<T> >& A,
    ...
    return ((A_real < B_real)
                ? true
                : ((A_real == B_real) ? (A.val.imag() < B.val.imag()) : false));
  }
};
