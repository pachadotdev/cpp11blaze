...
inline void op_strans::apply_mat_noalias_tinysq(Mat<eT>& out, const TA& A) {
...
inline void op_strans::block_worker(eT* Y, const eT* X, const uword X_n_rows,
                                    const uword Y_n_rows, const uword n_rows,
                                    const uword n_cols) {
...
template <typename eT>
inline void op_strans::apply_mat_noalias_large(Mat<eT>& out, const Mat<eT>& A) {
  arma_debug_sigprint();
  ...
  for (uword row = 0; row < n_rows_base; row += block_size) {
    ...
    for (uword col = 0; col < n_cols_base; col += block_size) {
      ...
      op_strans::block_worker(&Y[col + Y_offset], &X[row + X_offset], n_rows, n_cols,
                              block_size, block_size);
    }

    const uword X_offset = n_cols_base * n_rows;

    op_strans::block_worker(&Y[n_cols_base + Y_offset], &X[row + X_offset], n_rows,
                            n_cols, block_size, n_cols_extra);
  }
  ...
  for (uword col = 0; col < n_cols_base; col += block_size) {
    ...
    op_strans::block_worker(&Y[col + Y_offset], &X[n_rows_base + X_offset], n_rows,
                            n_cols, n_rows_extra, block_size);
  }

  const uword X_offset = n_cols_base * n_rows;

  op_strans::block_worker(&Y[n_cols_base + Y_offset], &X[n_rows_base + X_offset], n_rows,
                          n_cols, n_rows_extra, n_cols_extra);
}

//! Immediate transpose of a dense matrix
template <typename eT, typename TA>
inline void op_strans::apply_mat_noalias(Mat<eT>& out, const TA& A) {
  arma_debug_sigprint();
...
template <typename eT>
inline void op_strans::apply_mat_inplace(Mat<eT>& out) {
  arma_debug_sigprint();
  ...
  if (n_rows == n_cols) {
    ...
    if (((n_rows == 1) || (n_cols == 1)) && (out.vec_state == 0) &&
        (out.mem_state == 0)) {
...
template <typename eT, typename TA>
inline void op_strans::apply_mat(Mat<eT>& out, const TA& A) {
  arma_debug_sigprint();
...
inline void op_strans::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_strans::apply_direct(Mat<typename T1::elem_type>& out, const T1& X) {
  arma_debug_sigprint();
...
template <typename T1>
inline void op_strans::apply(Mat<typename T1::elem_type>& out,
...
inline void op_strans_cube::apply_noalias(Cube<eT>& out, const Cube<eT>& X) {
  ...
  for (uword s = 0; s < X.n_slices; ++s) {
    ...
    const Mat<eT> X_slice(const_cast<eT*>(X.slice_memptr(s)), X.n_rows, X.n_cols, false,
                          true);
