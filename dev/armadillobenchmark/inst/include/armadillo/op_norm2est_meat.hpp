...
inline norm2est_randu_filler<eT>::norm2est_randu_filler() {
  arma_debug_sigprint();
...
template <typename eT>
inline void norm2est_randu_filler<eT>::fill(eT* mem, const uword N) {
  arma_debug_sigprint();
...
template <typename T>
inline norm2est_randu_filler<std::complex<T> >::norm2est_randu_filler() {
  arma_debug_sigprint();
...
template <typename T>
inline void norm2est_randu_filler<std::complex<T> >::fill(std::complex<T>* mem,
                                                          const uword N) {
  arma_debug_sigprint();
...
inline typename T1::pod_type op_norm2est::norm2est(
    const Base<typename T1::elem_type, T1>& X, const typename T1::pod_type tolerance,
  ...
  arma_conform_check((max_iter == uword(0)),
                     "norm2est(): parameter 'max_iter' must be > 0");
  ...
  if (A.internal_has_nonfinite()) {
    arma_warn(1, "norm2est(): given matrix has non-finite elements");
  }

  if ((A.n_rows == 1) || (A.n_cols == 1)) {
    return op_norm::vec_norm_2(Proxy<Mat<eT> >(A));
  }
  ...
  for (uword i = 0; i < max_iter; ++i) {
    ...
    if (x_norm == T(0) || (arma_isfinite(x_norm) == false) ||
        (x.internal_has_nonfinite())) {
    ...
    if (arma_isfinite(est_cur) == false) {
      return est_old;
    }

    if (((std::abs)(est_cur - est_old)) <= (tol * (std::max)(est_cur, est_old))) {
      break;
    }
  }
...
template <typename T1>
inline typename T1::pod_type op_norm2est::norm2est(
    const SpBase<typename T1::elem_type, T1>& X, const typename T1::pod_type tolerance,
    const uword max_iter) {
  arma_debug_sigprint();
  ...
  if ((A.n_rows == 1) || (A.n_cols == 1)) {
    return spop_norm::vec_norm_k(A.values, A.n_nonzero, 2);
  }
