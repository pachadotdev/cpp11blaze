...
template <typename elem_type, typename derived>
inline elem_type Base<elem_type, derived>::min() const {
  return op_min::min((*this).get_ref());
}

template <typename elem_type, typename derived>
inline elem_type Base<elem_type, derived>::max() const {
...
template <typename elem_type, typename derived>
inline elem_type Base<elem_type, derived>::min(uword& row_of_min_val,
...
template <typename elem_type, typename derived>
inline elem_type Base<elem_type, derived>::max(uword& row_of_max_val,
...
template <typename elem_type, typename derived>
inline uword Base<elem_type, derived>::index_min() const {
  const Proxy<derived> P((*this).get_ref());
...
template <typename elem_type, typename derived>
inline uword Base<elem_type, derived>::index_max() const {
  const Proxy<derived> P((*this).get_ref());
...
inline bool Base<elem_type, derived>::is_hermitian() const {
  ...
  for (uword j = 0; j < N; ++j) {
    if (access::tmp_imag(A_col[j]) != T(0)) {
      return false;
    }
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      if (A_col[i] != access::alt_conj(*A_row)) {
        return false;
      }
...
inline bool Base<elem_type, derived>::is_diagmat() const {
  ...
  for (uword A_col = 0; A_col < A_n_cols; ++A_col) {
    for (uword A_row = 0; A_row < A_n_rows; ++A_row) {
      if ((A_mem[A_row] != elem_type(0)) && (A_row != A_col)) {
        return false;
      }
    }
...
template <typename elem_type, typename derived>
inline const Op<derived, op_vectorise_col> Base<elem_type, derived>::as_col() const {
  return Op<derived, op_vectorise_col>((*this).get_ref());
}

template <typename elem_type, typename derived>
inline const Op<derived, op_vectorise_row> Base<elem_type, derived>::as_row() const {
...
template <typename elem_type, typename derived>
inline const Op<derived, op_inv_gen_default> Base_extra_yes<elem_type, derived>::i()
    const {
