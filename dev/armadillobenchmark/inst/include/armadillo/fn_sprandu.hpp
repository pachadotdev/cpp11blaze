...
arma_warn_unused inline obj_type sprandu(
...
arma_warn_unused inline sp_mat sprandu(const uword n_rows, const uword n_cols,
...
arma_warn_unused inline sp_mat sprandu(const SizeMat& s, const double density) {
  arma_debug_sigprint();
...
//! given sparse matrix X, with the non-zero values set to random values in the [0,1]
//! interval (uniform distribution)
template <typename T1>
arma_warn_unused inline SpMat<typename T1::elem_type> sprandu(
